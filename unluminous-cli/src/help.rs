...
//!
//! [`as_json`] is the same list as data, which is what `unluminous-cli commands --json` prints. That is
...
pub fn overall() -> String {
    ...
    out.push_str("unluminous-cli — drive a running Unluminous window from the command line.\n\n");
    out.push_str("Usage:\n  unluminous-cli [flags] <area> <verb> [arguments]\n\n");
    ...
    out.push_str(
        "\n`unluminous-cli commands --json` prints every command as data, which is what to give a\n\
         program or an agent. `unluminous-cli <area> <verb> --help` explains one command.\n\
         The written documentation is in unluminous-cli/docs/commands.md.\n",
    );
    out
}
...
pub fn as_json(only: Option<&str>) -> Value {
    ...
    json!({
        "usage": "unluminous-cli [flags] <area> <verb> [arguments]",
...
mod tests {
    ...
    fn a_commands_help_leads_with_its_examples() {
        ...
        assert!(help.contains("unluminous-cli terminal send git status"));
    }
