# Unluminous's MCP server

Unluminous speaks the [Model Context Protocol](https://modelcontextprotocol.io), so an AI agent can
...
If you want to drive Unluminous yourself, read [commands.md](commands.md) instead. This is the same set of
commands with an agent's tool-calling machinery in front of it.

## The quickest way in

`Unluminous -> Edit -> Settings -> Tools -> MCP`, and press **Install for Claude Code** or **Install for
Codex**. Restart the agent. In Claude Code, `/mcp` will list Unluminous.

The same thing from a terminal:

```sh
unluminous-cli mcp install both
unluminous-cli mcp install claude --scope project   # this folder only, in .mcp.json
unluminous-cli mcp install codex --remove           # take it out again
```
...
it does whenever anybody runs the command, not something Unluminous asked for.

For a client with no button of its own, `unluminous-cli mcp config` prints the block to paste. It is the
same block the Settings page shows.

## What an agent is given

**The tools are generated from `catalogue.rs`** — the one list of commands that `unluminous-cli` parses
against and the window dispatches on. A command added to Unluminous is a tool the day it is added, and a
...
| `mcp.tools` | | |
|---|---|---|
| `grouped` | **the default** | One tool an area — `unluminous_tab`, `unluminous_editor`, `unluminous_git` — plus narrow generated aliases for semantic definition, references, and rename. Area verbs are an `enum`; every usage line and summary stays in the description. |
| `every` | | One tool a command — `unluminous_tab_open` — with every argument and flag a typed property. |

`grouped` costs an agent less than half the context `every` does, on every conversation the server
is connected to, and still names every command Unluminous has. The three aliases make the native semantic
answer as specific as a generic grep or edit tool without removing the compatible `unluminous_editor`
...
`unluminous-cli mcp tools --count` prints the real figures for both, against the catalogue as it is now:

```sh
$ unluminous-cli mcp tools --count
...
- **`instance`** — which running Unluminous to drive. See the next section.
...
- **A screenshot comes back as a picture.** `window screenshot` writes the PNG and the reply carries
  ...
  shapes.
- **The whole written reference as a resource**, at `unluminous://commands.md`. It costs nothing until it
...
3. Exactly one running Unluminous on the folder the server prefers — `CLAUDE_PROJECT_DIR`, which Claude
...
running Unluminous host it. Either way the endpoint is `http://127.0.0.1:7345/mcp`.
...
The same names in `settings.conf`, on the Settings page, and in `unluminous-cli settings set`. A change
takes effect at once: the listener starts, stops or moves in the same frame.

`unluminous-cli mcp status --json` says what a window is actually doing, which is not always what the
settings say — the port may be held by another Unluminous, and an Unluminous started with `--control off` has no
command channel for a server to drive.

## Writing a client of your own

The MCP server is a client of Unluminous's own control channel, which is a smaller and older thing: a
