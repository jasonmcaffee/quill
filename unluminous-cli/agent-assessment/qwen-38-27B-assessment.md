# Qwen 3.8 27B driving Unluminous from the command line
...
1. a short system prompt saying it drives Unluminous through a tool called `unluminous-cli`, and that its
   answer must be command lines and nothing else;
2. the whole of `unluminous-cli/docs/commands.md` — 32,123 characters, about 8,000 tokens.
...
- **executed** — against a **live Unluminous window**, for real;
...
`_agent_output/task-1661-unluminous-cli/assessment/*.json`.
...
`unluminous-cli set-root src`, `unluminous-cli status set "step three done"`, `unluminous-cli open README.md`.

So the 97-point gap between 3.13% and 100% is what the documentation is worth. The model is not
recognising Unluminous from its training data, and it is not guessing a conventional CLI. It is reading.

## How it did, task by task

**It was strongest where the vocabulary was Unluminous's own.** All nine settings tasks passed every
...
- **`unluminous-cli version --json` printed a sentence.** The documentation tells an agent to pass
...
- **"What version of the Unluminous editor am I running?"** was answered with `unluminous-cli version`, which
...
3. **Errors that say what to do instead.** `action run open-file` does not fail with "not
   ...
   `unluminous-cli tab open <path>` instead."* Every refusal names its alternative, which is what turns a
   dead end into a next step.

## Reproducing it

```sh
# an Unluminous on the sample project, and nothing else running
node unluminous-cli/agent-assessment/project.js _agent_output/task-1661-unluminous-cli/assessment-project
unluminous-cli launch _agent_output/task-1661-unluminous-cli/assessment-project
...
node unluminous-cli/agent-assessment/run.js --label mine --temperature 0
node unluminous-cli/agent-assessment/run.js --label mine-control --temperature 0 --no-docs
```
