...
//!   updates `Term`. Unluminate's own code never touches that thread.
...
pub struct SessionSettings {
    ...
    /// Variables laid **over** the environment Unluminate itself started with, never replacing it.
    ///
...
pub struct Session {
    ...
    /// after [`Session::kill`], because a program Unluminate killed did not choose a code.
    ///
...
impl Session {
    ...
    pub fn spawn(settings: &SessionSettings, size: Size, waker: Waker) -> std::io::Result<Self> {
        ...
        let options = alacritty_terminal::tty::Options {
            ...
            // the child from Unluminate's own environment and applies these on top.
        ...
        // A window identifier of zero: Unluminate has one window in a process, so there is nothing to tell
        ...
        // which is what made this look like it worked in `unluminate-terminal`'s own tests.
        //
    ...
    /// until Unluminate ran on Windows.
    ...
    pub fn pump(&mut self) {
        while let Ok(event) = self.events.try_recv() {
            match event {
                ...
                // the clipboard could read a password out of it, and nothing Unluminate runs needs to.
...
/// been it says `cmd.exe`. So Unluminate opened a `cmd.exe` while the machine's own commands live in a
...
/// Give the emulator Unluminate's colours, so that a program asking what colour something is gets an answer and
/// so that resetting a colour puts Unluminate's own back.
trait WithColors {
...
mod tests {
    ...
    fn a_session_takes_its_program_with_it_even_when_the_program_is_busy() {
        ...
        let nonce = format!("unluminate-reap-{}-{}", std::process::id(), std::time::SystemTime::now()
...
Start-Sleep -Seconds 900"),
    ...
    fn a_shell_runs_a_command_and_its_output_appears() {
        ...
        session.send(b"echo unluminate-terminal-works\r".to_vec());
        ...
        loop {
            session.pump();
            if session.snapshot().contains("unluminate-terminal-works") {
                break;
            }
            assert!(
    ...
    fn what_a_program_wrote_just_before_it_ended_is_still_on_the_screen() {
        ...
        let message = "unluminate-drain-on-exit-works";
    ...
    fn a_variable_in_the_settings_reaches_the_program() {
        ...
        // be in the child's environment. Asked of the program itself rather than of Unluminate, by
        // having it print the variable back.
        let (shell_flag, command) = if cfg!(target_os = "windows") {
            ("/c", "echo [%UNLUMINATE_RUN_TEST%]".to_owned())
        } else {
            ("-c", "echo \"[$UNLUMINATE_RUN_TEST]\"".to_owned())
        };
        let settings = SessionSettings {
            ...
            env: vec![("UNLUMINATE_RUN_TEST".to_owned(), "it-arrived".to_owned())],
        };
        let waker: Waker = Arc::new(|| {});
        let mut session =
            Session::spawn(&settings, Size::new(8, 60), waker).expect("start the program");
    ...
    #[test]
    fn the_environment_unluminate_started_with_is_still_there_underneath() {
        ...
        // the program would still start, because Unluminate names it in full.
        ...
        let settings = SessionSettings {
            ...
            args: vec![shell_flag.to_owned(), command],
            working_directory: None,
            env: vec![("UNLUMINATE_RUN_TEST".to_owned(), "one".to_owned())],
        };
        let waker: Waker = Arc::new(|| {});
        let mut session =
            Session::spawn(&settings, Size::new(8, 60), waker).expect("start the program");
    ...
    fn resizing_after_a_program_has_ended_keeps_what_it_wrote() {
        ...
        // by then is empty.
        let message = "unluminate-resize-after-exit";
    ...
    fn a_program_that_is_killed_stops_and_keeps_what_it_wrote() {
        ...
        session.send(b"echo unluminate-run-kill-test\r".to_vec());
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
        loop {
            session.pump();
            if session.snapshot().contains("unluminate-run-kill-test") {
                break;
            }
        ...
        assert_eq!(session.exit_code(), None, "a program Unluminate killed chose no code");
        assert!(
            session.snapshot().contains("unluminate-run-kill-test"),
    ...
    fn the_default_shell_is_powershell_rather_than_the_batch_interpreter() {
        // `COMSPEC` says `cmd.exe` on every Windows there is, so reading it meant Unluminate's terminal never
    ...
    fn a_shell_starts_in_the_folder_it_was_given_even_when_the_path_is_verbatim() {
        // `task-1670`: the folder reached the terminal as `\\?\C:\jason\dev\unluminate`, because that is what
        ...
        // came up in it.
        let folder = std::env::temp_dir().join("unluminate-verbatim-working-directory");
        ...
        // The shell's own answer to where it is standing, rather than Unluminate's. In brackets, because
