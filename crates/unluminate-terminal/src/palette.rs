...
//! everything else empty, so the defaults are ours to supply. These are Unluminate's own: the same blue as the
...
impl Palette {
    ...
    /// answer, and so that resetting a colour puts Unluminate's own back rather than nothing.
