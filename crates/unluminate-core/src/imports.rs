...
//! `unluminate_app::services::imports`. What is here is only the reading of the text.
//!
...
//! use unluminate_core::completion::Row;       the module is a path of segments
//! ```
//!
...
mod tests {
    ...
    fn a_module_path_is_read_back_to_its_keyword() {
        ...
        let grammar = rust();
        assert_eq!(
            at("use unluminate_core::comp|", &grammar),
            Some(Context::Segment { segments: vec!["unluminate_core".to_owned()], stem: 21..25 })
        );
        assert_eq!(at("use |", &grammar), Some(Context::Segment { segments: vec![], stem: 4..4 }));
        assert_eq!(
            at("use unluminate_core::|", &grammar),
            Some(Context::Segment { segments: vec!["unluminate_core".to_owned()], stem: 21..21 })
        );
    }
    ...
    fn a_braced_list_is_stepped_over_to_the_path_that_owns_it() {
        // Scenarios 22 and 23.
        let grammar = rust();
        let nested = at("use unluminate_core::completion::{Candidate, R|}", &grammar);
        assert!(
            matches!(&nested, Some(Context::Segment { segments, .. })
                if segments == &["unluminate_core".to_owned(), "completion".to_owned()]),
            "{nested:?}"
        );
        let deeper = at("use unluminate_core::{a, b::c|}", &grammar);
        assert!(
            matches!(&deeper, Some(Context::Segment { segments, .. })
                if segments == &["unluminate_core".to_owned(), "b".to_owned()]),
            "{deeper:?}"
        );
    }
    ...
    fn a_visibility_in_front_of_the_keyword_changes_nothing() {
        // Scenario 24.
        let found = at("pub use unluminate_core::comp|", &rust());
        assert!(matches!(&found, Some(Context::Segment { segments, .. })
            if segments == &["unluminate_core".to_owned()]));
    }
    ...
    fn a_path_written_across_lines_still_reaches_its_keyword() {
        let found = at("use unluminate_core::{\n    completion,\n    symb|,\n};", &rust());
        assert!(
    ...
    fn the_two_acceptance_ranges_differ_only_for_a_specifier() {
        ...
        let path = at("use unluminate_core::comp|", &rust()).expect("a segment");
