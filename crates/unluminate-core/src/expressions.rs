...
//!   every `use` line. A local is never reached through `::` in any language Unluminate ships a plugin
//!   for.
...
//! be values, which is a list of languages inside Unluminate — the exact thing `language.definers` and
...
mod tests {
    ...
    fn a_field_path_is_read_backwards_to_its_root() {
        ...
        // list of languages inside Unluminate, and the shape this module exists for reads it anyway.
