...
//! branches at the same depth; Unluminate does not, deliberately, because it makes the picture depend on
...
fn read(source: &Source) -> Result<Graph, Problem> {
    ...
    for line in source.statements() {
        ...
        // Unluminate draws every branch with its name, so they are read and ignored.
