...
//!   `config:` is not: Unluminate has one palette, read out of the design, and a document does not get to
...
mod tests {
    ...
    fn a_markdown_string_keeps_its_words_and_loses_its_backticks() {
        // Unluminate sets a diagram label in one style, so the marks inside are shown as written rather
