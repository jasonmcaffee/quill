...
//!
//! `unluminate-core` has no user interface dependency and cannot name a drawing type, and it should not
...
//! three lines, a dashed relationship is a line with a dash pattern. So the painter in `unluminate-app`
...
///
/// `style::Color` has no alpha, deliberately: text in Unluminate is always opaque. A diagram does want a
