...
mod tests {
    ...
    fn the_metadata_block_is_read_for_its_three_keys() {
        let text = "kanban\n todo[Todo]\n  a[Fix it]@{ assigned: \"Jason\", ticket: \"UNLUMINATE-12\", priority: \"High\" }\n";
        let board = board(text);
        let card = &board.columns[0].cards[0];
        assert_eq!(card.label, "Fix it");
        assert_eq!(card.assigned, "Jason");
        assert_eq!(card.ticket, "UNLUMINATE-12");
    ...
    fn a_kanban_board_is_drawn_and_keeps_every_property() {
        let text = "kanban\n\
            todo[Todo]\n  t1[Write the parser]@{ assigned: \"Jason\", ticket: \"UNLUMINATE-1\", priority: \"High\" }\n  t2[Draw the shapes]\n\
            doing[In progress]\n  d1[Lay it out]@{ assigned: \"Jason\" }\n\
            done[Done]\n  x1[Read the syntax]\n  x2[Write the TDD]\n  x3[Choose the approach]\n";
        check::drawn(
            text,
            &options(),
            &["Todo", "In progress", "Done", "Write the parser", "UNLUMINATE-1", "Lay it out", "Write the TDD"],
        );
    }
