...
//! Markdown file be drawn in the preview. `tasks/unluminate-mermaid-plugin-tdd.md` records why that is
...
//! actually needs on top of what Unluminate already has is arithmetic.
//!
...
//!
//! Not "identical to `mermaid.js`" — the curves are polylines, the fonts are Unluminate's and the colours
//! are Unluminate's. **Correct and readable**: the right nodes, the right edges in the right direction,
...
pub enum Kind {
    ...
    Radar,
    Treemap,
    /// A Mermaid diagram type this version of Unluminate does not draw. It carries the name to say so
...
impl Kind {
    ...
    /// True when Unluminate can draw this one.
...
pub struct Problem {
    ...
    pub text: String,
    pub reason: String,
    /// True when this is a diagram type Unluminate does not draw yet, rather than a fault in the source.
    /// The two are shown differently: one is the author's business and the other is Unluminate's.
...
impl Problem {
    ...
    pub fn not_drawn(name: &str) -> Self {
        Self {
            line: None,
            text: String::new(),
            reason: format!("Unluminate does not draw a {name} diagram yet."),
            unsupported: true,
        }
    }
...
impl<'a> Options<'a> {
    /// The usual options for `metrics`: Unluminate's palette, and a plain fourteen point face.
...
pub fn render(text: &str, options: &Options) -> Result<Scene, Problem> {
    ...
    if source.lines.len() > LINE_LIMIT {
        return Err(Problem::whole(format!(
            "This diagram has more than {LINE_LIMIT} lines, which is more than Unluminate draws."
        )));
    }
...
mod tests {
    ...
    fn every_keyword_mermaid_ships_is_recognised() {
        // The whole list from Mermaid's own navigation, so a type that is added to Unluminate later shows
    ...
    #[test]
    fn the_ten_unluminate_does_not_draw_are_named_rather_than_mistaken_for_something_else() {
        for keyword in [
    ...
    #[test]
    fn a_type_unluminate_does_not_draw_is_a_problem_that_names_it() {
        let metrics = FixedMetrics::default();
        let problem = render("wardley\nvalue chain\n", &Options::new(&metrics)).expect_err("not drawn");
        assert!(problem.unsupported, "this is Unluminate's limitation, not the author's fault");
...
/// One sample of every diagram type Unluminate draws, and the words each one has to end up saying.
///
...
pub const SAMPLES: &[(&str, &str, &[&str])] = &[
    ...
    (
        "mindmap",
        "mindmap\nroot((Unluminate))\n  Editing\n    Undo\n  Panes\n    Terminal\n",
        &["Unluminate", "Editing", "Undo", "Panes", "Terminal"],
    ),
    (
        "timeline",
...
mod every_type {
    ...
    #[test]
    fn there_is_a_sample_for_every_type_unluminate_draws() {
    ...
    #[test]
    fn a_type_unluminate_does_not_draw_is_named_rather_than_mis_parsed() {
        for keyword in [
            "c4Diagram", "zenuml", "architecture", "swimlanes", "eventModeling", "venn",
            "ishikawa", "wardley", "cynefin", "treeView",
        ] {
            ...
            assert!(problem.unsupported, "{keyword} is Unluminate's limitation, not the author's fault");
            assert!(
