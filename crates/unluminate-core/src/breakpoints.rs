...
//! top of the file, and every offset in Unluminate is already a byte offset into the real text — which
//! means [`crate::document::Document::insert`] and `remove_range`, the only two places in Unluminate that
...
/// adapter in `SourceBreakpoint`, so Unluminate's whole cost for two of the reference editor's features is these two
...
impl Breakpoints {
    ...
    ///
    /// What a file that changed on the disk underneath Unluminate needs, and what reading
    /// `.unluminate/breakpoints.conf` needs: the offsets were written against bytes that may no longer be
...
mod tests {
    ...
    /// A file rewritten outside Unluminate gives a misplaced dot rather than a panic in the layout
