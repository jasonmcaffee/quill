...
pub enum Command {
    ...
    ///
    /// This is what a rename is applied by. Undo in Unluminate restores a snapshot, so a whole
...
///
/// A tab from `Tab`, a space from `Space`. Unluminate has no tab width and no "insert spaces for tabs"
...
/// wrong. The cost is bounded by capping the history at `UNDO_LIMIT` states, and Unluminate opens plain
...
pub struct Document {
    ...
    /// Unluminate that know a range of bytes moved. See `crate::breakpoints`.
    breakpoints: Breakpoints,
    selection: Selection,
...
impl Document {
    ...
    /// so is never written, and `unluminate_app` refuses to save a tab showing a picture in any case.
    ...
    /// document as having unsaved changes, because a file Unluminate saves is plain text and carries no
    ...
    /// changes, because what Unluminate saves is plain text and carries no formatting, so nothing about
    ...
    /// when it was closed and something outside Unluminate may have rewritten it since. A mark in the
    ...
    /// undo history and the file is not marked as having unsaved changes, because what Unluminate saves
    ...
    /// because what Unluminate saves is plain text and a fold is not in it.
    ...
    /// the bytes the file had when it was closed, and something outside Unluminate may have rewritten it
    ...
    /// because what Unluminate saves is plain text and a breakpoint is not in it. The revision moves,
    ...
    /// draws. `unluminate-core` counts paragraphs from zero everywhere else, so the conversion is here
...
mod tests {
    ...
    /// from anything `ReplaceMany` arranges: undo in Unluminate restores a state, so the state it
    ...
    fn replacing_many_ranges_is_one_edit_to_undo() {
        ...
        let edits = unluminate_core_replacements(&before, "value", "total");
    ...
    fn undoing_a_rename_brings_back_the_marks_it_moved() {
        ...
        document.apply(Command::ReplaceMany(unluminate_core_replacements(&text, "value", "v")));
...
");
        ...
        assert!(!document.is_modified(), "what Unluminate saves is plain text, and a dot is not in it");
...
");
    ...
    /// A set read from a file that was rewritten outside Unluminate is brought inside the text rather
...
");
    ...
    fn unluminate_core_replacements(
        text: &str,
        name: &str,
        to: &str,
...
".to_owned()),
    ...
    fn opening_a_file_written_on_windows_normalises_its_line_breaks() {
        let directory = std::env::temp_dir().join("unluminate-core-test-crlf");
