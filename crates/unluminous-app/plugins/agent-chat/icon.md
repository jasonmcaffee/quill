...
The mark is a speech bubble holding three dots. The three dots are what a model does while it is
thinking, which is what this pane is showing more of the time than anything else. An outlined
bubble rather than a filled one, because a filled bubble at 32 by 32 is a blob.
...
curl -s -X POST http://localhost:8091/image-creation/generateImageToProjectFile \
  ...
  -d '{
        "prompt": "A flat vector app icon of a single rounded speech bubble with a small tail at the bottom left, containing three evenly spaced dots in a horizontal row. Bold even line weight, geometric, perfectly centred, generous margin. Bright cyan outline with white dots on a completely flat solid dark navy background. Minimal, crisp, high contrast, icon design, no text, no letters, no words, no gradients, no shadows, no perspective.",
        "negativePrompt": "text, letters, words, numbers, watermark, signature, photorealistic, 3d render, gradient, drop shadow, noise, texture, busy background, clutter, person, face, robot, hands, two bubbles",
        "width": 1024,
        "height": 1024,
        "projectId": "unluminous",
        "relativePath": "_agent_output/task-1795-icons/agent-chat-source.png",
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1795-icons/agent-chat-source.png crates/unluminous-app/plugins/agent-chat
```
...
```bash
cargo run --example scale -- crates/unluminous-app/plugins/agent-chat/icon.png /tmp/icon-large.png 8
```
