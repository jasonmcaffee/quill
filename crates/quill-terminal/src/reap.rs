...
//! * **Quill crashes, or is killed** — the same, because a handle a dead process held is a handle
...
pub struct Reaper {
    ...
    ///
    /// A job is the only thing that survives Quill being killed, so it is the one that matters; this
...
impl Reaper {
    ...
    /// Whether the guarantee that survives Quill itself being killed is in place.
