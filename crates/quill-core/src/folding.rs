...
/// `FoldingRange` is. `head` and `body` count paragraphs from zero, which is what `quill-core` calls
...
/// `Document::insert` and `Document::remove_range` — the only two places in Quill that know a range
