...
//! branches at the same depth; Quill does not, deliberately, because it makes the picture depend on
...
fn read(source: &Source) -> Result<Graph, Problem> {
    ...
    for line in source.statements() {
        ...
        // Quill draws every branch with its name, so they are read and ignored.
