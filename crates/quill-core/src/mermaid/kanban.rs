...
mod tests {
    ...
    fn a_kanban_board_is_drawn_and_keeps_every_property() {
        let text = "kanban\n\
            todo[Todo]\n  t1[Write the parser]@{ assigned: \"Jason\", ticket: \"QUILL-1\", priority: \"High\" }\n  t2[Draw the shapes]\n\
            doing[In progress]\n  d1[Lay it out]@{ assigned: \"Jason\" }\n\
            done[Done]\n  x1[Read the syntax]\n  x2[Write the TDD]\n  x3[Choose the approach]\n";
        check::drawn(
            text,
            &options(),
            &["Todo", "In progress", "Done", "Write the parser", "QUILL-1", "Lay it out", "Write the TDD"],
        );
    }
