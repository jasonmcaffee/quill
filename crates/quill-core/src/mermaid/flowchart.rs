...
fn read(source: &Source) -> Result<Chart, Problem> {
    ...
    for line in source.statements() {
        ...
        if let Some(rest) = line.after_word("direction") {
            // Mermaid lets a subgraph set its own direction. Quill lays a subgraph out with the
...
fn read_node(piece: &str, line: &Line) -> Result<(String, Option<String>, Shape), Problem> {
    ...
    // `:::class` picks a colour scheme out of the document, which Quill reads and ignores.
    ...
    let Some((shape, inner)) = read_shape(body) else {
        return Err(Problem::at(line, format!("`{body}` is not a node shape Quill knows")));
    };
...
mod drawing {
    ...
    fn a_shape_that_was_never_closed_says_which_line_it_was_on() {
        ...
        assert!(!problem.unsupported, "a mistake in the source is the author's, not Quill's");
    }

    #[test]
    fn brackets_quill_has_no_shape_for_are_kept_as_the_label() {
        // Being permissive here is deliberate: Mermaid keeps adding shapes, and a file using one
        // Quill has not learnt yet should still draw with its words in it rather than refuse.
