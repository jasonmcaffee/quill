...
//! what a window is. The default is Quill's own palette, so a test here needs no application.
//!
//! **A document does not choose these.** `style`, `classDef` and `:::` are read and ignored, which
//! §13 of `tasks/quill-mermaid-plugin-tdd.md` records: honouring arbitrary colours out of a document
