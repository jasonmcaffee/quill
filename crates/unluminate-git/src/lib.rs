...
//! fonts — the same bar `unluminate-core` and `unluminate-terminal` are held to.
//!
//! ## It runs the `git` program
//!
//! Not libgit2 and not gitoxide. Both were considered and both were rejected, and the reason is not
//! size or build time: it is **what the machine's own git already knows**. A person running Unluminate has
...
//! particular, `safe.directory`. `git push` from Unluminate has to be the same push they get from their
...
impl Repository {
    ...
    pub fn discover(folder: &Path) -> Option<Self> {
        ...
        // way the path Unluminate shows is the path git shows.
...
mod tests {
    ...
    fn a_folder_that_is_not_in_a_repository_has_none() {
        let folder = std::env::temp_dir().join("unluminate-git-not-a-repository");
    ...
    fn a_path_is_made_relative_the_way_git_spells_one() {
        let repository = Repository { root: PathBuf::from("/home/jason/unluminate") };
        assert_eq!(
            repository.relative(Path::new("/home/jason/unluminate/crates/unluminate-git/src/lib.rs")),
            Some("crates/unluminate-git/src/lib.rs".to_owned())
        );
        assert_eq!(repository.relative(Path::new("/home/jason/other/thing.md")), None);
        assert_eq!(repository.relative(Path::new("/home/jason/unluminate")), None, "the root itself is not a path in it");
    }
    ...
    fn a_path_reached_through_a_symlink_is_still_inside_the_repository() {
        let base = std::env::temp_dir().join("unluminate-git-symlink");
    ...
    fn a_path_outside_the_repository_is_still_none_when_symlinks_are_resolved() {
        let base = std::env::temp_dir().join("unluminate-git-outside");
        std::fs::remove_dir_all(&base).ok();
    ...
    fn a_repository_is_named_after_its_folder() {
        let repository = Repository { root: PathBuf::from("/home/jason/unluminate") };
        assert_eq!(repository.name(), "unluminate");
    }
}
