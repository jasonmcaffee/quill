...
//! hold anything the file system allows, and a path Unluminate cannot spell is still a path it should be
//! able to list.
...
pub fn run<S: AsRef<OsStr>>(folder: &Path, arguments: &[S]) -> Outcome {
    ...
    // Git will not open an editor or ask for a password on the terminal from inside Unluminate: there is
