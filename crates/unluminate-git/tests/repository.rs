...
//! asks **git** what happened rather than asking Unluminate.
//!
...
use unluminate_git::branch::{self, MergeOptions, Resume};
use unluminate_git::command::run;
use unluminate_git::ops::{self, PullStrategy, PushTarget, ResetMode};
use unluminate_git::{diff, log, Repository, State, Status};

/// A repository with one commit in it, at a folder named after the test.
fn repository(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join("unluminate-git-tests").join(name);
...
fn committing_writes_a_commit_and_amending_changes_the_one_before() {
    ...
    assert_eq!(commits[0].author, "Unluminate Test");
...
fn a_merge_that_conflicts_says_so_and_can_be_abandoned() {
    ...
    // Unluminate can open. There is no three way merge editor and this is why one is not needed to
    // finish.
...
fn a_conflict_can_also_be_settled_and_the_merge_finished() {
    ...
    // Settling a conflict is editing the file and staging it, which is what Unluminate does with it.
...
fn a_push_to_another_folder_really_arrives_and_a_pull_brings_it_back() {
    ...
    let bare = std::env::temp_dir().join("unluminate-git-tests").join("push-remote.git");
    ...
    let elsewhere = std::env::temp_dir().join("unluminate-git-tests").join("push-clone");
    ...
    for (name, value) in [("user.name", "Someone Else"), ("user.email", "else@unluminate.invalid"), ("commit.gpgsign", "false")] {
...
fn blame_says_who_wrote_each_line_and_ranks_the_commits_by_age() {
    ...
    assert_eq!(blame.lines[0].author, "Unluminate Test");
...
fn a_repository_with_no_commits_in_it_has_an_empty_history_rather_than_an_error() {
    let root = std::env::temp_dir().join("unluminate-git-tests").join("empty");
    std::fs::remove_dir_all(&root).ok();
    std::fs::create_dir_all(&root).expect("make the folder");
    init(&root);
...
fn a_diff_is_git_s_own_and_a_failure_carries_git_s_own_message() {
    ...
    // than a message of Unluminate's own invention.
