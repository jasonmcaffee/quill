...
which is how the other three bundled plugins' icons were made — `tasks/quill-ide-tdd.md` §6 records
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1660-mermaid/icon-source.png crates/quill-app/plugins/mermaid
```
...
```bash
cargo run --example scale -- crates/quill-app/plugins/mermaid/icon.png /tmp/icon-large.png 8
```
