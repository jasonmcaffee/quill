...
`tasks/quill-ide-tdd.md` §6 record the same recipe. Written down here so the icon can be made again
without guessing.
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1671-css-plugin/icon-source.png crates/quill-app/plugins/css
```
...
```bash
cargo run --example scale -- crates/quill-app/plugins/css/icon.png _agent_output/task-1671-css-plugin/icon-large.png 8
```
