//! Two things that have to happen while `quill.exe` is being built: the icon and version block go
...
//! handed to the compiler as `QUILL_BUILD_DATE`, which `build_info::BUILD_DATE` reads.
//!
//! **When this script reruns is the whole design.** The value it emits is part of the crate's
//! fingerprint, so a script that restamped on every invocation would recompile `quill-app` and
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../../quill-cli");
    println!("cargo:rerun-if-env-changed=QUILL_BUILD_DATE");
    println!("cargo:rustc-env=QUILL_BUILD_DATE={}", build_date());
...
fn build_date() -> String {
    if let Ok(given) = std::env::var("QUILL_BUILD_DATE") {
...
///
/// Quill has no dates library and does not want one: `quill_git::blame` computes a civil date from a
...
/// `quill-git` makes when it runs `git` rather than reimplementing it. `None` when the command
...
/// date is Howard Hinnant's `civil_from_days`, which is the same arithmetic `quill_git::blame` uses
