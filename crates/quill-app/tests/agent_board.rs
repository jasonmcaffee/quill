//! Tests that start a real agent on a real board and grade what happened against Quill's own state.
//!
...
//!
//! This is the bargain `tools/agent-study` makes, applied to the board. What is asserted is **a state Quill
...
//! gateway `~/.zshrc` exports. `QUILL_TEST_MODEL` names the model, and without it the ticket names none and the
...
use quill_app::services::agent_tasks::model::{Assignee, Author, Status};
use quill_app::services::agent_tasks::store::{NewTask, Store};
use quill_app::services::agent_tasks::{AgentTasks, Configuration};
use quill_app::services::plugin_ui::{Context, UiProvider};
...
/// everything was working. `QUILL_TEST_MODEL` names a quicker model when somebody wants a quicker run, and the
...
/// The folder the agent is allowed to work in, named by `QUILL_TEST_PROJECT`, or `None` to skip.
///
...
/// `tasks/quill-issues-tdd.md` §16.4 records that as a finding about the plugin.
///
...
///
/// So the operator names the folder. `QUILL_TEST_PROJECT` is a folder they have already opened Claude Code in, and
...
fn the_folder_the_agent_may_work_in() -> Option<PathBuf> {
    let named = std::env::var("QUILL_TEST_PROJECT").ok().filter(|value| !value.trim().is_empty());
    let Some(named) = named else {
        eprintln!(
            "skipped: set QUILL_TEST_PROJECT to a folder you have already opened Claude Code in, which is how \
             ...
             begins. See tasks/quill-issues-tdd.md section 16.4."
        );
        return None;
    };
    let folder = PathBuf::from(named.trim());
    if !folder.is_dir() {
        eprintln!("skipped: QUILL_TEST_PROJECT names {}, which is not a folder", folder.display());
        return None;
    }
    Some(folder)
}
...
fn there_is_a_key() -> bool {
    let found = quill_app::services::agent_tasks::the_key().is_some();
    if !found {
        eprintln!(
...
impl Bench {
    ...
    fn new(name: &str) -> Self {
        let folder = std::env::temp_dir().join(format!("quill-agent-board-{name}-{}", std::process::id()));
        ...
        let configuration = Configuration {
            ...
            model: std::env::var("QUILL_TEST_MODEL").ok().filter(|value| !value.trim().is_empty()),
            lease_minutes: 45,
    ...
    fn ticket(&mut self, title: &str, description: &str) -> String {
        ...
        let sprint = store
            .create_sprint("Current Sprint", quill_app::services::agent_tasks::model::SprintStatus::Active, &now())
            .expect("a sprint");
        let task = store
            .create_task(
                NewTask {
...
fn now() -> String {
    quill_app::services::agent_tasks::clock::now()
}
...
fn the_board_shows_the_agents_work_while_it_is_happening() {
    ...
    let said = format!("quill-agent-board-said-{}.txt", std::process::id());
...
fn a_claim_whose_worker_is_gone_is_struck_and_the_board_says_why() {
    ...
    let an_hour_ago = quill_app::services::agent_tasks::clock::from_unix(
        quill_app::services::agent_tasks::clock::to_unix(&now()).expect("now") - 60 * 60,
    );
...
fn the_board_on_this_machine_opens_and_can_be_claimed_on() {
    ...
    let folder = std::env::temp_dir().join(format!("quill-real-board-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&folder);
    std::fs::create_dir_all(&folder).expect("a folder");
    let copy = folder.join("board.sqlite3");
    // Through Quill's own copy, which is `VACUUM INTO` rather than a file copy: this board is in write ahead
