...
//! visible.
//!
//! `cargo run --release -p quill-app --example frame_cost -- <file> [width]`
//!
...
use quill_app::services::text_renderer::TextRenderer;
use quill_core::{layout, relayout, Command, Document, Layout};
...
fn collect_glyphs(renderer: &TextRenderer, lines: &[quill_core::PlacedLine]) -> usize {
...
fn main() {
    ...
    let path = arguments.next().unwrap_or_else(|| "crates/quill-app/src/app/mod.rs".to_owned());
    ...
    let (plugins, _) = quill_app::services::plugins::Plugins::load(None);
    let coloured = plugins.for_path(std::path::Path::new(&path)).map(|plugin| {
        let base = quill_core::Color::rgb(0xF2, 0xF2, 0xF2);
        let start = Instant::now();
        let tokens = quill_core::syntax::highlight(&source, &plugin.grammar);
        let tokenised = start.elapsed().as_secs_f64() * 1000.0;
        let spans: Vec<(std::ops::Range<usize>, quill_core::Color)> = tokens
            .into_iter()
    ...
    let style = quill_core::CharStyle::default();
    let ms = timed(200_000, || {
        std::hint::black_box(quill_core::FontMetrics::advance(&renderer, "a", &style));
    });
