//! Run a program in Quill's terminal and write a picture of the window, for a person to look at.
//!
...
fn main() {
    ...
    let mut harness = Harness::builder().with_size(window).wgpu().build_eframe(move |cc| {
        let mut app = QuillApp::new(project);
...
fn save(harness: &mut Harness<'static, QuillApp>, path: &std::path::Path) {
