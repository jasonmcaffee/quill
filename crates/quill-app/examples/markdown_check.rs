...
use quill_core::markdown::{self, Options};
use quill_core::{CharStyle, PreviewColors};
...
fn main() {
    ...
    for path in &files {
        ...
        let tables = preview
            .panels
            .iter()
            .filter(|panel| panel.kind == quill_core::PanelKind::Table)
            .count();
        let code = preview
            .panels
            .iter()
            .filter(|panel| panel.kind == quill_core::PanelKind::Code)
            .count();
...
fn problems(preview: &quill_core::Preview) -> Vec<String> {
    let lines = preview.text.len_lines();
    let mut out = Vec::new();
