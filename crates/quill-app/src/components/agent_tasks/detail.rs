...
//!
//! The description is markdown and Quill already reads markdown, so it is drawn by the same reader the
...
pub(crate) fn terminal(
    ...
    if outcome.take_focus {
        // Both halves, because they are two different things. `QuillApp::focus` is the one value that says who in
