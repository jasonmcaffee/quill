//! A ticket's description, drawn by Quill's own editor.
//!
//! The description is markdown and Quill reads and draws markdown, so it is a `quill_core::Document` and
...
///
/// `components::markdown_text` is what renders and paints it, which is `quill_core::markdown` and the editor's
