...
//! holds, and `QuillApp` writes `.quill/run-configurations.conf` when the dialog closes — by the
...
/// How large the modal is before it is shrunk to fit a small Quill window.
///
...
pub fn show(
    ...
    let (inner, should_close) = modal::show(ctx, "quill-run-configurations", WIDTH, HEIGHT, |ui, area| {
