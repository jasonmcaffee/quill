//! The About box: who wrote Quill, what version this is, and when it was built.
//!
//! `Quill -> About Quill` used to write one line into the status bar, which is gone as soon as
...
impl About {
    /// What this build of Quill is, from the one place either fact is read.
    pub fn current() -> Self {
        Self {
...
pub fn show(ctx: &egui::Context, about: &About) -> bool {
    let (pressed, closed) = modal::show(ctx, "quill-about", WIDTH, HEIGHT, |ui, area| {
        let crossed = modal::header(ui, area, "About Quill");
