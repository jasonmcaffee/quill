...
//!
//! `components::markdown_text` is `quill_core::markdown::render` plus a layout, which is exactly what
...
use quill_chat::model::{Message, Part, Role, ToolCall};
...
fn pieces(
    ...
    // plain text, which is within a point or two of what `quill_core` will lay the markdown out at,
...
///
/// `StatusTopicEl`'s own shape, in Quill's palette: open while it is running and collapsed once it
...
fn tool_block(
    ...
    // Quill's blue when it worked, red when it did not.
...
///
/// egui's own layout rather than `quill_core`'s, because this is asked before the markdown has been
