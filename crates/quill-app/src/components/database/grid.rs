...
fn where_and_order(
    ...
    // Enter sends it, which is what a filter field does everywhere else in Quill. Typing alone must
...
/// string the same way is a grid nobody could trust — which is the reason `quill_db::Value` keeps them
