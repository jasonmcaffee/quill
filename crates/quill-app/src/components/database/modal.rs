...
fn source_modal(
    ...
    let (_, escaped) = modal::show(ctx, "quill-database-source", 520.0, 470.0, |ui, area| {
    ...
    match secret {
        ...
        Secret::None => {
            "Kept in this machine’s own credential store - the keychain on macOS, the Secret Service \
             on Linux, Credential Manager on Windows. Quill writes no password to a file, ever: \
             what the settings file records is the name of the entry."
        }
    }
}
...
///
/// **The server’s own version string**, rather than a sentence of Quill’s about it, which is
/// `quill-git`’s rule about quoting a program. Read only for the test whatever the data source is
...
///
/// **The statement on the right is the one that will be sent**, from `quill_db::sql::create_table`,
...
fn new_table(
    ...
    let statement = quill_db::sql::create_table(&form.schema, &form.name, &form.columns, engine);

    let (_, escaped) = modal::show(ctx, "quill-database-new-table", 760.0, 520.0, |ui, area| {
