...
//! highlighter**, not a second copy of Quill's editor. It has selection, undo and the clipboard, and
...
fn sql_editor(
    ...
    if let Some(state) = egui::TextEdit::load_state(child.ctx(), response.id) {
        if let Some(range) = state.cursor.char_range() {
            // egui counts the caret in **characters** and `quill_db::sql` reads **bytes**, so the two
...
fn the_results(
    ...
    match (&result, &failure) {
        (_, Some(why)) => {
            ...
            // rule `quill-git` keeps about quoting a program rather than summarising it.
