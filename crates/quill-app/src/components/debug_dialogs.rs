...
//! they agree.
//!
//! Neither knows what it is for. The window holds them and `QuillApp::run_action` decides what an
...
pub fn breakpoint(ctx: &egui::Context, dialog: &mut BreakpointDialog) -> DialogOutcome {
    ...
    let (_, closed) = modal::show(ctx, "quill-breakpoint", BREAKPOINT_WIDTH, BREAKPOINT_HEIGHT, |ui, area| {
...
pub fn evaluate(ctx: &egui::Context, dialog: &mut EvaluateDialog, paused: bool) -> DialogOutcome {
    let mut outcome = DialogOutcome::default();
    let (_, closed) = modal::show(ctx, "quill-evaluate", EVALUATE_WIDTH, EVALUATE_HEIGHT, |ui, area| {
