//! `Settings -> Tools -> MCP`: how an AI agent is given Quill.
//!
...
//!
//! The buttons write the **stdio** server: the agent launches `quill-cli mcp serve` itself, nothing
...
//! first is the ordinary one: a page is a fixed rectangle and a settings page in Quill does not
...
use quill_cli::mcp::install::{self, Client, Wanted};
use quill_cli::mcp::Shape;
...
pub struct McpState {
    ...
    pub port: String,
    /// Whether each client already has Quill, read when the page opens and after every click.
...
fn install_section(
    ...
    pen = note(
        ...
        area,
        pen,
        "Writes Quill into the agent's own configuration. The agent then launches Quill's server \
    ...
    match &state.said {
        ...
        None => note(
            ...
            area,
            pen,
            "Restart the agent afterwards. In Claude Code, `/mcp` then lists Quill's tools.",
        ),
    }
}
...
fn server_section(
    ...
    pen = note(
        ...
        pen,
        &format!(
            "{} tools. One an area names every command Quill has for about a third of the context \
             one a command costs; one a command lets an agent permit a single tool by name.",
            quill_cli::mcp::tools::tools(settings.mcp_tools).len()
        ),
    );
