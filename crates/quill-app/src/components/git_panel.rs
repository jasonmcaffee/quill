...
//! staging everything at the moment of commit — means Quill's idea of what is staged and git's
//! disagree for as long as the panel is open, and anyone who runs `git status` in Quill's own
//! terminal meanwhile sees the disagreement. Staging as you tick keeps one truth.

use egui::{Pos2, Rect, Sense, Vec2};
use quill_git::status::Entry;
use quill_git::Status;
...
pub fn show(
    ...
    let (_, closed) = modal::show(ctx, "quill-commit", WIDTH, HEIGHT, |ui, area| {
...
fn recent_messages(ui: &mut egui::Ui, counts: Rect, panel: &mut CommitPanel, recent: &[String]) {
    ...
    let chosen = egui::Popup::new(
        egui::Id::new("quill-commit-messages"),
        ui.ctx().clone(),
        clock.left_bottom(),
        ui.layer_id(),
    )
