...
//! `bundled` feature compiles SQLite's own C into Quill, so there is nothing to install, nothing
...
//! the file was last written by, and a file from a **newer** Quill is refused with a message rather
...
/// What this version of Quill writes. A file saying more than this is refused.
///
...
impl Store {
    ...
    ///
    /// `~/Library/Application Support/Quill/plugins/agent-tasks/board.sqlite3` on macOS, and the
    ...
    /// Refuse a file a newer Quill wrote.
    ///
    ...
    fn check_version(&self) -> Result<(), String> {
        ...
        match found {
            Some(version) if version > SCHEMA_VERSION => Err(format!(
                "this board was written by a newer Quill: it is schema {version} and this Quill reads {SCHEMA_VERSION}"
            )),
            _ => Ok(()),
        }
    }

    /// Write down which schema this file is at now, which is this Quill's, since [`migrate`] has just brought it
    /// there.
    ///
    ...
    ///
    /// Two Quill windows can have the same board file open, and each runs its own watchdog over the same
...
mod tests {
    ...
    fn opening_a_file_that_is_not_there_makes_the_schema_and_opening_it_again_changes_nothing() {
        let folder = std::env::temp_dir().join(format!("quill-board-{}", std::process::id()));
    ...
    fn a_board_made_before_owner_existed_gains_it_and_a_ticket_can_then_be_claimed() {
        let folder = std::env::temp_dir().join(format!("quill-board-owner-{}", std::process::id()));
        ...
        // Opening it again is what migrates it, which is what happens when Quill starts.
        let again = Store::open(&path).expect("the same board");
    ...
    fn the_copy_taken_before_a_clear_holds_the_tickets_that_were_there() {
        let folder = std::env::temp_dir().join(format!("quill-board-clear-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&folder);
        let path = folder.join(FILE);
    ...
    #[test]
    fn a_board_written_by_a_newer_quill_is_refused_with_a_message() {
        ...
        assert!(problem.contains("newer Quill"), "{problem}");
