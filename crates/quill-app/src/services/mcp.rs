...
//!
//! The protocol, the tools and both transports live in `quill_cli::mcp`, where the client can reach
...
//!
//! The endpoint this starts drives Quill exactly as `quill-cli` does — down the control channel, with
...
//!
//! - **One server drives every window.** It finds Quills by reading the instances folder, so the
//!   endpoint hosted by this window answers for the one next to it too, with `instance` saying
//!   which. That turns the obvious collision — two Quills, one `mcp.port` — from a bug into the
...
use quill_cli::mcp::{self, http::Endpoint, Quills, Server, Shape};
...
pub enum State {
    ...
    Listening(u16),
    /// The setting is on, but the port is held by something else — nearly always the Quill in the
    /// next window, whose endpoint serves this one too.
    PortTaken(u16, String),
    /// The setting is on and there is no command channel to drive, because Quill was started with
    /// `--control off`.
    NoChannel,
}
...
impl State {
    ...
    pub fn message(&self) -> String {
        match self {
            State::Off => {
                "Not listening. Agents that launch Quill's server themselves still work.".to_owned()
            }
            State::Listening(port) => format!("Listening at {}", mcp::endpoint(*port)),
            State::PortTaken(port, problem) => format!(
                "Port {port} is already in use ({problem}). If that is another Quill, its server \
                 drives this window too."
            ),
            State::NoChannel => {
                "Quill was started with --control off, so there is no command channel for an MCP \
                 server to drive."
                    .to_owned()
            }
        }
...
pub struct Hosted {
    ...
    /// preference for which Quill to drive is that folder.
...
impl Hosted {
    ...
    ///
    /// `has_channel` is false when Quill was started with `--control off`.
    pub fn reconcile(
        &mut self,
        enabled: bool,
        port: u16,
        ...
        self.endpoint = None;
        let server = Server::new(shape, Quills::for_window(folder.to_path_buf()));
...
mod tests {
    ...
    /// driver when several Quills are running.
    ...
    fn a_port_somebody_else_holds_is_reported_rather_than_swallowed() {
        ...
        assert!(hosted.state().message().contains("another Quill"));
    }
}
