...
//!
//! `quill_core::markdown` says where a picture goes and what file it names; it cannot read a file or
...
//! read. Quill makes no network requests, and a preview that quietly fetched from the internet while
...
impl PreviewImages {
    ...
    pub fn ready(
        ...
        let name = format!("quill-preview-{}", path.display());
...
/// somebody who writes one means that file, and Quill already opens any file the person running it
/// can read.
...
mod tests {
    ...
    fn folder() -> PathBuf {
        let folder = std::env::temp_dir().join("quill-preview-images");
