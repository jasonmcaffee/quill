...
//!
//! `quill_core::symbols` says what a definition *is*; this says where the project's are. It is the
...
use quill_core::symbols::{self, Confidence, SymbolKind};
...
mod tests {
    ...
    fn the_index_holds_what_the_project_defines_and_says_where() {
        let (folder, files) = a_project("quill-symbol-index-build");
    ...
    fn only_the_files_a_language_claims_are_read_at_all() {
        ...
        let (folder, files) = a_project("quill-symbol-index-claims");
        let index = Index::build(&files, &grammars(), &|| false).expect("a build");
    ...
    fn a_build_that_has_been_overtaken_stops_where_it_is() {
        ...
        let (folder, files) = a_project("quill-symbol-index-cancel");
    ...
    fn reading_the_same_project_twice_gives_an_identical_index() {
        ...
        let (folder, files) = a_project("quill-symbol-index-determinism");
    ...
    fn a_file_that_is_not_text_or_cannot_be_read_is_skipped_rather_than_fatal() {
        let folder = folder("quill-symbol-index-unreadable");
    ...
    fn the_thread_answers_the_newest_question_and_abandons_the_ones_before_it() {
        let (folder, files) = a_project("quill-symbol-index-thread");
