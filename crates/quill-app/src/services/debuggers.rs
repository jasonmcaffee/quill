...
//! sentence, as the debug tile's Install button, and as `quill-cli debug adapters` and
...
//! `%LOCALAPPDATA%\Quill\adapters` — no elevation, fifty megabytes rather than LLVM's two and a half
...
pub struct Debugger {
    ...
    /// refusal into something a person or an agent can act on without leaving Quill.
    pub install: Install,
...
impl Install {
    /// The command for the machine Quill is running on.
...
impl Debugger {
    ...
    /// answer means Quill has nothing to offer here, and the refusal then says what the settings key
...
/// installed Quill that has no source checkout beside it — and because a person reading it in the
/// status bar can see exactly what it will do before pressing anything. **Quill does not run this by
...
pub const CODELLDB_FETCH: &str = concat!(
    ...
    r"$into=Join-Path $env:LOCALAPPDATA 'Quill\adapters'; ",
...
pub const JS_DEBUG_FETCH_UNIX: &str = concat!(
    "sh -c \"",
    "set -e; into=$HOME/.local/share/quill/adapters; mkdir -p $into; ",
...
pub const ALL: &[Debugger] = &[
    Debugger {
        ...
        // On Windows, CodeLLDB's own `.vsix` unpacked into Quill's adapters folder: it is a zip
        ...
        // written out as one command so an installed Quill with no source checkout can offer it.
    ...
    Debugger {
        ...
        // installing the extension is not a way to get the thing Quill needs, and the release asset
        // is.
        install: Install {
...
pub enum Refusal {
    ...
    /// empty when this version of Quill does not know the adapter at all and so cannot say.
    NotInstalled {
        name: String,
...
fn prepare_lldb(
    ...
    if let Some(tool) = build_tool(&program) {
        return Err(Refusal::BuildTool {
            ...
            advice: "Quill debugs `cargo run` and `cargo test` by building them first; anything else needs a configuration that names the built binary, such as `target\\debug\\myapp.exe`.".to_owned(),
        });
    }
...
/// as it was written; `PATH` is the machine's own answer; [`well_known`] is Quill guessing, correctly
...
fn codelldb() -> Option<PathBuf> {
    ...
    // Quill's own folder first, which is where `tools/get-debug-adapter.ps1` and the Install button
    ...
    if let Some(own) = quill_adapters()
...
/// Where Quill keeps an adapter it unpacked for itself.
///
...
pub fn quill_adapters() -> Option<PathBuf> {
    match cfg!(windows) {
        true => std::env::var_os("LOCALAPPDATA").map(|local| PathBuf::from(local).join("Quill").join("adapters")),
        false => home_folder().map(|home| home.join(".local").join("share").join("quill").join("adapters")),
    }
}
...
fn js_debug() -> Option<PathBuf> {
    // Quill's own folder first, which is where the release asset unpacks to.
    if let Some(own) = quill_adapters()
...
/// `QuillApp::adapter_for`, for a command line none of this recognises.
///
...
/// What `quill-cli debug adapters` says about one debugger, and what the debug tile's own panel
...
pub struct Report {
    ...
    pub comes_from: &'static str,
    /// The command that installs it on this machine, empty when Quill has nothing to offer.
    pub install: String,
...
mod tests {
    ...
    fn the_command_line_says_which_debugger_it_wants() {
        ...
        assert_eq!(adapter_for("./target/debug/quill"), Some("lldb"), "no extension is a program");
    ...
    fn cargo_is_debugged_as_the_binary_it_built_rather_than_being_refused() {
        ...
        let built =
            Some(("C:\\p\\target\\debug\\quill.exe".to_owned(), vec!["--control".to_owned(), "off".to_owned()]));
        ...
        assert_eq!(prepared.body["program"], "C:\\p\\target\\debug\\quill.exe");
