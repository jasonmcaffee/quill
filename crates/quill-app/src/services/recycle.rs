...
//! new crate and cannot be compiled, let alone tested, on the machine Quill is built on.
//!
...
pub enum Destination {
    ...
    Bin,
    /// Gone. Nothing in Quill can bring it back.
    Gone,
}
...
///
/// `FOF_NOCONFIRMATION` and `FOF_SILENT` are asked for because Quill has already asked the question
...
fn to_the_recycle_bin(path: &Path) -> std::io::Result<()> {
    ...
    let plain = quill_terminal::paths::plain(&full);
...
mod tests {
    ...
    fn folder(name: &str) -> std::path::PathBuf {
        let root = std::env::temp_dir().join("quill-recycle").join(name);
