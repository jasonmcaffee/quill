...
//!
//! `quill_core::mermaid` turns a diagram's source into a `Scene`, and that is arithmetic rather than
...
use quill_core::mermaid::{self, Options, Problem, Scene, Theme};
use quill_core::metrics::{FontMetrics, LineMetrics};
use quill_core::CharStyle;

/// Measuring a diagram's text with the fonts `egui` will actually draw it in.
///
/// **A diagram is the one thing in the window that `quill-core` measures and `egui` draws.** The
/// editor measures through `TextRenderer` and paints through its own glyph atlas, so those two agree
/// by construction; a diagram's boxes are worked out in `quill-core` and its text is handed to
...
/// like any other, and `quill-core` still knows nothing about what is drawing.
...
/// Quill's palette, as a diagram's theme.
///
...
pub fn theme() -> Theme {
    ...
    let of = |colour: egui::Color32| quill_core::Color::rgb(colour.r(), colour.g(), colour.b());
    Theme {
        text: of(color::TEXT),
        dim: of(color::TEXT_DIM),
        node_fill: quill_core::mermaid::Paint::solid(of(color::TITLE_BAR)),
        node_stroke: of(color::CONTROL_BORDER),
        group_fill: quill_core::mermaid::Paint::faded(of(color::ACCENT), 26),
        ...
        series: [
            ...
            quill_core::Color::rgb(0xE0, 0x7A, 0x5F),
            quill_core::Color::rgb(0x9A, 0x8C, 0xE8),
            of(color::BLAME_OLD),
        ],
    }
}
...
mod tests {
    ...
    #[test]
    fn the_theme_is_quills_own_palette_and_every_series_colour_differs() {
        let theme = theme();
        assert_eq!(theme.accent, quill_core::Color::rgb(0x48, 0x9F, 0xF8));
    ...
    #[test]
    fn a_diagram_drawn_in_quills_palette_still_keeps_every_property() {
        ...
        options.theme = theme();
        let scene = quill_core::mermaid::render("flowchart LR\n A[One] --> B[Two]\n", &options)
