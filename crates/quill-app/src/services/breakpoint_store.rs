//! The project's breakpoints, kept beside the project in `.quill/breakpoints.conf`.
//!
...
//! and `quill_core::Breakpoints` inside each — sorted by offset, so the one on a line is a binary
...
//! ```
//!
//! Read and written by [`crate::services::store::Values`] like every other file Quill writes. A block
...
//!
//! Offsets are bytes against the file as Quill last had it. A file rewritten outside Quill moves its
//! own bytes and Quill cannot know it, so `Document::set_breakpoints` clamps them: the worst case is
...
//!
//! [`BreakpointStore::load`] is called from `QuillApp::restore_project` and nowhere else, exactly as
//! the marks and the project state are: a test must not touch a person's files, and a `.quill` folder
