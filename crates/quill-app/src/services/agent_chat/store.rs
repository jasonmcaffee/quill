...
use quill_chat::base64;
use quill_chat::model::{Conversation, Message, Part, Role, ToolCall, Usage};
...
mod tests {
    ...
    fn a_folder(name: &str) -> PathBuf {
        let folder = std::env::temp_dir().join(format!("quill-agent-chat-{name}-{}", std::process::id()));
    ...
    fn a_conversation_with_a_picture_in_it_comes_back_byte_for_byte() {
        ...
        answered.tools.push(ToolCall::new("t1", "quill_git", "{}"));
    ...
    fn a_store_with_no_folder_does_nothing_at_all() {
        ...
        // `QuillApp::load_settings` and `services::file_marks` already keep.
