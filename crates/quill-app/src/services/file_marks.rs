//! What Quill remembers about the *files* in a project, kept beside the project like everything else.
//!
...
//! - Inside a file, `quill_core::Highlights` — sorted by where each mark starts and never
//!   overlapping — so the one under the pointer is a binary search and the handful on the screen is
//!   a binary search and a walk.
//! - **One file on the disk for the whole project**, `.quill/highlights.txt`, rather than one beside
...
//!
//! Offsets are bytes, against the file as it was when Quill last had it. A file rewritten outside
//! Quill — a `git checkout`, another editor — moves its own bytes and Quill cannot know it, so the
...
//!
//! [`FileMarks::load`] is called from `QuillApp::restore_project` and from nowhere else, exactly as
//! the project state is: a test must not touch a person's files, and a `.quill` folder written into
...
use quill_core::highlights::{Highlight, Highlights, Rgba};
...
/// What Quill remembers about the files in one project.
...
impl FileMarks {
    ...
    pub fn save(&mut self, root: &Path) {
        ...
        if self.files.values().all(quill_core::Highlights::is_empty) {
    ...
    /// Quill that marked the same passages write the same file.
    ...
    pub fn total(&self) -> usize {
        self.files.values().map(quill_core::Highlights::len).sum()
    }
...
mod tests {
    ...
    fn a_project_that_has_never_been_marked_holds_nothing() {
        let root = project("quill-file-marks-fresh");
    ...
    fn what_was_marked_is_what_comes_back() {
        let root = project("quill-file-marks-round-trip");
    ...
    fn the_paths_are_written_relative_so_a_project_that_moves_keeps_its_marks() {
        let root = project("quill-file-marks-relative");
        let mut marks = FileMarks::new();
    ...
    fn nothing_is_written_until_something_changes() {
        let root = project("quill-file-marks-dirty");
        let mut marks = FileMarks::new();
    ...
    fn clearing_a_file_empties_it_on_the_disk_rather_than_leaving_what_was_cleared() {
        let root = project("quill-file-marks-cleared");
