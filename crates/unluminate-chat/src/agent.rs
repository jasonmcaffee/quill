...
//!
//! - **Unluminate holds no key.** There is nothing to put in a settings file, nothing to read out of an
//!   environment variable, nothing to redact out of an error message. The agent's own credentials
//!   are the agent's own business.
//! - **The agent brings its own tools**, its own sandbox and its own permission model. Unluminate does not
...
//!   you without a word of it being uploaded by Unluminate.
...
//! that is how Unluminate sends one — not as the last word of the command line. **Because a batch file
...
//! *"batch file arguments are invalid"*. Unluminate's first line tells the agent it is answering in a
...
//! `unluminate_terminal::Session::kill` does and for the same reason: the alternative is a window that
...
pub fn run(
    ...
    #[cfg(windows)]
    {
        // No console window, which is what `unluminate_git` does for the same reason: a flashing black
    ...
    // says is kept for the refusal, which is `unluminate-git`'s rule about git's own stderr.
...
impl Decoder {
    ...
    fn claude(&mut self, value: &serde_json::Value) -> Vec<Reply> {
        ...
        match value["type"].as_str().unwrap_or_default() {
            ...
            // **The results of the tools the agent ran itself.** Unluminate did not run them and does not
            ...
            "result" => {
                ...
                // **An error is the agent's own words**, which is `unluminate-git`'s rule: it explains
                // itself better than Unluminate could.
    ...
    fn item(&mut self, item: &serde_json::Value, completed: bool) -> Vec<Reply> {
        ...
        match item["type"].as_str().unwrap_or_default() {
            ...
            // Unluminate's, so a tool block means one thing whichever agent produced it.
            "command_execution" => {
                let mut out = Vec::new();
...
pub fn write_a_picture(folder: &Path, name: &str, bytes: &[u8]) -> Option<std::path::PathBuf> {
    ...
    for _ in 0..64 {
        ...
        let path = folder.join(format!("unluminate-chat-{}-{at}-{safe}", std::process::id()));
...
mod tests {
    ...
    fn a_second_question_carries_the_agents_own_session_rather_than_the_transcript() {
        ...
        // context the agent has built is the context it keeps, and Unluminate sends one turn's words.
        let ask = Ask {
    ...
    fn a_tool_the_agent_ran_itself_is_shown_with_the_answer_it_got() {
        // **Unluminate runs nothing here.** The agent has its own tools and its own permission model, so
    ...
    fn a_program_is_found_the_way_a_shell_finds_one() {
        ...
        assert!(crate::provider::program("unluminate-no-such-agent-anywhere").is_none());
        assert!(crate::provider::program("/definitely/not/here/claude").is_none());
        let folder = std::env::temp_dir().join(format!("unluminate-chat-program-{}", std::process::id()));
        std::fs::create_dir_all(&folder).expect("a folder");
        let name = match cfg!(windows) {
            true => "unluminate-chat-test-agent.exe",
            false => "unluminate-chat-test-agent",
        };
    ...
    fn an_agent_that_is_not_installed_says_so_before_anything_is_run() {
        ...
        provider.command = "unluminate-no-such-agent-anywhere".to_owned();
        let why = provider.why_not().expect("a refusal");
        assert!(why.contains("unluminate-no-such-agent-anywhere"), "{why}");
