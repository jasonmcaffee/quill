...
//! tools, one that fails after saying something. `unluminate_dap::session` is arranged the same way and
//! for the same reason.
//!
...
impl Session {
    ...
    pub fn reply(&mut self, reply: Reply) {
        match reply {
            ...
            // **A tool the agent ran itself.** Unluminate did not run it and has nothing to send back: it
    ...
    /// running for ever and the turn stopped with no way out of it. A position is Unluminate's own and
    /// cannot collide.
