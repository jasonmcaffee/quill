...
pub struct Message {
    ...
    ///
    /// Empty in the ordinary case, because Unluminate does not ask for extended thinking. A gateway that
    ...
    /// The server's own words, when the request failed. Never invented — see `unluminate-git`'s rule.
...
impl Message {
    ...
    /// assistant message holding only Unluminate's own note about why — and sent back as
...
pub struct Conversation {
    ...
    /// HTTP endpoint, which keeps no session of its own — there the transcript Unluminate holds *is* the
