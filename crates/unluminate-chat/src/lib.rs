...
//! chose. `unluminate-dap` is arranged the same way and for the same reason: its tests run against
...
//! serialise. That is the choice `unluminate-cli/src/protocol.rs` and `unluminate-dap` already made.
//!
...
//! the better one. Unluminate holds **no key at all**; the agent brings its own tools, its own sandbox and
...
//! one part of Unluminate that opens a socket or starts a program.
