...
//! - [`messages`] — the requests, responses and events Quill uses, typed, with the JSON written by
//!   hand for the reason `quill-cli/src/protocol.rs` writes its own by hand: this is an open
...
//!   on, arranged exactly as `quill_git::Worker` is.
//!
//! **No user interface dependency**, for the reason `quill-core` and `quill-terminal` have none:
...
//! `quill_app::services::debuggers` beside the settings file that can override it.
