...
//! `unluminate-chat`'s server-sent-event reader arranged the same way and for the same reason: it makes
...
/// the process — the rule `unluminate_chat::sse::LARGEST_EVENT` already keeps. A row can legitimately be
...
fn read(tag: u8, body: &[u8]) -> Result<Message, Failure> {
    ...
    Ok(match tag {
        b'R' => match at.int32()? {
            ...
            other => {
                return Err(Failure::said(format!(
                    "the server asked for authentication method {other}, and Unluminate speaks \
...
mod tests {
    ...
    fn the_same_stream_split_at_every_byte_boundary_produces_the_same_frames() {
        // `unluminate-chat`'s test, applied to a different protocol. A message that arrives in three reads
