...
/// flag — which is the fault `unluminate-chat` records about stopping an agent, and the reason stopping a
...
impl Session {
    ...
    pub fn connect(source: &Source, password: Option<&str>) -> Answer<Session> {
        ...
        if source.read_only {
            // **The guarantee is the server's, not a parser's.** Unluminate also hides the editing
            ...
            match self.next_message()? {
                ...
                Message::AuthenticationSasl(mechanisms) => {
                    if !mechanisms.iter().any(|name| name == scram::MECHANISM) {
                        return Err(Failure::said(format!(
                            "the server offers {} and Unluminate speaks {}.",
                            mechanisms.join(", "),
                            scram::MECHANISM
                        )));
                    }
...
fn sending(why: std::io::Error) -> Failure {
    Failure::said(format!("the connection would not take what Unluminate sent: {why}"))
}

fn unexpected(message: &Message, wanted: &str) -> Failure {
    Failure::said(format!("the server sent {message:?} where Unluminate was waiting for {wanted}."))
}
