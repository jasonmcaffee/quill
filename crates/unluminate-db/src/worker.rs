...
//!
//! `unluminate_git::Worker` with a different payload, and the argument for it is the same one: the window
...
//! symbol index and `unluminate-dap` all already use.
//!
...
pub enum Job {
    ...
    /// A statement Unluminate composed, with values bound.
...
impl Worker {
    ...
    pub fn open(
        ...
        let thread = std::thread::Builder::new()
            .name(format!("unluminate-db {}", source.name))
            .spawn(move || {
...
mod tests {
    ...
    fn a_database(name: &str) -> std::path::PathBuf {
        let folder = std::env::temp_dir().join(format!("unluminate-db-worker-{}-{name}", std::process::id()));
