...
impl Session {
    ...
    /// rather than a check in Unluminate — the same arrangement the PostgreSQL session makes with a
...
mod tests {
    ...
    fn a_database(name: &str) -> PathBuf {
        let folder = std::env::temp_dir().join(format!("unluminate-db-{}-{name}", std::process::id()));
    ...
    fn a_file_that_is_not_there_is_said_rather_than_created() {
        ...
        let missing = std::env::temp_dir().join("unluminate-db-no-such-file-ever.db");
    ...
    fn a_table_that_declares_its_own_rowid_column_does_not_get_it_as_a_key() {
        ...
        // exists to prevent.
        let folder = std::env::temp_dir().join(format!("unluminate-db-shadow-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&folder);
    ...
    #[test]
    fn a_read_only_source_is_refused_by_sqlite_rather_than_by_unluminate() {
        // The guarantee is the engine's. Unluminate also hides the editing controls, but a statement that
