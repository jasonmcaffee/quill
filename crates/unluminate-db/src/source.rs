...
impl Engine {
    ...
    pub fn column_types(self) -> &'static [&'static str] {
        match self {
            Engine::Sqlite => &["TEXT", "INTEGER", "REAL", "BLOB", "NUMERIC", "BOOLEAN", "DATE", "DATETIME"],
            Engine::Postgres => &[
                "text",
                "varchar(255)",
                "integer",
    ...
    pub fn key_column(self, type_name: &str) -> String {
        ...
        match self {
            Engine::Sqlite if type_name.eq_ignore_ascii_case("integer") => "INTEGER".to_owned(),
...
///
/// **`Debug` is written by hand and redacts the one variant that holds a value.** Nothing in Unluminate
...
pub enum Secret {
    ...
    ///
    /// The only route on Windows, which has no keychain Unluminate can write to — see
...
pub struct Source {
    ...
    pub ssl: SslMode,
    pub secret: Secret,
    /// The server enforces this, and Unluminate hides the editing controls as well. See
    ...
    pub fn parse(name: &str, text: &str) -> Answer<Source> {
        ...
        if text.contains("://") {
            ...
            return Err(Failure::said(format!(
                "`{scheme}` is not an engine this version of Unluminate speaks, and it speaks {}.",
                Engine::ALL.join(", ")
            )));
        }
...
fn parse_postgres(name: &str, rest: &str) -> Answer<Source> {
    ...
    if let Some((user_only, _)) = user.split_once(':') {
        ...
        return Err(Failure::said(format!(
            "that URL has a password in it, and Unluminate does not write one down. Set it as an \
            ...
            match key {
                "sslmode" => {
                    source.ssl = SslMode::from_name(value).ok_or_else(|| {
                        Failure::said(format!(
                            "sslmode is `{value}`, and Unluminate has {}.",
                            SslMode::ALL.join(", ")
                        ))
                    })?
                }
                ...
                "password" => {
                    return Err(Failure::said(
                        "that URL has a password in it, and Unluminate does not write one down. Name an \
                ...
                // rule every other named-value key in Unluminate keeps.
                other => {
                    return Err(Failure::said(format!(
                        "`{other}` is not something Unluminate's PostgreSQL connections take, and they \
...
mod tests {
    ...
    fn anything_that_is_not_a_url_is_a_sqlite_file() {
        let source = Source::parse("tasks", r"C:\jason\dev\unluminate\tasks.db").expect("read");
        assert_eq!(source.engine, Engine::Sqlite);
        assert_eq!(source.database, r"C:\jason\dev\unluminate\tasks.db");
    ...
    fn a_typed_password_is_redacted_even_by_debug() {
        ...
        assert_eq!(
            format!("{:?}", Secret::Environment("UNLUMINATE_DB_AI".to_owned())),
            "Environment(\"UNLUMINATE_DB_AI\")"
        );
    }
