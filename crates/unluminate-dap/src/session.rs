...
//! **every optional feature asks them first**, so Unluminate never sends what an adapter did not offer.
//! Then `launch`, whose body the caller built, because each adapter names the program and its
//! arguments slightly differently and that knowledge is Unluminate's rather than the protocol's.
//!
...
/// Two hundred is `unluminate_app::git::HISTORY_LIMIT`'s reasoning applied to a stack: far more than
...
pub enum Event {
    ...
    /// The adapter refused something. Its own message, never one of Unluminate's.
...
pub struct Session {
    ...
    /// True once the adapter has asked to run the debuggee in Unluminate's terminal, so the session
...
impl Session {
    ...
    /// See `unluminate_app::app::debug::DebugState::is_ready`.
    ...
    ///
    /// Unluminate holds no list of its own — the names come from the capabilities — so an adapter that
    ...
    ///
    /// **Its own message, never one of Unluminate's** — a debugger explains a bad expression or a
    /// constant that cannot be assigned to far better than an editor could, which is the rule
    /// `unluminate-git` already keeps about git's standard error.
    fn on_refusal(
        &mut self,
        command: &str,
...
mod tests {
    ...
    /// Unluminate draws the adapter's answer rather than its own hope: an unbound breakpoint stays
    ...
    #[test]
    fn a_message_unluminate_does_not_read_changes_nothing() {
        let mut session = paused_session();
    ...
    /// The adapter's own words, never Unluminate's — the rule `unluminate-git` keeps about git's stderr.
    #[test]
