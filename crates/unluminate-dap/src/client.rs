//! The thread the adapter is spoken to on, arranged exactly as `unluminate_git::Worker` is.
//!
...
//! process Unluminate started, the same as any run.
...
///
/// `UNLUMINATE_DAP_TRACE=1` switches it on, read once. It exists because a protocol client with no way to
...
fn tracing() -> bool {
    ...
    *TRACING.get_or_init(|| {
        std::env::var("UNLUMINATE_DAP_TRACE").is_ok_and(|value| !matches!(value.as_str(), "" | "0"))
    })
}
...
pub enum Reply {
    /// One message the adapter sent. Boxed for the reason `unluminate_git::Reply::Snapshot` is: a
...
impl Client {
    ...
    pub fn start(command: &AdapterCommand, waker: Waker) -> Result<Self, String> {
        ...
        // explains itself, and swallowing that is the one thing `unluminate-git`'s rule about git's
        // stderr says never to do.
        if let Some(errors) = connection.errors {
            let said = sender.clone();
            let woken = waker.clone();
            std::thread::Builder::new()
                .name("unluminate-dap-errors".to_owned())
                .spawn(move || read_errors(errors, said, woken))
                .map_err(|problem| {
                    format!("Unluminate could not start a thread to read the debugger: {problem}")
                })?;
        }
        std::thread::Builder::new()
            .name("unluminate-dap".to_owned())
            .spawn(move || read_frames(reader, sender, waker))
            .map_err(|problem| format!("Unluminate could not start a thread to read the debugger: {problem}"))?;
        Ok(Self {
            replies: receiver,
    ...
    pub fn adopt_child(&mut self, command: &AdapterCommand, waker: Waker) -> Result<(), String> {
        ...
        std::thread::Builder::new()
            .name("unluminate-dap-child".to_owned())
            .spawn(move || read_child_frames(reader, sender, waker))
            .map_err(|problem| {
                format!("Unluminate could not start a thread to read the child session: {problem}")
            })?;
    ...
    ///
    /// What the screenshot tests use, exactly as `unluminate_terminal::Session::detached` is what the
    ...
    pub fn is_running(&mut self) -> bool {
        match self.child.as_mut() {
            ...
            // An adapter Unluminate did not start is somebody else's process, so its being there is not
...
mod tests {
    ...
    fn an_adapter_that_will_not_start_is_a_message_rather_than_a_client() {
        let waker: Waker = Arc::new(|| {});
        let command = AdapterCommand::stdio("unluminate-no-such-debug-adapter", Vec::new());
    ...
    #[test]
    fn an_adapter_unluminate_did_not_start_is_never_reported_as_dead() {
        let (sender, receiver) = std::sync::mpsc::channel();
        let mut client = Client {
            replies: receiver,
            writer: Box::new(Vec::new()),
