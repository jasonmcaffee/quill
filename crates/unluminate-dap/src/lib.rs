...
//! - [`messages`] — the requests, responses and events Unluminate uses, typed, with the JSON written by
//!   hand for the reason `unluminate-cli/src/protocol.rs` writes its own by hand: this is an open
...
//!   on, arranged exactly as `unluminate_git::Worker` is.
//!
//! **No user interface dependency**, for the reason `unluminate-core` and `unluminate-terminal` have none:
...
//! `unluminate_app::services::debuggers` beside the settings file that can override it.
