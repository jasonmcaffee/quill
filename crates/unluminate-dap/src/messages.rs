//! The messages Unluminate sends and the ones it reads, as Rust types with the JSON written by hand.
//!
//! **Only what Unluminate uses is modelled.** The protocol is large and deliberately additive — an
...
//! speak, so what crosses is a value. `unluminate-cli/src/protocol.rs` made the same choice about Unluminate's
//! own control channel.
//!
    ...
    pub fn arguments(&self) -> Option<Value> {
        Some(match self {
            Request::Initialize { client_id, lines_start_at_one } => json!({
                "clientID": client_id,
                "clientName": "Unluminate",
                "adapterID": "unluminate",
                "locale": "en",
...
/// so Unluminate's whole cost for two features the reference editor has is two `Option<String>`s and the modal that
/// edits them.
...
///
/// **Unluminate draws this rather than its own hope.** An adapter that moved a breakpoint to the next
...
///
/// **Every optional feature asks this first**, so Unluminate never sends what an adapter did not offer
/// and a control whose capability is absent is absent — the rule the `F` button already follows.
/// A capability this file does not name is one nothing in Unluminate gates on.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct Capabilities {
    ...
    /// The named exception filters this adapter offers, as `(filter, label, default)`. Unluminate holds
...
pub struct Stopped {
    ...
    /// The sentence an adapter adds for an exception, which is better than anything Unluminate could say.
...
/// Everything an adapter can say that Unluminate reads.
///
...
pub enum Message {
    ...
    Response {
        ...
        command: String,
        success: bool,
        /// The adapter's own message when it refused. **Never replaced with one of Unluminate's**: a
    ...
    /// id was. Unluminate answers it with the run tile.
...
fn read_reverse_request(value: &Value) -> Message {
    ...
    Message::RunInTerminal {
        seq: value.get("seq").and_then(Value::as_i64).unwrap_or(0),
        // `integrated` or `external`. Unluminate has one terminal and runs both in it, which is what
...
/// one often cannot say which process it became. Unluminate is one of those — `unluminate_terminal::Session`
...
pub fn run_in_terminal_response(
    ...
    match (started, process) {
        ...
        (false, _) => {
            message.insert(
                "message".to_owned(),
                json!("Unluminate could not start the program in its run tile."),
            );
        }
    }
    Value::Object(message)
...
mod tests {
    ...
    /// logs, so Unluminate's whole cost for two of the reference editor's features is two optional strings.
    #[test]
    ...
    /// The adapter's own refusal is carried whole, because nothing in Unluminate could say it better.
    #[test]
    ...
    #[test]
    fn the_events_unluminate_reads_are_read() {
    ...
    #[test]
    fn an_event_unluminate_does_not_read_is_carried_rather_than_dropped() {
