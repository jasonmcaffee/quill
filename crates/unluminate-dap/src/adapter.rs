...
//! on a port.
//!
//! Both shapes are common and Unluminate needs both, which is why this is a type rather than a
...
//!
//! Nothing here decides *which* adapter to run. That is `unluminate-app`'s registry, because knowing
...
pub enum Transport {
    ...
    /// server is one Unluminate has to launch, which is what js-debug needs; a `None` is an adapter
...
impl AdapterCommand {
    ...
    /// A server, started by Unluminate, connected to on `port`.
...
pub struct Connection {
    ...
    ///
    /// **Read rather than swallowed**, which is `unluminate-git`'s rule about git's stderr: nothing in
    /// Unluminate invents a message, and a debugger explains itself better than an editor could. It is not
...
fn start_server(command: &AdapterCommand, port: u16) -> Result<Connection, String> {
    ...
    let stream = loop {
        ...
        if started.elapsed() >= CONNECT_GRACE {
            ...
            return Err(format!(
                "Unluminate could not reach the debug adapter on 127.0.0.1:{port} or [::1]:{port}: {problem}"
            ));
        }
    ...
    let reader = stream
        .try_clone()
        .map_err(|problem| format!("Unluminate could not read from the debug adapter: {problem}"))?;
    Ok(Connection {
...
fn spawn(
    ...
    // inherited — which would print into whatever terminal Unluminate was started from — nor swallowed,
    ...
    {
        // Do not flash a console window, which is what `unluminate-git` does for each git command and for
        // the same reason. 0x08000000 is CREATE_NO_WINDOW.
        use std::os::windows::process::CommandExt;
        process.creation_flags(0x0800_0000);
    }
    process.spawn().map_err(|problem| {
        format!("Unluminate could not start {}: {problem}", program.display())
    })
}

fn standard_stream_missing(program: &std::path::Path) -> String {
    format!("{} was started but gave Unluminate no pipe to talk on.", program.display())
}
