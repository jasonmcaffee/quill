//! Run a program in Unluminate's terminal and write a picture of the window, for a person to look at.
//!
...
fn main() {
    ...
    let mut harness = Harness::builder().with_size(window).wgpu().build_eframe(move |cc| {
        let mut app = UnluminateApp::new(project);
...
fn save(harness: &mut Harness<'static, UnluminateApp>, path: &std::path::Path) {
