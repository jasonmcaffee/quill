//! What starting Unluminate costs, piece by piece, on this machine.
//!
//! `UNLUMINATE_FRAME_TRACE` says when the program had got to each step of `main`, which is what
//! `task-1805` used to find that a third of startup was a stale instance file being dialled. It
//! could say nothing about the one step that is not Unluminate's own code — creating the window and
...
//! for a DX12 device, against a whole Unluminate window's 220 MB and 435 MB — so most of what an
//! editor drawn on the graphics card holds is the driver's rather than the editor's, and there is no
//! amount of care inside Unluminate that would get it back.
//!
...
fn main() {
    println!("Starting Unluminate, piece by piece. Times are milliseconds on this machine.");
    println!();
    ...
    let renderer = unluminate_app::services::text_renderer::TextRenderer::new();
    println!(
    ...
    let start = Instant::now();
    let (plugins, _) = unluminate_app::services::plugins::Plugins::load(None);
    ...
    let start = Instant::now();
    let tree = unluminate_app::services::file_tree::FileTree::new(&folder);
    println!(
...
fn measure_the_graphics() {
    ...
    let device = pollster_block(adapter.request_device(&eframe::wgpu::DeviceDescriptor {
        label: Some("unluminate startup probe"),
        ..Default::default()
    }));
...
/// runtime — which is the whole point, because Unluminate has none and this example must not be the
