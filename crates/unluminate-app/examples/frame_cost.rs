...
//! visible.
//!
//! `cargo run --release -p unluminate-app --example frame_cost -- <file> [width]`
//!
...
use unluminate_app::services::text_renderer::TextRenderer;
use unluminate_core::{layout, relayout, Command, Document, Layout};
...
fn collect_glyphs(renderer: &TextRenderer, lines: &[unluminate_core::PlacedLine]) -> usize {
...
fn main() {
    ...
    let path = arguments.next().unwrap_or_else(|| "crates/unluminate-app/src/app/mod.rs".to_owned());
    ...
    let coloured = plugins.for_path(std::path::Path::new(&path)).map(|plugin| {
        ...
        let start = Instant::now();
        let tokens = unluminate_core::syntax::highlight(&source, &plugin.grammar);
        let tokenised = start.elapsed().as_secs_f64() * 1000.0;
        let spans: Vec<(std::ops::Range<usize>, unluminate_core::Color)> = tokens
            .into_iter()
    ...
    let style = unluminate_core::CharStyle::default();
    let ms = timed(200_000, || {
        std::hint::black_box(unluminate_core::FontMetrics::advance(&renderer, "a", &style));
    });
    ...
    if let Some(plugin) = plugins.for_path(std::path::Path::new(&path)) {
        ...
        let ms = timed(20, || {
            ...
            let spans: Vec<(std::ops::Range<usize>, unluminate_core::Color)> =
                unluminate_core::syntax::highlight(&text, &plugin.grammar)
                    .into_iter()
