...
//! applied to it.
//!
//! `cargo run --release -p unluminate-app --example folding_cost -- <file> [width]`
//!
...
use unluminate_app::services::file_kind;
use unluminate_app::services::plugins::Plugins;
use unluminate_core::folding::{self, Folds, Hidden};
use unluminate_core::{layout, relayout, FixedMetrics, ParagraphStyles, Rope, StyleSpans};
...
fn main() {
    ...
    let path = arguments
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("crates/unluminate-app/src/app/mod.rs"));
    ...
    let spans = StyleSpans::new(rope.len_bytes(), unluminate_core::CharStyle::default());
    ...
    let spans_after = StyleSpans::new(rope_after.len_bytes(), unluminate_core::CharStyle::default());
    ...
    let read_after = match reading {
        unluminate_core::folding::Reading::Code(grammar) => folding::tokens(&edited, grammar),
        _ => unluminate_core::folding::Tokens::default(),
    };
