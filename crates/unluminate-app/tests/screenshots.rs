...
//! `crates/unluminate-app/tests/snapshots`. When a run differs from the accepted image the harness also
...
//!
//! Run `UPDATE_SNAPSHOTS=1 cargo test -p unluminate-app` to accept new images.
...
use unluminate_app::UnluminateApp;
use unluminate_app::app::ViewMode;
use unluminate_app::app::actions::{Action, FoldAction, HighlightColor, RunAction};
use unluminate_app::components::about_dialog::About;
use unluminate_app::components::title_bar::MenuPlacement;
use unluminate_app::settings;
use unluminate_core::{Align, Color, Command, StyleChange};
...
fn build_sample_folder() -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminate-screenshot-folder");
    ...
    std::fs::write(root.join("readme.md"), "# Unluminate\n").expect("write readme.md");
    ...
    // A file Unluminate has no special handling for. It opens as plain text, which is what
...
fn harness(text: &str) -> Harness<'static, UnluminateApp> {
    let folder = sample_folder();
    let text = text.to_owned();
    let mut harness = builder()
        .with_size(vec2(WINDOW[0], WINDOW[1]))
        .build_eframe(move |cc| {
            let mut app = UnluminateApp::with_text(folder, &text);
...
fn harness_in(folder: &std::path::Path) -> Harness<'static, UnluminateApp> {
...
fn select_and(harness: &mut Harness<'static, UnluminateApp>, range: std::ops::Range<usize>, commands: &[Command]) {
...
/// large word.
fn select_phrase(harness: &mut Harness<'static, UnluminateApp>, phrase: &str, commands: &[Command]) {
...
fn open_text_options(harness: &mut Harness<'static, UnluminateApp>) {
...
fn collapse(harness: &mut Harness<'static, UnluminateApp>) {
...
/// top of the screen and the window buttons at the left; Windows draws the menus in Unluminate's own title bar
...
/// Unluminate draws there. macOS keeps the folder it already had, because those images were looked at and
...
fn pane_label(harness: &Harness<'static, UnluminateApp>, key: &str) -> String {
    let slot = harness
        .state()
        .plugin_ui
        .slot_of(key)
...
fn showing(harness: &Harness<'static, UnluminateApp>, key: &str) -> bool {
    harness
        .state()
        .plugin_ui
        .slot_of(key)
...
/// tints the explorer. A test folder that lives inside Unluminate's own repository therefore draws
...
fn git_folder(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminate-screenshot-repository").join(name);
    std::fs::remove_dir_all(&root).ok();
    std::fs::create_dir_all(&root).expect("make the folder");
    let git = |arguments: &[&str]| {
        let outcome = unluminate_git::command::run(&root, arguments);
        assert!(outcome.ok, "git {arguments:?}: {}", outcome.message());
    };
    git(&["init", "--initial-branch=main"]);
    for (name, value) in [
        ("user.name", "Unluminate Test"),
        ("user.email", "test@unluminate.invalid"),
...
fn pump(harness: &mut Harness<'static, UnluminateApp>) {
...
fn git_harness(name: &str) -> Harness<'static, UnluminateApp> {
...
fn typing_on_the_keyboard_puts_text_in_the_document() {
    ...
    for text in ["Unluminate", " typed", " this."] {
    ...
    harness.run();
    assert_eq!(harness.state().document().text().to_string(), "Unluminate typed this.\nA second line.");
...
fn a_long_paragraph_wraps_inside_the_editing_area() {
    let text = "Unluminate breaks a long paragraph into lines that fit the width of the editing area, \
...
fn the_background_fades_with_the_slider_and_the_text_stays_opaque() {
    /// A pixel fully covered by a glyph of red text: `unluminate_core::Color::RED` at full alpha.
    ...
    for (opacity, expected_alpha, name) in [(0.15_f32, 38_u8, "opacity_low"), (1.0_f32, 255, "opacity_high")] {
        ...
        // window. This is what the operating system compositor uses to blend Unluminate over the desktop.
...
fn a_document_holding_every_feature_at_once_renders() {
    ...
    select_phrase(&mut harness, "Unluminate", &[
...
fn the_status_bar_counts_the_line_and_column_from_one() {
    ...
    harness.state_mut().command(Command::MoveDocumentStart { extend: false });
    harness.run();
    assert_eq!(harness.state().caret_position(), unluminate_app::components::status_bar::Position { line: 1, column: 1 });
    harness.state_mut().command(Command::MoveDocumentEnd { extend: false });
    harness.run();
    assert_eq!(
        harness.state().caret_position(),
        unluminate_app::components::status_bar::Position { line: 2, column: 12 },
...
fn the_column_counts_characters_rather_than_bytes() {
    ...
    assert_eq!(
        harness.state().caret_position(),
        unluminate_app::components::status_bar::Position { line: 1, column: 6 },
...
fn the_editing_area_can_be_hidden_from_the_rail_and_the_explorer_takes_the_width() {
    ...
    let full = harness.state().panel_area(unluminate_app::app::dock::Panel::Explorer).width();
    ...
    let widened = harness.state().panel_area(unluminate_app::app::dock::Panel::Explorer).width();
    ...
    assert_eq!(
        harness.state().panel_area(unluminate_app::app::dock::Panel::Explorer).width(),
        full,
...
fn a_code_file_has_no_text_tools_and_nothing_below_them_moves() {
    // Everything in the tools is about how prose is shown, and Unluminate saves plain text and carries no
...
fn the_window_matches_the_design() {
    ...
    // Copied out of the repository first. `sample/` is inside Unluminate's own git repository, and the
    ...
    let sample = copy_out_of_the_repository(&sample, "unluminate-screenshot-sample");
    ...
    assert!(
        harness.state().document().text().to_string().starts_with("# Unluminate"),
    ...
    assert_eq!(
        harness.state().caret_position(),
        unluminate_app::components::status_bar::Position { line: 1, column: 1 }
    );
...
fn the_preview_removes_the_marks_and_applies_them() {
    ...
    assert!(!preview.contains("# Unluminate preview"), "the heading loses its hash");
    assert!(preview.contains("Unluminate preview"));
...
fn the_preview_puts_code_on_a_panel_and_inline_code_on_a_chip() {
    ...
    assert!(panels.iter().any(|panel| panel.kind == unluminate_core::PanelKind::Code));
    assert!(panels.iter().any(|panel| panel.kind == unluminate_core::PanelKind::Table));
...
fn the_whole_preview_can_be_selected_and_copied() {
    ...
    harness.run();
    let selected = harness.state().preview_selected_text().unwrap_or_default();
    assert!(selected.contains("Unluminate preview"), "the heading is in it");
...
///
/// On macOS Unluminate puts them in the bar along the top of the screen instead, which egui cannot draw and this
...
fn the_file_menu_holds_new_window_open_save_and_recent_projects() {
    ...
    harness.state_mut().recent = vec![
        std::path::PathBuf::from("/tmp/unluminate-recent-one"),
        std::path::PathBuf::from("/tmp/unluminate-recent-two"),
    ];
    harness.run();
    ...
    harness.get_by_label("unluminate-recent-one");
    harness.get_by_label("unluminate-recent-two");
...
fn opening_a_folder_shows_it_in_the_explorer() {
    ...
    let other = std::env::temp_dir().join("unluminate-other-folder");
...
fn the_explorer_draws_only_the_rows_that_are_on_screen() {
    let mut harness = harness("");
    let folder = std::env::temp_dir().join(format!("unluminate-many-rows-{}", std::process::id()));
...
/// The file types improvement: a file Unluminate has no special handling for opens as plain text.
#[test]
...
fn save_as_and_save_are_reachable_without_the_menu() {
    ...
    let folder = std::env::temp_dir().join("unluminate-save-action");
    std::fs::create_dir_all(&folder).expect("make the folder");
    ...
    let mut harness = builder()
        .with_size(vec2(WINDOW[0], WINDOW[1]))
        .build_eframe(move |cc| {
            let mut app = UnluminateApp::with_text(owned, text);
            app.prepare(&cc.egui_ctx);
            // A plugin's decoration is rasterised on the processor, and `vello_cpu` picks the widest SIMD it
            // has. Pinned here so an accepted image is a property of the code rather than of the machine that
            // took it — the same reason the terminal's screenshots feed fixed bytes to a session with no shell.
...
fn drag(harness: &mut Harness<'static, UnluminateApp>, from: egui::Pos2, to: egui::Pos2) {
...
/// Open the About box the way a person does: `Unluminate` in the bar, then `About Unluminate`.
fn open_about(harness: &mut Harness<'static, UnluminateApp>) {
    harness.state_mut().menu_placement = MenuPlacement::InWindow;
    harness.run();
    harness.get_by_label("Unluminate").click();
    harness.run();
    harness.get_by_label("About Unluminate").click();
    harness.run();
}
...
fn the_about_box_names_the_developer_the_version_and_the_build_date() {
    ...
    assert!(harness.state().about.is_some(), "Unluminate then About Unluminate should open it");
...
fn the_mcp_page_holds_the_install_buttons_the_server_and_the_configuration_to_copy() {
    ...
    // same on every machine. `UNLUMINATE_HOME` is where the installers look for an agent's own
    // configuration, so a folder of its own is what makes the buttons read `Install for ...`
    // whatever this machine happens to have; `UNLUMINATE_CLI_BIN` is the path written into the
    // configuration blocks, which would otherwise be wherever this checkout is.
    let home = std::env::temp_dir().join("unluminate-mcp-page-home");
    std::fs::remove_dir_all(&home).ok();
    std::fs::create_dir_all(&home).expect("make the folder");
    std::env::set_var("UNLUMINATE_HOME", &home);
    std::env::set_var("UNLUMINATE_CLI_BIN", r"C:\Program Files\Unluminate\unluminate-cli.exe");
    ...
    std::env::remove_var("UNLUMINATE_HOME");
    std::env::remove_var("UNLUMINATE_CLI_BIN");
    std::fs::remove_dir_all(&home).ok();
}
...
fn ticking_the_mcp_box_is_a_setting_and_not_a_listener_in_a_test() {
    ...
    let home = std::env::temp_dir().join("unluminate-mcp-tick-home");
    std::fs::remove_dir_all(&home).ok();
    std::fs::create_dir_all(&home).expect("make the folder");
    std::env::set_var("UNLUMINATE_HOME", &home);
...
fn changing_the_font_reaches_every_open_tab_and_not_only_the_one_showing() {
    ...
    // changing the font left two of them in the old one until Unluminate was restarted.
    let folder = sample_folder();
    let mut harness = harness_in(&folder);
...
fn the_keyboard_makes_the_text_bigger_and_smaller_and_puts_it_back() {
    ...
    harness.run();
    assert_eq!(harness.state().settings.font_size, 16.0, "back to what a new Unluminate has");
...
fn a_pinch_over_the_editing_area_steps_the_font_size() {
    ...
    let pinch = |harness: &mut Harness<'static, UnluminateApp>, factor: f32| {
...
fn a_folder_of_long_files() -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminate-zoom-folder");
...
/// is not focused on.
fn paragraph_in(file: &unluminate_app::app::files::OpenFile, above: f32) -> usize {
...
fn file_named<'a>(app: &'a UnluminateApp, name: &str) -> &'a unluminate_app::app::files::OpenFile {
    app.files
        .iter()
...
fn paragraph_under(app: &UnluminateApp, screen_y: f32) -> usize {
    let top = app.editor_area().top() + unluminate_app::theme::size::EDITOR_PADDING_Y;
...
fn caret_below_the_top(app: &UnluminateApp) -> f32 {
...
fn a_zoom_leaves_a_tab_that_was_not_showing_at_the_line_it_was_left_at() {
    ...
    harness.state_mut().files.active_mut().scroll = 900.0;
    harness.run();
    let top = harness.state().editor_area().top() + unluminate_app::theme::size::EDITOR_PADDING_Y;
...
fn a_pinch_in_a_split_is_the_pointers_pane_and_steps_the_size_once() {
    ...
    let down = right.top() + unluminate_app::theme::size::EDITOR_PADDING_Y + 300.0;
...
fn a_zoom_keeps_the_markdown_previews_place_too() {
    ...
    harness.run();
    let at_the_top = |app: &UnluminateApp| {
...
fn the_caret_is_no_taller_than_the_text_it_sits_in() {
    ...
    // reading leading Unluminate adds for prose and the paragraph's line spacing on top of the letters.
...
fn the_explorer_cannot_be_dragged_past_its_limits() {
    ...
    assert_eq!(harness.state().panes.explorer_width, unluminate_app::settings::EXPLORER_MAX);
    let handle = harness.get_by_label("Resize explorer").rect();
    drag(&mut harness, handle.center(), egui::pos2(handle.center().x - 2000.0, handle.center().y));
    assert_eq!(harness.state().panes.explorer_width, unluminate_app::settings::EXPLORER_MIN);
}
...
fn two_presses_on_a_panels_header_fill_the_window_with_it_and_two_more_put_it_back() {
    use unluminate_app::app::dock::Panel;
    let mut harness = harness("");
...
fn maximising_is_not_an_arrangement_and_a_toggle_inside_it_ends_it() {
    ...
    // away, and the project's own `.unluminate` is written every frame — so Unluminate closed while a pane filled the
...
fn escape_puts_a_maximised_pane_back_and_does_nothing_when_none_is() {
    ...
    harness.run();
    assert_eq!(harness.state().focus, unluminate_app::app::Focus::Explorer);
    did(&mut harness, "action run toggle-maximised-pane");
    harness.run();
    ...
    assert_eq!(
        harness.state().focus,
        unluminate_app::app::Focus::Explorer,
    ...
    harness.key_press(egui::Key::Escape);
    harness.run();
    assert_eq!(harness.state().focus, unluminate_app::app::Focus::Editor);
    assert_eq!(harness.state().maximised_pane(), None);
}
...
fn double_click_at(harness: &mut Harness<'static, UnluminateApp>, at: egui::Pos2) {
    let modifiers = Modifiers::default();
    harness.input_mut().events.push(egui::Event::PointerMoved(at));
    for _ in 0..2 {
...
/// where it is — see `UnluminateApp::zoom_over_a_panel`.
fn zoom_at(harness: &mut Harness<'static, UnluminateApp>, at: egui::Pos2, larger: bool) {
    harness.input_mut().events.push(egui::Event::PointerMoved(at));
    harness.run();
...
fn a_pane_is_as_wide_as_the_window_lets_it_be_once_the_editing_area_is_hidden() {
    ...
    assert!(
        (explorer.width() - unluminate_app::settings::EXPLORER_MIN).abs() < 1.5,
...
fn a_document_taller_than_its_pane_has_a_scrollbar_that_can_be_dragged() {
    ...
    let overflow = harness.state().layout().height - harness.state().editor_area().height()
        + unluminate_app::theme::size::EDITOR_PADDING_Y * 2.0;
...
fn the_explorers_menu_opens_from_the_empty_space_below_the_rows() {
    ...
    assert!(directory);
    assert_eq!(aimed, unluminate_app::app::actions::Aim::AtEmptySpace);
    harness.run();
    ...
    let entries = unluminate_app::app::actions::explorer_menu(
        sample_folder().as_path(),
        true,
        false,
        unluminate_app::app::actions::Aim::AtEmptySpace,
    );
    let live = |name: &str| {
        entries.iter().any(|entry| {
            matches!(
                entry,
                unluminate_app::app::actions::Entry::Item { name: found, enabled, .. }
...
fn the_explorer_can_make_a_folder() {
    let folder = std::env::temp_dir().join("unluminate-new-folder-test");
    std::fs::remove_dir_all(&folder).ok();
    ...
    // And from the command line, which is the other half of every feature in Unluminate.
...
fn a_file_made_by_another_program_appears_in_the_explorer() {
    let folder = std::env::temp_dir().join("unluminate-watch-test");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the project");
    std::fs::write(folder.join("readme.md"), "# here\n").expect("write a file");
    let mut harness = harness_in(&folder);
...
fn with_terminal(text: &str, rows: usize, columns: usize) -> Harness<'static, UnluminateApp> {
    let mut harness = harness(text);
...
fn feed(harness: &mut Harness<'static, UnluminateApp>, bytes: &[u8]) {
    harness
        .state_mut()
        .terminal
        .tabs
...
fn the_terminal_opens_along_the_bottom_and_shows_what_a_program_wrote() {
    ...
    feed(
        &mut harness,
        b"jason.mcaffee@unluminate ~ % cargo test\r\n   Compiling unluminate-terminal v0.1.0\r\n    Finished in 1.26s\r\n",
    );
    let screen = harness.state().terminal.tabs.active().expect("a tab").snapshot();
    assert!(screen.contains("Compiling unluminate-terminal"), "the output should be on the screen");
...
fn the_view_menu_shows_and_hides_the_terminal() {
    ...
    assert_eq!(harness.state().focus, unluminate_app::app::Focus::Terminal, "and the keyboard in it");

    harness.state_mut().run_action(Action::ToggleTerminal, &ctx);
    harness.run();
    assert!(!harness.state().terminal.visible);
    assert_eq!(harness.state().focus, unluminate_app::app::Focus::Editor, "the keyboard comes back");
}
...
fn typing_in_the_terminal_reaches_the_shell_and_not_the_document() {
    ...
    harness.state_mut().run_action(Action::ToggleTerminal, &ctx);
    pump(&mut harness);
    assert_eq!(harness.state().focus, unluminate_app::app::Focus::Terminal);
    ...
    for text in ["echo unluminate-typing-works"] {
        harness.input_mut().events.push(egui::Event::Text(text.to_owned()));
        pump(&mut harness);
    }
    harness.key_press(egui::Key::Enter);
    ...
    loop {
        ...
        let found = harness
            ...
            .tabs
            .active()
            .map(|session| session.snapshot().contains("unluminate-typing-works"))
            .unwrap_or(false);
        if found {
            break;
        }
...
fn the_recent_projects_are_remembered_across_windows() {
    ...
    let store_folder = std::env::temp_dir().join("unluminate-recent-projects-test");
    std::fs::remove_dir_all(&store_folder).ok();
    let first = std::env::temp_dir().join("unluminate-project-one");
    let second = std::env::temp_dir().join("unluminate-project-two");
    std::fs::create_dir_all(&first).expect("make the first project");
    std::fs::create_dir_all(&second).expect("make the second project");

    let mut harness = harness("");
    harness.state_mut().use_store(unluminate_app::services::store::Store::at(&store_folder));
    harness.state_mut().open_folder(&first);
    harness.state_mut().open_folder(&second);
    harness.run();

    let recent = harness.state().recent.clone();
    assert!(recent[0].ends_with("unluminate-project-two"), "the newest is first, got {recent:?}");
    assert!(recent.iter().any(|path| path.ends_with("unluminate-project-one")));
    ...
    second_window
        .state_mut()
        .use_store(unluminate_app::services::store::Store::at(&store_folder));
    second_window.run();
    assert!(
        second_window.state().recent.iter().any(|path| path.ends_with("unluminate-project-two")),
...
fn a_setting_is_written_and_read_back_by_the_next_window() {
    let store_folder = std::env::temp_dir().join("unluminate-settings-across-windows");
    std::fs::remove_dir_all(&store_folder).ok();

    let mut first_window = harness("");
    first_window.state_mut().use_store(unluminate_app::services::store::Store::at(&store_folder));
    ...
    next.state_mut().use_store(unluminate_app::services::store::Store::at(&store_folder));
    next.run();
...
fn a_box_that_takes_typing_keeps_the_keyboard_while_the_terminal_is_open() {
    ...
    let mut harness = with_terminal("", 10, 60);
    assert_eq!(harness.state().focus, unluminate_app::app::Focus::Editor);
    harness.state_mut().focus = unluminate_app::app::Focus::Terminal;
    harness.run();
...
fn click_at(harness: &mut Harness<'static, UnluminateApp>, at: egui::Pos2) {
    harness.input_mut().events.push(egui::Event::PointerMoved(at));
    for pressed in [true, false] {
        harness.input_mut().events.push(egui::Event::PointerButton {
            pos: at,
...
fn the_rest_of_the_menu_still_works_while_a_text_box_has_the_keyboard() {
    ...
    let folder = std::env::temp_dir().join("unluminate-save-while-filtering");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the folder");
...
fn typing_a_new_name_in_the_rename_prompt_leaves_the_document_alone() {
    let folder = std::env::temp_dir().join("unluminate-rename-prompt-keyboard");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the folder");
...
fn typing_in_the_plugin_search_leaves_the_document_alone() {
    ...
    harness.state_mut().settings_window.page = unluminate_app::settings::Page::Plugins;
    harness.run();
    harness.run();
...
fn enter_in_the_commit_message_is_a_new_line_and_the_command_key_commits() {
    ...
    let ctx = harness.ctx.clone();
    harness.state_mut().run_action(Action::Git(unluminate_app::app::actions::GitAction::Commit), &ctx);
    harness.run();
    ...
    harness
        .state_mut()
        .run_action(Action::Git(unluminate_app::app::actions::GitAction::Add(None)), &ctx);
    settle(&mut harness, "the file to be staged", |app| {
        app.git.as_ref().is_some_and(|git| {
            git.snapshot.status.entry("version.ts").is_some_and(unluminate_git::status::Entry::staged)
        })
    });
...
fn the_recent_projects_menu_opens_a_project_without_closing_this_one() {
    ...
    let mut harness = harness("");
    harness.state_mut().menu_placement = MenuPlacement::InWindow;
    let other = std::env::temp_dir().join("unluminate-recent-open-test");
    ...
    harness.get_by_label("File").click();
    harness.run();
    harness.get_by_label("unluminate-recent-open-test");
...
fn the_explorers_own_menu_holds_what_can_be_done_to_a_file() {
    ...
    harness.state_mut().explorer_menu =
        Some((
            ...
            false,
            unluminate_app::app::actions::Aim::AtARow,
        ));
    harness.run();
...
fn making_a_file_from_the_explorers_menu_opens_it() {
    let folder = std::env::temp_dir().join("unluminate-screenshot-new-file");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the folder");
    let mut harness = harness_in(&folder);
    let ctx = harness.ctx.clone();
...
fn renaming_a_file_moves_it_and_the_tab_follows() {
    let folder = std::env::temp_dir().join("unluminate-screenshot-rename");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the folder");
    std::fs::write(folder.join("before.md"), "# before\n").expect("write it");
    let mut harness = harness_in(&folder);
...
fn cutting_a_file_and_pasting_it_into_a_folder_moves_it() {
    let folder = std::env::temp_dir().join("unluminate-screenshot-clipboard");
    std::fs::remove_dir_all(&folder).ok();
...
fn the_view_menu_holds_the_font_size_and_ticks_the_mode_that_is_showing() {
    // The one menu in Unluminate with a checked row in it, so it is the one that shows the tick. It used
    // to be drawn as the character at U+2713, which no font in the stack Unluminate hands egui has a
...
fn the_commit_panel_shows_the_changes_and_the_unversioned_files() {
    ...
    // on how quickly a thread answered, which is not a difference in Unluminate.
    settle(&mut harness, "the history the panel asks for", |app| {
        app.git.as_ref().is_some_and(|git| git.message.is_none() && !git.history.is_empty())
    });
...
fn the_branches_dialog_lists_the_branches() {
    ...
    let ctx = harness.ctx.clone();
    harness.state_mut().run_action(Action::Git(unluminate_app::app::actions::GitAction::Branches), &ctx);
    harness.run();
...
fn the_gutter_annotates_with_git_blame_and_colours_by_age() {
    ...
    assert_eq!(blame[0].author, "Unluminate", "the first name, which is what fits in the column");
...
fn a_css_file_is_coloured_by_its_plugin() {
    ...
    let folder = std::env::temp_dir().join("unluminate-screenshot-css");
    std::fs::create_dir_all(&folder).expect("make the folder");
...
fn an_html_file_is_coloured_by_its_plugin() {
    ...
    let folder = std::env::temp_dir().join("unluminate-screenshot-html");
    std::fs::create_dir_all(&folder).expect("make the folder");
...
#[test]
fn the_plugins_page_lists_the_ones_that_ship_with_unluminate() {
    let mut harness = harness("");
    ...
    let store = std::env::temp_dir().join("unluminate-plugins-page-store");
    ...
    assert!(
        ...
        "uninstalling a bundled plugin puts it back to the one Unluminate ships rather than removing it",
    );
...
fn the_theme_page_lists_every_theme_with_the_colours_it_is_made_of() {
    let mut harness = harness("");
    harness.state_mut().settings_window.open();
    harness.state_mut().settings_window.page = unluminate_app::settings::Page::Theme;
    harness.run();
    harness.run();
    for name in ["Unluminate Dark", "Islands Dracula Colorful", "Material Deep Ocean", "Monokai Pro", "One Dark"] {
        harness.get_by_label(name);
    }
...
fn a_theme_repaints_the_window_and_recolours_the_code() {
    ...
    assert_eq!(
        ...
        unluminate_core::Color::rgb(0xFF, 0x79, 0xC6),
    ...
    assert_eq!(
        unluminate_app::theme::color::editor(),
    ...
    assert_eq!(
        harness.state().document().chars().style_at(keyword_at).color,
        unluminate_core::Color::rgb(0xFF, 0x61, 0x88),
...
fn a_theme_is_set_by_name_and_an_unknown_one_says_what_there_is() {
    ...
    harness.run();
    assert_eq!(
        unluminate_app::theme::color::accent(),
...
fn an_accent_is_set_over_the_theme_and_cleared_back_to_it() {
    let mut harness = harness("");
    run(&mut harness, "theme set unluminate/dark --accent #FF79C6");
    harness.run();
    assert_eq!(unluminate_app::theme::color::accent(), egui::Color32::from_rgb(0xFF, 0x79, 0xC6));
    assert_eq!(unluminate_app::theme::color::folder_open(), egui::Color32::from_rgb(0xFF, 0x79, 0xC6));
    assert_eq!(
        unluminate_app::theme::color::editor(),
    ...
    run(&mut harness, "theme set unluminate/dark --accent none");
    harness.run();
    assert_eq!(unluminate_app::theme::color::accent(), egui::Color32::from_rgb(0x48, 0x9F, 0xF8));

    let reply = run(&mut harness, "theme set unluminate/dark --accent puce");
    assert!(!reply.ok);
    assert!(reply.message.contains("#RRGGBB"), "{}", reply.message);
}

/// Switching the themes plugin off puts the window back to Unluminate's own in the same frame, which is
...
fn switching_the_themes_plugin_off_puts_the_window_back() {
    ...
    harness.run();
    assert_eq!(unluminate_app::theme::color::editor(), egui::Color32::from_rgb(0x29, 0x2D, 0x3E));
    ...
    assert_eq!(
        unluminate_app::theme::color::editor(),
        egui::Color32::from_rgb(0x1A, 0x1F, 0x26),
        "back to Unluminate Dark rather than left in a palette nothing can name"
    );
...
fn the_icon_set_follows_the_theme_and_the_setting_wins() {
    let mut harness = harness("");
    assert_eq!(
        unluminate_app::theme::icons(),
        unluminate_app::theme::IconSet::Material,
        "a window comes up in the improved marks"
    );

    run(&mut harness, "theme set \"One Dark\"");
    harness.run();
    assert_eq!(unluminate_app::theme::icons(), unluminate_app::theme::IconSet::Classic, "One Dark names them");

    run(&mut harness, "theme set \"One Dark\" --icons material");
    harness.run();
    assert_eq!(unluminate_app::theme::icons(), unluminate_app::theme::IconSet::Material, "and the setting wins");

    run(&mut harness, "theme set \"One Dark\" --icons follow");
    harness.run();
    assert_eq!(unluminate_app::theme::icons(), unluminate_app::theme::IconSet::Classic, "and follow gives it back");
}
...
fn theme_list_and_show_answer_in_a_payload_proportionate_to_the_question() {
    ...
    assert_eq!(themes.len(), 6, "Unluminate's own and the bundle's five");
    assert_eq!(themes[0]["key"], "unluminate/dark");
    ...
    assert_eq!(
        ...
        unluminate_app::theme::Palette::NAMES.len(),
...
/// a fault in Unluminate. This is what a step between git operations uses instead.
fn nudge(harness: &mut Harness<'static, UnluminateApp>) {
    for _ in 0..4 {
        pump(harness);
    }
}

fn settle(harness: &mut Harness<'static, UnluminateApp>, what: &str, ready: impl Fn(&UnluminateApp) -> bool) {
    for _ in 0..600 {
        pump(harness);
...
/// What git says about a repository, asked directly rather than through Unluminate.
...
///
/// Every assertion asks **git** what happened rather than asking Unluminate, so a step that only changed
...
fn every_git_operation_can_be_driven_from_the_window() {
    ...
    prompt.value = "from-unluminate".to_owned();
    harness.state_mut().run_prompt_for_test(prompt);
    harness.state_mut().prompt = None;
    settle(&mut harness, "the branch", |app| {
        app.git.as_ref().is_some_and(|git| git.snapshot.status.branch.as_deref() == Some("from-unluminate"))
    });
    assert_eq!(ask_git(&root, &["branch", "--show-current"]), "from-unluminate");
    ...
    assert!(unluminate_git::branch::switch(&root, "main").ok);
    ...
    if let Some(state) = harness.state_mut().git.as_mut() {
        state.dialogs.target = "from-unluminate".to_owned();
    }
    ...
    settle(&mut harness, "the merge", move |_| {
        unluminate_git::command::run(&merged, &["log", "--format=%s", "-n1"]).stdout.trim()
            == "task-1649: driven from the window"
    });
    assert_eq!(
        ask_git(&root, &["log", "--format=%s", "-n1"]),
...
fn the_rails_version_control_button_opens_the_commit_panel_and_shuts_it() {
    let root = git_folder("unluminate-rail-git");
    ...
    // for a reason that is not a fault in Unluminate, which is what made this test fail in the full suite and
    // pass on its own.
    nudge(&mut harness);
    assert!(
        harness.state().git.as_ref().is_some_and(|git| git.panel.open),
...
/// What was open in a project is written into a `.unluminate` folder beside it and read back next time.
#[test]
fn what_was_open_in_a_project_comes_back_when_it_is_opened_again() {
    let root = copy_out_of_the_repository(&sample_folder(), "unluminate-project-state-window");
    {
        let mut harness = harness_in(&root);
    ...
    assert!(
        root.join(".unluminate/open-files.txt").is_file(),
    ...
    second.run();
    let names: Vec<String> =
        second.state().files.iter().map(unluminate_app::app::files::OpenFile::name).collect();
...
fn the_keyboard_zooms_a_picture_and_leaves_the_editors_font_alone() {
    ...
    let scale = |harness: &Harness<'static, UnluminateApp>| {
...
fn saving_a_tab_that_holds_a_picture_does_not_write_over_the_picture() {
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-picture-save");
...
fn open_go_to_file(harness: &mut Harness<'static, UnluminateApp>) {
    let ctx = harness.ctx.clone();
...
fn double_click(harness: &mut Harness<'static, UnluminateApp>, label: &str) {
...
fn found_names(harness: &Harness<'static, UnluminateApp>) -> Vec<String> {
    harness
        .state()
        .go_to_file
        .as_ref()
...
fn double_clicking_a_row_in_go_to_file_opens_the_file_and_shuts_the_modal() {
    ...
    assert_eq!(harness.state().document().text().to_string(), "# Unluminate\n");
    assert_eq!(
...
fn search_for(harness: &mut Harness<'static, UnluminateApp>, text: &str) {
    let ctx = harness.ctx.clone();
...
fn matches(harness: &Harness<'static, UnluminateApp>) -> Vec<String> {
    harness
        .state()
        .find_in_files
        .as_ref()
...
fn find_in_files_finds_text_anywhere_in_the_project() {
    let mut harness = harness("");
    search_for(&mut harness, "Unluminate");
    let found = matches(&harness);
    assert!(found.contains(&"readme.md:1".to_owned()), "readme.md says `# Unluminate`: {found:?}");
...
fn the_case_of_a_search_can_be_insisted_on() {
    ...
    assert!(
        ...
        "`unluminate` should find `# Unluminate` while case is being ignored"
    );
    ...
    assert!(
        ...
        "with the case insisted on, `unluminate` no longer matches `Unluminate`: {found:?}"
    );
}
...
fn placement(harness: &Harness<'static, UnluminateApp>, id: &str) -> unluminate_app::components::modal::Placement {
    unluminate_app::components::modal::placement(&harness.ctx, id)
}
...
fn a_modal_is_moved_by_dragging_its_header() {
    let mut harness = harness("");
    open_go_to_file(&mut harness);
    assert!(placement(&harness, "unluminate-go-to-file").is_untouched(), "it opens in the middle");
    let bar = harness.get_by_label("Move go to file").rect();
    let from = bar.center();
    drag(&mut harness, from, from + egui::vec2(-120.0, 60.0));
    let moved = placement(&harness, "unluminate-go-to-file");
...
fn a_modal_is_resized_by_dragging_a_corner() {
    ...
    let grown = placement(&harness, "unluminate-go-to-file");
...
fn double_clicking_a_modals_header_puts_it_back_in_the_middle() {
    ...
    let from = bar.center();
    drag(&mut harness, from, from + egui::vec2(-120.0, 60.0));
    assert!(!placement(&harness, "unluminate-go-to-file").is_untouched());
    double_click(&mut harness, "Move go to file");
    assert!(
        placement(&harness, "unluminate-go-to-file").is_untouched(),
...
fn picture_document_folder() -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminate-preview-pictures");
    std::fs::create_dir_all(&root).expect("make the folder");
    write_sample_picture(&root.join("picture.png"));
    std::fs::write(
...
fn a_wide_picture_is_scaled_down_to_the_width_of_the_pane() {
    let folder = std::env::temp_dir().join("unluminate-preview-wide-picture");
    std::fs::create_dir_all(&folder).expect("make the folder");
...
fn typing_in_find_in_files_leaves_the_document_alone() {
    ...
    harness.key_press(egui::Key::Backspace);
    harness.run();
    assert_eq!(harness.state().find_in_files.as_ref().unwrap().query, "unluminat");
    assert_eq!(harness.state().document().text().to_string(), before);
    assert_eq!(harness.state().document().is_modified(), modified);
}
...
fn the_divider_in_find_in_files_moves_the_split_between_the_results_and_the_preview() {
    ...
    assert!(
        (harness.state().panes.find_split - unluminate_app::components::find_in_files::SPLIT).abs() < 0.001
    );
}
...
use unluminate_app::services::run_configurations::{Configuration, RunConfigurations};
...
fn with_run(name: &str, command: &str, rows: usize) -> Harness<'static, UnluminateApp> {
...
fn feed_run(harness: &mut Harness<'static, UnluminateApp>, bytes: &[u8]) {
    harness
        .state_mut()
        .run
        .active_mut()
...
fn a_run_that_ended_keeps_its_tab_and_the_strip_says_what_it_ended_with() {
    // The reference editor prints its epilogue into the console; Unluminate puts it in the strip, because a line
...
fn the_run_tile_and_the_terminal_tile_take_the_same_place_and_never_both() {
    ...
    // `UnluminateApp` are for. This is the fault they were written for: `terminal show` from the command
...
fn the_title_bar_carries_a_run_button_and_a_debug_button_beside_it() {
    ...
    // And one nothing can debug has one button, which is Unluminate's rule for a control that cannot
...
/// `UnluminateApp::debug_a_configuration` being the one place means.
#[test]
...
fn the_debug_tile_says_what_is_missing_and_offers_to_install_it() {
    ...
    harness.state_mut().debug_adapters.insert(
        ...
        (
            ...
            unluminate_app::services::debuggers::Report {
                name: "lldb",
                found: None,
                configured: false,
...
fn the_widgets_play_button_starts_the_chosen_configuration() {
    ...
    // `UnluminateApp::run_action` being the one place means. A program that is not there is what is run
    ...
    harness
        .state_mut()
        .run_configurations
        .add_permanent(configuration("Nothing", "unluminate-no-such-program-at-all"));
    ...
    assert!(
        said.contains("unluminate-no-such-program-at-all"),
...
fn the_flyout_lists_the_permanents_the_temporaries_and_the_suggestions() {
    ...
    let folder = std::env::temp_dir().join("unluminate-run-suggestions");
    std::fs::create_dir_all(&folder).expect("make the project");
    std::fs::write(
...
fn running_a_suggestion_keeps_it_as_a_temporary_so_it_can_be_run_again() {
    let folder = std::env::temp_dir().join("unluminate-run-suggestion-kept");
    std::fs::create_dir_all(&folder).expect("make the project");
...
fn run_current_file_is_offered_for_a_javascript_file_and_not_for_a_rust_one() {
    ...
    let folder = std::env::temp_dir().join("unluminate-run-current-file");
    std::fs::create_dir_all(&folder).expect("make the project");
...
fn a_program_that_prints_and_stops_leaves_what_it_printed_in_its_tab() {
    ...
    //
    // The fix is that `UnluminateApp::run_grid_size` works the size out from the rectangle the tile
    // really has, so there is no resize at all. What proves it is a program that prints and stops.
    let folder = std::env::temp_dir().join("unluminate-run-prints-and-stops");
    std::fs::create_dir_all(&folder).expect("make the project");
    let mut harness = harness_in(&folder);

    let marker = "unluminate-printed-this";
...
fn a_project_comes_back_with_its_configurations_and_the_choice_it_still_has() {
    // What `.unluminate` remembers: the permanents in a file of their own, and which of them was chosen
    ...
    let root = std::env::temp_dir().join("unluminate-run-remembered");
    std::fs::remove_dir_all(&root).ok();
    ...
    unluminate_app::services::run_configurations::save(&root, &configurations);
    let folder = unluminate_app::services::project_state::folder(&root);
    std::fs::write(
...
// words are parsed against `unluminate_cli::catalogue`, dispatched by `UnluminateApp::run_cli`, and what is
...
fn run(harness: &mut Harness<'static, UnluminateApp>, line: &str) -> unluminate_cli::protocol::Reply {
    let ctx = harness.ctx.clone();
    let reply = harness
        .state_mut()
...
fn did(harness: &mut Harness<'static, UnluminateApp>, line: &str) -> serde_json::Value {
...
fn did_while_waiting(harness: &mut Harness<'static, UnluminateApp>, line: &str) -> serde_json::Value {
    let ctx = harness.ctx.clone();
    let reply = harness
        .state_mut()
        .run_command_line(line, &ctx)
...
fn refused(harness: &mut Harness<'static, UnluminateApp>, line: &str) -> String {
    let reply = run(harness, line);
...
fn late_repository_project() -> (std::path::PathBuf, std::path::PathBuf) {
    ...
    let initialized = unluminate_git::command::run(&ancestor, &["init", "--initial-branch=main"]);
...
fn git_status_rediscovers_a_repository_created_after_the_window_opened() {
    ...
    let initialized = unluminate_git::command::run(&project, &["init", "--initial-branch=master"]);
    ...
    settle(&mut harness, "the project repository", |app| {
        app.git.as_ref().is_some_and(|git| {
            !git.is_busy()
                && unluminate_app::app::git::root_relation(git.repository.root(), &project)
                    == unluminate_app::app::git::RootRelation::Project
        })
    });
...
fn drag_through(harness: &mut Harness<'static, UnluminateApp>, from: egui::Pos2, path: &[egui::Pos2]) {
    let modifiers = Modifiers::default();
    harness.input_mut().events.push(egui::Event::PointerMoved(from));
    harness.run();
    harness.input_mut().events.push(egui::Event::PointerButton {
...
fn a_folder_with_a_long_source_file(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminate-performance").join(name);
    std::fs::create_dir_all(&root).expect("make the folder");
...
fn status_answers_for_every_part_of_the_window_at_once() {
    ...
    assert!(status["project"].as_str().unwrap().contains("unluminate-screenshot-folder"));
}
...
fn settings_list_keeps_a_long_value_and_its_help_apart() {
    let mut harness = harness_in(&sample_folder());
    let long = "C:\\jason\\AppData\\Local\\Unluminate\\adapters\\codelldb\\extension\\adapter\\codelldb.exe";
    ...
    for row in &rows {
        ...
        // because Unluminate writes those keys into its own settings file and would not name them back.
...
/// Send a request the way something other than `unluminate-cli` sends one: a JSON object down the
...
fn over_the_wire(
    harness: &mut Harness<'static, UnluminateApp>,
    command: &str,
    arguments: serde_json::Value,
) -> unluminate_cli::protocol::Reply {
    let ctx = harness.ctx.clone();
    let request = unluminate_cli::protocol::Request::from_json(&serde_json::json!({
        "token": "",
        "command": command,
        "arguments": arguments,
    }))
...
///
/// Both halves were real faults found by driving Unluminate through the MCP tools. `tab open` with
...
/// A file changed by something other than Unluminate is read again before it is answered about.
///
/// Found by driving Unluminate through the MCP tools while editing the same files from outside: the tab
...
#[test]
fn a_file_changed_outside_unluminate_is_read_again_before_it_is_answered_about() {
    let folder = std::env::temp_dir().join("unluminate-changed-underneath");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the project");
    ...
    // Something else rewrites it. Nothing tells Unluminate.
...
fn a_tab_with_unsaved_changes_is_not_reread_from_the_file() {
    let folder = std::env::temp_dir().join("unluminate-changed-underneath-dirty");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the project");
    let path = folder.join("notes.md");
    std::fs::write(&path, "first\n").expect("write notes.md");
...
fn every_command_in_the_catalogue_is_one_the_window_knows() {
    ...
    let mut harness = harness_in(&sample_folder());
    for command in unluminate_cli::catalogue::COMMANDS {
        if command.local {
        ...
        let ctx = harness.ctx.clone();
        let request = unluminate_cli::protocol::Request::new("", &command.wire(), Default::default());
...
fn browser_tabs_open_through_the_shared_cli_and_action_paths() {
    let folder = std::env::temp_dir().join(format!("unluminate-browser-paths-{}", std::process::id()));
...
// many files as you like. The set itself is tested in `unluminate-core` with no window, and the file
...
// looks like the rest of Unluminate.
...
fn open_text_menu(harness: &mut Harness<'static, UnluminateApp>, offset: usize) {
    let at = harness.state().editor_area().left_top() + vec2(120.0, 60.0);
    harness.state_mut().text_menu =
        Some(unluminate_app::components::text_menu::TextMenu::new(at, offset));
    harness.run();
}
...
fn choosing_a_colour_marks_the_selection_and_shuts_the_menu() {
    ...
    assert_eq!(marks.iter().next().unwrap().color, unluminate_core::Rgba::new(0x48, 0x9F, 0xF8, 0x59));
}
...
///
/// The one interaction in Unluminate that has to be sent as raw events: `kittest` can click a control it
...
fn right_click_at(harness: &mut Harness<'static, UnluminateApp>, at: egui::Pos2) {
...
fn the_command_line_marks_a_passage_and_lists_it() {
    ...
    assert_eq!(rows[0]["text"], "# Unluminate");
    assert_eq!(harness.state().document().highlights().len(), 1);
}

#[test]
fn the_command_line_marks_every_occurrence_of_some_words() {
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-highlight-occurrences");
...
fn a_bulk_request_marks_passages_across_several_files_in_one_call() {
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-highlight-bulk");
    let mut harness = harness_in(&folder);
    ...
    let marks = harness.state().document().highlights();
    assert_eq!(marks.len(), 1);
    assert_eq!(marks.iter().next().unwrap().color, unluminate_core::Rgba::new(0xFF, 0x00, 0xFF, 0x80));
...
// the layout are tested in `unluminate-core` with no window at all, where the numbers can be checked by
...
/// way through writing is a failure that has nothing to do with Unluminate.
fn diagram_folder() -> std::path::PathBuf {
    static FOLDER: OnceLock<std::path::PathBuf> = OnceLock::new();
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminate-mermaid-samples");
            std::fs::create_dir_all(&root).expect("make the folder");
...
#[test]
fn a_diagram_type_unluminate_does_not_draw_is_named_rather_than_left_blank() {
    let folder = diagram_folder();
...
fn choose(harness: &mut Harness<'static, UnluminateApp>, action: Action) {
    let ctx = harness.ctx.clone();
    harness.state_mut().run_action(action, &ctx);
    harness.run();
}
...
fn a_split_project_opens_split_again() {
    // The whole round trip through `.unluminate`, on a folder of its own so that no other test's window
    // is reading or writing the same state file.
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-screenshot-split-project");
    {
        let mut harness = harness_in(&folder);
        harness.state_mut().restore_project();
        harness.state_mut().open_path_permanently(&folder.join("readme.md"));
...
// behaviour underneath them is tested with no window in `unluminate_core::symbols`, `app::symbols` and
...
fn code_folder() -> std::path::PathBuf {
    ...
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminate-screenshot-code");
...
fn code_harness(open: &str) -> Harness<'static, UnluminateApp> {
...
fn caret_on(harness: &mut Harness<'static, UnluminateApp>, needle: &str, into: usize) -> usize {
    let text = harness.state().document().text().to_string();
...
fn settle_the_references(harness: &mut Harness<'static, UnluminateApp>) {
    for _ in 0..400 {
        let searching =
            harness.state().references.as_ref().is_some_and(unluminate_app::components::references::References::is_searching);
        if !searching {
            break;
        }
        std::thread::sleep(std::time::Duration::from_millis(10));
...
fn references(harness: &Harness<'static, UnluminateApp>) -> Vec<String> {
    harness
        .state()
        .references
        .as_ref()
        ...
        .map(|hit| {
            format!(
                ...
                hit.line,
                match hit.role {
                    unluminate_core::symbols::Role::Code => String::new(),
...
fn references_heading(file: &str) -> String {
    let path = std::path::Path::new("unluminate-screenshot-code").join(file);
...
fn a_file_whose_language_says_nothing_has_none_of_the_three_entries() {
    ...
    let names: Vec<String> = unluminate_app::app::actions::menus(&harness.state().menu_state())
        .iter()
        ...
        .filter_map(|entry| match entry {
            unluminate_app::app::actions::Entry::Item { name, .. } => Some(name.clone()),
            _ => None,
        })
        .collect();
...
fn point_of(harness: &Harness<'static, UnluminateApp>, offset: usize) -> egui::Pos2 {
    let area = harness.state().editor_area();
    let caret = harness.state().layout().caret_at(offset);
    let scroll = harness.state().files.active().scroll;
    egui::pos2(
        area.left() + unluminate_app::components::editor_view::PADDING + caret.x + 1.0,
        area.top() + unluminate_app::theme::size::EDITOR_PADDING_Y - scroll + caret.y
            + caret.height / 2.0,
    )
}
...
fn hover_over(harness: &mut Harness<'static, UnluminateApp>, offset: usize, modifier: bool) {
...
fn modifier_click(harness: &mut Harness<'static, UnluminateApp>, offset: usize) {
    let at = point_of(harness, offset);
...
// `unluminate_core::imports`, `services::imports` and `app::completion`.
...
/// change that picture for a reason that is not a change to Unluminate.
fn import_folder() -> std::path::PathBuf {
    static FOLDER: OnceLock<std::path::PathBuf> = OnceLock::new();
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminate-screenshot-imports");
...
// underneath is tested with no window in `unluminate_core::completion` and `app::completion`; what is
...
/// a reason that is not a change to Unluminate.
///
...
fn completion_folder() -> std::path::PathBuf {
    ...
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminate-screenshot-completion");
...
fn type_letters(harness: &mut Harness<'static, UnluminateApp>, text: &str) {
...
fn completions(harness: &Harness<'static, UnluminateApp>) -> Vec<String> {
    harness
        .state()
        .completion()
...
fn typing_a_word_offers_the_names_it_could_become() {
    ...
    assert!(anchor.pane.contains_rect(
        unluminate_app::components::completion::where_it_goes(8, anchor.caret, anchor.pane)
    ));
...
fn the_list_flips_above_the_caret_at_the_bottom_of_the_pane() {
    ...
    let area = unluminate_app::components::completion::where_it_goes(rows, anchor.caret, anchor.pane);
    assert!(
...
fn the_editor_page_holds_the_gutter_and_the_suggestions() {
    ...
    assert!(harness.state().settings.suggestions.is_automatic(), "on in a fresh Unluminate");
...
fn scratch_folder(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join(format!("unluminate-1681-{name}"));
    std::fs::remove_dir_all(&root).ok();
...
fn a_modal_takes_the_keyboard_from_the_editing_area_and_the_explorer() {
    ...
    let before = harness.state().document().text().to_string();
    harness.state_mut().command(unluminate_core::Command::PlaceCaret { offset: 0, extend: false });
    harness.run();
...
fn row_middle(harness: &mut Harness<'static, UnluminateApp>, name: &str) -> egui::Pos2 {
...
fn run_project(name: &str) -> Harness<'static, UnluminateApp> {
...
fn the_command_line_keeps_a_configuration_and_lists_it_with_the_suggestions() {
    let mut harness = run_project("unluminate-cli-run-add");
...
fn the_command_line_chooses_removes_and_refuses_a_name_nothing_holds() {
    let mut harness = run_project("unluminate-cli-run-select");
...
fn a_run_that_could_not_start_is_a_failure_rather_than_a_success() {
    ...
    let mut harness = run_project("unluminate-cli-run-cannot-start");
...
fn adding_a_configuration_whose_program_cannot_be_found_says_so_and_still_adds_it() {
    ...
    let mut harness = run_project("unluminate-cli-run-add-path");
...
fn every_run_command_reaches_the_window() {
    ...
    let mut harness = run_project("unluminate-cli-run-known");
...
fn folding_folder(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminate-screenshot-folding").join(name);
    std::fs::remove_dir_all(&root).ok();
    std::fs::create_dir_all(&root).expect("make the folder");
    std::fs::write(
        root.join("source.rs"),
...
fn folding_harness(name: &str) -> Harness<'static, UnluminateApp> {
...
fn laid_out_paragraphs(harness: &Harness<'static, UnluminateApp>) -> Vec<usize> {
...
fn a_function_can_be_collapsed_from_the_gutter_and_the_line_numbers_stay_right() {
    ...
    // more than the paragraph numbers Unluminate counts in.
...
fn collapse_all_but_highlighted_leaves_the_marked_passage_showing() {
    ...
    harness.state_mut().document_mut().highlight(start..end, unluminate_core::Rgba::new(0xC9, 0xA2, 0x27, 0x66));
    harness.run();
...
fn a_caret_put_inside_a_collapsed_block_expands_it() {
    ...
    harness
        .state_mut()
        .document_mut()
        .apply(unluminate_core::Command::PlaceCaret { offset, extend: false });
...
fn a_fold_stays_on_its_block_when_a_line_is_typed_above_it() {
    ...
    harness.state_mut().document_mut().apply(unluminate_core::Command::PlaceCaret { offset: 0, extend: false });
    harness.state_mut().document_mut().apply(unluminate_core::Command::Insert("// a new line\n".to_owned()));
    harness.run();
    harness.run();
...
fn a_picture_has_no_folding_entries_at_all() {
    ...
    assert!(unluminate_app::app::actions::folding_menu(&state).is_empty());
    assert!(unluminate_app::app::actions::folding_here_menu(&state).is_empty());
}
...
use unluminate_app::app::actions::DebugAction;
use unluminate_dap::Message;
...
fn debug_folder(name: &str) -> std::path::PathBuf {
    let root = std::env::temp_dir().join("unluminate-screenshot-debug").join(name);
    std::fs::remove_dir_all(&root).ok();
    std::fs::create_dir_all(&root).expect("make the folder");
    std::fs::write(
        root.join("main.rs"),
...
fn debug_harness(name: &str) -> Harness<'static, UnluminateApp> {
...
fn javascript_folder() -> std::path::PathBuf {
    ...
    FOLDER
        .get_or_init(|| {
            let root = std::env::temp_dir().join("unluminate-screenshot-javascript");
            std::fs::create_dir_all(&root).expect("make the folder");
...
fn asked(harness: &mut Harness<'static, UnluminateApp>) -> Vec<serde_json::Value> {
...
fn asked_for(harness: &mut Harness<'static, UnluminateApp>, command: &str) -> i64 {
    let batch = asked(harness);
    seq_of(&batch, command)
}

/// Hand a message to the session and let the window settle.
fn feed_debug(harness: &mut Harness<'static, UnluminateApp>, message: Message) {
...
fn paused_harness(name: &str) -> Harness<'static, UnluminateApp> {
    ...
    feed_debug(
        &mut harness,
        Message::Stopped(unluminate_dap::Stopped {
...
fn the_gutter_draws_an_enabled_a_disabled_an_unverified_and_a_conditional_breakpoint() {
    ...
    let conditional: Vec<bool> = harness
        ...
        .breakpoints()
        .iter()
        .map(unluminate_core::Breakpoint::is_conditional)
        .collect();
    assert_eq!(conditional, vec![false, true, false, false]);

    // A session that answered "I could not bind the last one", which is what makes it hollow. Unluminate
...
/// — puts the file **and** `.unluminate/breakpoints.conf` back together, consistent with each other. A
...
///
/// Unluminate already re-reads a **tab** whose file changed underneath it. This is that rule applied to
...
fn a_checkout_under_a_running_window_leaves_the_breakpoints_working() {
    let folder = std::env::temp_dir().join("unluminate-breakpoints-checkout");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the project");
    ...
    let written = folder.join(".unluminate").join("breakpoints.conf");
    ...
    // The checkout: something outside Unluminate puts back a longer `main.rs` **and** the breakpoints file
    ...
    std::thread::sleep(unluminate_app::app::WATCH_INTERVAL + std::time::Duration::from_millis(120));
    for _ in 0..8 {
        pump(&mut harness);
    }
...
///
/// The scripted adapter is used rather than a real one because what is asserted is **what Unluminate
/// sends**, which is Unluminate's half and is the same on every machine.
...
fn a_breakpoint_in_a_shut_file_with_windows_line_breaks_names_the_line_it_is_on() {
    let folder = std::env::temp_dir().join("unluminate-screenshot-debug").join("crlf");
    std::fs::remove_dir_all(&folder).ok();
    ...
    // What Unluminate believes, read back the way `debug breakpoint list` reads it.
...
/// Unluminate cannot see the difference, because `Path` compares components; the adapter can, and takes
...
fn a_breakpoint_binds_wherever_the_editor_is_scrolled() {
    for open in [true, false] {
        ...
        // was invisible from inside Unluminate, so it is asserted first and is not the assertion that
        // catches anything.
        assert_eq!(
            std::path::PathBuf::from(&sent),
            report,
            "{name}: the adapter is handed the path Unluminate holds"
        );
        ...
        // And Unluminate knows the adapter bound it, which is what stops the gutter drawing it hollow:
...
fn the_value_tooltip_shows_a_structure_and_opens_it_into_its_fields() {
    ...
    harness.state_mut().document_mut().apply(unluminate_core::Command::PlaceCaret {
        offset: offset + 2,
        extend: false,
    });
    harness.run();
...
fn a_file_whose_language_names_no_debugger_has_no_debug_controls_at_all() {
    // Unluminate's rule for a control that can never apply: absent, not dimmed. A stylesheet has nothing
    ...
    let entries = unluminate_app::app::actions::gutter_menu(&state);
    let names: Vec<String> = entries
        .iter()
        .filter_map(|entry| match entry {
            unluminate_app::app::actions::Entry::Item { name, .. } => Some(name.clone()),
...
fn the_command_line_can_set_a_breakpoint_read_the_stack_and_read_a_variable() {
    ...
    // driving Unluminate can observe a program's actual state instead of reasoning about it.
...
///
/// The scripted test above asserts the bytes that go on the wire, which is Unluminate's half. This is the
...
fn a_real_debugger_binds_a_breakpoint_in_a_file_that_is_not_open() {
    let Some(adapter) = std::env::var_os("UNLUMINATE_LLDB_ADAPTER")
        .map(std::path::PathBuf::from)
        .filter(|path| path.is_file())
        .or_else(|| unluminate_app::services::debuggers::on_path("codelldb"))
        .or_else(|| unluminate_app::services::debuggers::on_path("lldb-dap"))
    else {
        eprintln!(
            "skipped: no lldb adapter on this machine. `pwsh tools/get-debug-adapter.ps1` fetches \
             CodeLLDB and prints the path; point UNLUMINATE_LLDB_ADAPTER at it."
        );
        return;
    };

    let folder = std::env::temp_dir().join("unluminate-real-debug-not-open");
    std::fs::remove_dir_all(&folder).ok();
    ...
    harness.state_mut().run_configurations.add_permanent(configuration(
        "fleet",
        &unluminate_app::services::run_configurations::quote_part(&binary.to_string_lossy()),
    ));
...
/// taken of a state machine that was handed fixed messages. That proves Unluminate's half of the
...
/// a skipped test that says why is honest, and a red one that lies about Unluminate is not. `lldb-dap`
...
fn a_real_debugger_stops_at_a_breakpoint_and_reads_a_variable() {
    // `UNLUMINATE_LLDB_ADAPTER` first, which is the test's own spelling of the `debug.lldb` setting: an
    ...
    let Some(adapter) = std::env::var_os("UNLUMINATE_LLDB_ADAPTER")
        ...
        eprintln!(
            "skipped: no lldb adapter on this machine. `debug start` would say so too — \
             ...
             `codelldb.exe` is inside its .vsix. Point UNLUMINATE_LLDB_ADAPTER at either one."
        );
        return;
    };

    // A ten-line program, built here, so the test carries no binary and nothing is checked in.
    let folder = std::env::temp_dir().join("unluminate-real-debug");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the project");
    ...
    // build has no `answer` to read, which would make this test fail for a reason that is not Unluminate.
    let built = std::process::Command::new("rustc")
        .arg("-g")
        .arg("-C")
        .arg("opt-level=0")
...
/// seconds and failed — which is exactly what a person driving Unluminate saw.
///
/// It also proves the address. js-debug's own default host is `localhost`, which resolves to `::1`
/// before `127.0.0.1` on macOS, so an adapter left to its default binds where `unluminate_dap` never
...
fn a_real_node_debugger_stops_at_a_breakpoint_and_reads_a_variable() {
    let Some(adapter) = std::env::var_os("UNLUMINATE_NODE_ADAPTER")
        .map(std::path::PathBuf::from)
        .filter(|path| path.is_file())
    else {
        eprintln!(
            "skipped: no js-debug on this machine. `debug start` would say so too. \
             ...
             UNLUMINATE_NODE_ADAPTER at its dapDebugServer.js."
        );
        return;
    };
    let Some(node) = unluminate_app::services::debuggers::on_path("node") else {
        eprintln!("skipped: no node on this machine, and js-debug is a script node runs.");
        return;
    };

    let folder = std::env::temp_dir().join("unluminate-real-node-debug");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the project");
    ...
    let command = format!(
        "{} {}",
        unluminate_app::services::run_configurations::quote_part(&node.to_string_lossy()),
        unluminate_app::services::run_configurations::quote_part(&source.to_string_lossy())
    );
...
/// round-trips and `unluminate_core::breakpoints` proves the offsets move with the text, and both passed
/// while **the reading half was not wired up at all**: `.unluminate/breakpoints.conf` was written
...
fn a_breakpoint_is_still_there_when_the_project_is_opened_again() {
    let folder = std::env::temp_dir().join("unluminate-breakpoints-persist");
    std::fs::remove_dir_all(&folder).ok();
    std::fs::create_dir_all(&folder).expect("make the project");
    let source = folder.join("main.rs");
    ...
    let written = std::fs::read_to_string(folder.join(".unluminate/breakpoints.conf"))
...
/// only happens there.
fn type_and_paint(harness: &mut Harness<'static, UnluminateApp>, text: &str) {
    for letter in text.chars() {
        if letter == '\n' {
            harness.key_press(egui::Key::Enter);
        } else {
...
// unless the widget that holds focus says those keys are its own. Unluminate's editing area never held
...
fn window_commands(harness: &Harness<'static, UnluminateApp>) -> Vec<String> {
    harness
        .output()
        .viewport_output
        .values()
...
fn a_window_button_holds_the_keyboard(harness: &mut Harness<'static, UnluminateApp>) -> Option<String> {
...
fn typing_a_space_after_a_tab_or_an_arrow_key_cannot_close_or_minimise_the_window() {
    ...
    for key in [
        ...
        // And it is Unluminate's own holder that has it, so this test is passing for the reason it is meant
        // to and not because the title bar happened not to be drawn.
        assert_eq!(
            harness.ctx.memory(|memory| memory.focused()),
            Some(egui::Id::new(unluminate_app::app::KEYBOARD_HOLDER)),
            "the focus stays where Unluminate put it after {key:?}"
        );
...
fn an_idle_window_always_asks_to_be_woken_again() {
    ...
    let asked_for = harness
        ...
        .expect("the window's own output")
        .repaint_delay;
    assert!(asked_for <= unluminate_app::app::HEARTBEAT, "and still after a frame of typing: {asked_for:?}");
}

/// What holds egui's keyboard focus this frame.
fn what_holds_the_keyboard(harness: &Harness<'static, UnluminateApp>) -> Option<egui::Id> {
    harness.ctx.memory(|memory| memory.focused())
}

/// Unluminate's own holder, which is where the focus belongs while a pane is being typed into.
fn the_holder() -> egui::Id {
    egui::Id::new(unluminate_app::app::KEYBOARD_HOLDER)
}
...
fn panel_handle(harness: &Harness<'static, UnluminateApp>, label: &str) -> egui::Pos2 {
...
fn carry(harness: &mut Harness<'static, UnluminateApp>, from: egui::Pos2, to: egui::Pos2) {
    harness.input_mut().events.push(egui::Event::PointerMoved(from));
    harness.run();
    harness.input_mut().events.push(egui::Event::PointerButton {
        pos: from,
...
fn side_of(
    harness: &Harness<'static, UnluminateApp>,
    panel: unluminate_app::app::dock::Panel,
) -> unluminate_app::app::dock::Side {
    harness.state().panes.dock.side_of(panel)
}

#[test]
fn dragging_the_terminals_header_to_the_right_makes_it_a_column_down_that_edge() {
    use unluminate_app::app::dock::{Panel, Side};
    let mut harness = with_terminal("A document with the terminal beside it.", 12, 80);
    feed(&mut harness, b"jason.mcaffee@unluminate ~ % cargo build\r\n    Finished in 1.26s\r\n");
...
fn where_the_panels_are_survives_being_written_to_the_settings_file_and_read_back() {
    ...
    let mut values = unluminate_app::services::store::Values::new();
...
#[test]
fn a_panel_nobody_has_is_refused_with_the_ones_unluminate_does_have() {
    let mut harness = harness("");
...
// the answer is read back through `unluminate-cli` rather than out of the window's own state, which is the
...
///
/// Unluminate\'s pane machinery is untouched, and this is the test that says so: the board contributes no pane,
...
fn the_board_contributes_a_tab_and_no_pane() {
    ...
    assert_eq!(
        ...
        "`panel list` is Unluminate\'s own four; a contributed pane is moved with `plugins pane`: {names:?}"
    );
...
/// manifest. That is a better test than the one it replaces: Unluminate\'s pane machinery used to be verified
...
fn the_pane_is_moved_and_put_away_from_the_command_line() {
    use unluminate_app::app::dock::{Panel, Side};
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-plugin-pane");
    let settings = folder.join(".unluminate-settings");
    ...
    let mut harness = harness_in(&folder);
    harness.state_mut().use_store(unluminate_app::services::store::Store::at(&settings));
    harness.run();
...
#[test]
fn the_plugins_menu_is_after_unluminates_own_six_and_its_entries_run_through_one_path() {
    let mut harness = harness("");
    let listed = did(&mut harness, "action list");
    ...
    let built_in: Vec<usize> = ["Unluminate", "File", "Edit", "View", "Run", "Git"]
        .iter()
        .map(|name| at(name).unwrap_or_else(|| panic!("{name} is missing: {menus:?}")))
        .collect();
    assert!(
        built_in.windows(2).all(|pair| pair[0] < pair[1]),
        "Unluminate's own six are still in their own order: {menus:?}"
    );
...
fn nothing_the_plugin_owns_is_built_until_its_button_is_pressed() {
    ...
    // plugin code. Here it is the difference between opening a database when Unluminate starts and opening it
...
fn switching_the_plugin_off_withdraws_every_contribution_in_the_same_frame() {
    ...
    // Unluminate opening a blank one to avoid showing none at all.
    let mut harness = harness("Some prose.");
    did(&mut harness, "plugins tab agent-tasks/board --open");
    harness.run();
...
fn the_board_can_be_read_and_changed_entirely_from_the_command_line() {
    // Unluminate's rule: everything a person can do in the window an agent can do too, through the same code.
...
fn a_manifest_changed_by_hand_takes_effect_on_a_reload_with_no_restart() {
    ...
    // box.
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-plugin-reload");
    let settings = folder.join(".unluminate-settings");
    let plugin = settings.join("plugins").join("agent-tasks");
    std::fs::create_dir_all(&plugin).expect("a plugin folder");
    ...
    // And a manifest that will not parse is skipped with its reason rather than stopping Unluminate.
    std::fs::write(plugin.join("plugin.conf"), "plugin.id = agent-tasks\nplugin.kind = wasm\n")
        .expect("a manifest Unluminate refuses");
...
fn the_plugins_pane_takes_the_windows_own_transparency_and_font() {
    ...
    use unluminate_app::services::plugin_ui::Look;
    let mut harness = harness("");
...
fn the_board_in_a_narrow_editing_area() {
    ...
    did(&mut harness, "plugins run agent-tasks new-task Unluminate \u{2014} Plugin architecture for UI");
...
///
/// The off switch is a control like any other and Unluminate's rule is that a control has a test. It is also
...
fn the_same_board_in_two_windows_is_the_same_picture() {
    fn board(harness: &mut Harness<'static, UnluminateApp>) {
        did(harness, "plugins run agent-tasks new-sprint Current Sprint");
        did(harness, "plugins run agent-tasks new-task Unluminate \u{2014} Plugin architecture for UI");
...
fn a_board_with_sprints() -> Harness<'static, UnluminateApp> {
    let mut harness = harness("");
    did(&mut harness, "plugins tab agent-tasks/board --open");
    did(&mut harness, "plugins run agent-tasks new-sprint August 2nd Half");
    did(&mut harness, "plugins run agent-tasks new-epic Unluminate");
    did(&mut harness, "plugins run agent-tasks epic-colour Unluminate #8B6BFF");
    ...
    for title in ["Upgrade and GPU stuck", "Unluminate agent chat plugin", "Rust db scoring system"] {
...
fn the_epics_view_is_a_grid_of_cards_that_can_be_renamed_recoloured_and_deleted() {
    ...
    harness.run();
    harness.get_by_label("Unluminate");
    harness.snapshot(shot("agent_tasks_epics").as_str());

    // Recolouring is one press, and it is the same command the command line runs.
    harness.get_by_label("Unluminate colour #2FCFA6").click();
    harness.run();
    let epics = did(&mut harness, "plugins view agent-tasks")["epics"].clone();
    let unluminate = epics
        .as_array()
        .expect("the epics")
        .iter()
        .find(|epic| epic["name"] == "Unluminate")
        .expect("the Unluminate epic")
        .clone();
    assert_eq!(unluminate["color"], "#2FCFA6");
    ...
    harness.get_by_label("Delete Unluminate").click();
    harness.run();
    harness.get_by_label("Really delete Unluminate");
    harness.get_by_label("Keep Unluminate").click();
    harness.run();
    assert_eq!(
...
fn the_reviews_findings_about_the_boards_commands() {
    ...
    let epics = did(&mut harness, "plugins view agent-tasks")["epics"].clone();
    assert!(epics.as_array().expect("the epics").iter().any(|epic| epic["name"] == "Unluminate"));
    did(&mut harness, "plugins run agent-tasks view epics");
    harness.run();
    harness.get_by_label("Rename Unluminate").click();
    harness.run();
    harness.get_by_label("Epic name").click();
    harness.run();
    harness.input_mut().events.push(egui::Event::Text(" IDE".to_owned()));
    harness.run();
    harness.get_by_label("Save Unluminate").click();
    harness.run();
    let epics = did(&mut harness, "plugins view agent-tasks")["epics"].clone();
    assert!(
        epics.as_array().expect("the epics").iter().any(|epic| epic["name"] == "Unluminate IDE"),
...
fn the_sprints_and_the_epics_are_driven_entirely_from_the_command_line() {
    ...
    assert_eq!(refused(&mut harness, "plugins run agent-tasks epic-colour Unluminate sideways"), "failed");
}
...
fn where_a_contributed_pane_was_left_is_remembered_against_its_own_name() {
    ...
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-plugin-pane-layout");
    let store = unluminate_app::services::store::Store::at(folder.join(".unluminate-settings"));
    let mut panes = settings::Panes::new();
...
fn a_pane_that_asks_for_a_project_is_absent_when_there_is_none() {
    // Unluminate's rule everywhere: a control that cannot apply is absent rather than dimmed. Agent-Tasks asks for
    ...
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-plugin-in-project");
    let settings = folder.join(".unluminate-settings");
    ...
    .expect("a manifest");
    let (plugins, problems) = unluminate_app::services::plugins::Plugins::load(Some(
        &unluminate_app::services::store::Store::at(&settings),
    ));
    assert!(problems.is_empty(), "the manifest should parse: {problems:?}");
    let mut ui = unluminate_app::app::plugin_panes::PluginUi::default();
    ui.refresh(&plugins);
    let slot = ui
...
fn a_pane_that_is_showing_stays_the_pane_that_is_showing_when_the_plugins_change() {
    ...
    // does.
    let folder = copy_out_of_the_repository(&sample_folder(), "unluminate-plugin-pane-persists");
    let settings = folder.join(".unluminate-settings");
    let plugin = settings.join("plugins").join("agent-tasks");
    std::fs::create_dir_all(&plugin).expect("a plugin folder");
    std::fs::write(
...
fn chosen_agent(harness: &mut Harness<'static, UnluminateApp>) -> String {
    ...
    let tasks = provider
        .as_any_mut()
        .and_then(|any| any.downcast_mut::<unluminate_app::services::agent_tasks::AgentTasks>())
...
fn chosen_card(harness: &mut Harness<'static, UnluminateApp>) -> Option<(String, usize)> {
    let provider = harness.state_mut().plugin_ui.provider("agent-tasks").expect("the board is open");
    let tasks = provider
        .as_any_mut()
        .and_then(|any| any.downcast_mut::<unluminate_app::services::agent_tasks::AgentTasks>())
...
/// `unluminate-core`, so the buttons are always there.
#[test]
...
fn choosing_in_each_dropdown_writes_the_field_it_names() {
    use unluminate_app::services::agent_tasks::agent;
    use unluminate_app::services::agent_tasks::model::Assignee;
    let mut harness = harness("");
    did(&mut harness, "plugins tab agent-tasks/board --open");
    did(&mut harness, "plugins run agent-tasks new-sprint Current Sprint");
    ...
    let pick = |harness: &mut Harness<'static, UnluminateApp>, control: &str, said: &str| {
...
fn add_task_opens_an_editor_with_every_field_a_ticket_needs() {
    ...
    harness.run();
    for model in unluminate_app::services::agent_tasks::agent::models_for(
        unluminate_app::services::agent_tasks::model::Assignee::Claude,
        None,
    ) {
        assert!(
    ...
    use unluminate_app::services::agent_tasks::Field;
...
// answers is not something a test can know. `unluminate-chat`'s own tests drive the whole client against a
...
const NO_SUCH_AGENT: &str = "unluminate-no-such-agent-anywhere";
...
fn with_the_chat(
    harness: &mut Harness<'static, UnluminateApp>,
    act: impl FnOnce(&mut unluminate_app::services::agent_chat::AgentChat),
) {
    let provider =
        harness.state_mut().plugin_ui.opened("agent-chat", "agent-chat").expect("the chat provider");
    let chat = provider
        .as_any_mut()
        .and_then(|any| any.downcast_mut::<unluminate_app::services::agent_chat::AgentChat>())
...
fn a_chat(said: &[(unluminate_chat::Role, &str)]) -> Harness<'static, UnluminateApp> {
    let mut harness = harness("");
    did(&mut harness, "plugins pane agent-chat/chat --show");
    harness.run();
    let said: Vec<(unluminate_chat::Role, String)> =
    ...
    with_the_chat(&mut harness, move |chat| {
        for (role, text) in said {
            ...
            chat.session_mut().chat.push(unluminate_chat::Message::said(id, role, text));
        }
    });
    harness
}
...
/// `panes.agent-chat/chat.width` — a key **Unluminate writes into its own settings file**. So the chat
...
fn a_contributed_panes_size_is_settable_from_the_command_line() {
    ...
    // The same numbers through the settings, which is the other half of the ticket: a key Unluminate
    // writes is a key Unluminate reads.
    ...
    // `panel list` stays Unluminate's own four — a contributed pane is listed by `plugins list` and
    ...
    // Reset puts it back to what the **manifest** asked for rather than to a number inside Unluminate,
    // because that is what "what a new Unluminate has" means for a pane Unluminate did not write.
...
fn a_question_and_an_answer_are_two_bubbles_on_opposite_sides() {
    let mut harness = a_chat(&[
        (unluminate_chat::Role::User, "Why is `relayout` keeping a paragraph it should have thrown away?"),
        (
            unluminate_chat::Role::Assistant,
...
fn an_answer_that_is_still_arriving_shows_what_has_come_so_far() {
    ...
    with_the_chat(&mut harness, |chat| {
        chat.session_mut().ask(unluminate_chat::Message::said(0, unluminate_chat::Role::User, "Explain the fold."));
        chat.session_mut().reply(unluminate_chat::Reply::Started { model: "claude-opus-5".to_owned() });
        chat.session_mut()
            .reply(unluminate_chat::Reply::Text("A hidden paragraph produces no lines and keeps".to_owned()));
    });
...
fn a_tool_the_model_asked_for_is_run_by_the_window_and_its_answer_comes_back() {
    let mut harness = harness("Some prose.");
    did(&mut harness, "plugins pane agent-chat/chat --show");
    // **An address row, because only an address row asks Unluminate to run anything.** The two rows that
    ...
    with_the_chat(&mut harness, |chat| {
        ...
        chat.session_mut().ask(unluminate_chat::Message::said(0, unluminate_chat::Role::User, "What is open?"));
        chat.session_mut().reply(unluminate_chat::Reply::ToolCall {
            id: "t1".to_owned(),
            name: "unluminate_tab".to_owned(),
            arguments: "{\"command\":\"list\"}".to_owned(),
        });
        chat.session_mut().reply(unluminate_chat::Reply::Finished { reason: "tool_use".to_owned() });
    });
...
fn a_tool_call_is_shown_while_it_runs_and_when_it_has_finished() {
    ...
    with_the_chat(&mut harness, |chat| {
        ...
        chat.chat.push(unluminate_chat::Message::said(id, unluminate_chat::Role::User, "What does git say?"));
        let id = chat.chat.next_id();
        let mut answering = unluminate_chat::Message::said(id, unluminate_chat::Role::Assistant, "Let me look.");
        answering.tools.push(unluminate_chat::ToolCall::new("t1", "unluminate_git", "{\"command\":\"status\"}"));
    ...
    with_the_chat(&mut harness, |chat| {
        ...
        tool.took = Some(84);
        let id = chat.chat.next_id();
        chat.chat.push(unluminate_chat::Message::said(
            id,
            unluminate_chat::Role::Assistant,
    ...
    let view = did(&mut harness, "plugins view agent-chat");
    let tools = view["conversation"]["messages"][1]["tools"].as_array().expect("the tool calls");
    assert_eq!(tools[0]["name"], "unluminate_git");
...
fn a_picture_attached_to_a_question_is_drawn_in_the_bubble_it_was_sent_with() {
    ...
    with_the_chat(&mut harness, |chat| {
        ...
        let id = chat.session_mut().chat.next_id();
        let mut asked = unluminate_chat::Message::new(id, unluminate_chat::Role::User);
        asked.parts.push(unluminate_chat::Part::Text("What is wrong with this diagram?".to_owned()));
        asked.parts.extend(parts);
        chat.session_mut().chat.push(asked);
        let id = chat.session_mut().chat.next_id();
        chat.session_mut().chat.push(unluminate_chat::Message::said(
            id,
            unluminate_chat::Role::Assistant,
...
fn a_refusal_is_drawn_in_the_servers_own_words() {
    ...
    harness.run();
    with_the_chat(&mut harness, |chat| {
        chat.session_mut().ask(unluminate_chat::Message::said(0, unluminate_chat::Role::User, "Hello?"));
        chat.session_mut().reply(unluminate_chat::Reply::Text("Half an ans".to_owned()));
        chat.session_mut()
            .reply(unluminate_chat::Reply::Failed("HTTP 429: rate_limit_error: too many requests".to_owned()));
    });
    let view = did(&mut harness, "plugins view agent-chat");
    assert_eq!(view["state"], "failed");
...
fn agents_on_the_path() {
    ...
    ONCE.get_or_init(|| {
        let folder = std::env::temp_dir().join("unluminate-screenshot-agents");
...
fn the_chat_answers_the_command_line() {
    ...
    // and codex etc through cli"* — so what a row holds is a program, and Unluminate holds no key for
    // either of them.
    ...
    with_the_chat(&mut harness, |chat| {
        chat.session_mut().ask(unluminate_chat::Message::said(0, unluminate_chat::Role::User, "Hello"));
        chat.session_mut().reply(unluminate_chat::Reply::Text("Hello yourself.".to_owned()));
        chat.session_mut().reply(unluminate_chat::Reply::Finished { reason: "stop".to_owned() });
    });
...
// cannot be assumed on the machine running a test — `crates/unluminate-db/tests/scripted_server.rs` is
// where the wire protocol is tested against a server made of fixed bytes, and
// `cargo run -p unluminate-db --example connect` is how the real one is. This is the layer above both:
...
fn a_database_file(name: &str) -> std::path::PathBuf {
    ...
    let folder = std::env::temp_dir().join(format!("unluminate-database-shot-{name}"));
...
fn a_database(name: &str) -> Harness<'static, UnluminateApp> {
...
fn until_the_database_settles(harness: &mut Harness<'static, UnluminateApp>) {
    for _ in 0..400 {
        harness.step();
...
fn press_at(harness: &mut Harness<'static, UnluminateApp>, at: egui::Pos2) {
    harness.input_mut().events.push(egui::Event::PointerMoved(at));
    harness.input_mut().events.push(egui::Event::PointerButton {
        pos: at,
        button: egui::PointerButton::Primary,
...
fn a_table_opens_in_the_workspace_with_its_rows() {
    ...
    // the engine, which is the fault `unluminate_db::Value` exists to keep out of a grid.
...
fn a_view_is_drawn_read_only_with_the_reason_where_the_buttons_were() {
    ...
    // which is the distinction `unluminate-cli`'s two codes already draw.
...
/// *server's* rather than a parser in Unluminate: `SET SESSION CHARACTERISTICS AS TRANSACTION READ ONLY`
...
fn the_database_settings_page_says_where_a_password_is_and_never_what_it_is() {
    ...
    did(
        &mut harness,
        "plugins run database add-source library postgres://postgres@localhost:5432/library UNLUMINATE_DB_LIBRARY",
    );
    did(&mut harness, "action run settings");
    harness.run();
    ...
    let view = did(&mut harness, "plugins view database");
    assert_eq!(view["sources"][0]["password"], "environment UNLUMINATE_DB_LIBRARY");
...
fn the_open_page(harness: &mut Harness<'static, UnluminateApp>) -> serde_json::Value {
...
///
/// It comes from `unluminate_db::sql::create_table`, which is the same call `Create` makes — the rule the
