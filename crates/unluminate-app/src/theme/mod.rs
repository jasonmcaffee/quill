...
//!
//! Every colour in [`Palette::UNLUMINATE_DARK`] was read out of `design/intial-design-screenshot.png` rather than
...
//! the style guide says about the palette is still true: this module is the whole list of colours Unluminate
...
//! `crates/unluminate-app/tests/screenshots.rs` holds 169 accepted pictures, cargo runs them in parallel in one
...
//!
//! What it asks is that nothing paints off the thread that drew: the background workers — `unluminate_git`, the
...
///
/// Each entry becomes a field on [`Palette`], its value in [`Palette::UNLUMINATE_DARK`], a name in
...
macro_rules! palette {
    ($( $(#[$note:meta])* $name:ident = $default:expr; )*) => {
        ...
        /// `unluminate_core::mermaid::Scene` and `services::vello_canvas::Decor` already are.
        ...
        impl Palette {
            /// The palette Unluminate shipped with, and the one every theme inherits the names it does not set.
            pub const UNLUMINATE_DARK: Palette = Palette { $( $name: $default, )* };
...
palette! {
    ...
    /// `tasks/unluminate-ide-tdd.md` section 2, in the same way: the two colours covering the annotation
    ...
    ///
    /// **This is the one place a second blue is right, and it took two reviews to be sure of it.** Unluminate's
    ...
    /// Unluminate's own accent still means *this is where the keyboard is* everywhere, the board included, so
    ...
    /// beside itself. Unluminate's palette has a red, an amber, a green, two blues and a pink, and the board
...
pub mod derived {
    ...
    use unluminate_core::Rgba;

    /// The four colours a passage can be marked in, on the editor's right click menu.
    ///
    /// The palette is closed and these do not open it: they are the accents Unluminate shipped with — the
    ...
    ///
    /// They are `unluminate_core::Rgba` rather than `Color32` because a highlight is a value that is written to
    ...
    ///
    /// Git's added green rather than a colour of its own: it is the green Unluminate already means "there is
...
pub enum IconSet {
    /// The marks Unluminate shipped with: a solid disclosure triangle, stroked rail buttons, no folder mark.
    ///
...
/// drawn.
///
/// Read from a `plugin.kind = theme` manifest, or [`Theme::unluminate_dark`], which is the one built in.
#[derive(Debug, Clone, PartialEq)]
pub struct Theme {
    /// `unluminate/dark`, `themes-bundle-1/dracula` — the plugin and the theme, as a contributed pane is named.
    pub key: String,
    /// What a person reads in the Settings list.
    pub name: String,
    /// Which plugin it came from, for `theme list`. `unluminate` for the built-in one.
    pub plugin: String,
...
impl Theme {
    /// The theme Unluminate shipped with: the design's own numbers, and **no syntax colours at all**.
    ///
    ...
    pub fn unluminate_dark() -> Theme {
        Theme {
            key: "unluminate/dark".to_owned(),
            name: "Unluminate Dark".to_owned(),
            plugin: "unluminate".to_owned(),
            dark: true,
            palette: Palette::UNLUMINATE_DARK,
            syntax: None,
...
/// `UnluminateApp::apply_the_theme` — so the two can never be half done.
...
pub mod size {
    ...
    /// because Unluminate's holds three buttons rather than a dozen. Twenty four for the button, six either
...
///
/// The one place the two spellings of a colour meet. `unluminate_core::Rgba` is what a mark is stored,
...
pub fn color32(color: unluminate_core::Rgba) -> Color32 {
...
pub fn apply_scaled(ctx: &egui::Context, interface_scale: f32) {
    ...
    // plus is pressed. That is a browser's zoom, and it is not what an editor's zoom means: Unluminate's
...
pub const BOLD_FAMILY: &str = "unluminate-bold";
...
/// shows a genuinely bold B, so the family Unluminate is using is handed to egui as well, with its bold face
...
pub fn install_fonts(ctx: &egui::Context, family: &str, regular: Option<Vec<u8>>, bold: Option<Vec<u8>>) {
    ...
    if let Some(bytes) = bold {
        fonts.font_data.insert("unluminate-ui-bold".to_owned(), std::sync::Arc::new(egui::FontData::from_owned(bytes)));
        bold_stack.push("unluminate-ui-bold".to_owned());
    }
    if let Some(bytes) = regular {
        fonts.font_data.insert("unluminate-ui".to_owned(), std::sync::Arc::new(egui::FontData::from_owned(bytes)));
        if let Some(list) = fonts.families.get_mut(&egui::FontFamily::Proportional) {
            list.insert(0, "unluminate-ui".to_owned());
        }
        bold_stack.push("unluminate-ui".to_owned());
    }
...
///
/// Markdown gets the blue square because Unluminate treats it differently, having a preview for it. Every
/// other kind of text gets the grey one, whether Unluminate knows the extension or not. What the status bar
...
mod tests {
    ...
    /// The test that makes the 689-site rewrite safe: every colour Unluminate shipped with, still there.
    #[test]
    fn unluminate_dark_is_exactly_what_shipped() {
        activate(Theme::unluminate_dark());
    ...
    fn a_derived_colour_follows_the_one_it_is_defined_as() {
        ...
        let mut theme = Theme::unluminate_dark();
    ...
    fn every_role_is_readable_and_writable_by_name() {
        let mut palette = Palette::UNLUMINATE_DARK;
        ...
        assert!(palette.get("editor_background").is_none(), "a name Unluminate has not got");
    ...
    fn an_accent_reaches_everything_that_means_the_accent() {
        ...
        activate(Theme::unluminate_dark().with_accent(Color32::from_rgb(0xFF, 0x79, 0xC6)));
