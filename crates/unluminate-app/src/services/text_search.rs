...
//! like a crash. So the reading happens on a thread, the same arrangement `unluminate_git::Worker` and
...
use unluminate_core::symbols::{self, Role};
...
impl Searcher {
    ...
    pub fn start(wake: Arc<dyn Fn() + Send + Sync>) -> Self {
        ...
        std::thread::Builder::new()
            .name("unluminate-search".to_owned())
            .spawn(move || {
...
/// Read a file if it is one worth searching: text, and not larger than Unluminate will open.
...
///
/// The reference half of the searcher. `unluminate_core::symbols::occurrences` decides what a whole word
/// is and what each one was found inside — the same function `unluminate_core` tests with no window —
...
pub fn references_in(
    ...
    text: &str,
    name: &str,
    grammar: &unluminate_core::Grammar,
    limit: usize,
) -> Vec<Hit> {
...
mod tests {
    ...
    fn rust() -> unluminate_core::Grammar {
    ...
    fn a_reference_is_a_whole_word_and_says_which_line_it_is_on() {
        // Scenario 23 through the app's own half of it: the same rule `unluminate_core` tests, turned
    ...
    fn the_thread_answers_the_newest_question_and_abandons_the_ones_before_it() {
        let folder = std::env::temp_dir().join("unluminate-text-search");
