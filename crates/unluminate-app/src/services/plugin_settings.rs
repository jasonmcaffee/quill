...
/// before, and a plugin's file may hold keys this version of Unluminate has never heard of.
///
...
mod tests {
    ...
    fn folder_for(name: &str) -> PathBuf {
        let folder = std::env::temp_dir().join(format!("unluminate-plugin-settings-{name}"));
