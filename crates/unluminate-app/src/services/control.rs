//! The channel a running Unluminate is driven down.
//!
...
//! `unluminate_cli::catalogue`, which both halves share so they cannot drift apart.
//!
...
//! language with a socket and a JSON library can drive Unluminate in three lines, so `unluminate-cli` is the
...
//! reliable way to wake a window. Two mechanisms lose it, both in code Unluminate does not own.
...
//! the event loop, where Unluminate sits. `app::cli::pump_control` already does the same thing one step
...
use unluminate_cli::instances::{self, Instance};
    ...
    fn drop(&mut self) {
        if let Some(channel) = self.reply.take() {
            let _ = channel.send(Reply::failed(
                "",
                code::FAILED,
                "Unluminate dropped the request without answering it.",
            ));
        }
    }
}
...
pub struct Server {
    ...
    /// Where the instance file was written, so the same one is removed when Unluminate stops.
    file: PathBuf,
    health: Arc<Health>,
}
...
impl Server {
    ...
    pub fn start(folder: PathBuf, wake: Arc<dyn Fn() + Send + Sync>) -> Option<Self> {
        let listener = match bind() {
            ...
            Err(problem) => {
                eprintln!("Unluminate could not open its command channel: {problem}");
                return None;
            }
        };
        let port = match listener.local_addr() {
            ...
            Err(problem) => {
                eprintln!("Unluminate could not read its own command channel's port: {problem}");
                return None;
            }
        };
        ...
        let file = match advertise(&instance) {
            Ok(file) => file,
            Err(problem) => {
                eprintln!("Unluminate could not write its instance file: {problem}");
                return None;
            }
        };
        ...
        std::thread::Builder::new()
            .name("unluminate-control".to_owned())
            .spawn(move || accept(listener, sender, expected, wake, theirs))
            .ok()?;
        Some(Server { instance, requests, file, health })
    }

    /// Which port this Unluminate is listening on, for a test and for the status command.
...
impl Drop for Server {
    ...
    /// telling the next `unluminate-cli instances` about a window that is no longer there.
    fn drop(&mut self) {
...
/// Open the listener.
///
/// Loopback only, and port zero so the operating system finds a free one. Several Unluminates run at
...
/// Write the instance file, readable only by the person running Unluminate.
...
fn accept(
    ...
    for stream in listener.incoming() {
        ...
        let _ = std::thread::Builder::new()
            .name("unluminate-control-connection".to_owned())
...
/// what makes a request that reached Unluminate *after* its caller had gone answerable at all: the
...
fn read_and_queue(
    ...
    if request.token != expected {
        return Reply::failed(
            &request.command,
            code::REFUSED,
            "That is not this Unluminate's token. It is in the instance file; `unluminate-cli instances` \
             finds it.",
        );
    }
    ...
    if watched && caller_has_gone(stream) {
        ...
        return Reply::failed(
            ...
            code::TIMED_OUT,
            format!(
                "The caller had closed the connection before {command} reached Unluminate, so it was \
                 not run. {}",
...
fn wait_for_the_window(
    ...
    loop {
        ...
        let ran = match picked_up {
            true => "Unluminate had taken it, so it may already have been run.",
        ...
        return Reply::failed(
            ...
            code::TIMED_OUT,
            format!(
                "Unluminate did not answer {command} within {asked} ms. {} {ran}",
                health.what_was_seen()
            ),
        );
    }
...
mod tests {
    ...
    fn a_server(name: &str) -> Running {
        ...
        let folder = std::env::temp_dir().join(format!("unluminate-control-{name}"));
    ...
    fn a_caller_that_closed_the_connection_is_never_waited_for() {
        ...
        // is the one that covers a request which reached Unluminate *after* its caller gave up — where
        ...
        // been read, and that is the network's business rather than Unluminate's.
