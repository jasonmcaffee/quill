//! The project's breakpoints, kept beside the project in `.unluminate/breakpoints.conf`.
//!
...
//! and `unluminate_core::Breakpoints` inside each — sorted by offset, so the one on a line is a binary
...
//! ```
//!
//! Read and written by [`crate::services::store::Values`] like every other file Unluminate writes. A block
...
//!
//! Offsets are bytes against the file as Unluminate last had it. A file rewritten outside Unluminate moves its
//! own bytes and Unluminate cannot know it, so `Document::set_breakpoints` clamps them: the worst case is
...
//!
//! [`BreakpointStore::load`] is called from `UnluminateApp::restore_project` and nowhere else, exactly as
//! the marks and the project state are: a test must not touch a person's files, and a `.unluminate` folder
...
use unluminate_core::breakpoints::{Breakpoint, Breakpoints};
...
impl BreakpointStore {
    ...
    /// What would be written, the files in a settled order so that two runs of Unluminate that put
