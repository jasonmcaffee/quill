...
//! program rather than from the menu, so Unluminate's own menu is put first. What AppKit calls the running program
//! is the process name, which for a program run straight from `target/release` is the file name, `unluminate`. So
//! the process name is set to `Unluminate` before the bar is built, and the bar reads
//! `Unluminate  File  Edit  View`.
//!
    ...
    fn rebuild(&mut self, menus: &[Menu]) {
        ...
        for menu in menus {
            ...
            if let Err(problem) = append(&submenu, &menu.entries, &mut actions) {
                eprintln!("Unluminate could not build the {} menu: {problem}", menu.name);
            }
            if let Err(problem) = bar.append(&submenu) {
                eprintln!("Unluminate could not add the {} menu to the bar: {problem}", menu.name);
                return;
            }
        }
        bar.init_for_nsapp();
...
/// Tell AppKit the application is called `Unluminate`, so the application menu says so.
///
/// The name in that menu is the process name, and the process name of a program run from `target/release` is
/// the file name of the program, which is `unluminate` in lower case. `NSProcessInfo` lets it be set, and setting
/// it before the menu bar is built is what makes the bar read `Unluminate` rather than `unluminate`. A packaged
...
fn name_the_application() {
    ...
    NSProcessInfo::processInfo().setProcessName(ns_string!("Unluminate"));
}

/// Put Unluminate's entries into a native submenu, remembering what each identifier means.
#[cfg(target_os = "macos")]
fn append(
...
/// Turn one of Unluminate's shortcuts into a macOS key equivalent.
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    #[test]
    fn the_application_is_called_unluminate_rather_than_the_name_of_the_program_file() {
        // The name in the application menu is the process name, and the program file is `unluminate` in lower
        // case. This checks the setting took; what the bar draws can only be seen by looking at it.
        use objc2_foundation::NSProcessInfo;
        name_the_application();
        assert_eq!(NSProcessInfo::processInfo().processName().to_string(), "Unluminate");
    }
