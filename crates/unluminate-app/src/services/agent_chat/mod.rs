...
//!
//! `tasks/task-1767-agent-chat-tdd.md` is the design. `unluminate-chat` is the half that can be tested
...
//! `UnluminateApp::run_cli` — **the one place a command turns into a change** — and hands the answer back
...
//! Unluminate with a socket in it.
...
use unluminate_chat::model::{Message, Part, Role};
use unluminate_chat::provider::{Provider, Wire, WIRES};
use unluminate_chat::{Client, Conversation, Session, State};
...
/// `name = value` form, so a person can open it in Unluminate and change it. **No key is in it** — a
...
pub struct Configuration {
    ...
    pub chosen: String,
    pub stream: bool,
    /// Whether Unluminate's own commands are offered to the model. Off unless somebody says so.
    pub tools: bool,
    ...
    /// safest mode. Unluminate's own `tools` and `shell` switches are the equivalent for an HTTP endpoint,
    /// where the model asks and Unluminate runs it — the two are never both in play, because the
    /// transport decides which of them applies.
    pub permission: unluminate_chat::Permission,
    /// The person's own system prompt, added after Unluminate's own line.
    pub system: String,
    pub history: usize,
}
...
impl Default for Configuration {
    fn default() -> Self {
        Self {
            ...
            permission: unluminate_chat::Permission::default(),
            system: String::new(),
...
impl Configuration {
    ...
    /// is simply not there and nothing says why — the fault every other registry in Unluminate is refused
    ...
    pub fn of(values: &Values) -> (Self, Vec<String>) {
        ...
        if let Some(named) = values.text("permission") {
            match unluminate_chat::Permission::from_name(named) {
                ...
                // to anything else would be Unluminate widening a permission nobody granted.
                None => refused.push(format!(
                    "`{}` is not something an agent may be allowed to do, and this version of Unluminate has {}.",
                    named.trim(),
                    unluminate_chat::PERMISSIONS.join(", ")
                )),
            }
        }
        ...
        std::fs::write(
            ...
            values.to_text_headed(
                "# The Agent-Chat plugin's settings. `Settings -> Agent-Chat` writes this file and reads it back.\n\
                 ...
                 # written here or anywhere else by Unluminate.",
            ),
        )
...
fn provider_at(values: &Values, index: usize) -> Result<Option<Provider>, String> {
    ...
    let Some(wire) = Wire::from_name(named) else {
        return Err(format!(
            "`{name}` speaks `{named}`, and this version of Unluminate speaks {}.",
            WIRES.join(", ")
        ));
    };
    Ok(Some(Provider {
        ...
        max_tokens: values
            ...
            .unwrap_or(unluminate_chat::provider::DEFAULT_MAX_TOKENS as f32)
...
///
/// On the provider rather than in the component, because a component in Unluminate takes a rectangle,
...
pub struct PaneState {
    ...
    /// `UnluminateApp::follow_the_open_file` already uses.
...
///
/// A component in Unluminate takes a rectangle, draws and reports what happened — it changes nothing. So
...
impl AgentChat {
    ...
    pub fn attach(&mut self, path: &Path) -> Result<(), String> {
        ...
        let media = media_type_of(path, &bytes)
            .ok_or_else(|| format!("{} is not a picture Unluminate can send.", path.display()))?;
        let name = path
            .file_name()
    ...
    fn attach_from(&mut self, value: &serde_json::Value) -> Result<(), String> {
        ...
        let bytes = unluminate_chat::base64::decode(value["data"].as_str().unwrap_or_default())
    ...
    fn dispatch(&mut self, provider: &Provider) {
        if let Some(why) = provider.why_not() {
            self.problem = Some(why.clone());
            self.session.reply(unluminate_chat::Reply::Failed(why));
            return;
        }
        ...
        let body = unluminate_chat::wire::request(
            provider,
            &self.session.chat,
            &self.system_prompt(),
            &tools,
    ...
    /// confuse an agent whose own record of the turn includes tools Unluminate never saw.
    ///
    /// Unluminate's own line about where it is goes in front of the **first** question only, for the same
    /// reason: after that the agent knows, and repeating it every turn would be a paragraph of
    /// preamble on every message.
    fn what_to_ask_the_agent(&mut self) -> unluminate_chat::Ask {
        let newest = self
            .session
            .chat
            .messages
        ...
        if let Some(message) = newest {
            ...
            for (name, _, bytes) in message.pictures() {
                if let Some(path) = unluminate_chat::agent::write_a_picture(&folder, name, bytes) {
                    pictures.push(path);
                }
            }
        }
    ...
    /// The line Unluminate puts in front of an agent's first question.
    ///
    ...
    fn what_the_agent_should_know(&self) -> String {
        let mut lines = vec![
            "You are answering in a chat pane inside Unluminate, a code editor, beside the person's work. Be brief and concrete; they can see their own screen, and your answer is read as markdown rather than in a terminal.".to_owned(),
        ];
    ...
    /// What Unluminate tells the model about where it is.
    ///
    /// Unluminate's own line first, then the person's. **Which project is open and which file is showing,
    ...
    pub fn system_prompt(&self) -> String {
        let mut lines = vec![
            "You are answering inside Unluminate, a code editor, in a pane beside the person's work. Be brief and concrete; they can see their own screen.".to_owned(),
        ];
        ...
        if self.configuration.tools {
            lines.push(
                "You can drive this window with the tools you have been given. They are Unluminate's own commands, so anything you do this way is exactly what the person would get from the menu, and it is one undo step. Read a file before changing it."
                    .to_owned(),
            );
        }
        if !self.configuration.system.trim().is_empty() {
    ...
    fn take_the_replies(&mut self) -> bool {
        ...
        // **A command-line agent runs its own tools, so Unluminate runs none of them.** Measured against
        // a real `claude`: it asked for `Grep` and `Read`, which are its own tools and are not in
        // Unluminate's catalogue, and this window answered both with "there is no tool called `Grep`" —
    ...
    fn answered_at(&mut self, at: usize, answer: Result<String, String>) {
        ...
        if self.session.round() >= self.configuration.tool_limit {
            self.session.reply(unluminate_chat::Reply::Failed(format!(
    ...
    /// What the pane holds, as data — which is what `unluminate-cli plugins view agent-chat` prints.
...
impl UiProvider for AgentChat {
    ...
    fn command(&mut self, command: &str, arguments: &[String]) -> Result<Answer, String> {
        ...
        match command {
            ...
            "send" => {
                ...
                // is the sentence `unluminate_git::Worker` exists for. `state` says when it has finished.
                Ok(Answer::said("sent")
            ...
            "last" => {
                ...
                Ok(
                    Answer::said(last.map(unluminate_chat::Message::text).unwrap_or_default()).with(
                        serde_json::json!({
                            "text": last.map(unluminate_chat::Message::text),
            ...
            "tools" => {
                ...
                Ok(Answer::said(match self.configuration.tools {
                    true => "Unluminate's own commands are offered to the model",
    ...
    fn commands(&self) -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            (
                "tools",
                "`on` or `off`: whether Unluminate's own commands are offered to the model.",
            ),
...
mod tests {
    ...
    fn a_folder(name: &str) -> PathBuf {
        let folder = std::env::temp_dir().join(format!("unluminate-chat-plugin-{name}-{}", std::process::id()));
    ...
    fn the_configuration_round_trips_through_the_plugins_own_folder() {
        ...
        configuration.permission = unluminate_chat::Permission::Edit;
    ...
    fn a_row_naming_a_wire_this_version_has_not_got_is_refused_rather_than_half_loaded() {
        ...
        assert!(unluminate_chat::provider::WIRES.contains(&configuration.providers[0].wire.name()));
    ...
    fn sending_with_nothing_typed_and_with_a_broken_endpoint_both_refuse_before_a_request_goes_out() {
        ...
        chat.configuration.providers[0].command = "unluminate-no-such-agent-anywhere".to_owned();
        let problem = chat.send().expect_err("not installed");
        assert!(problem.contains("unluminate-no-such-agent-anywhere"), "{problem}");

        // And the missing-key refusal for a row that really does send to an address.
        chat.configuration.providers[0].wire = unluminate_chat::Wire::Anthropic;
        ...
        chat.configuration.providers[0].key_env = "UNLUMINATE_A_VARIABLE_NOTHING_SETS".to_owned();
        let problem = chat.send().expect_err("no key");
        assert!(problem.contains("UNLUMINATE_A_VARIABLE_NOTHING_SETS"), "{problem}");
    ...
    fn the_system_prompt_says_where_it_is_and_never_sends_the_file() {
        ...
        assert!(prompt.contains("Unluminate"), "{prompt}");
    ...
    fn a_tool_call_becomes_a_request_the_window_runs_and_its_answer_goes_back_up() {
        ...
        chat.session.reply(unluminate_chat::Reply::ToolCall {
            id: "t1".to_owned(),
            name: "unluminate_git".to_owned(),
            arguments: "{\"command\":\"status\"}".to_owned(),
        });
        chat.session.reply(unluminate_chat::Reply::Finished {
    ...
    fn a_tool_that_cannot_be_resolved_answers_at_once_rather_than_hanging_the_turn() {
        ...
        chat.session.reply(unluminate_chat::Reply::ToolCall {
            id: "t1".to_owned(),
            name: "unluminate_levitate".to_owned(),
            arguments: "{}".to_owned(),
        });
        chat.session.reply(unluminate_chat::Reply::Finished {
            reason: "tool_use".to_owned(),
    ...
    fn a_conversation_is_written_when_the_turn_ends_and_read_back_when_the_pane_is_opened_again() {
        ...
        chat.session.reply(unluminate_chat::Reply::Text("I will.".to_owned()));
        chat.session.reply(unluminate_chat::Reply::Finished {
    ...
    fn a_picture_off_the_clipboard_is_attached_and_an_empty_clipboard_says_so() {
        ...
        let answer = serde_json::json!({
            "media": "image/png",
            "name": "pasted-8x8.png",
            "data": unluminate_chat::base64::encode(&bytes),
        });
    ...
    fn what_is_written_down_is_read_back_and_a_key_is_never_among_it() {
        ...
        configuration.tool_limit = 3;
        configuration.permission = unluminate_chat::Permission::Full;
        configuration.providers[0].wire = unluminate_chat::Wire::Anthropic;
        ...
        assert_eq!(read.providers[1].wire, unluminate_chat::Wire::CodexCli);
        assert_eq!(read.permission, unluminate_chat::Permission::Full);
