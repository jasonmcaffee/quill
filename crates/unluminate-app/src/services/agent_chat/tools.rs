//! Unluminate's own commands, offered to the model as tools.
//!
//! **Nothing here writes a tool out.** `unluminate_cli::mcp::tools` already turns the catalogue into
...
//!
//! `instance` names which Unluminate to drive, and this chat is inside the one it would name. `timeout`
...
//! refused with a sentence naming it. `UnluminateApp::run_cli` answers such a command with `Outcome::Hold`
...
use unluminate_cli::mcp::tools::{self, Shape};
...
/// then read the answer back with `terminal read`. The key Unluminate takes such care never to write down
...
pub const RUNS_A_PROGRAM: &[&str] = &[
    ...
    "debug.install",
    // Starts another Unluminate on a folder of the model's choosing.
    "launch",
];

/// Whether `command` is one that hands the model a program of its own choosing.
pub fn runs_a_program(command: &unluminate_cli::catalogue::Command) -> bool {
    RUNS_A_PROGRAM.contains(&command.wire().as_str())
}

/// Every tool Unluminate offers a model, built from the catalogue.
pub fn offered() -> Vec<unluminate_chat::Tool> {
    tools::tools(SHAPE)
        .into_iter()
        .map(|tool| unluminate_chat::Tool {
...
#[derive(Debug)]
pub struct Resolved {
    pub command: &'static unluminate_cli::catalogue::Command,
    pub arguments: Map<String, Value>,
}

/// Turn a model's tool call into the request `unluminate-cli` would have sent.
///
/// The one path, so a tool call and a person typing the same command reach the same code —
/// `UnluminateApp::run_cli` — rather than two paths that agree today.
...
/// settle, a git action waiting for the worker — is caught by `UnluminateApp::run_cli_for_a_plugin`
...
mod tests {
    ...
    fn a_tool_that_does_not_exist_is_a_sentence_rather_than_a_panic() {
        let problem = resolve("unluminate_nothing", &serde_json::json!({}), false).expect_err("a refusal");
        assert!(problem.contains("unluminate_nothing"), "{problem}");
        // And a verb the area has not got names the ones it has.
        let problem = resolve("unluminate_tab", &serde_json::json!({ "command": "levitate" }), false)
    ...
    fn a_call_that_asks_to_wait_is_refused_and_the_same_command_without_it_is_not() {
        ...
        let allowed = resolve("unluminate_terminal", &serde_json::json!({ "command": "read" }), false)
    ...
    fn arguments_that_are_not_an_object_are_refused_rather_than_guessed_at() {
        let problem =
            resolve("unluminate_tab", &serde_json::json!("open README.md"), false).expect_err("a refusal");
        ...
        let area = resolve("unluminate_git", &Value::Null, false).expect_err("an area tool needs a verb");
        assert!(area.contains("needs a `command`"), "{area}");
        let no_arguments =
            resolve("unluminate_git", &serde_json::json!({ "command": "status" }), false).expect("resolved");
    ...
    fn a_command_that_runs_a_program_is_held_back_until_the_second_switch_is_on() {
        ...
        assert!(resolve("unluminate_terminal", &serde_json::json!({ "command": "read" }), false).is_ok());
    }
    ...
    fn every_command_held_back_is_a_command_the_catalogue_really_has() {
        ...
        for name in RUNS_A_PROGRAM {
            assert!(
                unluminate_cli::catalogue::find(name).is_some(),
