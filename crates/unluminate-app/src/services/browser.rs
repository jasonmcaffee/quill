...
/// The browser engines Unluminate embeds on the platforms it ships.
...
impl BrowserLocation {
    ...
    pub fn parse(value: &str, project: &Path) -> Result<Self, String> {
        ...
        if let Ok(url) = Url::parse(value) {
            return match url.scheme() {
                ...
                scheme => Err(format!("Unluminate opens HTTP and HTTPS addresses, not {scheme} addresses.")),
            };
        }
    ...
    fn local(value: &str, project: &Path) -> Result<Self, String> {
        ...
        let path = candidate
            .canonicalize()
            .map_err(|problem| format!("Unluminate could not open {}: {problem}", candidate.display()))?;
...
/// A command shared by the browser toolbar and `unluminate-cli browser`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl BrowserHost {
    /// A lazy host. Constructing an ordinary Unluminate window starts no browser process.
    pub fn new() -> Self {
    ...
    /// Keep browser cookies and caches in Unluminate's per-user settings folder.
...
///
/// WebView2 has no way to serve a scheme of its own, so wry maps `unluminate://tab-1/page.html` to
/// `http://unluminate.tab-1/page.html` and reports that back. WKWebView reports the `unluminate://` form. A tab
...
fn canonical(url: &str) -> String {
    ...
    match (parsed.scheme(), host.strip_prefix("unluminate.")) {
        ("http" | "https", Some(origin)) => format!("unluminate://{origin}{}", parsed.path()),
        _ => url.to_owned(),
    }
}
...
/// its own, so wry serves `unluminate://` under `http://unluminate.<origin>/`. It rewrites the address a view
...
fn engine_url(url: &str) -> String {
    ...
    match url.strip_prefix("unluminate://") {
        Some(rest) => format!("http://unluminate.{rest}"),
        None => url.to_owned(),
    }
}
...
mod native {
    ...
    /// The platform objects, kept behind this module so the rest of Unluminate stays portable and testable.
    ...
    impl NativeHost {
        ...
        fn create(&mut self, request: &Settle<'_>) -> Result<(), String> {
            let Some(parent) = &self.parent else {
                return Err("Unluminate has not finished opening its window yet.".to_owned());
            };
            ...
            let webview = WebViewBuilder::new_with_web_context(context)
                ...
                .with_custom_protocol("unluminate".to_owned(), move |_, request| {
                    protocol_response(&resources, protocol_showing.load(Ordering::Relaxed), request)
                })
                .build_as_child(parent)
                .map_err(|problem| format!("Unluminate could not start the browser: {problem}"))?;
    ...
    /// Allow ordinary web navigation and Unluminate's one local resource origin.
    fn allowed_navigation(url: String) -> bool {
        Url::parse(&url).ok().is_some_and(|url| matches!(url.scheme(), "http" | "https" | "unluminate"))
    }
...
mod tests {
    ...
    fn fixture(name: &str) -> PathBuf {
        let folder = std::env::temp_dir().join(format!("unluminate-browser-{name}-{}", std::process::id()));
    ...
    fn local_pages_load_linked_resource_types_and_head() {
        ...
        let html = store.resolve(7, "GET", "unluminate://tab-7/index.html");
        let css = store.resolve(7, "GET", "unluminate://tab-7/site.css");
        let script = store.resolve(7, "HEAD", "unluminate://tab-7/app.js");
    ...
    fn local_origin_refuses_traversal_missing_files_and_writes() {
        ...
        assert_eq!(store.resolve(3, "GET", "unluminate://tab-3/../secret.txt").status, 404);
        assert_eq!(store.resolve(3, "GET", "unluminate://tab-3/%2E%2E/secret.txt").status, 404);
        assert_eq!(store.resolve(3, "GET", "unluminate://tab-3/missing.css").status, 404);
        assert_eq!(store.resolve(3, "POST", "unluminate://tab-3/index.html").status, 405);
    }
    ...
    fn local_resource_changes_are_reported_once() {
        ...
        assert_eq!(store.resolve(9, "GET", "unluminate://tab-9/site.css").status, 200);
    ...
    fn local_addresses_stay_readable_and_still_resolve() {
        ...
        assert_eq!(url, "unluminate://tab-4/my%20site/index.html");
        let store = LocalResourceStore::new();
    ...
    fn a_closed_tab_releases_its_root_and_its_remembered_resources() {
        ...
        assert_eq!(resources.resolve(tab.id, "GET", &format!("unluminate://tab-{}/index.html", tab.id)).status, 200);
        host.close_tab(tab.id);
        assert_eq!(resources.resolve(tab.id, "GET", &format!("unluminate://tab-{}/index.html", tab.id)).status, 404);
    ...
    fn tab_ids_are_unique_and_dropped_tabs_are_retained_away() {
        ...
        assert_eq!(host.resources.resolve(first.id, "GET", &format!("unluminate://tab-{}/index.html", first.id)).status, 404);
    }
    ...
    fn a_local_page_is_one_address_under_either_engines_name() {
        ...
        assert_eq!(asked_for, "unluminate://tab-1/index.html");
        let mut tab = tab;
        tab.arrived_at("http://unluminate.tab-1/index.html".to_owned());
        ...
        if cfg!(windows) {
            assert_eq!(engine_url(&asked_for), "http://unluminate.tab-1/index.html");
    ...
    fn only_the_resources_a_page_requested_are_watched() {
        ...
        store.resolve(5, "GET", "unluminate://tab-5/index.html");
