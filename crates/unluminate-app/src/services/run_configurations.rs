...
//! ## Where they live
//!
//! `.unluminate/run-configurations.conf`, read by [`crate::services::store::Values`] like every other
//! file Unluminate writes, numbered the way `files.panes` already numbers a list:
//!
//! ```text
...
//! already chose. It is in `.unluminate` because a run configuration belongs to the **project** — the
...
/// The file inside `.unluminate` that holds them.
...
pub struct Configuration {
    ...
    /// `.unluminate/open-files.txt` already follows, for the same reason: the project may move.
...
impl Configuration {
    ...
    /// The variables to lay over the environment Unluminate started with.
...
pub enum Origin {
    /// Made deliberately and kept in `.unluminate/run-configurations.conf` until it is removed.
    Permanent,
    /// Made by running a file or a suggestion. In memory only, capped at [`TEMPORARY_LIMIT`], and
    /// **never written to disk**: The reference editor writes its temporaries into `workspace.xml` and Unluminate
...
pub fn write(configurations: &RunConfigurations) -> String {
    ...
    values.to_text_headed(
        "# The run configurations for this project. Written by Unluminate, and safe to edit by hand.",
    )
}
...
///
/// The parsing is Unluminate's own code, shipped in the binary, which is what keeps a third-party
...
/// better list than anything Unluminate could guess at. A `package.json` that will not parse, or that
...
mod tests {
    ...
    fn the_configurations_survive_being_written_and_read_back() {
        let root = project("unluminate-run-round-trip");
        let mut configurations = RunConfigurations::new();
        configurations.add_permanent(Configuration {
    ...
    fn a_temporary_is_never_written_down() {
        // The reference editor writes its temporaries into `workspace.xml`; Unluminate deliberately does not,
        // because a file the project shares should hold what somebody chose to keep.
        let root = project("unluminate-run-temporaries-not-written");
        let mut configurations = RunConfigurations::new();
...
",
    ...
    fn a_file_that_cannot_be_read_is_a_file_that_is_not_there() {
        let root = project("unluminate-run-missing");
    ...
    fn the_cargo_detector_offers_a_run_when_the_project_has_a_manifest() {
        let root = project("unluminate-run-detect-cargo");
    ...
    fn the_npm_detector_offers_one_run_for_each_script_in_the_package() {
        let root = project("unluminate-run-detect-npm");
        std::fs::write(
    ...
    fn a_package_with_no_scripts_or_one_that_will_not_parse_offers_nothing() {
        let root = project("unluminate-run-detect-npm-empty");
    ...
    fn a_detector_named_twice_runs_once() {
        ...
        // two projects.
        let root = project("unluminate-run-detect-twice");
    ...
    fn a_detector_name_this_version_does_not_have_offers_nothing() {
        ...
        let root = project("unluminate-run-detect-unknown");
    ...
    fn a_program_is_looked_for_on_the_path_and_a_path_is_looked_for_where_it_points() {
        ...
        let folder = std::env::temp_dir().join("unluminate-run-found-on-path");
    ...
    fn a_bare_name_on_windows_is_completed_from_pathext() {
        ...
        let folder = std::env::temp_dir().join("unluminate-run-pathext");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
