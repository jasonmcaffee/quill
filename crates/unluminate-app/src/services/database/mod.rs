...
//!
//! `crates/unluminate-db` is the half that talks to a database and has never heard of a window; this is the
//! half that has never heard of a wire protocol. Between them is `unluminate_db::Worker`, a connection on a
...
//! a fault `unluminate-db`'s own tests found before this file existed.
//!
...
//! `unluminate_db::Pending::statements` refuses to compose a statement for such a table even if something
//! here asked it to.
...
use unluminate_db::catalog::{Item, Kind, Table};
use unluminate_db::edit::{Pending, Row};
use unluminate_db::rows::{Failure, Rows};
use unluminate_db::source::{Engine, Source};
use unluminate_db::value::Value;
use unluminate_db::worker::{Job, Reply, Worker};
...
impl Grid {
    ...
    /// left as NULL. `unluminate_db::Value` keeps the two apart all the way from the wire and this is the
...
///
/// `unluminate_db::sql::NewColumn` itself rather than a form of its own beside it: the dialog is typed
...
pub use unluminate_db::sql::NewColumn as ColumnForm;
...
pub fn keychain_entry_for(source: &str) -> String {
    let kept: String = source
        ...
        .collect();
    format!("unluminate-database-{}", kept.trim_matches('-'))
}
...
impl DatabaseExplorer {
    ...
    pub fn write_the_configuration(&self) -> Result<(), String> {
        match &self.folder {
            ...
            // the rule `UnluminateApp::load_settings` sets: a test must not read or write the settings of
    ...
    fn execute_with(&mut self, id: u64) -> Result<String, String> {
        ...
        let statement = unluminate_db::sql::at(&console.text, console.caret)
        ...
        if read_only && !unluminate_db::sql::only_reads(&statement) {
            return Err(format!(
    ...
    /// that refuses says so in its own words rather than in a sentence of Unluminate’s about it. It waits
    ...
    pub fn preview(&self, id: u64) -> Result<Vec<unluminate_db::Statement>, String> {
        let Some(Page { sheet: Sheet::Grid(grid), .. }) = self.page(id) else {
    ...
    fn apply(
        ...
        ticket: u64,
        wanted: Wanted,
        answer: unluminate_db::Answer<Reply>,
    ) {
    ...
    /// what says which, and it is the same reason `unluminate_db::Worker` hands one out at all.
    ...
    fn view_value(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            // What the New Table dialog is about to send, from `unluminate_db::sql::create_table` — the
            ...
            "new_table_sql": match &self.modal {
                Some(Modal::NewTable(form)) => {
                    let engine = self
                        .configuration
                        .source(&form.source)
                        .map_or(unluminate_db::source::Engine::Postgres, |source| source.engine);
                    match unluminate_db::sql::create_table(&form.schema, &form.name, &form.columns, engine) {
...
/// writes back is bound — see `unluminate_db::edit`.
///
...
pub fn select_for(grid: &Grid, engine: Engine, limit: usize, at: usize) -> String {
    ...
    // `unluminate_db::sqlite::ROWID_ALIASES` decides. Asking for the literal `rowid` on a table that
    ...
    let by_rowid = engine == Engine::Sqlite
        && grid
            ...
            .key
            .first()
            .is_some_and(|name| unluminate_db::sqlite::ROWID_ALIASES.contains(&name.as_str()));
    ...
    match grid.order_by.trim() {
        ...
        _ if !grid.table.key.is_empty() => {
            let by = grid
                ...
                .key
                .iter()
                .map(|name| unluminate_db::catalog::quoted(name, '"'))
                .collect::<Vec<String>>()
                .join(", ");
