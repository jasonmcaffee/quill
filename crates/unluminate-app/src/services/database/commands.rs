//! What the menu entries, the buttons in the pane and `unluminate-cli plugins run database …` all call.
//!
//! **One path**, which is `UnluminateApp::run_action`'s rule for the menus and `run_cli`'s for the command
...
//! would stop the window drawing for the length of a query — the sentence `unluminate_git::Worker` exists
...
/// sentence `unluminate_git::Worker` exists for, and the reason `query` answers with a ticket instead.
...
fn ddl(explorer: &mut DatabaseExplorer, rest: &str) -> Result<Answer, String> {
    ...
    let kind = explorer
        ...
        .map(|item| item.kind)
        .unwrap_or(unluminate_db::Kind::Table);
...
///
/// **The place, never the value.** There is deliberately no way to give Unluminate a password on the
/// command line: it would be in a shell history, in a process list and in whatever log the caller
/// keeps, which is three copies of a secret Unluminate has gone to some trouble never to write down once.
...
fn password(explorer: &mut DatabaseExplorer, rest: &str) -> Result<Answer, String> {
    ...
    let secret = match words.next() {
        ...
        // credential store rather than into anything Unluminate writes. The value is the rest of the line,
        ...
        Some(other) => {
            return Err(format!(
                "`{other}` is not somewhere a password lives. Unluminate knows `set <secret>`, \
...
pub fn a_new_table(explorer: &DatabaseExplorer, source: &str, schema: &str) -> TableForm {
    let engine = explorer
        ...
        .unwrap_or(unluminate_db::source::Engine::Postgres);
    TableForm {
        ...
        columns: vec![ColumnForm {
            ...
            type_name: match engine {
                unluminate_db::source::Engine::Sqlite => "INTEGER".to_owned(),
                unluminate_db::source::Engine::Postgres => "integer".to_owned(),
            },
            in_key: true,
            not_null: true,
        }],
...
fn new_table(explorer: &mut DatabaseExplorer, arguments: &[String]) -> Result<Answer, String> {
    ...
    let engine = explorer
        ...
        .map(|source| source.engine)
        .unwrap_or(unluminate_db::source::Engine::Postgres);
    let sql = unluminate_db::sql::create_table(&schema, &name, &columns, engine)?;
...
fn drop_table(explorer: &mut DatabaseExplorer, rest: &str) -> Result<Answer, String> {
    ...
    let sql = unluminate_db::sql::drop_table(&schema, &name)?;
    explorer.run_the_ddl(&source, &schema, &sql)?;
