...
///
/// The flag itself stays, because it is not a check in Unluminate: it is what asks a **server** for a
        ...
        std::fs::write(
            ...
            values.to_text_headed(
                "The data sources the Database plugin knows about.\n\
                 Written by Unluminate; safe to edit by hand.\n\n\
...
fn one(values: &Values, index: usize) -> Result<Source, String> {
    ...
    let engine = Engine::from_name(engine_name).ok_or_else(|| {
        format!(
            "`{name}` names the engine `{engine_name}`, and this version of Unluminate speaks {}.",
            Engine::ALL.join(", ")
        )
    })?;
    ...
    match engine {
        ...
        Engine::Postgres => {
            ...
            source.ssl = SslMode::from_name(mode).ok_or_else(|| {
                format!(
                    "`{}` names sslmode `{mode}`, and Unluminate has {}.",
                    source.name,
                    SslMode::ALL.join(", ")
                )
            })?;
    ...
    if values.text(&at("password")).is_some() {
        return Err(format!(
            "`{}` has a `password` in the file. Unluminate does not store one: use `password.env` to \
...
mod tests {
    ...
    fn a_folder(name: &str) -> std::path::PathBuf {
        let folder = std::env::temp_dir().join(format!("unluminate-database-{name}-{}", std::process::id()));
    ...
    fn a_configuration_round_trips_and_carries_no_secret() {
        ...
        postgres.secret = Secret::Environment("UNLUMINATE_DB_AI".to_owned());
        ...
        assert!(text.contains("UNLUMINATE_DB_AI"), "the name of the variable is written down");
        ...
        assert_eq!(read.sources[0].secret, Secret::Environment("UNLUMINATE_DB_AI".to_owned()));
