...
//! `crates/unluminate-db/tests/scripted_server.rs` is where the wire protocol is tested, and
//! `cargo run -p unluminate-db --example connect` is how the real server is.

use std::path::PathBuf;

use unluminate_db::source::{Secret, Source};
...
fn a_database(name: &str) -> PathBuf {
    let folder = std::env::temp_dir().join(format!("unluminate-database-plugin-{name}-{}", std::process::id()));
...
fn the_tree_reads_one_level_at_a_time() {
    ...
    assert!(items.iter().any(|item| item.name == "member" && item.kind == unluminate_db::Kind::Table));
    assert!(items.iter().any(|item| item.name == "members" && item.kind == unluminate_db::Kind::View));
...
fn a_console_runs_the_statement_under_the_caret() {
    // Which is the whole reason `unluminate_db::sql::at` exists: a console holds several statements and
...
fn the_view_answers_the_same_numbers_the_pane_is_drawing() {
    // Unluminate's rule: a pane drawn with `egui` is invisible to a test and to an agent unless it can be
...
fn where_a_password_is_can_be_said_on_the_command_line_and_the_password_itself_cannot() {
    ...
    run(&mut explorer, "password", &["test", "env", "UNLUMINATE_DB_TEST"]);
    assert_eq!(
        explorer.configuration.source("test").map(|source| source.secret.clone()),
        Some(Secret::Environment("UNLUMINATE_DB_TEST".to_owned()))
    );
    ...
    explorer
        .command(
            "add-source",
            &["remote".to_owned(), "postgres://me@example.com/db".to_owned(), "UNLUMINATE_DB_REMOTE".to_owned()],
        )
        .expect("added");
    assert_eq!(
        explorer.configuration.source("remote").map(|source| source.secret.clone()),
        Some(Secret::Environment("UNLUMINATE_DB_REMOTE".to_owned()))
    );
    explorer
    ...
    // There is deliberately no way to give Unluminate the password itself: it would be in a shell history,
...
fn a_grids_statement_asks_for_one_more_row_than_it_keeps() {
    ...
    let grid = Grid {
        ...
        table: unluminate_db::Table {
            schema: String::new(),
            name: "member".to_owned(),
            columns: Vec::new(),
            key: vec!["id".to_owned()],
        },
        kind: unluminate_db::Kind::Table,
        where_clause: "name like 'A%'".to_owned(),
        order_by: "name desc".to_owned(),
        at: 2,
        rows: unluminate_db::Rows::default(),
        pending: unluminate_db::Pending::default(),
        failure: None,
        running: None,
        chosen: None,
        editing: None,
    };
    let statement = select_for(&grid, unluminate_db::Engine::Sqlite, 200, 2);
    assert_eq!(
        statement,
    ...
    assert_eq!(
        select_for(&unordered, unluminate_db::Engine::Postgres, 200, 0),
    ...
    assert_eq!(
        select_for(&compound, unluminate_db::Engine::Postgres, 10, 0),
    ...
    let no_key = Grid {
        table: unluminate_db::Table { key: Vec::new(), ..unordered.table.clone() },
        ..unordered.clone()
    };
    assert_eq!(
        select_for(&no_key, unluminate_db::Engine::Postgres, 10, 0),
        "select * from \"member\" limit 11 offset 0"
    );
    // A SQLite table addressed by its `rowid` has to ask for it by name: it is not in `select *`.
    let by_rowid = Grid {
        table: unluminate_db::Table { key: vec!["rowid".to_owned()], ..grid.table.clone() },
        where_clause: String::new(),
        order_by: String::new(),
        ..grid
    };
    let statement = select_for(&by_rowid, unluminate_db::Engine::Sqlite, 10, 0);
    ...
    let statement = select_for(&by_rowid, unluminate_db::Engine::Postgres, 10, 0);
    ...
    let by_alias = Grid {
        table: unluminate_db::Table { key: vec!["_rowid_".to_owned()], ..by_rowid.table.clone() },
        ..by_rowid.clone()
    };
    assert_eq!(
        select_for(&by_alias, unluminate_db::Engine::Sqlite, 10, 0),
...
fn a_cell_shows_what_is_pending_on_it_rather_than_what_was_read() {
    let mut rows = unluminate_db::Rows::default();
    rows.columns = vec![unluminate_db::Column::new("id", "int"), unluminate_db::Column::new("name", "text")];
    ...
    assert_eq!(row, unluminate_db::Row::Keyed(vec!["1".to_owned()]));
...
fn an_answer_to_a_superseded_question_is_thrown_away_rather_than_drawn() {
    ...
    let mut rows = unluminate_db::Rows::default();
    rows.columns = vec![unluminate_db::Column::new("who", "text")];
