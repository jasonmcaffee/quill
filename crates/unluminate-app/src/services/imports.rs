...
//!
//! `unluminate_core::imports` reads the text and says what the caret is in the middle of; this is the
...
//! over paths, and walking `unluminate_core::completion` down a folder tree is the same arithmetic with
...
use unluminate_core::syntax::{Grammar, PathRoot};
...
/// package, and one with a scheme in it would be a fetch, which nothing in Unluminate ever does.
...
pub struct Module {
    ...
    pub root: PathBuf,
    /// How the package is spelt in a path: `unluminate_app`.
    pub package: String,
...
mod tests {
    use super::*;
    use unluminate_core::syntax::ImportStyle;
    ...
    /// A little Rust workspace, shaped like Unluminate's own.
    fn workspace() -> (PathBuf, Vec<PathBuf>) {
        let root = PathBuf::from("/unluminate");
        let files = [
            "crates/unluminate-core/src/lib.rs",
            "crates/unluminate-core/src/completion.rs",
            "crates/unluminate-core/src/mermaid/mod.rs",
            "crates/unluminate-core/src/mermaid/layered.rs",
            "crates/unluminate-app/src/lib.rs",
            "crates/unluminate-app/src/app/mod.rs",
            "crates/unluminate-app/src/app/actions.rs",
            "crates/unluminate-app/src/components/editor_view.rs",
        ]
        .iter()
        .map(|name| root.join(name))
        .collect();
    ...
    fn a_module_path_walks_the_source_root_of_the_package_it_names() {
        ...
        let project = project(&root, &files);
        let from = root.join("crates/unluminate-app/src/components/editor_view.rs");
        let reach = |path: &[&str]| {
        ...
        assert_eq!(
            ...
            Some(Reached {
                folder: None,
                file: Some(root.join("crates/unluminate-app/src/app/actions.rs")),
            })
        );
        assert_eq!(
            reach(&["unluminate_core", "completion"]),
            Some(Reached {
                folder: None,
                file: Some(root.join("crates/unluminate-core/src/completion.rs")),
            })
        );
        ...
        assert_eq!(reach(&["unluminate-core", "completion"]), None);
    }
    ...
    fn a_folder_with_a_module_file_answers_with_both() {
        let (root, files) = workspace();
        let project = project(&root, &files);
        let from = root.join("crates/unluminate-core/src/lib.rs");
        let segments = ["crate".to_owned(), "mermaid".to_owned()];
        assert_eq!(
            resolve_segments(&project, &from, &segments, &rust()),
            Some(Reached {
                folder: Some(root.join("crates/unluminate-core/src/mermaid")),
                file: Some(root.join("crates/unluminate-core/src/mermaid/mod.rs")),
            })
        );
    }
    ...
    fn an_unresolved_segment_answers_nothing_rather_than_everything() {
        ...
        let project = project(&root, &files);
        let from = root.join("crates/unluminate-app/src/app/mod.rs");
        let typo = ["unluminat_core".to_owned(), "completion".to_owned()];
    ...
    fn a_folders_children_are_its_subfolders_and_its_files_but_not_its_own_module() {
        let (root, files) = workspace();
        let project = project(&root, &files);
        let names: Vec<String> = children(&project, &root.join("crates/unluminate-core/src"), &rust())
            .into_iter()
            .map(|(name, _)| name)
            .collect();
    ...
    fn nothing_written_offers_the_reserved_roots_and_the_packages() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "self".to_owned(),
                "super".to_owned(),
                "unluminate_core".to_owned(),
                "unluminate_app".to_owned(),
            ]
        );
    }
