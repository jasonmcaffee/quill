//! What kind of file a path is, whether Unluminate can open it, and what to call it.
//!
//! Unluminate opens text. It used to open `.md` and `.txt` and nothing else, and dim everything else in the
//! explorer. `tasks/improvements.md` asks for the rest: a file Unluminate has no special handling for, such as
...
//! second kind of thing Unluminate opens rather than a file it refuses. [`is_image`] is the whole of the
...
/// Extensions Unluminate can show as a picture. Every one of them is a format the `image` crate is built
...
impl Refusal {
    ...
    pub fn reason(self) -> &'static str {
        match self {
            Refusal::NotText => "Unluminate opens text files. This one is not text.",
            Refusal::TooLarge => "This file is larger than 16 MB, which is more than Unluminate opens.",
            Refusal::NotAFile => {
                "This is not a file. It is a device, a pipe or a socket, and Unluminate opens files."
            }
        }
    }
}
...
/// Whether Unluminate can open this entry, decided from its name, its kind and its size alone.
///
...
/// Whether Unluminate can open this file, and why not when it cannot.
///
...
/// True for a file Unluminate shows as a picture rather than as text.
///
...
/// Whether a path names an HTML document that Unluminate can render in a browser tab.
...
///
/// Markdown is named because Unluminate treats it differently, having a preview for it. The rest are named
...
///
/// Unluminate saves plain text and carries no formatting to disk, so bold, a colour, an alignment and a
...
/// extension to go on, it is very often the beginning of a Markdown file, and it is the one Unluminate
/// starts with.
...
/// entry — **absent**, not dimmed, which is Unluminate's rule for a control that can never apply.
///
...
///
/// The window knows which plugin claims a file and whether it is Markdown; `unluminate_core::folding`
/// deliberately holds no list of languages, so this is where the two are put together.
pub fn folding_reading<'a>(path: Option<&Path>, grammars: &'a Grammars) -> unluminate_core::folding::Reading<'a> {
    if let Some(grammar) = path.and_then(|path| grammars.for_path(path)) {
        return unluminate_core::folding::Reading::Code(grammar);
    }
    if is_markdown(path) || path.is_none() {
        return unluminate_core::folding::Reading::Markdown;
    }
    unluminate_core::folding::Reading::Plain
}
...
/// them, because the buttons, the `View` menu, the keyboard and `unluminate-cli editor view` all ask
    ...
    fn the_listing_question_agrees_with_the_opening_one_for_every_name_it_knows() {
        ...
        for extension in BINARY_EXTENSIONS {
            ...
            let expected = match IMAGE_EXTENSIONS.contains(extension) {
                // A picture is a second kind of thing Unluminate opens rather than a file it refuses.
                true => Ok(()),
...
mod tests {
    ...
    fn how_a_file_is_read_for_its_blocks_follows_the_plugin_and_then_the_extension() {
        let rust = unluminate_core::Grammar { language: "rust".to_owned(), ..Default::default() };
        let grammars = Grammars::of(vec![("rs".to_owned(), rust)]);
        assert!(matches!(
            folding_reading(Some(Path::new("main.rs")), &grammars),
            unluminate_core::folding::Reading::Code(_)
        ));
        assert!(matches!(
            folding_reading(Some(Path::new("notes.md")), &grammars),
            unluminate_core::folding::Reading::Markdown
        ));
        assert!(matches!(
    ...
    #[test]
    fn the_file_types_unluminate_used_to_open_are_still_text() {
    ...
    #[test]
    fn a_source_file_unluminate_has_no_special_handling_for_is_text() {
    ...
    fn a_picture_is_offered_even_though_it_is_not_text() {
        ...
        // A picture Unluminate has no decoder for stays refused, rather than opening into an empty tab.
    ...
    fn a_name_with_no_extension_is_decided_by_reading_it() {
        let folder = folder("unluminate-file-kind-sniff");
    ...
    fn an_unknown_extension_holding_writing_is_text() {
        let folder = folder("unluminate-file-kind-unknown");
        let path = folder.join("recipe.unluminatenotes");
    ...
    fn a_file_of_accented_letters_cut_in_half_by_the_sniff_is_still_text() {
        let folder = folder("unluminate-file-kind-utf8");
        let path = folder.join("accents.unluminatenotes");
    ...
    fn a_file_too_large_to_open_says_so_rather_than_being_called_binary() {
        let folder = folder("unluminate-file-kind-large");
    ...
    fn the_status_bar_names_the_kind_of_file() {
        ...
        assert_eq!(kind_name(Some(Path::new("a.unluminatenotes"))), "Plain text");
    ...
    fn formatting_is_offered_for_prose_and_not_for_code() {
        for prose in ["a.md", "a.markdown", "a.txt", "a.text", "a.unluminatenotes"] {
