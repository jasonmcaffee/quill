...
/// open, so opening `crates/unluminate-core` in a checkout still honours the checkout's own file.
...
mod tests {
    ...
    fn the_scratch_folder_that_polluted_go_to_definition_is_left_out() {
        ...
        assert!(!rules.ignores("crates/unluminate-core/src/layout.rs", false));
    }
    ...
    fn the_rules_are_read_off_the_disk_and_the_settings_line_is_read_after_them() {
        let folder = std::env::temp_dir().join("unluminate-ignore-test");
    ...
    fn the_repository_root_is_found_from_a_folder_inside_it() {
        let folder = std::env::temp_dir().join("unluminate-ignore-root-test");
