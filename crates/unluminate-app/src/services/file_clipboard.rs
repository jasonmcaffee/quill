...
//!
//! This is Unluminate's own clipboard, not the operating system's. The system's *file* clipboard is a
//! different interface on each platform — `CF_HDROP` on Windows, `NSFilenamesPboardType` on macOS —
//! and `arboard`, which Unluminate already uses for text, exposes neither. Adding a platform-specific
//! dependency for each platform to make cut and paste reach outside Unluminate is not worth it for
//! version one.
//!
//! So the consequence is stated plainly rather than hidden: **a file cut in Unluminate cannot be pasted
...
mod tests {
    ...
    fn folder(name: &str) -> PathBuf {
        let root = std::env::temp_dir().join("unluminate-file-clipboard").join(name);
    ...
    fn a_hidden_file_keeps_its_whole_name() {
        ...
        std::fs::write(root.join("to/.unluminaterc"), "one\n").expect("write it");
        assert_eq!(free_name(&root.join("to"), ".unluminaterc"), root.join("to/.unluminaterc 2"));
    }
}
