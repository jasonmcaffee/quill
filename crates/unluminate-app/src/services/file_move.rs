...
//! move.
//!
//! It changes nothing itself. `UnluminateApp::move_path` moves the bytes and applies the plan, following
...
//!
//! ## The tier is syntactic, and it is the one Unluminate is already on
//!
...
//!
//! So the two readings are `unluminate_core::imports`' own — `specifiers_in` for the quoted family and
...
use unluminate_core::imports::{self as reading, UseLeaf};
use unluminate_core::syntax::{Grammar, ImportStyle, PathRoot};
...
/// what lets `super::a`, `crate::b::a` and `unluminate_app::b::a` all be compared with one another.
#[derive(Debug, Clone, PartialEq, Eq)]
struct Abs {
    root: PathBuf,
    segments: Vec<String>,
...
/// and those references have to follow a move as much as any other do. `unluminate-app`'s own screenshot
