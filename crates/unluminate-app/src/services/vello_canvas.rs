...
//! same seam `unluminate_core::mermaid::Scene` is — a value a test can assert on with no window, no graphics
...
//! atlas, so this renderer is already inside Unluminate and using it here adds no third-party renderer to the
...
/// else does. At 1.5 it is under ten. `crates/unluminate-app/examples/vello_cost.rs` is how that is measured
/// again.
...
/// small and as plain as `unluminate_core::mermaid::Scene`'s items: a value with no behaviour, so the drawing
...
/// values in Unluminate are hashable, and because a test that wants to say "these two drawings are the same
...
impl Canvas {
    ...
    /// Unluminate. The tests pin the **baseline** level, which is the set of features the target itself
