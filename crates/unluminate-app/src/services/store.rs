//! What Unluminate remembers between runs: the settings, and the projects that have been open.
//!
...
//!
//! A file that cannot be read is treated as a file that is not there. Unluminate starting with its defaults is
//! better than Unluminate refusing to start because a settings file has a stray line in it.
...
impl Values {
    ...
    /// to an empty string: several settings here mean "whatever this Unluminate's own default is" by having
    ...
    /// and nothing in Unluminate knows the word `new` until the manifest is read. The order is the map's
    ...
    pub fn to_text(&self) -> String {
        self.to_text_headed("# Unluminate settings. Written by Unluminate, and safe to edit by hand.")
    }
...
/// The folder Unluminate keeps its settings in, and the two files inside it.
#[derive(Debug, Clone)]
pub struct Store {
    folder: PathBuf,
}
...
impl Store {
    ...
    /// what the ticket asks for in as many words and is the only rule available. Unluminate has no
    ...
    pub fn open_windows(&self) -> Vec<PathBuf> {
        ...
        for line in text.lines() {
            ...
            let path = unluminate_terminal::paths::plain(Path::new(line));
    ...
    pub fn write_session(&self, windows: &[PathBuf]) {
        ...
        if let Err(problem) = self.write(&self.session_path(), &text) {
            eprintln!("Unluminate could not write its open windows: {problem}");
        }
    }
    ...
    /// Write the settings. A failure is reported on the error output and otherwise ignored: Unluminate going
    ...
    pub fn write_values(&self, values: &Values) {
        if let Err(problem) = self.write(&self.settings_path(), &values.to_text()) {
            eprintln!("Unluminate could not write its settings: {problem}");
        }
    }
    ...
    pub fn recent_projects(&self) -> Vec<PathBuf> {
        ...
        for line in text.lines() {
            ...
            // Through `plain`, so that a list written by an earlier Unluminate — every line of which held a
    ...
    /// `\\?\C:\jason\dev\unluminate` — and this list is not only read back by Unluminate. It is the explorer's
    /// root, the folder the `.unluminate` state is written beside, and the directory a shell is started in,
    ...
    /// shares. `unluminate_terminal::paths` says what the prefix is and why the terminal strips it again.
    pub fn remember_project(&self, folder: &Path) {
        ...
        if let Err(problem) = self.write(&self.recent_path(), &text) {
            eprintln!("Unluminate could not write its recent projects: {problem}");
        }
    }
...
///
/// Both lists Unluminate keeps of folders go through this. Absolute, so that opening `.` and opening the
/// folder it stands for are one entry rather than two; and plain because on Windows `canonicalize`
/// gives back a **verbatim** path — `\\?\C:\jason\dev\unluminate` — which `cmd.exe` will not start in.
...
fn plain_absolute(folder: &Path) -> PathBuf {
    ...
    unluminate_terminal::paths::plain(&folder)
}
...
/// `~/.config` when it is not. With no home directory at all the current folder is used, so that Unluminate
/// still runs.
/// Where Unluminate keeps its things for this person, without opening a store first.
///
...
mod tests {
    ...
    /// `task-1693`: the windows Unluminate had open, so that starting it again brings them all back.
    #[test]
    fn every_window_that_was_open_is_remembered_and_a_line_is_kept_when_one_closes() {
        let folder = temporary("unluminate-store-session");
    ...
    fn the_session_list_stops_at_its_limit_and_drops_the_oldest() {
        let folder = temporary("unluminate-store-session-limit");
        let store = Store::at(&folder);
    ...
    fn a_project_that_has_gone_is_not_a_window_to_open() {
        let folder = temporary("unluminate-store-session-gone");
        let store = Store::at(&folder);
    ...
    fn values_survive_being_written_and_read_back() {
        let store = Store::at(temporary("unluminate-store-round-trip"));
    ...
    fn a_missing_settings_file_reads_as_no_values_rather_than_failing() {
        let store = Store::at(temporary("unluminate-store-missing"));
...
");
    ...
    fn the_newest_project_is_first_and_a_repeat_moves_up_rather_than_appearing_twice() {
        let folder = temporary("unluminate-store-recent");
        let store = Store::at(&folder);
    ...
    fn the_recent_list_is_capped_and_drops_folders_that_have_gone() {
        let folder = temporary("unluminate-store-cap");
        let store = Store::at(&folder);
    ...
    fn a_remembered_project_is_written_down_as_a_plain_path() {
        ...
        let folder = temporary("unluminate-store-plain-path");
        let store = Store::at(&folder);
    ...
    #[test]
    fn a_list_written_by_an_earlier_unluminate_is_read_as_plain_paths() {
        // Every line of the list on a machine that had run the earlier Unluminate was verbatim. Reading it
        // repairs it rather than leaving somebody to edit the file by hand.
        let folder = temporary("unluminate-store-old-list");
        let store = Store::at(&folder);
        let project = folder.join("project");
        std::fs::create_dir_all(&project).expect("make a project");
    ...
    fn the_settings_folder_is_under_the_home_directory() {
        ...
        assert!(
            folder.ends_with("Unluminate") || folder.ends_with("unluminate") || folder.ends_with(".unluminate"),
            "the settings folder should be named after Unluminate, it was {}",
            folder.display()
        );
    }
}
