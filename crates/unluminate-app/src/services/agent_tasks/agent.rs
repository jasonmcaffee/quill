...
//! `unluminate_terminal::Session`, which is the same session and the same emulator the terminal tile uses.
//!
//! ## Which agents exist is a list in Unluminate, and which one a ticket uses is data on the row
//!
...
//! also why another Unluminate window can resume a session this one started.

use super::model::Assignee;

/// The agents this version of Unluminate can launch.
///
...
/// draws it as the chosen one, because a board written by hand or by a later Unluminate must not silently lose a
...
pub struct Plan {
    ...
    ///
    /// The program and the flags in front of it, replacing Unluminate's own choice of both. Empty means the
...
/// one answer in Unluminate to what a written command line means: a double-quoted word holds its spaces, a
...
///
/// A command naming nothing at all is a refusal rather than a fall back to Unluminate's own, because a
...
/// names nothing and [`can_resume`] says that a Codex ticket cannot be resumed by an id Unluminate chose. The
...
/// still goes **first** — before those flags and before Unluminate's own — because it is a subcommand and
...
/// assigns its own, so the id in the ticket is Unluminate's own marker and means nothing to Codex.
///
...
pub fn why_it_cannot_resume(key: &str) -> String {
    format!(
        "{key} is Codex's, and Codex names its own sessions rather than taking one, so the id on the ticket \
         is only Unluminate's marker that a worker was here. Starting it again begins a new conversation: press \
...
///
/// So the handoff carries the protocol itself. Unluminate's board is a SQLite file driven through the
...
/// two things it cannot work out for itself: where `unluminate-cli` is, since nothing puts it on `PATH`, and
...
pub fn handoff(agent: Assignee, key: &str) -> String {
    ...
    format!(
        "You are working {key} on Unluminate's own Agent-Tasks board. Do not use the /task skill or look for \
...
/// The variable that carries the path to `unluminate-cli` to a launched agent.
///
/// Named rather than left to be found, because nothing puts `unluminate-cli` on `PATH`: on macOS it lives
/// inside the application bundle beside `unluminate`, and an agent told to run `unluminate-cli` would answer that
/// there is no such command. `services::agent_tasks::beside_this_program` is what fills it in.
pub const ENV_CLI: &str = "UNLUMINATE_CLI";

/// The variable that carries which window to drive, as the window's own process id.
///
/// Several Unluminates may be running, each on its own project, and `unluminate-cli` with no `--instance` given
/// several has no way to know which board the ticket is on.
pub const ENV_INSTANCE: &str = "UNLUMINATE_INSTANCE";
...
mod tests {
    ...
    fn a_command_naming_nothing_is_a_refusal_rather_than_a_silent_fall_back() {
        ...
        assert_eq!(
            ...
            "claude",
            "a setting nobody filled in means Unluminate's own command"
        );
        asked.command = Some("\"\"".to_owned());
    ...
    fn a_new_codex_session_names_nothing_because_codex_names_its_own() {
        ...
        // failed to start, and passing Unluminate's own id to `codex resume` would be naming a conversation
    ...
    fn only_claude_can_be_handed_its_conversation_back() {
        ...
        // own, so the id on the ticket is Unluminate's marker that a worker was here and nothing more.
