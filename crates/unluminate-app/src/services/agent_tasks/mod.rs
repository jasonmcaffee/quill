...
pub struct Configuration {
    ...
    /// The command a Claude ticket is launched with, when Unluminate's own is not what is wanted.
    ///
    /// The program and the flags in front of it — `claude --dangerously-skip-permissions
    /// --add-dir /tmp` — and Unluminate still puts the ticket's `--model`, `--effort` and the session flag
    ...
    /// The same for a Codex ticket. `codex resume <id>` puts its subcommand first, which Unluminate goes on
...
/// What the key is called in this machine's keychain, under the service `unluminate-agent-tasks`.
///
/// **A constant rather than a setting.** `task-28`: the page used to ask for a `Key name`, a `Key variable`
/// and then the key, which is three values to describe one connection and two of them are Unluminate's own
...
impl Configuration {
        ...
        std::fs::write(
            ...
            values.to_text_headed(
                "The Agent-Tasks board. No secret is in here: the agent's authentication key lives in this \
                 ...
                 command Unluminate builds itself.",
            ),
        )
    ...
    /// alternative to pass instead. Nothing Unluminate writes ever carries the value: the settings file holds the
...
///
/// `unluminate-cli` is not on anybody's `PATH`: on macOS it is inside the application bundle next to `unluminate`
/// and on Windows it sits in the installation folder. So an agent told to run `unluminate-cli` would answer
...
pub struct AgentTasks {
    ...
    waker: Option<unluminate_terminal::Waker>,
}

/// A ticket's own terminal.
///
/// A `unluminate_terminal::Session`, which is the same session and the same emulator the terminal tile
...
pub struct TicketTerminal {
    pub task_id: i64,
    pub session: unluminate_terminal::Session,
...
impl std::fmt::Debug for TicketTerminal {
    /// Written by hand because `unluminate_terminal::Session` holds a channel and a thread and has no
...
impl AgentTasks {
    ...
    /// folder Unluminate happens to have started in.
    ...
    /// after this Unluminate has closed.
    pub fn start(&mut self, key: &str) -> Result<Answer, String> {
        ...
        // on: Codex names its own sessions, so its recorded id is Unluminate's marker and Resume refuses it. Refusing
    ...
    pub fn resume(&mut self, key: &str) -> Result<Answer, String> {
        ...
        // Codex names its own sessions, so the id on a Codex ticket is Unluminate's marker that a worker was
    ...
    fn spawn(
        ...
        // Unluminate started from the Finder or the Dock has — launchd gives it `/usr/bin:/bin:/usr/sbin:
        ...
        let settings = unluminate_terminal::SessionSettings {
            ...
            env: {
                ...
                environment.push((
                    "UNLUMINATE_AGENT_TASKS".to_owned(),
                ...
                environment.push((agent::ENV_CLI.to_owned(), beside_this_program("unluminate-cli")));
        ...
        let session = unluminate_terminal::Session::spawn(
            &settings,
            unluminate_terminal::Size::new(40, 120),
            waker,
        )
    ...
    ///
    /// The same `command` the menu entry and `unluminate-cli plugin run` go through, so a button and an agent
    ...
    ///
    /// This does not, and cannot, take away a key Unluminate inherited in its own environment. The page says which of
    ...
    pub fn clear_the_key(&mut self) -> Result<String, String> {
        ...
        match std::env::var("ANTHROPIC_API_KEY").ok().filter(|value| !value.trim().is_empty()) {
            Some(_) => Ok(format!(
                "`{KEY_NAME}` is out of the keychain. Unluminate's own environment still has ANTHROPIC_API_KEY, and \
    ...
    pub fn set_waker(&mut self, waker: unluminate_terminal::Waker) {
...
///
/// Two Unluminate windows can have one board file open, and each runs its own watchdog over the same rows. A
...
/// because Unluminate has none and adding one to name a session would be a dependency for a string. Two ids
...
impl UiProvider for AgentTasks {
    ...
    fn open(&mut self, context: &Context) -> Result<(), String> {
        ...
        // board somebody is using. The released binary always has one, because `UnluminateApp::load_settings`
        // is what hands it over; a window a test builds has none, exactly as it has no settings file and
        // no `.unluminate` folder. The rule is `UnluminateApp::load_settings`' own, kept once more.
    ...
    fn command(&mut self, command: &str, arguments: &[String]) -> Result<Answer, String> {
        ...
        match command {
            ...
            // same way by an agent — one function behind both, which is Unluminate's own rule about a control.
            "show" => {
            ...
            // tab itself — only the window can — so it answers and `UnluminateApp` acts, which is the rule every
            ...
            // Unluminate's rule is that everything a person can do reaches the same code from the command line.
            ...
            "epic-colour" | "epic-color" => {
                if arguments.len() < 2 {
                    return Err(
                        "say which epic and what colour: `epic-colour Unluminate #8B6BFF`".to_owned()
                    );
                }
            ...
            "sync" => Err(
                "this board does not sync with JIRA. That is the one part of the survey the plugin does not \
                 do: reading JIRA is HTTP and Unluminate has no HTTP client, so it is its own piece of work. The \
...
mod tests_task_28 {
    ...
    fn a_configuration_nobody_has_written_points_at_iliad() {
        ...
        let folder = std::env::temp_dir().join(format!("unluminate-board-iliad-{}", std::process::id()));
    ...
    fn a_settings_file_naming_a_key_and_a_variable_is_read_and_written_back_without_them() {
        let folder = std::env::temp_dir().join(format!("unluminate-board-oldkeys-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&folder);
        std::fs::create_dir_all(&folder).expect("a folder");
        std::fs::write(
