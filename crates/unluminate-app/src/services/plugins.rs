...
//! rebuilt for every release of Unluminate, and a plugin that crashes takes the editor with it. For
...
//!
//! It carries one new key, `language.renders`, which names a renderer that is **built into Unluminate**.
...
use unluminate_core::symbols::SymbolKind;
use unluminate_core::syntax::{Grammar, ImportStyle, PathRoot, Token};
use unluminate_core::Color;
...
/// The renderers built into this version of Unluminate that a plugin may name.
///
/// Checked rather than taken on trust, so a manifest asking for a picture Unluminate cannot draw says so
/// plainly instead of loading as a language whose files quietly never draw.
pub const RENDERERS: &[&str] = &["mermaid"];

/// The project detectors built into this version of Unluminate that a plugin's `run.project` may name.
///
/// Checked the same way [`RENDERERS`] is, and for the same reason: a manifest asking for a detector
/// Unluminate does not have should say so plainly rather than load as a language whose projects are
...
/// The debuggers built into this version of Unluminate that a plugin's `debug.adapter` may name.
///
/// The third registry of this shape, checked the same way and for the same reason: a manifest naming
/// a debugger Unluminate cannot drive should say so plainly rather than load as a language whose files
...
/// Unluminate.** `services::debuggers` is what each name knows how to find and how to start — where
...
/// [`PROJECT_RUNNERS`] and [`DEBUGGERS`]: a manifest naming a provider Unluminate does not have should say
/// so plainly rather than load as a plugin whose pane is permanently empty.
///
/// **What a plugin contributes is data and the code that draws it is Unluminate's.** A manifest says there
...
/// The icon sets built into this version of Unluminate that a theme's `icons` may name.
///
/// The sixth registry of this shape, checked the same way and for the same reason as the five above: a
/// theme naming a set Unluminate has not got should say so plainly rather than load as a theme whose rail
...
///
/// Unluminate's answer to VS Code's `when` expressions, which are the most copied part of its contribution
...
pub enum Kind {
    ...
    /// in Unluminate, named by `ui.provider`.
    Ui,
...
/// Unluminate's `theme::color::editor()` stays, because the window letting the desktop show through is the
...
/// A menu a plugin contributes, added after the six Unluminate has.
#[derive(Debug, Clone, PartialEq)]
...
impl Plugins {
    ...
    /// Unluminate starting with one plugin fewer is better than Unluminate refusing to start — the same rule
                ...
                for entry in entries.flatten() {
                    ...
                    match read_folder(&path) {
                        Ok(plugin) => {
                            ...
                            // one can be corrected by hand without rebuilding Unluminate.
    ...
    /// Every theme that can be chosen: Unluminate's own, then each theme plugin that is switched on.
    ///
    ...
    /// `UnluminateApp::apply_the_theme` falls back to `unluminate/dark` when the active one is no longer in this
    ...
    pub fn themes(&self) -> Vec<crate::theme::Theme> {
        let mut found = vec![crate::theme::Theme::unluminate_dark()];
        for plugin in self.installed.iter().filter(|plugin| plugin.enabled) {
    ...
    /// what makes ```` ```rs ```` and ```` ```rust ```` one question without Unluminate holding a table
    ...
    ///
    /// The other direction of [`Plugins::debugger_for`], and it exists for `unluminate-cli debug
...
pub fn parse(values: &Values, bundled: bool) -> Result<Plugin, String> {
    ...
    // Checked rather than assumed, so a manifest for something Unluminate cannot run is refused with a
    ...
    let kind = match values.text("plugin.kind").unwrap_or("language") {
        ...
        other => {
            return Err(format!(
                "plugin.kind is `{other}`, and this version of Unluminate runs `language`, `ui` and `theme` plugins"
            ))
        }
    };
    ...
    // manifest naming a picture Unluminate does not have should say so rather than load as a language
    ...
    let renders = match values.text("language.renders").map(str::trim).filter(|name| !name.is_empty())
    {
        ...
        Some(other) => {
            return Err(format!(
                "language.renders is `{other}`, and this version of Unluminate draws {}",
                RENDERERS.join(", ")
            ))
        }
        None => None,
    ...
    Ok(Plugin {
        ...
        vendor: values.text("plugin.vendor").unwrap_or("Unluminate").to_owned(),
...
fn one_theme(values: &Values, plugin: &str, id: &str) -> Result<crate::theme::Theme, String> {
    ...
    if !dark {
        return Err(format!(
            "theme.{id}.dark is false, and this version of Unluminate draws dark themes only"
        ));
    }
    let icons = match at("icons").filter(|named| !named.is_empty()) {
        Some(named) => crate::theme::IconSet::parse(named).ok_or_else(|| {
            format!("theme.{id}.icons is `{named}`, and this version of Unluminate draws {}", ICON_SETS.join(", "))
        })?,
        None => crate::theme::IconSet::default(),
    };

    // A role that is not named keeps Unluminate Dark's, which is the reference editor's `parentTheme` in one line and is
    // what keeps a manifest to the thirty colours that matter rather than all forty.
    let mut palette = crate::theme::Palette::UNLUMINATE_DARK;
    for (role, value) in values.starting_with(&format!("theme.{id}.ui.")) {
        ...
        if !palette.set(&role, egui::Color32::from_rgb(read.r, read.g, read.b)) {
            return Err(format!(
                "theme.{id}.ui.{role} names a colour Unluminate has not got. It has {}",
...
fn contributions(values: &Values, kind: Kind) -> Result<Contributions, String> {
    let provider = match word(values, "ui.provider") {
        ...
        Some(named) => {
            return Err(format!(
                "ui.provider is `{named}`, and this version of Unluminate has {}",
                UI_PROVIDERS.join(", ")
            ))
        }
    ...
    let chrome = match word(values, "ui.chrome") {
        ...
        Some(named) => {
            return Err(format!(
                "ui.chrome is `{named}`, and this version of Unluminate has {}",
                CHROME.join(", ")
            ))
        }
        None => None,
...
fn pane(values: &Values) -> Result<Option<PaneContribution>, String> {
    ...
    let icon = match word(values, "pane.icon") {
        ...
        Some(named) => {
            return Err(format!(
                "pane.icon is `{named}`, and this version of Unluminate draws {}",
                PANE_ICONS.join(", ")
            ))
        }
        None => "board".to_owned(),
    ...
    let applies = match word(values, "pane.applies") {
        ...
        Some(named) => {
            return Err(format!(
                "pane.applies is `{named}`, and this version of Unluminate knows {}",
...
fn page(values: &Values) -> Result<Option<PageContribution>, String> {
    ...
    let icon = match word(values, "settings.icon") {
        ...
        Some(named) => {
            return Err(format!(
                "settings.icon is `{named}`, and this version of Unluminate draws {}",
                PANE_ICONS.join(", ")
            ))
        }
        None => "board".to_owned(),
...
/// `run.project`: the name of a project detector built into Unluminate.
///
...
fn run_project(values: &Values) -> Result<Option<String>, String> {
    ...
    Err(format!(
        "run.project is `{named}`, and this version of Unluminate detects {}",
        PROJECT_RUNNERS.join(", ")
    ))
}

/// `debug.adapter`: the name of a debugger built into Unluminate.
///
...
fn debug_adapter(values: &Values) -> Result<Option<String>, String> {
    ...
    Err(format!(
        "debug.adapter is `{named}`, and this version of Unluminate drives {}",
        DEBUGGERS.join(", ")
    ))
}
...
///
/// The kind is checked against what `unluminate_core::symbols` actually has, for the same reason
...
fn definers(values: &Values) -> Result<Vec<(String, SymbolKind)>, String> {
    ...
    for entry in list(values, "language.definers") {
        ...
        let Some(parsed) = SymbolKind::parse(kind) else {
            ...
            return Err(format!(
                "language.definers says `{entry}`, and a definition in Unluminate is one of {}",
                known.join(", ")
            ));
        };
...
fn import_style(values: &Values) -> Result<Option<ImportStyle>, String> {
    ...
    match ImportStyle::parse(&named) {
        ...
        None => {
            ...
            Err(format!(
                "language.imports is `{named}`, and an import in Unluminate is written {}",
                known.join(" or ")
            ))
        }
    }
...
fn path_roots(values: &Values) -> Result<Vec<(String, PathRoot)>, String> {
    ...
    for entry in list(values, "language.path_roots") {
        ...
        let Some(parsed) = PathRoot::parse(meaning) else {
            ...
            return Err(format!(
                "language.path_roots says `{entry}`, and a root in Unluminate is one of {}",
                known.join(", ")
            ));
        };
        let word = word.trim();
...
/// registry-shaped key in Unluminate that is not validated against a list: the name is resolved by
...
mod tests {
    ...
    fn a_manifest_becomes_a_plugin() {
        ...
        // should not have to know which Unluminate wanted.
    ...
    fn a_kind_this_version_cannot_run_is_refused_rather_than_half_loaded() {
        // The seam a later version widens. A manifest asking for something Unluminate cannot do must say
    ...
    fn a_renderer_this_version_does_not_have_is_refused_with_the_list_of_the_ones_it_does() {
        ...
        // naming something Unluminate has not got says so plainly rather than loading as a plugin whose pane is
...
";
    ...
    /// pane. Reading a pane is Unluminate\'s side of the plugin contract rather than one plugin\'s arrangement, so
    ...
    fn installing_then_uninstalling_leaves_the_bundled_plugin() {
        let folder = std::env::temp_dir().join(format!("unluminate-plugins-{}", std::process::id()));
    ...
    fn the_css_plugin_reads_the_three_things_a_stylesheet_needs() {
        ...
        use unluminate_core::syntax::{highlight, Token};
    ...
    fn the_four_code_plugins_say_how_their_imports_are_written() {
        ...
        assert_eq!(rust.path_root("unluminate_core"), None, "a package is not a reserved word");
    ...
    fn a_theme_plugin_carries_several_themes() {
        ...
        // And One Dark keeps the marks Unluminate shipped with, so the bundle has one of each.
        assert_eq!(bundle.themes[4].icons, crate::theme::IconSet::Classic);
    }

    /// A role a theme does not name keeps Unluminate Dark's, which is the reference editor's `parentTheme` in one line.
    #[test]
    fn a_theme_inherits_every_colour_it_does_not_name() {
        ...
        assert_eq!(
            ...
            crate::theme::Palette::UNLUMINATE_DARK.editor,
            "and every other is Unluminate's own"
        );
    ...
    fn a_theme_manifest_is_refused_rather_than_half_loaded() {
        ...
        assert!(problem.contains("explorer_footer"), "and it lists what Unluminate has: {problem}");
    ...
    fn switching_a_theme_plugin_off_withdraws_its_themes() {
        ...
        assert_eq!(plugins.themes().len(), 6, "Unluminate's own, then the bundle's five");
        plugins.set_enabled(None, "themes-bundle-1", false);
        assert!(plugins.theme("Monokai Pro").is_none(), "and gone when the plugin is off");
        assert_eq!(plugins.themes().len(), 1, "leaving only Unluminate's own");
    }
    ...
    fn a_theme_is_found_by_its_key_or_by_its_name() {
        ...
        assert_eq!(plugins.theme("unluminate/dark").map(|theme| theme.name), Some("Unluminate Dark".to_owned()));
    ...
    /// so every debug control is **absent** for their files — which is Unluminate's rule for a control
    ...
    fn a_debugger_this_version_cannot_drive_is_refused_rather_than_half_loaded() {
        ...
        let refused = parse(&Values::parse(&format!("{head}debug.adapter = gdb")), false)
            .expect_err("gdb is not one Unluminate drives");
    ...
    #[test]
    fn a_manifest_asking_for_an_import_shape_or_a_root_unluminate_does_not_have_is_refused() {
    ...
    fn what_the_definers_add_up_to_read_through_the_reader_the_window_uses() {
        ...
        let found: Vec<(&str, SymbolKind)> =
            unluminate_core::symbols::file_definitions(source, &rust.grammar)
                .into_iter()
        ...
        let found: Vec<&str> = unluminate_core::symbols::file_definitions(source, &typescript.grammar)
            .into_iter()
    ...
    #[test]
    fn a_manifest_naming_a_detector_unluminate_does_not_have_is_refused_with_a_reason() {
    ...
    #[test]
    fn a_manifest_naming_a_renderer_unluminate_does_not_have_is_refused_with_a_reason() {
    ...
    fn installing_writes_the_folder_and_reads_it_back_from_disk() {
        let folder = std::env::temp_dir().join("unluminate-plugins-install");
    ...
    fn a_plugin_that_is_switched_off_is_remembered() {
        let folder = std::env::temp_dir().join("unluminate-plugins-disabled");
        std::fs::remove_dir_all(&folder).ok();
        let store = Store::at(&folder);
    ...
    fn a_folder_with_a_broken_manifest_is_skipped_and_reported() {
        let folder = std::env::temp_dir().join("unluminate-plugins-broken");
        std::fs::remove_dir_all(&folder).ok();
        let store = Store::at(&folder);
        ...
        assert_eq!(
            plugins.all().len(),
            bundled::ALL.len(),
            "Unluminate still has every one of its bundled plugins"
        );
    }
}
