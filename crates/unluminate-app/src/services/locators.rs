...
//! `cargo run` is the configuration Unluminate's own project suggests, and pressing Debug on it said
...
//! `UnluminateApp::begin_a_build` is what starts the process, on a thread, the way `unluminate-git` runs git.
...
/// The cargo flags that take their value as the next word, which is what stops `--bin unluminate` being
...
mod tests {
    ...
    fn the_flags_that_choose_what_is_built_are_kept_and_the_programs_arguments_are_not() {
        let built = build("cargo run --release -p unluminate-app --bin unluminate -- --control off");
        assert_eq!(
            built.args,
            vec![
                "build".to_owned(),
        ...
        assert_eq!(built.wanted.as_deref(), Some("unluminate"), "--bin also picks the artifact");
        assert_eq!(built.what, "Building unluminate");
    }

    #[test]
    fn a_flag_written_with_an_equals_sign_is_read_the_same_way() {
        let built = build("cargo run --bin=unluminate --features=fast");
        assert_eq!(built.wanted.as_deref(), Some("unluminate"));
    ...
    fn cargo_test_builds_the_test_binary_and_the_filter_is_the_programs_own() {
        let built = build("cargo test the_name -p unluminate-core -- --nocapture");
        assert_eq!(
            built.args,
            vec![
                "test".to_owned(),
                "-p".to_owned(),
                "unluminate-core".to_owned(),
                "--no-run".to_owned(),
                MESSAGE_FORMAT.to_owned(),
            ]
        );
    ...
    fn a_subcommand_that_builds_nothing_and_a_program_that_is_one_need_no_build() {
        ...
        assert_eq!(locate("target\\debug\\unluminate.exe"), None);
...
{"reason":"compiler-artifact","target":{"kind":["lib"],"name":"unluminate_core"},"executable":null}
{"reason":"compiler-artifact","target":{"kind":["bin"],"name":"unluminate"},"executable":"C:\\p\\target\\debug\\unluminate.exe"}
{"reason":"build-finished","success":true}"#;
    ...
    fn the_binary_is_taken_and_the_library_and_the_build_script_are_not() {
        ...
        assert_eq!(found, PathBuf::from("C:\\p\\target\\debug\\unluminate.exe"));
    }
    ...
    fn a_named_bin_is_taken_out_of_a_workspace_that_built_several() {
        let several = format!(
            "{TRANSCRIPT}\n{}",
            r#"{"reason":"compiler-artifact","target":{"kind":["bin"],"name":"unluminate-cli"},"executable":"C:\\p\\target\\debug\\unluminate-cli.exe"}"#
        );
        assert_eq!(
            executable(&several, None),
            Some(PathBuf::from("C:\\p\\target\\debug\\unluminate-cli.exe")),
            "with nothing asked for, the last is the one cargo would have run"
        );
        assert_eq!(
            executable(&several, Some("unluminate")),
            Some(PathBuf::from("C:\\p\\target\\debug\\unluminate.exe")),
            "and --bin picks"
        );
    ...
    fn a_library_only_build_produces_nothing_rather_than_a_panic() {
        let library = r#"{"reason":"compiler-artifact","target":{"kind":["lib"],"name":"unluminate_core"},"executable":null}
{"reason":"build-finished","success":true}"#;
