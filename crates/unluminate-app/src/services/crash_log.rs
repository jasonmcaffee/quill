//! Writing down what Unluminate was doing when it stopped.
//!
...
//! visible: every panic, on any thread, is appended to `crash.log` in the folder Unluminate keeps its
//! settings in, with the version, the time, where in the code it happened and a backtrace.
//!
//! It changes nothing else. The message still goes to standard error for anybody running Unluminate from a
...
pub fn install(folder: PathBuf) {
    ...
    std::panic::set_hook(Box::new(move |panic| {
        ...
        if let Err(problem) = append(&folder, &report) {
            eprintln!("Unluminate could not write {}: {problem}", folder.join(FILE).display());
        } else {
            eprintln!("Unluminate wrote what happened to {}", folder.join(FILE).display());
        }
        previous(panic);
    }));
}
...
fn report(panic: &std::panic::PanicHookInfo<'_>, backtrace: &str) -> String {
    ...
    out.push_str("---\n");
    out.push_str(&format!("Unluminate {}\n", env!("CARGO_PKG_VERSION")));
...
///
/// A civil date would need a calendar, and `unluminate-core` has one for git dates but this crate would
...
mod tests {
    ...
    fn a_panic_is_written_down_with_where_and_what_and_a_backtrace() {
        let folder = temporary("unluminate-crash-log");
        ...
        assert!(
            written.contains("unluminate_app") || written.contains("backtrace"),
    ...
    fn a_log_that_has_grown_too_large_is_rotated_rather_than_growing_for_ever() {
        let folder = temporary("unluminate-crash-log-rotate");
