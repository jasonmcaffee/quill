...
//! program Unluminate starts gets is what a command typed in Unluminate's own terminal would have got. That is
...
//! own exports matter to their own agent is not a decision Unluminate can make correctly — that is exactly
...
///
/// The process's own is kept rather than replaced because it is the honest answer when Unluminate was
/// started from a terminal — where it is the `PATH` the person typed `unluminate` on, which may be one
...
/// `PATH` and [`search_path`], as the pair a `unluminate_terminal::SessionSettings` lays over the
/// environment of a program Unluminate starts.
///
...
pub fn required(program: &str) -> Result<PathBuf, String> {
    find(program).ok_or_else(|| {
        format!(
            "{program} is not installed, or is in a folder that is not on the PATH Unluminate searched: {}",
...
mod tests {
    ...
    fn a_program_is_found_in_a_folder_on_the_path_and_a_missing_one_is_not() {
        let folder = std::env::temp_dir().join("unluminate-login-shell-test");
        std::fs::create_dir_all(&folder).expect("a folder under the temporary folder");
        let program = folder.join(match cfg!(windows) {
            true => "unluminate-test-agent.exe",
            false => "unluminate-test-agent",
        });
        ...
        // `PATHEXT` reads `.EXE` it answers `unluminate-test-agent.EXE` for the very file written here as
        // `.exe` - which the file system finds, and which an equality on the spelling does not.
        let found = look_up("unluminate-test-agent", &path).expect("the program is found");
        assert_eq!(
        ...
        assert!(look_up("unluminate-no-such-agent", &path).is_none());
    ...
    fn a_name_with_a_separator_in_it_is_a_path_rather_than_something_to_look_up() {
        ...
        assert!(find("unluminate-no-such-program-anywhere").is_none());
    }

    #[test]
    fn a_program_that_is_not_there_is_refused_by_name_with_the_folders_that_were_searched() {
        let problem = required("unluminate-no-such-agent").expect_err("nothing of that name exists");
        assert!(problem.starts_with("unluminate-no-such-agent is not installed"), "{problem}");
        assert!(
