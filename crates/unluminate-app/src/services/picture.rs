...
impl Picture {
    ...
    pub fn texture(&mut self, ctx: &egui::Context, name: &str) -> Option<egui::TextureHandle> {
        if let Some(image) = self.pending.take() {
            ...
            self.texture = Some(upload(ctx, format!("unluminate-picture-{name}"), image, options));
        }
        self.texture.clone()
    }
...
/// reaches Unluminate is the key going back up (see `agent_chat::pasting`), which arrives after an ordinary
...
mod tests {
    ...
    fn sample(name: &str) -> std::path::PathBuf {
        let folder = std::env::temp_dir().join("unluminate-picture-tests");
