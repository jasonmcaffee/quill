...
/// The rectangle the `TextEdit` inside one of Unluminate's fields is given.
///
/// Every field in Unluminate draws its own frame — `FIELD` with a one point stroke, the corner radius the
...
pub fn wants_the_keyboard() -> egui::Id {
    egui::Id::new("unluminate-field-wants-the-keyboard")
}
...
/// reads out — egui names a text box after whatever has been typed into it, so every field in Unluminate
/// says its own name.
pub fn search_field(
    ui: &mut egui::Ui,
    area: Rect,
...
/// toolbar returns a `unluminate_core::Command` and the Settings window returns a font size.
pub fn dropdown<T>(
    ui: &mut egui::Ui,
    area: Rect,
    value: &str,
...
pub fn choice_button_over(
    ...
    let painter = ui.painter();
    if !ground {
        // A hover still has to answer, or a button on a canvas would be the one control in Unluminate that never
...
///
/// This lives here rather than in `menu_bar` because there are three menus in Unluminate now — the bar
...
pub fn menu_rows(ui: &mut egui::Ui, entries: &[Entry], indent: f32) -> Option<Action> {
    ...
    if height > room {
        return egui::ScrollArea::vertical()
            ...
            .id_salt("unluminate-menu-scroll")
...
pub fn menu_row(
    ...
    if checked {
        // Drawn, not the character at U+2713. No font in the stack Unluminate hands egui has a shape for
...
/// A word in a bar that opens a menu when clicked, which is what `Unluminate`, `File`, `Edit` and `View` are.
