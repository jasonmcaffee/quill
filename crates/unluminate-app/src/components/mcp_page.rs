//! `Settings -> Tools -> MCP`: how an AI agent is given Unluminate.
//!
...
//!
//! The buttons write the **stdio** server: the agent launches `unluminate-cli mcp serve` itself, nothing
...
//! first is the ordinary one: a page is a fixed rectangle and a settings page in Unluminate does not
...
use unluminate_cli::mcp::install::{self, Client, Wanted};
use unluminate_cli::mcp::Shape;
...
pub struct McpState {
    ...
    pub port: String,
    /// Whether each client already has Unluminate, read when the page opens and after every click.
...
fn install_section(
    ...
    pen = note(
        ...
        area,
        pen,
        "Writes Unluminate into the agent's own configuration. The agent then launches Unluminate's server \
    ...
    match &state.said {
        ...
        None => note(
            ...
            area,
            pen,
            "Restart the agent afterwards. In Claude Code, `/mcp` then lists Unluminate's tools.",
        ),
    }
}
...
fn server_section(
    ...
    pen = note(
        ...
        pen,
        &format!(
            "{} tools. One an area names every command Unluminate has for about a third of the context \
             one a command costs; one a command lets an agent permit a single tool by name.",
            unluminate_cli::mcp::tools::tools(settings.mcp_tools).len()
        ),
    );
