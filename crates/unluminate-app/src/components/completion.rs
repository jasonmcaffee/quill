...
//!
//! It reports what was clicked and decides nothing, which is the rule every component in Unluminate
...
pub fn show(ui: &mut egui::Ui, state: &CompletionState, caret: Rect, pane: Rect) -> Outcome {
    ...
    egui::Area::new(egui::Id::new("unluminate-completion"))
        .order(egui::Order::Foreground)
        .fixed_pos(area.min)
        .interactable(true)
        .show(ui.ctx(), |ui| {
            // Reserve the whole of it, so egui knows where the list is for the pointer's sake; the
            // drawing itself is at absolute positions, as everything in Unluminate is.
...
/// The popup frame: the menu fill and the one point border every menu in Unluminate is drawn with.
...
fn draw_row(ui: &mut egui::Ui, area: Rect, row: &unluminate_core::completion::Row, chosen: bool) -> bool {
