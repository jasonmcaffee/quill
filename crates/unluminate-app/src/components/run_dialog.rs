...
//! holds, and `UnluminateApp` writes `.unluminate/run-configurations.conf` when the dialog closes — by the
...
/// How large the modal is before it is shrunk to fit a small Unluminate window.
///
...
pub fn show(
    ...
    let (inner, should_close) = modal::show(ctx, "unluminate-run-configurations", WIDTH, HEIGHT, |ui, area| {
