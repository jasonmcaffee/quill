...
/// How large the window is, before it is shrunk to fit a small Unluminate window.
///
...
/// still shrinks the whole thing to whatever room a small Unluminate window has.
...
pub fn show(
    ...
    // `unluminate_cli` is the client an agent is told to launch. It is passed in rather than worked out
    // in the page, because `current_exe` in the window is `unluminate.exe`, and because a screenshot
    // test has to be able to pin it: a picture holding this machine's own path is a picture no
    // other machine can match.
    unluminate_cli: &std::path::Path,
    ...
    // The window is drawn into one rectangle, the way every other part of Unluminate is, rather than
    ...
    let (inner, should_close) = modal::show(ctx, "unluminate-settings", WIDTH, HEIGHT, |ui, area| {
        contents(
            ui,
            area,
            state,
...
fn contents(
    ...
    match state.page {
        ...
        Page::Mcp => {
            outcome.changed |=
                mcp_page::show(ui, page_area, &mut state.mcp, settings, mcp_running, unluminate_cli)
                    .changed;
        }
...
///
/// Unluminate's own five answer for themselves; a contributed page's name is `settings.page` in its manifest,
...
fn appearance_page(
    ...
    // The reference editor's `Appearance -> Use custom font`, which Unluminate had no equivalent of: the window's own text
...
fn theme_page(
    ...
    // Empty means Unluminate's own, which is what the settings file says by saying nothing. Matched on the key
    // rather than remembered, so a theme whose plugin has been switched off leaves the row on Unluminate Dark
    // rather than on a name that is no longer in the list.
    let chosen = match settings.theme.is_empty() {
        true => crate::theme::Theme::unluminate_dark().key,
    ...
    for theme in &themes {
        ...
        if theme_row(ui, row, &theme.name, &theme.plugin, theme.palette, on) && !on {
            // Unluminate's own is written as an empty setting rather than as its key, so a settings file that
    ...
    pen = note(
        ...
        area,
        pen + 6.0,
        "A theme says what every colour in Unluminate's own palette means. One that names the nine token colours also recolours code, in every language at once, in the same frame.",
    );
...
/// One theme in the list: the pill every list in Unluminate draws for its chosen row, the name, where it came
...
fn theme_row(
    ...
    // Where it came from, in the faintest colour, so a bundle's five and Unluminate's own are told apart.
...
fn editor_page(ui: &mut egui::Ui, area: Rect, settings: &mut Settings) -> bool {
    ...
    note(
        ...
        area,
        pen,
        "A number against each line of the file. Unluminate wraps, so a paragraph that runs over several rows is numbered once, against its first row. Right clicking the gutter puts the numbers away and annotates with git blame.",
    );
    pen += 44.0;
