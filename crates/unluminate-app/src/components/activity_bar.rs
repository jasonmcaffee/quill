...
//! pane is showing or not. Unluminate's is narrower than the reference editor's — 36 points against about 40 — because it
...
//! window is on, and Unluminate has one rail on one edge, so its two groups say what a panel *is* (a
...
//!
//! A button that is on is the pill every list in Unluminate draws for its chosen row — `SELECTED_ROW`, the row
...
//! and the keyboard all go down the one path in `UnluminateApp::run_action`.
...
/// bottom — and a contributed button joins the group its manifest named. It is drawn after Unluminate's own,
...
pub fn show_with(
    ...
    // The plugins' buttons, after Unluminate's own in whichever group each manifest named. A button that
...
///
/// The names are `plugins::PANE_ICONS`, and a manifest naming one Unluminate cannot draw was refused when it
