...
//!
//! `unluminate_core::mermaid` works out where everything goes and hands back a `Scene`: rectangles,
...
//! diagram type can be added to `unluminate-core` without a line changing here.
//!
...
//! blown up, which is what `fit` means everywhere else in Unluminate — the wheel scrolls it, a pinch or
...
use unluminate_core::mermaid::scene::{Anchor, Dash, Item, Paint, Point, Scene};
...
/// short solid ones. Worked out here rather than in `unluminate-core` because how long a mark should be
/// depends on how large the diagram is being drawn, which only this side knows.
fn paint_line(
    painter: &egui::Painter,
    points: &[Pos2],
    stroke: &unluminate_core::mermaid::scene::Stroke,
    dash: Dash,
    scale: f32,
) {
...
fn stroke_of(stroke: &Option<unluminate_core::mermaid::scene::Stroke>, scale: f32) -> Stroke {
    match stroke {
...
/// A diagram's colour as egui knows it. Always fully opaque, like every other glyph in Unluminate.
fn colour_of(colour: unluminate_core::Color) -> Color32 {
...
/// type Unluminate does not draw yet is Unluminate's limitation and says so. Either way the **source is shown
/// underneath**, so nothing a person wrote disappears behind an error.
pub fn show_problem(
    ui: &egui::Ui,
    area: Rect,
    problem: &unluminate_core::mermaid::Problem,
    source: &str,
) {
    let painter = ui.painter();
