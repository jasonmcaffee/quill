...
//!
//! The terminal itself is in `unluminate-terminal`, which has no user interface dependencies. This draws what
...
//! background colour, then the selection over them, then one glyph a cell out of Unluminate's own atlas, then the
...
use unluminate_terminal::keys::{self, KeyPress, Modifiers as TermModifiers};
use unluminate_terminal::mouse;
use unluminate_terminal::screen::{CursorShape, Screen};
use unluminate_terminal::session::{SelectionKind, SessionSettings, Size};
use unluminate_terminal::Tabs;
...
/// [`unluminate_terminal::Tabs::move_tab`], which is what `unluminate-cli terminal move` calls too, so a
...
pub(crate) fn grid(
    ui: &mut egui::Ui,
    area: Rect,
    session: Option<&mut unluminate_terminal::Session>,
    selecting: &mut bool,
    focused: bool,
    id: &str,
    empty: &str,
...
fn handle_input(
    ui: &mut egui::Ui,
    session: &mut unluminate_terminal::Session,
    selecting: &mut bool,
    focused: bool,
    ...
    // the ones something else in Unluminate also listens for: Escape closes every modal, Enter confirms one and opens
...
fn key_press(key: egui::Key, modifiers: &egui::Modifiers) -> Option<KeyPress> {
    use unluminate_terminal::keys::Key as TermKey;
    let control = modifiers.ctrl && !modifiers.mac_cmd;
...
/// `Ctrl+Space` is how a null is sent, and in Unluminate's terminal all three did nothing.
///
...
mod tests {
    ...
    fn control_and_a_letter_becomes_a_terminal_key_press() {
        ...
        assert_eq!(press.key, unluminate_terminal::keys::Key::Character('c'));
    ...
    fn the_special_keys_are_all_carried_across() {
        let plain = egui::Modifiers::default();
        for (key, expected) in [
            (egui::Key::Enter, unluminate_terminal::keys::Key::Enter),
            (egui::Key::Tab, unluminate_terminal::keys::Key::Tab),
            (egui::Key::Escape, unluminate_terminal::keys::Key::Escape),
            (egui::Key::ArrowUp, unluminate_terminal::keys::Key::Up),
            (egui::Key::Delete, unluminate_terminal::keys::Key::Delete),
            (egui::Key::F5, unluminate_terminal::keys::Key::Function(5)),
        ] {
