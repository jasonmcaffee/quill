...
//! settled by `UnluminateApp::show_the_debug_tile` and its two siblings, not here.
//!
...
//!
//! Like every component in Unluminate this one takes a rectangle, draws itself and reports what happened.
...
pub struct Missing {
    ...
    pub sentence: String,
    /// The command that would install it, empty when Unluminate has nothing to offer.
    pub install: String,
}
...
pub struct DebugOutcome {
    ...
    pub step: Option<unluminate_dap::Step>,
...
fn show_header(
    ...
    // The adapter's own exception filters, when it offered any. **Unluminate holds no list of its own**,
...
fn show_watch(ui: &mut egui::Ui, row: Rect, watch: &Watch, outcome: &mut DebugOutcome) {
    ...
    // debugger explains a bad expression better than Unluminate could.
...
pub fn show_row(
    ...
    // stops it being the sixth field in Unluminate to put its words against its own top edge.
