...
//! rather than the drawing. See `UnluminateApp::follow_the_open_file`.
//!
...
//! what was dropped where. `UnluminateApp::move_path` does the rest.
//!
...
struct RowClick {
    ...
    /// a file Unluminate cannot show the text of can still be the one you meant to delete.
    picked: bool,
}
...
fn file_row(
    ...
    let row = allocate_row(ui, view.at(size::ROW));
    // A file Unluminate cannot open is drawn dimmed and does not open on a click, so the tree says what is
