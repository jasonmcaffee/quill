...
//! Unluminate wraps, so one paragraph is several `PlacedLine`s; a number is drawn against a visual line
...
///
/// Deliberately not `unluminate_git::BlameLine`: a component draws and does not know where its text came
...
pub struct Gutter<'a> {
    ...
    /// is, which is the rule every component in Unluminate follows. `unluminate_core::folding` works it out
    ...
    ///
    /// **Absent rather than dimmed**, which is Unluminate's rule for a control that can never apply: a
...
fn text_band(line: &unluminate_core::PlacedLine, top: f32) -> egui::Rangef {
...
///
/// **Unluminate draws the adapter's answer rather than its own hope**, which is `task-1675`'s honesty rule
...
///
/// A file whose language names no debugger takes no click at all — Unluminate's rule for a control that
...
mod tests {
    ...
    /// One line as `unluminate_core::layout` really builds it: the baseline `ascent` from the top, and
    /// every scrap of extra leading below the letters.
    fn line(y: f32, ascent: f32, descent: f32, leading: f32) -> unluminate_core::PlacedLine {
        unluminate_core::PlacedLine {
            y,
            ...
            runs: Vec::new(),
            empty_style: unluminate_core::CharStyle::default(),
        }
    }
    ...
    fn the_drift_grows_with_the_type_size() {
        ...
        let error = |line: &unluminate_core::PlacedLine| {
    ...
    /// A file whose language names no debugger gets no column and no click, which is Unluminate's rule
