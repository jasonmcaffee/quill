...
//!
//! `Unluminate` is first and sits at the very left of the bar, so it reads `Unluminate  File  Edit  View`, which is
...
pub fn show(ui: &mut egui::Ui, area: Rect, menus: &[Menu]) -> Option<Action> {
    ...
    for menu in menus {
        ...
        let response = controls::bar_button(ui, button, &menu.name, menu.name == "Unluminate");
