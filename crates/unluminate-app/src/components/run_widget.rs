...
//!
//! Three parts, and each is absent when it cannot apply, which is Unluminate's rule:
//!
...
//!   button would start **resolves to a debugger at all**, which `UnluminateApp::run_widget_state` works
...
//!   Unluminate's rule for a control that cannot apply, asked of the thing the button acts on rather than
...
//! rail and the keyboard all go down the one path in `UnluminateApp::run_action`.
...
pub struct WidgetState {
    ...
    /// file's language — see [`crate::app::UnluminateApp::adapter_for`].
    pub debuggable: bool,
...
pub fn show(ui: &mut egui::Ui, area: Rect, state: &WidgetState) -> Option<Action> {
    ...
    // Stop, drawn only while it can apply — Unluminate's rule for a control that cannot.
    if state.running {
