//! A ticket's description, drawn by Unluminate's own editor.
//!
//! The description is markdown and Unluminate reads and draws markdown, so it is a `unluminate_core::Document` and
...
///
/// `components::markdown_text` is what renders and paints it, which is `unluminate_core::markdown` and the editor's
