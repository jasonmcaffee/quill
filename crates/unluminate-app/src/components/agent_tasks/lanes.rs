...
fn live_for(
    ...
    card::Live {
        ...
        // the id on the ticket is only Unluminate's marker that a worker was here and there is nothing to resume. The
...
pub fn show(board: &mut AgentTasks, ui: &mut egui::Ui, look: &Look<'_>, area: Rect) -> Vec<Request> {
    ...
    // Named, because every control in Unluminate has a plain name and a test finds one by it. This is the one that
...
fn header(ui: &mut egui::Ui, look: &Look<'_>, lane: Rect, status: Status, count: usize) {
    ...
    // already Unluminate's; the violet is `color::agent()`, and the reason it was added is written down beside it.
