...
//!
//! The description is markdown and Unluminate already reads markdown, so it is drawn by the same reader the
...
pub(crate) fn terminal(
    ...
    if outcome.take_focus {
        // Both halves, because they are two different things. `UnluminateApp::focus` is the one value that says who in
