...
//! division every component in Unluminate makes, and it is what lets the board be tested with no window.
//!
...
/// other panel in Unluminate already has: the four drop bands, the strong rectangle showing where it would land
...
pub fn pane(board: &mut AgentTasks, ui: &mut egui::Ui, look: &Look<'_>) -> Vec<Request> {
    ...
    // be added to the painter after it and would wash the decoration out. See `UnluminateApp::paint_the_chrome`.
    let mut requests = Vec::new();
    // **No pump here.** Every open provider is pumped once a frame by `UnluminateApp::let_the_plugins_catch_up`,
...
///
/// **This is the board's own rail and not Unluminate's.** `components::activity_bar` is the thin strip on the
...
fn view_switch(
    ...
    // those lanes; neither means anything on a page of sprints or of epics, and Unluminate's rule is that a
...
/// `unluminate-core`. See `components::markdown_text`.
...
/// already use, so it opens, closes and reads like every other dropdown in Unluminate. `options` is `(value, said)`
...
pub(crate) fn value_dropdown_over(
    ...
    let picked =
        ...
        for (value, said) in &rows {
            ...
            // is what every other dropdown in Unluminate puts in one.
