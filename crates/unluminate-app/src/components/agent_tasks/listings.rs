...
///
/// A value rather than a call, for the reason every component in Unluminate reports rather than acts: the
...
fn group_heading(
    ...
    // The buttons, from the right. A control that cannot apply is absent, which is Unluminate's rule and is why
