...
//!
//! It is a `unluminate_core::Document` and `components::editor_view` draws it, so the description gets Unluminate's own
...
//! to a person rather than disabled, which is Unluminate's rule and the one place this deliberately differs from
//! the browser.
...
/// the most crowded thing on this board. Every other modal in Unluminate keeps its fixed size: they are a
...
fn contents(
    ...
    // and every dialog in Unluminate shares.
...
fn fields(
    ...
    // which is Unluminate's rule: a ticket with an agent already running offers `Stop` instead, and a new one
    ...
    if !new {
        ...
        let (label, command) = match () {
            ...
            // Codex ticket is only Unluminate's marker that a worker was here and there is no conversation to hand
    ...
    // **Absent** for a ticket assigned to a person rather than disabled, which is Unluminate's rule: the `F` button
    ...
    //
    // **What it does not do is sync.** There is no HTTP client in Unluminate, which
    ...
    if !new {
        ...
        pen += tall;
        // Copy only when there is something to copy, which is Unluminate's rule about a control that cannot apply.
...
///
/// The triangle is the disclosure every tree in Unluminate draws, and the whole heading is the target rather than
...
/// dropdown in Unluminate. `options` is `(value, said)` pairs — the value written to the row and the words a person
...
fn dropdown_row(
    ...
    // right everywhere else in Unluminate and wrong here: this modal is on a board drawn in dark neumorphism, and
