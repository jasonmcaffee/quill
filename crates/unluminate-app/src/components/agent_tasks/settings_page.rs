...
fn rows(board: &mut AgentTasks, ui: &mut egui::Ui, look: &Look<'_>) -> Vec<Request> {
    ...
    // runs. Empty means the command Unluminate builds itself, which is what these hints say.
    ...
    // Key variable naming an environment variable, and then the key — three of which are Unluminate's own plumbing
    ...
    // Whether this machine has a keychain Unluminate can drive at all. macOS has `security` and Linux has
    ...
    let key_state = match (found, has_a_keychain) {
        ...
        (None, false) => format!(
            "not set, and Unluminate has no keychain on {}: export ANTHROPIC_API_KEY in your shell profile",
            std::env::consts::OS
        ),
    };
        ...
        match has_a_keychain {
            true =>
                "Typed once and never shown again. It goes to this machine's keychain under `iliad`, not to a \
                 file, and Unluminate reads it at the moment an agent is launched and hands it over as \
                 ANTHROPIC_API_KEY, OPENAI_API_KEY and ILIAD_API_KEY. Nothing needs typing here when your shell \
                 profile already exports ANTHROPIC_API_KEY: Unluminate reads that profile whether it was started from \
            ...
            false =>
                "There is no keychain here for Unluminate to write to, so nothing can be typed. Export \
...
fn heading(ui: &mut egui::Ui, look: &Look<'_>, area: Rect, pen: &mut f32, said: &str) {
    ...
    // Named, because every control in Unluminate has a plain name and a test finds one by it. A heading over a group
