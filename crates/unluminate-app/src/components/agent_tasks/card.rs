...
/// in **this** window, and whether Start could do anything. Unluminate's rule is that a control which cannot
...
pub fn show(
    ...
    // Named, because every control in Unluminate has a plain name and a test finds one by it. A card is the control
    ...
    // A card under the pointer wears the pill every list in Unluminate draws for its chosen row, rather than a
    ...
    if look.chrome.is_recording() {
        ...
        // nothing — and it is the same pill every list in Unluminate draws for the row it is on.
    ...
    if let Some(key) = &task.jira_key {
        if chip + 60.0 < right {
            ...
            // colour Unluminate uses for something you can act on, so it read as a link — and clicking it opened the
            // ticket modal, because the card's own click target covers the whole card and there is nothing here
            // that could open a browser. Unluminate has no way to open a URL: `services::launcher` opens a folder in
            // the file manager and a new Unluminate window, and that is all. The link lives on `Copy issue link` in the
