...
//! `tasks/unluminate-ide-tdd.md` says it goes.
//!
//! The tab that is showing is a filled row with a two point accent line along its bottom edge, which
//! is the underline `task-1649` asks for. A file with unsaved changes shows the amber dot Unluminate
...
//! Unluminate is painted at an absolute position and one container that is not would behave differently
...
pub struct TabsOutcome {
    ...
    /// Unluminate has a header to press; the editing area has this strip, and the part of it no tab wanted is
...
fn draw_tab(
    ...
    // family Unluminate installs, and a fake slant is worse than a change of weight.
