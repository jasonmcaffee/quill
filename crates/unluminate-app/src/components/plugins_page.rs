...
//!
//! Laid out like the capture in `tasks/unluminate-ide-tdd.md`, and built out of the Settings window's own
...
//! so; `task-1795` took it out, because it answered a question about Unluminate on every plugin’s own
...
fn detail(
    ...
    let where_from = match (on_disk, plugin.bundled) {
        (true, true) => "Its folder is under the settings folder and can be edited by hand. Uninstalling takes that folder away and puts the plugin back to the one Unluminate ships.",
        (true, false) => "Its folder is under the settings folder and can be edited by hand. Uninstalling removes it.",
        (false, _) => "Installed with Unluminate and working. Customising writes its folder out where it can be edited by hand.",
    };
    ...
    egui::ScrollArea::vertical().id_salt("plugin-detail").show(&mut child, |ui| {
        ...
        match plugin.kind {
            ...
            crate::services::plugins::Kind::Ui => {
                ...
                note(
                    ui,
                    "It draws with Unluminate's own controls, so it takes the font and the transparency from Appearance and cannot name a colour of its own.".to_owned(),
                );
            }
            crate::services::plugins::Kind::Theme => {
                ...
                note(
                    ui,
                    "A theme says what every name in Unluminate's own palette means, so the list of colours stays closed while what they are changes. Choose one in Appearance & Behavior -> Theme.".to_owned(),
                );
            }
        }
