...
//!
//! Every control sends a `unluminate_core::Command`. None of them changes the document directly, so the tools
...
use unluminate_core::{Align, Color, Command, Document, StyleChange};
...
fn text_options(
    ...
    // everything else in Unluminate is drawn. The rule between the two halves adds its own gap.
    let rule_gap = 11.0;
