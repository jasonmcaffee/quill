...
//!
//! `components::markdown_text` is `unluminate_core::markdown::render` plus a layout, which is exactly what
...
use unluminate_chat::model::{Message, Part, Role, ToolCall};
...
fn pieces(
    ...
    // plain text, which is within a point or two of what `unluminate_core` will lay the markdown out at,
...
///
/// `StatusTopicEl`'s own shape, in Unluminate's palette: open while it is running and collapsed once it
...
fn tool_block(
    ...
    // Unluminate's blue when it worked, red when it did not.
...
///
/// Both are colours Unluminate already has. The panel is the well every field in the window is, and the
...
///
/// egui's own layout rather than `unluminate_core`'s, because this is asked before the markdown has been
