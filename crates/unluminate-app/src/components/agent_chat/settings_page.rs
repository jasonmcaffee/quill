...
//! that mean nothing there. They are **absent** rather than dimmed for such a row, which is Unluminate's
...
use unluminate_chat::provider::{Provider, Wire, DEFAULT_MAX_TOKENS, WIRES};
...
fn rows(chat: &mut AgentChat, ui: &mut egui::Ui, look: &Look<'_>) -> Vec<Request> {
    ...
    pen = crate::components::modal::note(
        ...
        "What answers. `claude-cli` and `codex-cli` run the agent already installed on this machine: \
         ...
         is never stored by Unluminate, so name the environment variable it is in.",
    );
    ...
    // question is which of Unluminate's own commands it may ask for. Drawing both would be a page with
    ...
    if an_agent {
        ...
        if crate::components::modal::check(
            ui,
            row,
            "Let the model use Unluminate's own commands",
        ...
        pen = crate::components::modal::note(
            ...
            inner,
            pen,
            "Off unless you say so. On, the model is offered Unluminate's whole command catalogue as \
    ...
    pen = crate::components::modal::note(
        ...
        inner,
        pen,
        "Added after Unluminate's own line, which says which file is showing. The file's text is never \
         sent by Unluminate.",
    );
...
fn permission_row(
    ...
    for named in unluminate_chat::PERMISSIONS {
        ...
        if crate::components::controls::choice_button(ui, at, named, on) {
            if let Some(permission) = unluminate_chat::Permission::from_name(named) {
    ...
    let said = match configuration.permission {
        unluminate_chat::Permission::Read => {
