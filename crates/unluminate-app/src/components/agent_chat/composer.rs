...
//! Unluminate's rule about a control that cannot apply being absent rather than present and refusing.
//!
...
//! can: its own server knows the window the model was loaded with. Unluminate does not — a URL and a model
...
fn pill(parts: &Parts<'_>, ui: &mut egui::Ui, look: &Look<'_>, area: Rect) -> Vec<Act> {
    ...
    // handing them Unluminate's would be offering a switch that does nothing, and they always stream, so
...
fn prompt(parts: Parts<'_>, ui: &mut egui::Ui, look: &Look<'_>, area: Rect) -> Vec<Act> {
    ...
    if field.width() > 30.0 {
        ...
        // Named, because every control in Unluminate has a plain name and a test finds one by it. Its hint
    ...
    //
    // **It senses a click only when it can do something**, which is Unluminate's rule about a control that
