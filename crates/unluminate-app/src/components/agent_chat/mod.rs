...
fn surface(mut parts: Parts<'_>, ui: &mut egui::Ui, look: &Look<'_>, area: Rect) -> Vec<Act> {
    ...
    // decoration off it is the flat bordered panel every list in Unluminate draws, so switching the
...
fn conversation(parts: &mut Parts<'_>, ui: &mut egui::Ui, look: &Look<'_>, area: Rect) -> Vec<Act> {
    ...
    let session: &unluminate_chat::Session = parts.session;
    ...
    let scrolled = scroller.show(&mut body, |ui| {
        ...
        for one in &session.chat.messages {
            ...
            if one.role == unluminate_chat::Role::Tool {
                continue;
            }
...
///
/// `ChatPage.module.css`'s `.chatEmpty`, with the four starter prompts made about what Unluminate is: a
    ...
    for act in acts {
        match act {
            ...
            Act::ToggleTools => {
                ...
                requests.push(Request::Message(match now {
                    true => "Unluminate's own commands are offered to the model.".to_owned(),
                    false => "No tools are offered.".to_owned(),
                }));
            }
            // The picker is the platform's, and `rfd` is how Unluminate already opens one. It blocks the
...
/// the one report of the chord that reaches Unluminate. Asking the window for a picture on the way up is a
...
/// Whether this frame's events carry the paste chord as Unluminate can actually see it.
///
...
/// holding a position from before the drag started. Nothing else in Unluminate reads `dropped_files`, so a
...
/// gating on it silently threw the picture away. Nothing else in Unluminate reads `dropped_files`, so a drop
...
/// something that is refused by the server rather than by Unluminate.
...
mod tests {
    ...
    /// A drop the platform cannot place is this pane's, because nothing else in Unluminate wants one.
    #[test]
    ...
    #[test]
    fn the_starter_chips_are_about_what_unluminate_is() {
