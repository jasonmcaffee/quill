...
//! Unluminate is resized by dragging its edge and a pane inside a modal is still a pane.
//!
...
pub fn show(ctx: &egui::Context, state: &mut FindInFiles, split: f32) -> FindOutcome {
    ...
    let (_, closed) = modal::show(ctx, "unluminate-find-in-files", WIDTH, HEIGHT, |ui, area| {
...
/// The frame round one of the two panes, drawn the way every sunken box in Unluminate is.
