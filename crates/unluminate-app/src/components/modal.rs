//! The furniture every modal in Unluminate is built from.
//!
...
//! under the modal's id rather than in `UnluminateApp`. A modal's geometry belongs to the modal: the
...
/// Unluminate window larger moves a modal that was dragged to one side along with the middle it was
...
/// Put the modal with this id where a caller says, which is what `unluminate-cli modal move` and
...
///
/// Every control in Unluminate has a plain name, and the drag strip and the eight grips are controls. The
/// id is what [`show`] is given, so `unluminate-find-in-files` names them `Move find in files` and
/// `Resize find in files: bottom right`, which is what a test asks for and what assistive technology
/// reads out.
fn plain_name(id: &str) -> String {
    id.trim_start_matches("unluminate-").replace('-', " ")
}
...
/// window, so a modal in a small Unluminate window is smaller rather than running off the edges. Where it
...
pub fn header_of(ui: &mut egui::Ui, area: Rect, key: Option<&str>, title: &str) -> bool {
    ...
    // Every control in Unluminate has a plain name and a test finds one by it; a modal's title is what says which of
...
pub enum Confirm {
    /// Enter, which is what every modal in Unluminate but one uses.
    Enter,
...
/// controls in Unluminate must not share a name. So the word is what a person reads and the name is what a
...
/// A field to type into, drawn the way every other one in Unluminate is.
...
mod tests {
    ...
    fn a_modals_controls_are_named_after_it() {
        assert_eq!(plain_name("unluminate-find-in-files"), "find in files");
        assert_eq!(plain_name("unluminate-settings"), "settings");
    }
