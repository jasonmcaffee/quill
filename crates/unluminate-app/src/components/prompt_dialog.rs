//! The one text prompt, and the one confirmation, that the rest of Unluminate asks with.
//!
...
//! [`UnluminateApp::run_action`] decides what a confirmed prompt means, so a prompt cannot do anything by
//! itself.
...
pub fn show(ctx: &egui::Context, prompt: &mut Prompt) -> PromptOutcome {
    ...
    let closed = modal("unluminate-prompt", ctx, |ui, area| {
...
pub fn confirm(ctx: &egui::Context, question: &Confirmation) -> PromptOutcome {
    let mut outcome = PromptOutcome::default();
    let closed = modal("unluminate-confirm", ctx, |ui, area| {
