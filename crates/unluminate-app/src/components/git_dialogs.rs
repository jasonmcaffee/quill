...
//! none of them runs a git command: each returns what was asked for and `UnluminateApp::run_action` sends
//! it to the worker thread.

use egui::{Pos2, Rect, Vec2};
use unluminate_git::{Branch, Commit, PullStrategy, PushTarget, Remote, ResetMode, Status};
...
pub struct DialogOutcome {
    ...
    pub merge: Option<(String, unluminate_git::MergeOptions)>,
...
pub fn show(
    ...
    let (_, closed) = modal::show(ctx, "unluminate-git-dialog", width, height, |ui, area| {
