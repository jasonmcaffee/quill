...
//! holds a button each and pressing one puts the other away, which `UnluminateApp` settles, not this
//! file.
//!
//! ## What a run is
//!
...
//! never written into by Unluminate**: The reference editor prints its epilogue into the console, and that line
...
//! second press, is killed through [`unluminate_terminal::Session::kill`]. Closing a tab and closing the
...
use unluminate_terminal::session::{Session, SessionSettings, Size};
...
pub struct Run {
    ...
    /// one that ended without choosing a code, which is a program Unluminate killed or, on Unix, one a
    /// signal took.
    ///
...
impl Run {
    ...
    pub fn state(&self) -> State {
        match self.ended {
            ...
            // A program whose code could not be read — one Unluminate killed, or one a signal took on
...
impl State {
    ...
    /// `theme::color`, which is the whole list of colours Unluminate draws with.
...
pub struct RunPanel {
    ...
    /// `UnluminateApp::run_grid_size` explains and what loses a fast program's output.
    pub tile: Rect,
}
...
impl RunPanel {
    ...
    pub fn start(
        ...
        root: &std::path::Path,
        size: Size,
        waker: unluminate_terminal::session::Waker,
    ) -> Result<usize, String> {
        ...
        let session = Session::spawn(&settings, size, waker)
            .map_err(|problem| format!("Unluminate could not start {program}: {problem}"))?;
...
///
/// `controls::icon_button` has no such state, because until now every icon button in Unluminate could
...
mod tests {
    ...
    fn a_run_that_has_ended_keeps_its_tab_and_says_what_it_ended_with() {
        ...
        // exercised by `unluminate-terminal`'s own tests and by the walkthrough.
