...
//!
//! Unluminate draws its own title bar, which means the window is created with `with_decorations(false)`, and
...
//! started Unluminate resizes from all four edges and all four corners, and in `egui_kittest` all eight
...
//! on a maximised window, every later drag did nothing, and a freshly started Unluminate worked at once.
//!
//! So **no grip is added while the window is maximised**. That is Unluminate's own rule for a control
...
/// every component in Unluminate follows.
///
...
fn grip(ui: &mut egui::Ui, area: Rect, name: &str, cursor: egui::CursorIcon) -> bool {
    ...
    // Every control in Unluminate has a plain name, so a test can find this one without a pointer.
