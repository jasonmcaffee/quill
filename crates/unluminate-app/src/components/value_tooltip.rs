...
pub fn show(
    ...
    outcome.above = above;
    egui::Area::new(egui::Id::new("unluminate-value-tooltip"))
        .order(egui::Order::Foreground)
        .fixed_pos(area.min)
        .interactable(true)
        .show(ui.ctx(), |ui| {
            // Reserved so egui knows where the popup is for the pointer's sake; the drawing itself is
            // at absolute positions, as everything in Unluminate is.
...
///
/// **Its own words, never Unluminate's** — a debugger explains a name that does not resolve in this frame
/// far better than an editor could, which is the rule `unluminate-git` keeps about git's standard error.
