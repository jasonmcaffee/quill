...
fn where_and_order(
    ...
    // Enter sends it, which is what a filter field does everywhere else in Unluminate. Typing alone must
...
/// string the same way is a grid nobody could trust — which is the reason `unluminate_db::Value` keeps them
...
///
/// A console result is **never editable**: The reference editor tries to resolve one back to a table, and Unluminate
