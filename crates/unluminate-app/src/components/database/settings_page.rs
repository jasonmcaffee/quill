...
//! UNLUMINATE_DB_AI`, `keychain …`, `typed, until this window closes`, or `none` — and never what it is,
...
fn rows(explorer: &mut DatabaseExplorer, ui: &mut egui::Ui, look: &Look<'_>) -> Vec<Request> {
    ...
    pen = modal::note(
        ...
        body,
        pen,
        "Where each one points, and where its password is kept. Unluminate never writes a password into \
    ...
    let sources: Vec<unluminate_db::Source> = explorer.sources().to_vec();
