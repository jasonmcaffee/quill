...
pub enum Act {
    ...
    NullTheCell(u64),
    SetCell(u64, usize, usize, unluminate_db::Value),
    ...
    for act in acts {
        match act {
            ...
            Act::DropTable(source, schema, name) => {
                let sql = match unluminate_db::sql::drop_table(&schema, &name) {
                    Ok(sql) => sql,
                    Err(why) => {
                        requests.push(Request::Message(why));
                        continue;
            ...
            Act::CommitCell(id) => with_grid(explorer, id, |grid| {
                ...
                let value = match editing.text.is_empty() && was.is_null() {
                    true => unluminate_db::Value::Null,
                    false => unluminate_db::Value::Text(editing.text.clone()),
                };
            ...
            Act::NullTheCell(id) => with_grid(explorer, id, |grid| {
                ...
                if let Some(row) = grid.row_of(at) {
                    grid.pending.set(row, &name, unluminate_db::Value::Null);
                }
                grid.editing = None;
            }),
...
fn kind_of(explorer: &DatabaseExplorer, source: &str, schema: &str, name: &str) -> unluminate_db::Kind {
    explorer
        .loaded
        .get(source)
...
/// A raised card, or the flat bordered panel every list in Unluminate draws when the decoration is off.
///
