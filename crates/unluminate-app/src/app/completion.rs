...
//!
//! `unluminate_core::completion` says what a stem is, what matches it and in what order;
...
//! [`UnluminateApp::keep_the_completion_fresh`] compares two integers before it does anything at all. A
...
use unluminate_core::completion::{self, Candidate, Row, Source};
use unluminate_core::imports::{self as core_imports, Context as ImportContext};
use unluminate_core::symbols::SymbolKind;
use unluminate_core::{Command, Grammar, Role};

use crate::app::{Focus, UnluminateApp};
...
impl UnluminateApp {
    ...
    pub fn completion_candidates(&mut self, stem: &str, offset: usize) -> Vec<Candidate> {
        ...
        if let Some(grammar) = self.grammar_for(self.files.active().path()) {
            let lists: Vec<(&Vec<String>, &str)> = if grammar.markup {
                ...
                let position = unluminate_core::syntax::markup_position(
                    &self.document().text().to_string(),
                    offset,
                    &grammar,
                );
                match position {
                    Some(unluminate_core::syntax::MarkupPosition::TagName) => {
                        vec![(&grammar.keywords, "keyword")]
                    }
                    Some(unluminate_core::syntax::MarkupPosition::Attribute) => {
    ...
    ///
    /// One function, so `unluminate-cli editor complete` prints exactly the list the popup would show
    ...
    /// `Complete Word`, `Ctrl+Space`, and `unluminate-cli editor complete`.
    ///
    ...
    /// identifier character to the left of the caret at all it does what every honest miss in Unluminate
    /// does and says so in the status bar.
    pub fn complete_word(&mut self) {
        if !self.completion_applies_here() {
            self.message =
                Some("No plugin claims this file, so Unluminate has no words to offer.".to_owned());
            return;
        }
        let head = self.document().selection().head;
        let text = self.document().text().to_string();
...
mod tests {
    ...
    fn a_window(name: &str) -> (PathBuf, UnluminateApp) {
    ...
    fn typing(app: &mut UnluminateApp, text: &str) {
    ...
    fn a_quiet_frame(app: &mut UnluminateApp) {
    ...
    /// one left behind.
    fn an_open_popup(app: &mut UnluminateApp, original: &str) {
    ...
    fn offered(app: &UnluminateApp) -> Vec<String> {
        app.completion()
            .map(|state| state.rows.iter().map(|row| row.name.clone()).collect())
            .unwrap_or_default()
    }

    fn text_of(app: &UnluminateApp) -> String {
    ...
    fn typing_the_second_letter_of_a_word_opens_the_list_with_the_best_row_chosen() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-opens");
        typing(&mut app, "d");
    ...
    fn a_one_character_stem_asks_the_pool_for_nothing_at_all() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-one-letter");
        typing(&mut app, "d");
    ...
    fn the_automatic_popup_stays_out_of_a_comment_and_asking_by_hand_opens_one_there() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-comment");
    ...
    fn typing_on_until_nothing_matches_closes_it_and_the_next_letter_asks_again() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-narrows");
        typing(&mut app, "dra");
    ...
    fn backspace_inside_the_word_keeps_it_open_and_refilters_it() {
        // Scenario 16.
        let (folder, mut app) = a_window("unluminate-completion-backspace");
    ...
    fn typing_a_word_boundary_closes_it() {
        ...
        for boundary in ["(", " ", "."] {
            let (folder, mut app) = a_window("unluminate-completion-boundary");
            typing(&mut app, "dra");
            assert!(app.completion().is_some());
    ...
    fn the_caret_moving_by_anything_but_typing_closes_it() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-caret-moved");
    ...
    fn a_modal_opening_and_the_tab_changing_both_close_it() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-modal");
        typing(&mut app, "dra");
        assert!(app.completion().is_some());
    ...
    fn escape_closes_it_and_leaves_the_document_alone() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-escape");
        typing(&mut app, "dra");
    ...
    fn manual_suggestions_stop_the_unasked_popup_and_keep_the_asked_one() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-manual");
    ...
    fn asking_with_no_word_to_the_left_of_the_caret_says_so_and_opens_nothing() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-nothing-there");
        typing(&mut app, " ");
        app.complete_word();
        assert!(app.completion().is_none());
    ...
    fn down_twice_then_enter_replaces_the_stem_and_one_undo_puts_it_back() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-accept");
        let before = text_of(&app);
        typing(&mut app, "dra");
        assert_eq!(
            offered(&app),
    ...
    fn the_pill_is_clamped_at_the_ends_and_the_list_scrolls_with_it() {
        ...
        // cannot hold `Down` on.
        let (folder, mut app) = a_window("unluminate-completion-steering");
        typing(&mut app, "dr");
    ...
    fn the_list_scrolls_when_the_pill_walks_off_the_eight_that_are_drawn() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-scrolls");
        typing(&mut app, "ra");
        let rows = app.completion().expect("open").rows.len();
    ...
    fn tab_replaces_the_whole_word_and_enter_replaces_only_the_stem() {
        ...
        for (whole_word, expected) in [(true, "draw_frame;"), (false, "draw_framewing;")] {
            let (folder, mut app) = a_window("unluminate-completion-mid-word");
    ...
    fn typing_while_it_is_open_lands_in_the_document_and_refilters_the_list() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-typing-through");
        typing(&mut app, "dr");
    ...
    fn a_click_on_a_row_accepts_it_exactly_as_enter_does() {
        ...
        // screenshot test's.
        let (folder, mut app) = a_window("unluminate-completion-click");
        typing(&mut app, "dra");
    ...
    fn the_popup_takes_exactly_five_keys_and_only_while_it_is_open() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-non-interference");
        let original = text_of(&app);
        let five = [
            egui::Key::ArrowDown,
            egui::Key::ArrowUp,
    ...
    fn pressing(app: &mut UnluminateApp, key: egui::Key, modifiers: egui::Modifiers) -> bool {
        let context = egui::Context::default();
    ...
    fn the_index_answers_for_closed_files_and_never_for_open_ones() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-ownership");
    ...
    fn the_language_s_own_words_are_offered_and_labelled_as_such() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-keywords");
    ...
    fn the_position_in_a_tag_decides_which_language_words_are_offered() {
        ...
        // own words.
        let folder = std::env::temp_dir().join("unluminate-completion-markup");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
        std::fs::write(
    ...
    fn a_web_window(name: &str) -> (PathBuf, UnluminateApp) {
    ...
    /// A Rust workspace shaped like Unluminate's own: two packages, each with a source root.
    fn a_workspace(name: &str) -> PathBuf {
        let folder = std::env::temp_dir().join(name);
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(folder.join("crates/unluminate-core/src")).expect("make the core");
        std::fs::create_dir_all(folder.join("crates/unluminate-app/src/app")).expect("make the app");
        std::fs::write(folder.join("crates/unluminate-core/src/lib.rs"), "pub mod completion;\n")
            .expect("write core lib.rs");
        std::fs::write(
            folder.join("crates/unluminate-core/src/completion.rs"),
            "pub struct Candidate;\n\npub fn rank() {}\n\nfn hidden() {}\n",
        )
        .expect("write completion.rs");
        std::fs::write(folder.join("crates/unluminate-app/src/lib.rs"), "pub mod app;\n")
            .expect("write app lib.rs");
        std::fs::write(folder.join("crates/unluminate-app/src/app/mod.rs"), "")
            .expect("write app/mod.rs");
        std::fs::write(
            folder.join("crates/unluminate-app/src/app/actions.rs"),
            "pub enum Action {}\n\npub fn menus() {}\n",
        )
    ...
    /// A window on it, with `crates/unluminate-app/src/app/mod.rs` open.
    fn a_rust_window(name: &str) -> (PathBuf, UnluminateApp) {
    ...
    fn ask_at(app: &mut UnluminateApp, line: &str) {
    ...
    fn a_specifier_offers_the_projects_own_files_and_nothing_else() {
        ...
        let (folder, mut app) = a_web_window("unluminate-import-specifier");
    ...
    fn a_named_import_offers_what_the_module_exports_and_not_what_it_hides() {
        ...
        let (folder, mut app) = a_web_window("unluminate-import-named");
    ...
    fn a_module_that_resolves_to_nothing_offers_nothing() {
        ...
        let (folder, mut app) = a_web_window("unluminate-import-unresolved");
    ...
    fn tab_replaces_the_whole_specifier_and_enter_replaces_what_was_typed() {
        ...
        let (folder, mut app) = a_web_window("unluminate-import-accept");
    ...
    fn accepting_an_import_row_is_one_undo_step() {
        ...
        let (folder, mut app) = a_web_window("unluminate-import-undo");
        ask_at(&mut app, "import { Layout } from './la|zy'");
        assert!(app.choose_the_completion("./layout"));
        app.accept_the_completion(true);
        app.document_mut().apply(Command::Undo);
    ...
    fn a_module_path_offers_the_packages_then_walks_into_one() {
        ...
        let (folder, mut app) = a_rust_window("unluminate-import-path");
        ask_at(&mut app, "use |");
        let rows = offered(&app);
        assert!(rows.contains(&"crate".to_owned()), "{rows:?}");
        assert!(rows.contains(&"super".to_owned()), "{rows:?}");
        assert!(rows.contains(&"unluminate_core".to_owned()), "a folder named with a hyphen: {rows:?}");
        assert!(!rows.contains(&"unluminate-core".to_owned()), "{rows:?}");

        ask_at(&mut app, "use unluminate_core::|");
        assert_eq!(
            offered(&app),
        ...
        ask_at(&mut app, "use unluminate_core::completion::|");
        let rows = offered(&app);
    ...
    fn crate_and_super_are_read_from_where_the_file_is() {
        ...
        let (folder, mut app) = a_rust_window("unluminate-import-roots");
    ...
    fn typing_the_separator_opens_the_next_segments_list_in_the_same_frame() {
        ...
        let (folder, mut app) = a_rust_window("unluminate-import-separator");
        typing(&mut app, "use unluminate_core:");
        typing(&mut app, ":");
    ...
    fn an_ordinary_path_in_code_is_offered_the_ordinary_sources() {
        ...
        let (folder, mut app) = a_rust_window("unluminate-import-not-an-import");
        ask_at(&mut app, "let x = unluminate_core::comp|");
        let modules = app
            .completion()
    ...
    fn a_stylesheet_offers_the_projects_stylesheets_with_their_extensions() {
        ...
        let folder = std::env::temp_dir().join("unluminate-import-css");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
    ...
    fn a_frame_in_which_nothing_moved_recomputes_nothing() {
        ...
        let (folder, mut app) = a_window("unluminate-completion-idle");
        typing(&mut app, "dra");
