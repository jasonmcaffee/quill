...
//! away.
//!
//! `task-1696`. `unluminate_core::expressions` reads what the pointer is over, `app::debug::DebugState`
...
//! so somebody who finds the tooltip distracting can turn it down, and Unluminate's tick box already
...
/// only the *unasked* popup — `Debug -> Show Value` and `unluminate-cli debug hover` work either way,
...
impl UnluminateApp {
    ...
    ///
    /// One function, so the pointer, `Debug -> Show Value` and `unluminate-cli debug hover` cannot come to
    ...
    pub fn expression_at(&mut self, index: usize, offset: usize) -> Option<Range<usize>> {
        ...
        unluminate_core::expressions::at(&text, symbols, &grammar, offset)
    }
    ...
    fn word_box(&mut self, index: usize, range: &Range<usize>, origin: Pos2) -> Rect {
        ...
        let box_of = |caret: unluminate_core::layout::Caret, right: f32| {
            Rect::from_min_max(
