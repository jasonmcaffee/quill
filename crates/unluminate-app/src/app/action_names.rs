...
//! is the one thing a menu or a shortcut can ask for, so giving each one a name makes `unluminate-cli
//! action run <name>` a way in to everything the menus hold — including whatever is added to them
//! tomorrow, because `unluminate-cli action list` is built by walking the real menus rather than by
...
impl Action {
    ...
    pub fn name(&self) -> String {
        match self {
            ...
            // than from this list. `unluminate-cli action list` therefore offers them the day the manifest is
...
impl GitAction {
    ...
    /// Every entry, which is what `unluminate-cli git actions` lists.
