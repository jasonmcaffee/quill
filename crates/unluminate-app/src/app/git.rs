...
//! [`unluminate_git::Worker`]'s thread and the drawing is in `components::git_panel` and
...
use unluminate_git::worker::{Reply, Request, Snapshot};
use unluminate_git::{Commit, Repository, Worker};
...
impl GitState {
        ...
        for reply in self.worker.poll() {
            match reply {
                    ...
                    let changes: Vec<(usize, Change)> = changes
                        ...
                        .map(|(line, kind)| {
                            (
                                line,
                                match kind {
                                    unluminate_git::LineChange::Added => Change::Added,
                                    unluminate_git::LineChange::Modified => Change::Modified,
                                },
                            )
                        })
                        .collect();
                ...
                Reply::Done { label, outcome } => {
                    ...
                    // themselves better than anything Unluminate could say about them.
    ...
    pub fn state_of(&self, path: &Path) -> Option<unluminate_git::State> {
        let relative = self.relative(path)?;
        let entry = self.snapshot.status.entry(&relative)?;
        Some(if entry.index == unluminate_git::State::Unchanged { entry.worktree } else { entry.index })
    }
}

/// How the discovered repository root relates to the project Unluminate opened.
