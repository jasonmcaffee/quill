...
//! window out: a title bar Unluminate draws itself, which carries the menus, the project's name and the text
...
//!
//! Everything a menu or a keyboard shortcut can ask for is an `actions::Action`, and [`UnluminateApp::run_action`]
...
use unluminate_core::{layout, relayout, Command, Document, Highlights, Layout, Rgba};
...
/// How opaque the background is when Unluminate starts.
...
///
/// Two, because everything Unluminate used to ask about first was something git could not undo, and
/// `task-1681` added the one thing that is not: throwing a file away.
#[derive(Debug, Clone, PartialEq)]
pub enum Answer {
    /// Send this to the git thread.
    Git(unluminate_git::worker::Request),
...
///
/// The same shape as [`PlacedPicture`] and for the same reason: `unluminate_core::markdown` says which
...
/// `UnluminateApp::reveal_in_explorer` for why it is not one.
...
///
/// **Maximising is putting everything else away, and that is the whole of it.** Unluminate already has a way for
...
/// pointer is, and `UnluminateApp::settle_the_tab_drag` decides where it landed once every pane has said
...
/// [`UnluminateApp::settle_the_panel_drag`] decides once they have all been drawn.
#[derive(Debug, Clone, Copy, PartialEq)]
struct PanelDrag {
    panel: dock::Panel,
    at: Pos2,
...
/// `memory.focused().is_some()` and is true of **any** focusable widget. Every control Unluminate draws
...
/// explorer's cursor.
///
/// egui's own modal layer is what is asked, rather than a list of Unluminate's dialogs, so a modal added
...
/// The name of the widget that holds egui's keyboard focus while a pane of Unluminate's own is being typed
/// into.
pub const KEYBOARD_HOLDER: &str = "unluminate-keyboard-holder";

/// How long the window will sleep before it wakes itself up, whether or not anything has asked it to.
///
/// Unluminate draws only when something happens: a key press, the pointer, a program writing to a
...
/// Keep egui's keyboard focus on a widget of Unluminate's own, so that pressing `Tab` or an arrow key
/// cannot hand the keyboard to a button.
///
/// **This is what a report of Unluminate crashing while somebody typed turned out to be.** egui moves
/// keyboard focus when a bare `Tab` or a bare arrow key is pressed, and it moves it to the next
/// widget that can take focus — every control Unluminate draws with `Sense::click` can. The editing area,
...
/// `TextEdit` keeps `Tab` from moving the focus out of a text box. Unluminate has three surfaces that read
...
pub struct UnluminateApp {
    ...
    pub run: RunPanel,
    /// The project's run configurations, as `.unluminate/run-configurations.conf` holds them plus
    ...
    /// What was last written to the project's own `.unluminate` folder, so it is written again only when
    ...
    /// The project's breakpoints, as `.unluminate/breakpoints.conf` holds them. The authority for every
    ...
    ///
    /// See [`WATCH_INTERVAL`] and `UnluminateApp::notice_what_changed_on_disk`.
    ...
    /// was asked from.
    pub(crate) rename_kind: Option<unluminate_core::symbols::SymbolKind>,
    ...
    /// Unluminate lives where the text is — this is only the note that says which one to take back.
    ...
    /// released Unluminate started with `--control off`: a window with no channel is an ordinary window.
    ...
    /// rather than once a frame. See [`UnluminateApp::follow_the_execution_point`].
...
impl UnluminateApp {
    ...
    /// Open the command channel, so that `unluminate-cli` can drive this window.
    ///
    ...
    pub fn open_control_channel(&mut self, ctx: &egui::Context) {
        ...
        if let Some(server) = &self.control {
            self.message = Some(format!(
                "Unluminate {} \u{00B7} the command line is listening on port {}",
    ...
    /// machine with a different processor, for a reason that is not a fault in Unluminate. The screenshot tests
    ...
    pub fn prepare(&mut self, ctx: &egui::Context) {
        ...
        // starts from Unluminate's own rather than from whatever the last window on this thread was painted
        ...
        theme::activate(theme::Theme::unluminate_dark());
        theme::apply(ctx);
        self.themed = true;
        ...
        // the rail button, the menu and the Settings page a plugin adds — the drawing is Unluminate's own
    ...
    pub fn use_store(&mut self, store: Store) {
        ...
        // A plugin that will not parse is skipped and said out loud, rather than stopping Unluminate.
    ...
    /// must not read or write anything belonging to the person running it, and a `.unluminate` folder written
    ...
    fn project_state(&self) -> ProjectState {
        ...
        ProjectState {
            ...
            // chose — and this is written to the project's `.unluminate` every frame. Unluminate closed with the
    ...
    /// What the project's `.unluminate` would record about which panels are showing.
    ///
    ...
    /// True when this window is the one allowed to read and write the project's own `.unluminate` folder.
    ///
    ...
    pub fn run_action(&mut self, action: Action, ctx: &egui::Context) {
        match action {
            ...
            // is the same path `unluminate-cli plugin` takes. A button in the rail, an entry in the plugin's
            ...
            Action::OpenFile => {
                let start = self.tree.root().to_path_buf();
                // Every file is offered, not only Markdown and plain text, because Unluminate opens any file
                ...
                match arboard::Clipboard::new().and_then(|mut clipboard| clipboard.get_text()) {
                    Ok(text) if !text.is_empty() => {
                        if self.focus == Focus::Terminal {
                            ...
                            let bytes = unluminate_terminal::keys::paste(&text, mode);
                    ...
                    Ok(_) => {}
                    Err(problem) => eprintln!("Unluminate could not read the clipboard: {problem}"),
                }
            }
            Action::SelectAll => {
            ...
            // `unluminate-cli panel dock --position` is what says it in a script.
            ...
            Action::PasteInto(folder) => match self.clipboard.paste_into(&folder) {
                ...
                Err(problem) => self.message = Some(format!("Unluminate could not paste: {problem}")),
            },
    ...
    pub fn debug_a_configuration(&mut self, what: DebugAction) {
        match what {
            ...
            DebugAction::Resume => self.step(unluminate_dap::Step::Resume),
            DebugAction::StepOver => self.step(unluminate_dap::Step::Over),
            DebugAction::StepInto => self.step(unluminate_dap::Step::Into),
            DebugAction::StepOut => self.step(unluminate_dap::Step::Out),
            DebugAction::Pause => self.step(unluminate_dap::Step::Pause),
    ...
    fn install_an_adapter(&mut self, adapter: &str) {
        ...
        let Some(entry) = debuggers::find(&adapter) else {
            self.message = Some(format!("This version of Unluminate does not know a debugger called {adapter}."));
            return;
        };
        let command = entry.install_command();
        if command.is_empty() {
            self.message = Some(format!(
                "Unluminate has no way to install {adapter} here. {}. Set {} to it once you have one.",
                entry.comes_from,
    ...
    pub(crate) fn adapter_report(&mut self, adapter: &str) -> debuggers::Report {
        ...
        let Some(entry) = debuggers::find(adapter) else {
            return debuggers::Report {
                ...
                programs: Vec::new(),
                languages: Vec::new(),
                comes_from: "this version of Unluminate does not know it",
                install: String::new(),
    ...
    fn begin_a_build(
        ...
        std::thread::spawn(move || {
                ...
                .output()
            {
                ...
                // puts on standard error and is more use than anything Unluminate could write instead.
    ...
    fn step(&mut self, step: unluminate_dap::Step) {
    ...
    fn run_to_cursor(&mut self) {
        ...
        self.step(unluminate_dap::Step::Resume);
    }
    ...
    fn breakpoint_in_question(&self) -> Option<&unluminate_core::Breakpoint> {
    ...
    /// simply not sent to the adapter, because `enabled` is Unluminate's own idea and the protocol has no
    /// field for it.
    ...
    pub fn change_breakpoints(
        &mut self,
        path: &Path,
        change: impl FnOnce(&mut unluminate_core::Breakpoints),
    ) -> bool {
        if let Some(index) = self.files.index_of(path) {
    ...
    pub fn breakpoints_of(&self, path: &Path) -> unluminate_core::Breakpoints {
        if let Some(index) = self.files.index_of(path) {
    ...
    pub fn every_breakpoint(&self) -> Vec<(PathBuf, unluminate_core::Breakpoints)> {
        let mut files: Vec<(PathBuf, unluminate_core::Breakpoints)> = Vec::new();
    ...
    ///
    /// A disabled breakpoint is **not sent**: `enabled` is Unluminate's own idea and the protocol has no
    ...
    fn send_the_breakpoints_of(&mut self, path: &Path) {
        ...
        let lines: Vec<(usize, unluminate_dap::SourceBreakpoint)> = breakpoints
            .iter()
            ...
            .map(|breakpoint| {
                ...
                let carried = unluminate_dap::SourceBreakpoint {
                    line,
    ...
    /// Read `.unluminate/breakpoints.conf` again when something else has written it, and take what it says.
    ///
    ...
    fn adopt_the_breakpoints_from_disk(&mut self) -> bool {
        ...
        // that already exists. This is not Unluminate starting to watch files: it is reached only when
        // `.unluminate/breakpoints.conf` itself has been written by something else, which does not happen
    ...
    fn take_the_debug_replies(&mut self, ctx: &egui::Context) {
        ...
        for event in asked {
            match event {
                unluminate_dap::Event::RunInTerminal { seq, title, cwd, args, env } => {
                ...
                unluminate_dap::Event::StartDebugging { seq, request, configuration } => {
                    let opened = self
                        .debug
                        .as_mut()
    ...
    fn follow_the_execution_point(&mut self) {
        ...
        if !path.exists() {
            // A frame in a library Unluminate has no source for. The tile still lists it; there is simply
    ...
    fn show_the_plugin_panes(
        ...
        for slot_number in 0..self.plugin_ui.pane_count() {
            ...
            // The header, which is what every other panel in Unluminate has and what the board pane did not: it
            ...
            // **The colouring a plugin draws a fenced block or a SQL console with.** `unluminate-core` holds
            ...
            match problem {
                Some(problem) => {
                    ...
                    // because a blank pane is a pane somebody reports as a bug in Unluminate.
    ...
    fn act_on_a_plugin_request(
        ...
        match request {
            ...
            Request::RunCommand { id, command, arguments } => {
                let request = unluminate_cli::protocol::Request::new("", &command, arguments);
            ...
            // `services::launcher` is the one place Unluminate asks the operating system to open something.
            Request::Reveal(path) => {
    ...
    /// in the status bar rather than swallowed, which is what every honest miss in Unluminate does.
    ...
    pub fn open_the_plugin_tab(&mut self, tab: &str) {
        ...
        // in Unluminate is a `Document`, which is the picture precedent followed exactly.
        self.files.open_plugin_tab(
            unluminate_core::Document::new(),
            files::PluginTab {
                key: tab.to_owned(),
                plugin: surface.plugin.clone(),
    ...
    /// and `unluminate-cli plugin run` are one path. What it said goes in the status bar.
    pub fn run_plugin_command(
        ...
        // Unluminate's side of the plugin contract rather than Agent-Tasks's own code, and a manifest with a pane
        // in it is still a manifest Unluminate supports — `tasks/ui-plugin-architecture.md`.
        if answer.is_ok() {
            match command {
                "open-pane" => {
    ...
    ///
    /// What `Settings -> Plugins -> Reload` and `unluminate-cli plugins reload` both call. **No restart is
    ...
    pub fn refresh_the_plugins(&mut self) {
        ...
        // The same waker `unluminate_git::Worker` and the terminal tile use, so a plugin's terminal that prints
    ...
    /// came apart at all, so the question is asked in one place and every answer Unluminate gives about a
    ...
    fn run_grid_size(&self) -> unluminate_terminal::session::Size {
    ...
    pub fn new_detached_terminal_tab(&mut self, rows: usize, columns: usize) {
        let cell = self.renderer.cell_metrics(self.settings.terminal_font_size);
        let size =
            unluminate_terminal::session::Size::new(rows, columns).with_cell(cell.width, cell.height);
        self.terminal.visible = true;
    ...
    fn terminal_grid_size(&self) -> unluminate_terminal::session::Size {
        let cell = self.renderer.cell_metrics(self.settings.terminal_font_size);
    ...
    fn waker(&self) -> unluminate_terminal::session::Waker {
        match &self.context {
            Some(context) => {
    ...
    fn open_path_in_tab(&mut self, path: &Path, permanent: bool) -> Result<(), String> {
        ...
        match Document::open(path) {
            Ok(mut document) => {
                ...
                // outside Unluminate gives a dot on the wrong line — which the adapter's `verified`
    ...
    pub fn refresh_repository(&mut self) -> bool {
        let discovered = unluminate_git::Repository::discover(self.tree.root());
    ...
    /// mark the file as changed, for the same reasons setting the font does not: what Unluminate saves is
    ...
    fn colour_the_file(&mut self, index: usize) {
        ...
        let base = unluminate_core::Color::rgb(color::text().r(), color::text().g(), color::text().b());
    ...
    /// language.
    ///
    /// `unluminate-core` says where a `<style>` block is and what language it names and colours
    ...
    fn colour_the_embedded(
        &self,
        text: &str,
        embedded: &[unluminate_core::syntax::Embedded],
        spans: &mut Vec<(std::ops::Range<usize>, unluminate_core::Color)>,
        tokens: &mut unluminate_core::folding::Tokens,
    ) {
        for region in embedded {
            ...
            unluminate_core::syntax::scan(&text[region.range.clone()], &inside.grammar, |range, token| {
                let shifted = range.start + start..range.end + start;
                match token {
                    unluminate_core::Token::Comment => tokens.note(shifted.clone(), true),
                    unluminate_core::Token::String => tokens.note(shifted.clone(), false),
                    _ => {}
                }
                if token != unluminate_core::Token::Text {
    ...
    fn ask_git_about_the_open_file(&mut self) {
        ...
        if let Some(git) = self.git.as_mut() {
            if git.relative(&path).is_some() {
                git.send(unluminate_git::worker::Request::ChangedLines(path));
            }
        }
    }
    ...
    fn run_git(&mut self, what: GitAction) {
        use unluminate_git::worker::Request;
        ...
        match what {
            ...
            GitAction::Continue => {
                let request = match git.snapshot.in_progress {
                    Some("Rebasing") => Request::ResumeRebase(unluminate_git::Resume::Continue),
                    _ => Request::ResumeMerge(unluminate_git::Resume::Continue),
                };
                git.send(request);
            }
            GitAction::Abort => {
                let request = match git.snapshot.in_progress {
                    Some("Rebasing") => Request::ResumeRebase(unluminate_git::Resume::Abort),
                    _ => Request::ResumeMerge(unluminate_git::Resume::Abort),
                };
                git.send(request);
            }
            GitAction::Branches => {
    ...
    fn show_git_windows(&mut self, ctx: &egui::Context) -> Option<Action> {
        ...
        if let Some(push) = outcome.commit {
            ...
            git.panel.open = false;
            if push {
                let target = unluminate_git::PushTarget {
                    remote: git
                        .snapshot
                        .remotes
                        .first()
        ...
        if let Some((revision, mode)) = outcome.reset {
            ...
            if mode == unluminate_git::ResetMode::Hard {
                self.confirmation = Some(Confirmation {
    ...
    /// `unluminate-cli modal accept` presses the same button, and two arms that agreed today would be
    ...
    pub fn ask_before_deleting(&mut self, path: &Path) {
        ...
        // did is now misleading — and `unluminate-cli action run delete-path` answers with it, which
    ...
    pub fn delete_path(&mut self, path: &Path) {
        match recycle::delete(path) {
            ...
            Err(problem) => {
                self.message =
                    Some(format!("Unluminate could not delete {}: {problem}", path.display()))
            }
        }
    }
    ...
    pub fn move_path(&mut self, from: &Path, to: &Path, refactor: bool) -> bool {
        ...
        if let Some(folder) = to.parent() {
            if let Err(problem) = std::fs::create_dir_all(folder) {
                self.message = Some(format!("Unluminate could not make {}: {problem}", folder.display()));
                return false;
            }
        }
        if let Err(problem) = move_the_bytes(from, to) {
            self.message = Some(format!("Unluminate could not move {}: {problem}", from.display()));
            return false;
        }
    ...
    fn route_the_explorer_keys(&mut self, ui: &egui::Ui) -> Option<Action> {
        ...
        // the focus: `hold_the_keyboard` keeps the focus on a widget of Unluminate's own the rest of the
    ...
    ///
    /// Unluminate watches nothing and a tab is owned by its `Document`, which is the right rule while
    /// Unluminate is the only thing writing. It is the wrong answer the moment something else does: the tab
    ...
    pub fn reload_from_disk(&mut self, path: &Path, discard: bool) -> bool {
        ...
        match Document::open(path) {
            ...
            Err(problem) => {
                self.message =
                    Some(format!("Unluminate could not reload {}: {problem}", path.display()));
                false
            }
        }
    }
        ...
        match prompt.purpose {
            ...
            Purpose::NewFile(folder) => {
                ...
                match std::fs::write(&target, "") {
                    ...
                    Err(problem) => {
                        self.message =
                            Some(format!("Unluminate could not make {}: {problem}", target.display()))
                    }
                }
            }
            Purpose::NewFolder(folder) => {
            ...
            Purpose::NewBranch => self.send_git(unluminate_git::worker::Request::CreateBranch(name)),
            Purpose::NewTag => self.send_git(unluminate_git::worker::Request::Tag(name)),
            Purpose::Stash => self.send_git(unluminate_git::worker::Request::Stash {
                message: name,
                include_untracked: true,
            }),
            Purpose::Clone => {
                let parent = self.tree.root().to_path_buf();
                self.send_git(unluminate_git::worker::Request::Clone { parent, url: name });
            }
            Purpose::CompareWithRevision(path) => {
                self.send_git(unluminate_git::worker::Request::Diff {
                    path,
                    staged: false,
                    revision: Some(name),
                });
    ...
    fn send_git(&mut self, request: unluminate_git::worker::Request) {
        self.refresh_repository();
    ...
    /// editor puts a three-answer dialog here; Unluminate can give the simpler answer because it saves
    ...
    /// and `unluminate-cli tab close` all reach it — so it is one change in one function.
    ...
    fn save_before_closing(&mut self, index: usize) {
        ...
        match self.files.at_mut(index).document.save() {
            ...
            Err(problem) => {
                self.message =
                    Some(format!("Unluminate could not save {}: {problem}", path.display()))
            }
        }
    }

    /// Close a tab without writing it, which is what deleting its file means and what
    /// `unluminate-cli tab close --discard` asks for.
    ...
    fn save(&mut self) {
        ...
        // write nothing over the file. There is nothing in a picture Unluminate can change, so there is
        // nothing to save.
    ...
    fn write_settings(&mut self) {
        if let Some(store) = &self.store {
            ...
            // are written against its own name rather than being lost when Unluminate closes.
    ...
    /// `unluminate_core::Preview::source_lines`, which is what the two halves of the side by side view
    ...
    pub fn preview_style_at(&self, at: usize) -> unluminate_core::CharStyle {
        self.files
            .active()
            .cached
            .preview
    ...
    pub fn preview_panels(&self) -> Vec<unluminate_core::PreviewPanel> {
        self.files
            .active()
            .cached
            .preview
    ...
    ///
    /// The preview is produced by `unluminate_core::markdown`, which turns the source into the same three
    ...
    fn refresh_preview(&mut self, ctx: &egui::Context, width: f32) {
        ...
        let base = unluminate_core::CharStyle {
            ...
            color: unluminate_core::Color::rgb(color::text().r(), color::text().g(), color::text().b()),
            ..unluminate_core::CharStyle::default()
        };
        let colors = unluminate_core::PreviewColors {
            text: unluminate_core::Color::rgb(
                color::text_strong().r(),
                color::text_strong().g(),
                color::text_strong().b(),
            ),
            code: unluminate_core::Color::rgb(0x7E, 0xD3, 0x9B),
            link: unluminate_core::Color::rgb(color::accent().r(), color::accent().g(), color::accent().b()),
            quiet: unluminate_core::Color::rgb(
                color::text_dim().r(),
                color::text_dim().g(),
                color::text_dim().b(),
            ),
            rule: unluminate_core::Color::rgb(
                color::divider().r(),
                color::divider().g(),
                color::divider().b(),
            ),
        ...
        let mut preview = {
            ...
            let advance = unluminate_core::FontMetrics::advance(&self.renderer, "M", &code).max(1.0);
            let options = unluminate_core::PreviewOptions {
                base: base.clone(),
                colors,
                mono,
            ...
            unluminate_core::markdown::render(&self.document().text().to_string(), &options)
        };
        ...
        let file = self.files.active_mut();
        if rebuilt {
            file.preview_selection = unluminate_core::Selection::caret(0);
        } else {
    ...
    fn read_the_pictures(
        &mut self,
        ctx: &egui::Context,
        preview: &mut unluminate_core::Preview,
        width: f32,
    ) -> Vec<PlacedPicture> {
        let folder = self
            .document()
    ...
    ///
    /// Exactly the two passes the pictures take, and for the same reason: `unluminate_core::markdown`
    ...
    /// which is what `fit` means everywhere else in Unluminate.
    ///
    ...
    /// editor's. A diagram is the one thing in the window that is measured by `unluminate-core` and drawn
    ...
    fn diagram_style(&self) -> unluminate_core::CharStyle {
        unluminate_core::CharStyle {
    ...
    /// them here rather than in the settings dialog is what makes `unluminate-cli plugins disable` and
    ...
    pub fn set_plugin_enabled(&mut self, id: &str, on: bool) {
        ...
        // the window falls back to Unluminate's own in this frame rather than staying in colours nothing in the
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        // the reason rather than nothing, which is what every honest miss in Unluminate does.
        ...
        // other modal in Unluminate is drawn: `components::modal` places it, drags it and resizes it, and it has to
        ...
        if self.focus != self.last_focus {
            ...
            if let Some(session) = self.terminal.tabs.active() {
                if session.wants_focus_reports() {
                    session.send(unluminate_terminal::keys::focus(gained));
                }
            }
        ...
        let settings_outcome = settings_dialog::show(
            ...
            &unluminate_cli::mcp::install::unluminate_cli_program(),
            &on_disk,
            &icon_for,
            &page_names,
        ...
        // for why they exist at all, which is that Unluminate's window has no operating system frame.
        // Nothing is added at all while the window is maximised, which is Unluminate's rule for a
    ...
    ///
    /// Worked out here rather than in the widget for the reason every component in Unluminate decides
    ...
    /// come here, so none of them can leave the two halves — the palette Unluminate draws with and the copy
    ...
    /// so this falls back to Unluminate's own rather than leaving the window in a palette nothing can name.
    ...
    pub(crate) fn apply_the_theme(&mut self) {
        let wanted = match self.settings.theme.trim().is_empty() {
            true => theme::Theme::unluminate_dark(),
            false => self
                .plugins
                .theme(&self.settings.theme)
                .unwrap_or_else(theme::Theme::unluminate_dark),
        };
    ...
    /// next time Unluminate starts — which is what a person means by zooming an editor rather than
    ...
    /// left two of them in the old one until Unluminate was restarted, and left the Markdown preview in
    ...
    /// having unsaved changes, because what Unluminate saves is plain text and carries no formatting.
    ...
    ///
    /// The one place a panel moves, which is what the drag, the panel's own menu and `unluminate-cli
    ...
    /// Draw Unluminate's browser toolbar and reserve the rest of the pane for its native child view.
    ...
    /// Apply browser callbacks to ordinary tab state and open requested popup URLs as Unluminate tabs.
    ...
    fn browser_is_occluded(&self, ctx: &egui::Context) -> bool {
        ...
        // flyout in one answer; the fields after it are the surfaces Unluminate owns itself.
    ...
    /// through the text rather than through a proportion of the height. `unluminate_core::scroll_sync`
    ...
    /// scroll set by `unluminate-cli editor scroll` is applied before the frame draws anything, so the
    ...
    pub fn follow_the_other_half(&mut self, source_drives: bool, room: f32) {
        ...
        let (scroll, preview_scroll) = if source_drives {
            let to =
                unluminate_core::preview_y_for_source_y(source_page, preview_page, map, file.scroll);
            (file.scroll, to.clamp(0.0, (preview_page.height - room).max(0.0)))
        } else {
            let to = unluminate_core::source_y_for_preview_y(
                source_page,
                preview_page,
                map,
                file.preview_scroll,
    ...
    fn show_diagram(&mut self, ui: &mut egui::Ui, area: Rect) {
        if !self.mermaid_is_enabled() {
            let problem = unluminate_core::mermaid::Problem::whole(
    ...
    fn show_markdown_preview(&mut self, ui: &mut egui::Ui, area: Rect) {
        ...
        // taking a copy of what you are reading. See `UnluminateApp::reading_preview`.
    ...
    /// in Unluminate follows; the choice made here is that a press claims the copy for the preview
    ...
    pub fn select_the_whole_preview(&mut self) {
        let length = self
            ...
            .unwrap_or(0);
        self.files.active_mut().preview_selection = unluminate_core::Selection::new(0, length);
        self.reading_preview = true;
    }

    /// Draw a panel behind the code blocks, the tables and the front matter.
    ///
    /// `unluminate-core` says which paragraphs want a ground under them and this decides what one looks
    ...
    /// bar's drag, the resize grips, the platform's own snap, and `unluminate-cli window position`.
    ///
    ...
    ///
    /// `task-1693`: a file or a folder made by anything other than Unluminate — an agent with its own
    /// tools, a build, a command in the terminal tile — never appeared in the explorer, because the
    /// tree is only read when Unluminate is told to read it. The folders that are **showing** are asked,
    ...
    fn notice_what_changed_on_disk(&mut self) {
        ...
        // `git checkout` under a running window put `.unluminate/breakpoints.conf` back and the window
    ...
    ///
    /// Zero-based, which is what `unluminate-core` calls a source line everywhere and is one less than
    ...
    ///
    /// **DAP has no request for this; it is the client matching names**, and Unluminate already owns both
    ...
    /// debugger agreed to stop there**, which is the two-authority split §6.3 asks for: Unluminate draws
    ...
    fn show_editor(&mut self, ui: &mut egui::Ui, area: Rect, focused: bool) -> bool {
        ...
        // the caret somewhere without a click — a jump to a definition, a search hit, `unluminate-cli
...
/// component in Unluminate has — and the borrow checker will not hand out both halves of one `&mut self`
...
fn split_the_debug(app: &mut UnluminateApp) -> DebugSplit<'_> {
...
fn same_file(left: &Path, right: &Path) -> bool {
    ...
    // Through `paths::plain` first, because a path Unluminate wrote down may be **verbatim** —
    // `\\?\C:\jason\dev\unluminate` — while the one an adapter answers with never is. That module records
    ...
    let left = unluminate_terminal::paths::plain(left);
    let right = unluminate_terminal::paths::plain(right);
    if left == right {
        return true;
    }
    match cfg!(windows) {
...
fn line_number_in_file(path: &Path, offset: usize) -> usize {
    ...
    let Ok(text) = unluminate_core::document::read_to_normalised_string(path) else {
        return 1;
    };
    text.as_bytes()[..offset.min(text.len())].iter().filter(|byte| **byte == b'\n').count() + 1
}

fn git_colour(state: unluminate_git::State) -> Color32 {
    match state {
        unluminate_git::State::Untracked => color::git_untracked(),
        unluminate_git::State::Added | unluminate_git::State::Copied => color::git_added(),
        unluminate_git::State::Unmerged => color::close(),
        unluminate_git::State::Ignored | unluminate_git::State::Unchanged => color::text_faint().gamma_multiply(0.7),
        _ => color::git_modified(),
    }
}

impl eframe::App for UnluminateApp {
...
///
/// `unluminate-core` holds no plugin registry and must not learn about one, so it asks a question through
...
impl unluminate_core::CodeHighlighter for PluginHighlighter<'_> {
    fn colour(
        &self,
        language: &str,
        code: &str,
    ) -> Vec<(std::ops::Range<usize>, unluminate_core::Color)> {
        ...
        if !plugin.grammar.markup {
            return unluminate_core::syntax::highlight(code, &plugin.grammar)
                .into_iter()
        ...
        let mut spans: Vec<(std::ops::Range<usize>, unluminate_core::Color)> = Vec::new();
        let mut embedded: Vec<unluminate_core::syntax::Embedded> = Vec::new();
        unluminate_core::syntax::scan_with_embedded(code, &plugin.grammar, &mut embedded, |range, token| {
            if token != unluminate_core::Token::Text {
        ...
        for region in &embedded {
            ...
            let inside_theme = crate::services::plugins::scheme_of(inside);
            let start = region.range.start;
            unluminate_core::syntax::scan(&code[region.range.clone()], &inside.grammar, |range, token| {
                if token != unluminate_core::Token::Text {
                    if let Some(colour) = inside_theme.colour(token) {
