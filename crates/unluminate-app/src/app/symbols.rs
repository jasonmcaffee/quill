...
//!
//! `unluminate_core::symbols` says what a definition is; `services::symbol_index` says where the
...
//!
//! The index is a picture of the disk as it was when it was last read. A file changed outside Unluminate
...
//! confirmed to still be there, re-found by [`unluminate_core::symbols::file_definitions`] if it has
//! moved. `UnluminateApp::open_the_match` already re-checks a search hit the same way. A stale entry
...
use unluminate_core::symbols::{self, Confidence, FileSymbols, RankKey, Role, SymbolKind};
use unluminate_core::{Command, Grammar};
...
impl UnluminateApp {
    ...
    /// started lazily rather than in `UnluminateApp::new`, because a window that never asks a question
    ...
    /// `unluminate_core::symbols::rank`, which is where it can be tested with no window.
    fn rank_candidates(
        &self,
    ...
    /// Apply Unluminate's honest navigation rule to a ranked set of definition candidates.
    ...
    fn jump_to(&mut self, candidate: &Candidate, name: &str) {
        ...
        let Some(range) = range else {
            self.message = Some(format!(
                "'{name}' is no longer in {}, so Unluminate could not go to it.",
    ...
    /// comparison; then the file's definitions, in case an edit outside Unluminate moved it; and nothing
...
mod tests {
    ...
    /// by counting bytes.
    fn at(app: &UnluminateApp, needle: &str) -> usize {
    ...
    fn a_word_with_no_definition_says_so_rather_than_guessing() {
        ...
        app.document_mut().apply(unluminate_core::Command::PlaceCaret { offset: use_site, extend: false });
        app.document_mut().apply(unluminate_core::Command::Insert("nowhere_at_all;\n        ".to_owned()));
    ...
    fn two_files_defining_one_name_offer_both_rather_than_choosing() {
        ...
        let use_site = app.document().text().to_string().find("layout.draw()").expect("the call");
        app.document_mut()
            .apply(unluminate_core::Command::PlaceCaret { offset: use_site, extend: false });
        app.document_mut().apply(unluminate_core::Command::Insert("new();\n        ".to_owned()));
    ...
    fn an_open_tab_owns_its_definitions_and_the_index_does_not_answer_for_it() {
        ...
        app.document_mut().apply(unluminate_core::Command::ReplaceMany(vec![(range, "sketch".to_owned())]));
    ...
    fn a_closed_file_that_changed_since_the_search_is_skipped_whole_and_reported() {
        ...
        let (folder, mut app) = a_window("unluminate-symbols-changed-underneath");
    ...
    fn a_closed_files_stored_marks_are_shifted_by_the_rename_that_moved_its_bytes() {
        ...
        let (folder, mut app) = a_window("unluminate-symbols-rename-marks");
        ...
        app.marks.change(&caret, |marks| {
            marks.add(word..word + "paint".len(), unluminate_core::Rgba::parse("#ffff0080").expect("colour"));
        });
    ...
    fn a_comment_or_a_string_ticked_by_hand_is_applied_like_any_other_row() {
        ...
        let (folder, mut app) = a_window("unluminate-symbols-rename-a-comment");
        let caret = folder.join("caret.rs");
        let text = std::fs::read_to_string(&caret).expect("read");
    ...
    fn a_large_project_costs_definitions_rather_than_occurrences_and_still_cancels_at_once() {
        ...
        let folder = std::env::temp_dir().join("unluminate-symbols-large");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
    ...
    fn a_rename_with_no_files_in_it_changes_nothing_and_says_so() {
        ...
        let (folder, mut app) = a_window("unluminate-symbols-rename-nothing");
        let report = app.apply_rename(&RenameChange {
            from: "draw".to_owned(),
            to: "sketch".to_owned(),
            by_file: Vec::new(),
    ...
    fn what_a_build_wrote_is_never_indexed_or_searched() {
        ...
        let folder = a_project("unluminate-symbols-generated");
    ...
    fn the_index_is_read_again_when_a_file_is_saved_and_not_on_every_frame() {
        let (folder, mut app) = a_window("unluminate-symbols-staleness");
    ...
    fn the_command_line_can_open_a_definition_by_name_without_finding_an_occurrence_first() {
        let (folder, mut app) = a_window("unluminate-symbols-cli-name");
        let context = egui::Context::default();
