...
//!
//! `UnluminateApp::run_action` is the one place a menu entry turns into a change, and this is the same
//! rule for the command line: [`UnluminateApp::run_cli`] is the one place a request turns into a change.
...
//!
//! Nothing here decides what a command is called or what it takes — `unluminate_cli::catalogue` does,
...
use unluminate_cli::protocol::{code, Reply, Request};
...
use crate::app::{UnluminateApp, ViewMode};
use unluminate_core::symbols::Role;
...
pub enum Waiting {
    ...
    /// wait for the stop and read a variable — four commands, and an agent driving Unluminate can then
...
fn unknown_argument_refusal(request: &Request) -> Option<Outcome> {
    let command = unluminate_cli::catalogue::find(&request.command)?;
    let unknown = unluminate_cli::catalogue::unknown_arguments(command, &request.arguments);
    if unknown.is_empty() {
        return None;
    }
    let takes = unluminate_cli::catalogue::value_names(command);
    ...
    let hints = unknown
        .iter()
        .filter_map(|name| unluminate_cli::catalogue::argument_hint(command, name))
...
impl UnluminateApp {
        ...
        match waiting {
            ...
            Waiting::DebugEvaluate { expression, id, .. } => {
                ...
                Some(match answered {
                    ...
                    // expression far better than Unluminate could.
        ...
        let (command, message) = match waiting {
            ...
            Waiting::TerminalText { tab, needle, lines, .. } => {
                ...
                return Reply {
                    ...
                    error: Some(unluminate_cli::protocol::Failure {
    ...
    /// Run a whole command line, the way `unluminate-cli` would, and take the answer.
    ///
    /// `line` is what somebody types after the word `unluminate-cli`. It is parsed against the same
    /// catalogue the client parses against and run through the same [`Self::run_cli`], so a test
    /// driving Unluminate this way goes down the whole command line path apart from the socket — the
    ...
    pub fn run_command_line(&mut self, line: &str, ctx: &egui::Context) -> Option<Reply> {
        ...
        let typed = match unluminate_cli::parse::parse(&words) {
            Ok(typed) => typed,
    ...
    fn cli_panel(&mut self, request: &Request, verb: &str) -> Outcome {
        match verb {
            "list" => {
                // Unluminate's own four. A contributed pane is listed by `plugins list` and moved by
    ...
    /// The panel a `panel` command names: one of Unluminate's own four, or a contributed pane.
    ///
    ...
    fn cli_no_such_panel(&self, request: &Request) -> Outcome {
        ...
        no(
            request,
            code::NOT_FOUND,
            format!("There is no panel by that name. Unluminate has {}.", names.join(", ")),
        )
    }
    ...
    /// plugins are installed is not known until the manifests are read. `task-1794`: Unluminate wrote
    ...
    fn cli_fold(&mut self, request: &Request, verb: &str) -> Outcome {
        ...
        // A line the caller gave, turned into the paragraph number everything inside Unluminate counts in.
...
///
/// A shell has already done this by the time `unluminate-cli` sees its arguments. Anything driving
/// [`UnluminateApp::run_command_line`] has not had a shell, so it is done here — and it is the same two
...
impl UnluminateApp {
    ...
    fn cli_top(&mut self, request: &Request, verb: &str, ctx: &egui::Context) -> Outcome {
        match verb {
            ...
            "quit" => {
                ...
                done(request, "Unluminate is closing.")
            }
            _ => no(
                request,
                code::UNKNOWN_COMMAND,
    ...
    ///
    /// One command rather than eight, because the first thing anything driving Unluminate wants is to
...
impl UnluminateApp {
    ...
    /// `unluminate-cli browser ...` through the same host and commands as the menu and toolbar.
    ...
    /// `unluminate-cli tab move <position> [--tab] [--pane]` — what dragging a tab does.
    ///
    ...
    /// `unluminate-cli pane ...` — the editing area split into panes.
    ///
    ...
    fn cli_tab_save(&mut self, request: &Request) -> Outcome {
        ...
        match self.files.active().document.is_modified() {
            ...
            true => no(request, code::FAILED, "Unluminate could not write the file. The status bar says why."),
        }
    }
...
impl UnluminateApp {
    ...
    /// `unluminate-cli highlight ...` — the passages marked in the project's files.
    ///
    ...
    /// none of the files has to be opened first. [`UnluminateApp::change_highlights`] is where the choice
    ...
    fn highlight_text(&self, path: &Path) -> Result<unluminate_core::Rope, String> {
        if let Some(index) = self.files.index_of(path) {
            return Ok(self.files.at(index).document.text().clone());
        }
        std::fs::read_to_string(path)
            .map(|text| unluminate_core::Rope::from_str(&text.replace("\r\n", "\n")))
...
fn unknown(request: &Request) -> Outcome {
    no(
        ...
        code::UNKNOWN_COMMAND,
        format!(
            "There is no command called {}. `unluminate-cli commands` lists them.",
            request.command
        ),
    )
}
...
/// than being refused.
pub fn offset_at(text: &unluminate_core::Rope, line: usize, column: usize) -> usize {
...
impl UnluminateApp {
    ...
    /// `unluminate-cli editor scroll` — how far through the file the view is, and moving it.
    ///
    ...
    fn cli_editor_set_text(&mut self, request: &Request) -> Outcome {
        ...
        self.document_mut().apply(unluminate_core::Command::SelectAll);
        self.document_mut().apply(unluminate_core::Command::Insert(text.clone()));
        self.forget_layout();
        ok(
            request,
    ...
    fn cli_editor_select(&mut self, request: &Request) -> Outcome {
        if request.switch("all") {
            ...
            self.document_mut().apply(unluminate_core::Command::PlaceCaret { offset: head, extend: false });
        } else {
            let Some(from_line) = request.whole("from-line") else {
                return no(
                    request,
            ...
            let to_line = request.whole("to-line").unwrap_or(from_line);
            let to = offset_at(text, to_line, request.whole("to-column").unwrap_or(usize::MAX));
            self.document_mut().apply(unluminate_core::Command::PlaceCaret { offset: from, extend: false });
            self.document_mut().apply(unluminate_core::Command::PlaceCaret { offset: to, extend: true });
        }
        self.reveal_caret = true;
        let selection = self.document().selection();
    ...
    /// `unluminate-cli editor indent` — the agent's half of what `Tab` and `Space` do over a selection,
    ...
    fn cli_editor_indent(&mut self, request: &Request) -> Outcome {
        ...
        let unit = if request.switch("space") {
            unluminate_core::IndentUnit::Space
        } else {
            unluminate_core::IndentUnit::Tab
        };
        let selection = self.document().selection();
        let text = self.document().text();
        ...
        self.document_mut().apply(unluminate_core::Command::Indent { unit });
        self.forget_layout();
        self.reveal_caret = true;
        let selection = self.document().selection();
        let what = if unit == unluminate_core::IndentUnit::Space { "a space" } else { "a tab" };
        ok(
            request,
            format!("Indented {lines} line{} with {what}", if lines == 1 { "" } else { "s" }),
            json!({
                "lines": lines,
                "unit": if unit == unluminate_core::IndentUnit::Space { "space" } else { "tab" },
                "start": selection.start(),
                "end": selection.end(),
            }),
        )
    ...
    /// `unluminate-cli editor dedent` — the agent's half of what `Shift+Tab` and `Shift+Space` do over a
    ...
    fn cli_editor_dedent(&mut self, request: &Request) -> Outcome {
        ...
        let what = if unit == unluminate_core::IndentUnit::Space { "a space" } else { "a tab" };
        let changed = self.document_mut().apply(unluminate_core::Command::Dedent { unit });
        if !changed {
            return ok(
                request,
    ...
    fn cli_editor_history(&mut self, request: &Request, undo: bool) -> Outcome {
        ...
        let command =
            if undo { unluminate_core::Command::Undo } else { unluminate_core::Command::Redo };
    ...
    fn cli_editor_diagram(&mut self, request: &Request, ctx: &egui::Context) -> Outcome {
        ...
        let kind = unluminate_core::mermaid::kind(&source).map(|kind| kind.name().to_owned());
    ...
    /// `unluminate-cli editor definition [name]` — where a name is defined.
    ///
            ...
            .map(|candidate| {
                json!({
                    ...
                    "confidence": match candidate.confidence {
                        unluminate_core::symbols::Confidence::Sure => "sure",
                        unluminate_core::symbols::Confidence::Likely => "likely",
                    },
                    "open": candidate.open,
                })
            })
    ...
    /// `unluminate-cli editor complete` — the names the word being typed could become.
    ///
    ...
    fn cli_editor_complete(&mut self, request: &Request) -> Outcome {
        if !self.completion_applies_here() {
            return no(
                request,
                code::NOT_APPLICABLE,
                "No plugin claims this file, so Unluminate has no words to offer.",
            );
        }
    ...
    /// `unluminate-cli editor references` — every place a name is used.
    ///
            ...
            _ => {
                if !self.symbols_apply_here() {
                    return no(
                        request,
                        code::NOT_APPLICABLE,
                        "No plugin claims this file, so Unluminate cannot tell one of its words from another.",
                    );
                }
    ...
    /// `unluminate-cli editor rename` — the change set, and applying it.
    ///
    ...
    fn cli_editor_rename(&mut self, request: &Request) -> Outcome {
        ...
        if let Err(reason) = unluminate_core::symbols::check_name(to.trim(), &grammar) {
    ...
    fn cli_editor_preview(&mut self, request: &Request, ctx: &egui::Context) -> Outcome {
        ...
        let diagrams: Vec<Value> = self
            ...
            .map(|placed| {
                json!({
                    "paragraph": placed.paragraph,
                    "diagram": unluminate_core::mermaid::kind(&placed.source).map(|kind| kind.name()),
                    "width": placed.size.x,
                    "height": placed.size.y,
                    ...
                    .map(|panel| {
                        json!({
                            ...
                            "kind": match panel.kind {
                                unluminate_core::PanelKind::Code => "code",
                                unluminate_core::PanelKind::Table => "table",
                                unluminate_core::PanelKind::FrontMatter => "front-matter",
                            },
                        })
                    })
                    .collect::<Vec<Value>>(),
    ...
    /// `unluminate-cli editor preview-select` — reading and copying what the preview has selected.
    ///
    ...
    fn cli_editor_preview_select(&mut self, request: &Request, ctx: &egui::Context) -> Outcome {
        ...
        if request.switch("all") {
            ...
            self.files.active_mut().preview_selection = unluminate_core::Selection::caret(0);
            ...
            self.files.active_mut().preview_selection = unluminate_core::Selection::new(from, to);
    ...
    /// `unluminate-cli terminal rename [--tab <index>] <name>` — what the tab's own `Rename...` does.
    ...
    /// `unluminate-cli terminal move [--tab <index>] <position>` — what dragging a terminal tab does.
    ///
    /// It goes through `unluminate_terminal::Tabs::move_tab`, which is the same call the drag makes, so a
    /// rearrangement made from a script and one made with the pointer are the same rearrangement —
        ...
        if let Some(name) = request.text("key") {
            let Some(press) = key_named(&name) else {
                return no(
                    request,
                    code::USAGE,
                    format!("{name} is not a key this understands. `unluminate-cli commands \"terminal send\"` lists them."),
                );
            };
            match unluminate_terminal::keys::encode(press, mode) {
    ...
    /// `unluminate-cli run ...` — the whole of `task-1683` from the command line, which also makes every
    ...
    /// terminal still holds is [`unluminate_terminal::SCROLLBACK`] lines.
    ...
    /// that takes is a fact about the program rather than about Unluminate.
    ...
    fn cli_debug(&mut self, request: &Request, verb: &str) -> Outcome {
        match verb {
            ...
            "continue" => self.cli_debug_step(request, unluminate_dap::Step::Resume),
            "step-over" => self.cli_debug_step(request, unluminate_dap::Step::Over),
            "step-into" => self.cli_debug_step(request, unluminate_dap::Step::Into),
            "step-out" => self.cli_debug_step(request, unluminate_dap::Step::Out),
    ...
    /// `UnluminateApp::step`, which is the one place a stepping request turns into a change — so a thing
    /// done from a script and the same thing done from the tile are the same thing.
    fn cli_debug_step(&mut self, request: &Request, step: unluminate_dap::Step) -> Outcome {
            ...
            "add" | "remove" | "enable" | "disable" => {
                ...
                let changed = match action.as_str() {
                    "add" => self.change_breakpoints(&path, |breakpoints| {
                        breakpoints.set(unluminate_core::Breakpoint {
                            offset,
                            enabled: true,
                            condition,
                            log_message: log,
    ...
    /// string somebody typed; what this adds is `unluminate_core::expressions`' reading — an agent that
    ...
    /// carried whole, which is `unluminate-git`'s rule about git's standard error.
    ...
    /// install is a visible program that `run output` can read and `run stop` can stop. Unluminate itself
    ...
    fn cli_debug_install(&mut self, request: &Request) -> Outcome {
        ...
        let Some(entry) = debuggers::find(&name) else {
            ...
            return no(
                request,
                code::NOT_FOUND,
                format!("This version of Unluminate drives {}.", known.join(" and ")),
            );
        };
        ...
        if command.is_empty() {
            return no(
                ...
                code::NOT_APPLICABLE,
                format!(
                    "Unluminate has no way to install {name} here. {}. Set debug.{name} to it once you have one.",
                    entry.comes_from
                ),
            );
        }
    ...
    /// Serializes one stored DAP frame without changing which frames Unluminate retains for the UI.
    fn debug_frame_value(frame: &unluminate_dap::Frame) -> Value {
        json!({
            "name": frame.name,
            "path": frame.path,
            "line": frame.line,
        ...
        if let Some(above) = above {
            if let Err(problem) = std::fs::create_dir_all(above) {
                return no(
                    request,
                    code::REFUSED,
                    format!("Unluminate could not make {}: {problem}", above.display()),
                );
            }
        }
        if !directory {
            if let Err(problem) = std::fs::write(&path, "") {
                return no(
                    request,
                    code::REFUSED,
                    format!("Unluminate could not make {}: {problem}", path.display()),
                );
            }
        }
        self.tree.reload();
    ...
    /// the file it is asking about. Unluminate reloads the tree after each of its own file operations, and
    ...
    /// use.** Measured at 20 ms over Unluminate's own repository, which is what a read from a command line
    ...
    fn cli_modal_open(&mut self, request: &Request) -> Outcome {
        ...
        match name.as_str() {
            ...
            "about" => {
                ...
                ok(request, "About Unluminate is open", answer)
            }
    ...
    fn cli_modal_accept(&mut self, request: &Request, ctx: &egui::Context) -> Outcome {
        ...
        if self.about.take().is_some() {
            ...
            return done(request, "Closed About Unluminate.");
        }
    ...
    ///
    /// `pub(super)` because `run_action` is in `app` rather than here, and `About Unluminate` shuts the
    ...
    fn cli_settings(&mut self, request: &Request, verb: &str) -> Outcome {
        match verb {
            "list" => {
                ...
                // The static ones, then the contributed panes' — which Unluminate writes into its own
    ...
    /// because those two come apart in the two cases that matter: a port another Unluminate is holding,
            ...
            "status" => {
                ...
                ok(
                    ...
                    json!({
                        ...
                        "stdio": {
                            "command": unluminate_cli::mcp::install::unluminate_cli_program().to_string_lossy(),
                            "arguments": ["mcp", "serve"],
                        },
                    }),
                )
    ...
    fn cli_settings_reset(&mut self, request: &Request) -> Outcome {
        let fresh = crate::settings::Settings {
            // The family a fresh Unluminate has is decided by what this machine has installed, so it is
        ...
        match request.text("key") {
            Some(name) => {
                ...
                // number in Unluminate, which is what "what a new Unluminate has" means for a pane Unluminate did
            ...
            None => {
                ...
                self.unsaved_settings = true;
                ok(request, "Every setting is back to what a new Unluminate has.", self.settings_value())
            }
        }
    }
    ...
    fn apply_setting(&mut self, name: &str, value: &str) -> Result<(), String> {
        ...
        match name {
            ...
            "appearance.theme" => {
                ...
                // other. Empty is Unluminate's own, which is what the file says by saying nothing.
                if value.trim().is_empty() {
                    ...
                    settings.theme = match theme.key == crate::theme::Theme::unluminate_dark().key {
                        true => String::new(),
                        false => theme.key,
                    };
                }
            ...
            "appearance.icons" => {
                if value.trim().is_empty() {
                    ...
                    return Err(format!(
                        "There is no icon set called {value}. Unluminate draws {}, and empty follows the theme.",
        ...
        match verb {
            ...
            "show" => {
                ...
                let syntax = theme.syntax.as_ref().map(|scheme| {
                    ...
                    for token in unluminate_core::Token::ALL {
            ...
            "set" => {
                ...
                let mut settings = self.settings.clone();
                // Unluminate's own is remembered as an empty setting rather than as its key, so a settings file
                ...
                if let Some(icons) = request.text("icons") {
                    ...
                    if icons.eq_ignore_ascii_case("follow") {
                        ...
                        return no(
                            ...
                            code::USAGE,
                            format!(
                                "There is no icon set called {icons}. Unluminate draws {}, or `follow` for whichever the theme names.",
                                crate::services::plugins::ICON_SETS.join(" and ")
                            ),
                        );
                    }
            ...
            return no(
                ...
                format!(
                    "{name} opens the platform's file chooser, which nobody can click from a script. \
                     Use `unluminate-cli {instead}` instead."
                ),
            );
        }
        let path = self.cli_path_argument(request, "path");
...
const MODALS: &[(&str, &str)] = &[
    ...
    ("about", "Who wrote Unluminate, what version this is and when it was built."),
...
fn modal_id(name: &str) -> Option<&'static str> {
    Some(match name {
        "go-to-file" => "unluminate-go-to-file",
        "find-in-files" => "unluminate-find-in-files",
        "settings" => "unluminate-settings",
        "about" => "unluminate-about",
        "new-file" | "rename" | "prompt" => "unluminate-prompt",
        "confirmation" => "unluminate-confirmation",
        "commit" => "unluminate-commit",
        "git-dialog" => "unluminate-git-dialog",
        _ => return None,
    })
}
...
///
/// `task-1794`: Unluminate wrote `panes.agent-chat/chat.width` itself and then refused it back, because
...
/// Every setting, by the name it has in Unluminate's own settings file.
///
...
const SETTINGS: &[SettingKey] = &[
    ...
    SettingKey {
        ...
        accepts: "a theme's key or its name; `theme list` names them, and empty is Unluminate Dark",
    ...
    SettingKey {
        name: "mcp.enabled",
        accepts: "true or false",
        help: "Whether this Unluminate serves MCP over HTTP. An agent that launches the server itself needs neither this nor a port.",
    },
    SettingKey {
        name: "mcp.port",
    ...
    SettingKey {
        ...
        help: "Where the LLDB adapter lives, for Rust and native code. Empty means Unluminate looks for codelldb then lldb-dap on PATH. `tools/get-debug-adapter.ps1` fetches one and prints the line.",
    },
    SettingKey {
        name: "debug.node",
        accepts: "a path to js-debug's dapDebugServer.js",
        help: "Where js-debug lives, for JavaScript and TypeScript. There is no default: js-debug is a script rather than a program, so Unluminate has nothing to look for until it is told.",
    },
    SettingKey {
...
fn key_named(name: &str) -> Option<unluminate_terminal::keys::KeyPress> {
    use unluminate_terminal::keys::{Key, KeyPress, Modifiers};
...
mod tests {
    ...
    fn a_line_and_a_column_find_the_place_in_the_text() {
        let text = unluminate_core::Rope::from_str("one\ntwo\nthree\n");
    ...
    fn a_line_and_a_column_find_the_place_in_text_that_is_not_ascii() {
        let text = unluminate_core::Rope::from_str("héllo\nwörld\n");
