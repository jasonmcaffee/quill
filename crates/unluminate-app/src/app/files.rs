...
use unluminate_core::{Anchor, Document, Layout, Preview, Selection};
...
///
/// These were ten fields on `UnluminateApp` until `task-1664`, because there was one editing area and so
...
pub struct Cached {
    ...
    pub fold_tokens: Option<(u64, unluminate_core::folding::Tokens)>,
...
/// `key` is the `<plugin id>/<tab id>` the command line and the project's `.unluminate` folder name it by.
...
pub struct OpenFile {
    ...
    /// nothing — see `UnluminateApp::refresh_preview`.
    ...
    ///
    /// A tab is owned by its `Document` and Unluminate deliberately watches nothing, which is right until
    /// something else changes the file: the tab then goes on showing text that is no longer what is
    /// there, and `editor text` answers with it. `UnluminateApp::reread_if_the_file_changed` compares this
...
impl OpenFiles {
    ...
    /// into it.
    ///
    /// The tab **moves** rather than being copied, which is where Unluminate and the reference editor part company:
    ...
    /// does, and what `unluminate-cli tab move` asks for.
    ///
...
mod tests {
    ...
    fn document(name: &str) -> Document {
        ...
        let path = std::env::temp_dir().join("unluminate-open-files").join(name);
    ...
    fn a_tab_opened_to_keep_is_not_replaced_by_the_next_file() {
        ...
        let kept = files.index_of(&std::env::temp_dir().join("unluminate-open-files").join("kept.md"));
    ...
    fn a_picture_takes_a_tab_by_the_same_rules_as_a_file_of_text() {
        let mut files = OpenFiles::new(Document::new());
        let path = std::env::temp_dir().join("unluminate-open-files").join("photo.png");
        std::fs::create_dir_all(path.parent().expect("a parent")).expect("make the folder");
    ...
    fn a_tab_that_held_a_picture_and_is_reused_for_text_stops_being_a_picture() {
        let mut files = OpenFiles::new(Document::new());
        let path = std::env::temp_dir().join("unluminate-open-files").join("reused.png");
        std::fs::create_dir_all(path.parent().expect("a parent")).expect("make the folder");
        std::fs::write(&path, b"not really a png").expect("write it");
