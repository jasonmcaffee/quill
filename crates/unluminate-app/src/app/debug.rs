...
//!
//! Nothing here draws and nothing here speaks the protocol. The protocol is `unluminate_dap`, the drawing
//! is `components::debug_panel` and `components::gutter`, and this is what sits between them —
//! exactly the place `app::git::GitState` sits between `unluminate_git::Worker` and `components::git_panel`.
//!
//! **One session per window.** the reference editor runs several; the first version of this does not, and
//! everything is simpler for it: the state below is one `Option<DebugState>` on `UnluminateApp` rather
...
//! `unluminate_dap::Request::resumes` and acted on here. What is **not** thrown away is which rows were
...
//!
//! Unluminate draws the adapter's answer rather than its own hope. [`DebugState::answered`] holds, per
...
use unluminate_dap::messages::{Frame, OutputKind, Scope, SourceBreakpoint, Thread, Variable, VerifiedBreakpoint};
use unluminate_dap::session::{Event, Outcome, Session, State, Step};
use unluminate_dap::{AdapterCommand, Client, Reply};
...
/// The build runs **on a thread**, the way `unluminate-git` runs git, because a cold `cargo build` of this
/// repository is minutes and the window draws throughout it. The reply arrives on a channel and is
/// taken in `UnluminateApp::take_the_debug_replies`, beside every other thread's replies.
///
...
pub struct PendingBuild {
    ...
    /// What the tile says while it runs — `Building unluminate`.
    pub what: String,
...
pub enum Built {
    ...
    /// rule `unluminate-git` already follows about git's standard error.
    Failed(String),
...
pub struct HoverValue {
    ...
    pub id: u64,
    /// The expression as `unluminate_core::expressions::at` read it, which is also the root row's key.
    pub expression: String,
...
impl DebugState {
    ...
    ///
    /// The other half of [`unluminate_dap::Client::detached`], and it exists for the same reason
    ...
    pub fn feed(&mut self, message: unluminate_dap::Message) {
    ...
    /// itself.
    pub fn capabilities(&self) -> &unluminate_dap::Capabilities {
    ...
    fn send(&mut self, outcome: Outcome) -> bool {
        ...
        // The events an operation produces are about what Unluminate has just decided rather than about
    ...
    /// the third place deliberately, for the reason `unluminate_terminal::paths` gives about the verbatim
    ...
    pub fn set_breakpoints(&mut self, path: &Path, lines: Vec<(usize, SourceBreakpoint)>) {
        let path = unluminate_terminal::paths::native(&unluminate_terminal::paths::plain(path));
    ...
    /// real one answers `'total' is not a valid command`. Unluminate's box is called `Evaluate Expression`
    /// and the reference editor's evaluates an expression, so `watch` is the context that means what the control
    /// says. The distinction Unluminate keeps between the two is the reference editor's — persistent against one-off —
    /// which is Unluminate's own bookkeeping rather than anything on the wire.
    ...
    ///    Unluminate's own registry prefers, the first way alone would mean the commonest thing anybody
    ...
    /// is Unluminate's rule that a control which can never apply is absent. The children of a tooltip are
    ...
    fn absorb(&mut self, event: Event) {
        match event {
            Event::Ready => {
                ...
                // means. Unluminate holds no list of its own.
                self.filters = self
                    .session
                    .capabilities()
                    .exception_filters
            ...
            Event::Breakpoints { path, answered } => {
                ...
                // closes the half of that Unluminate controls, and a case difference is still the
                // adapter's own and still answered there.
                let path = unluminate_terminal::paths::native(&unluminate_terminal::paths::plain(Path::new(&path)));
...
mod tests {
    ...
    fn answer(request_seq: i64, command: &str, body: serde_json::Value) -> unluminate_dap::Message {
        unluminate_dap::Message::Response {
            seq: 500 + request_seq,
            request_seq,
    ...
    fn paused_state(capabilities: serde_json::Value) -> DebugState {
        ...
        state.requested();
        state.feed(unluminate_dap::Message::Initialized);
        let asked = state.requested();
        state.feed(answer(seq_of(&asked, "configurationDone"), "configurationDone", serde_json::Value::Null));
        state.feed(unluminate_dap::Message::Stopped(unluminate_dap::Stopped {
    ...
    /// And anything that is not a name the frame holds has no field at all, which is Unluminate's rule
    ...
    fn resuming_puts_the_tooltip_away() {
        ...
        state.step(unluminate_dap::Step::Over).expect("stopped, so it can step");
