...
//! the window stopped being a run of `let`s in `UnluminateApp::ui` and became a value. This module is that
...
//! is what Unluminate did before any of this: the terminal spans the whole width of the panes, including
...
/// How many slots [`Layout`] and [`Regions`] hold: Unluminate's own four, then the plugins'.
pub const SLOTS: usize = 4 + PLUGIN_PANES;

/// The panels that can be moved.
///
/// Unluminate's own four are variants, and the compiler names every place that has to answer for a fifth —
...
impl Panel {
    /// Unluminate's own four. A plugin's panes are not in it, because there is no compile time list of them;
    /// [`Panel::all`] is the one that includes them.
    pub const ALL: [Panel; 4] = [Panel::Explorer, Panel::Terminal, Panel::Run, Panel::Debug];

    /// Unluminate's own four, then `contributed` plugin panes.
    ///
    /// What the rail, the dock's menus and `unluminate-cli panels` walk. `contributed` comes from
...
impl Side {
    ...
    /// [`fill_the_depth`] and `UnluminateApp::move_a_divider_with_no_editor`.
...
impl Layout {
    /// The arrangement Unluminate has always had: the explorer down the left, the three tiles along the
    ...
    /// does not name — so a file written by an older Unluminate opens as the layout that Unluminate had.
...
/// - 120).max(MIN)` was written out three times in `UnluminateApp::ui` and is now written once.
...
pub struct Regions {
    ...
    /// showing. The first four are Unluminate's own and the rest are the plugin slots.
...
/// single-strip case come out at exactly the number `UnluminateApp::ui` used to clamp against: one strip
...
mod tests {
    ...
    fn all_showing() -> [bool; SLOTS] {
        // Unluminate's own four. A slot with no plugin in it is never showing, which is what makes the
    ...
    #[test]
    fn the_default_is_the_window_unluminate_has_always_had() {
    ...
    fn the_default_layout_is_the_arithmetic_the_window_used_to_do_inline() {
        ...
        // height, and the editing area filling the rest — which is what `UnluminateApp::ui` spelled out
    ...
    fn a_strip_asking_for_the_whole_window_leaves_the_document_its_minimum() {
        // The number `UnluminateApp::ui` used to clamp against, written once now.
        let mut sizes = Panes::new();
    ...
    #[test]
    fn a_settings_file_written_before_any_of_this_reads_as_the_window_unluminate_had() {
