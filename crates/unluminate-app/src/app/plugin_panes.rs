...
impl PluginUi {
    ...
    ///
    /// **A control that cannot apply is absent**, which is Unluminate's rule everywhere: the `F` button is not
    ...
    pub fn opened(
        ...
        let mut built = plugin_ui::provider(provider)
            .ok_or_else(|| format!("this version of Unluminate has no `{provider}` provider"))?;
        let context = Context {
    ...
    /// entry, a button inside a pane, or `unluminate-cli plugin run`.
    pub fn run(
        &mut self,
        plugin: &str,
        command: &str,
