...
//!
//! Usage: `unluminate [path] [--opacity N] [--view raw|side|preview] [--menu-bar native|in-window] [--terminal] [--control on|off]`
//!
...
//! folder that file is in. With no path at all it is the current directory — or, when Unluminate was started
//! from the desktop and the current directory is only wherever the shortcut points, the project that was
//! open last time. `unluminate_app::starting_folder` is that rule and says why it is drawn so narrowly.
//! Several Unluminates can run at once, each on its own project, which is what `File -> New Window` and
//! `File -> Recent Projects` start.
//!
//! `--control off` closes the command channel `unluminate-cli` drives the window down. It is open by
...
struct Arguments {
    ...
    terminal: bool,
    print_menus: bool,
    /// False when `--control off` was given, or `UNLUMINATE_CONTROL=off` is in the environment.
    control: bool,
}
...
fn parse_arguments() -> Arguments {
    ...
    let mut control = !matches!(
        std::env::var("UNLUMINATE_CONTROL").unwrap_or_default().trim(),
        "off" | "no" | "0" | "false"
    );
    ...
    while let Some(argument) = rest.next() {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Usage: unluminate [path] [--opacity N] [--view raw|side|preview] [--menu-bar native|in-window] [--terminal] [--control on|off] [--print-menus]"
                );
                ...
                println!("  --control WHICH on to let unluminate-cli drive this window, off to close the channel. On by default.");
...
fn print_menus() {
    use unluminate_app::app::actions::{self, Entry, MenuState};
    let state = MenuState {
        can_undo: true,
        can_redo: true,
        has_selection: true,
...
fn main() -> eframe::Result {
    ...
    //
    // The backtrace is asked for rather than left to `RUST_BACKTRACE`, because the person whose Unluminate
    ...
    if std::env::var_os("UNLUMINATE_PANIC_TEST").is_some() {
        panic!("UNLUMINATE_PANIC_TEST was set, so this is a panic on purpose");
    }

    // A file argument opens that file and shows the folder it sits in. A folder argument just shows the
    // folder. With no argument at all, the explorer shows the current directory — except when Unluminate was
    // started from the desktop rather than from a terminal, where the current directory is the folder
    // holding `unluminate.exe` and the project that was open last time is what was meant. Both rules live in
    ...
    // A project that already has a window is left alone. Two Unluminates on one folder would be two
    // processes writing one `.unluminate` folder, and whichever wrote last would win — which is the same
    // reason `OpenFiles::open` shows a file that is already open rather than opening it twice. The
    // instance files are how a running window says which project it is on; `unluminate-cli instances`
    // reads the same list.
    ...
    let (folder, file) = unluminate_app::resolve_target(arguments.path.as_deref(), &fallback);
    ...
    for other in &session {
        if *other != folder {
            unluminate_app::services::launcher::open_window(other);
        }
    }

    // Where this project's window was left, which `task-1693` asks for and which has to be known
    // before the window is built. It is read from the project's own `.unluminate` folder rather than from
    ...
    let options = eframe::NativeOptions {
        viewport: viewport
            ...
            // Unluminate draws its own title bar, because rounded corners and a translucent background need the
        ...
        // option "is only there so we can revert if we find any bugs". Unluminate runs the loop once and
    ...
    #[cfg(windows)]
    let options = unluminate_app::services::windows_transparency::with_direct_composition(options);
