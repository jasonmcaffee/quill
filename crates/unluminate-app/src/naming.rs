//! The rule that keeps the name of the tool Unluminate was measured against out of what ships.
//!
//! Unluminate's editing area, its tabs, its explorer menu, its debugger, its run configurations and the
...
//! Unluminate was designed: they are not shipped, are not reachable from the product, and rewriting a
...
mod tests {
    ...
    /// The workspace root: this crate is `crates/unluminate-app`, so it is two folders up.
    fn root() -> PathBuf {
    ...
    const SHIPPED: &[&str] = &["crates", "unluminate-cli", "documentation", "README.md"];
    ...
    /// Nothing that ships names the tool Unluminate was measured against.
    ///
    ...
    fn no_shipped_text_names_the_other_tool() {
        ...
        assert!(
            found.is_empty(),
            "the tool Unluminate was measured against is named in what ships. Say `the reference \
             editor` instead — `task-1795` and `unluminate_app::naming`:\n  {}",
            found.join("\n  "),
        );
    }
}
