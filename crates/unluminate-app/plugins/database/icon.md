...
The mark is a stack of three discs. That is the drum every database in every diagram is drawn as,
and it is the one mark somebody recognises before reading the name beside it. Three discs rather
than two, because two read as a coin at 32 by 32.
...
curl -s -X POST http://localhost:8091/image-creation/generateImageToProjectFile \
  ...
  -d '{
        "prompt": "A flat vector app icon of a database: three identical wide ellipse-topped cylinder discs stacked one directly above another, forming a classic database drum symbol. Bold even line weight, geometric, perfectly centred, generous margin, symmetrical. Bright cyan and white discs on a completely flat solid dark navy background. Minimal, crisp, high contrast, icon design, no text, no letters, no words, no gradients, no shadows, no perspective.",
        "negativePrompt": "text, letters, words, numbers, watermark, signature, photorealistic, 3d render, gradient, drop shadow, noise, texture, busy background, clutter, server rack, computer, person, face, coins, pancakes",
        "width": 1024,
        "height": 1024,
        "projectId": "unluminate",
        "relativePath": "_agent_output/task-1795-icons/database-source.png",
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1795-icons/database-source.png crates/unluminate-app/plugins/database
```
...
```bash
cargo run --example scale -- crates/unluminate-app/plugins/database/icon.png /tmp/icon-large.png 8
```
