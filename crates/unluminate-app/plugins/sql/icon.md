...
**It writes into the project called `unluminate`, which is the main checkout**, wherever this branch is
...
```bash
cargo run --example plugin_icon -- _agent_output/task-1777-database-plugin/icon-source.png crates/unluminate-app/plugins/sql
```
...
```bash
cargo run --example scale -- crates/unluminate-app/plugins/sql/icon.png _agent_output/task-1777-database-plugin/icon-large.png 8
```
