...
//! `api.anthropic.com` in every way that matters here: there is **no key for Quill to hold at all**,
//! the agent brings its own tools and its own permission model rather than being handed Quill's, it
...
impl Wire {
    ...
    /// catalogue from Quill, and its conversation is continued by its own session id rather than by
...
impl Provider {
    ...
    pub fn defaults() -> Vec<Provider> {
        vec![
            Provider {
                ...
                // tools, it reads the project's own `CLAUDE.md`, and Quill holds no key for it.
                wire: Wire::ClaudeCli,
    ...
    fn why_the_program_will_not_run(&self) -> Option<String> {
        ...
        match program(named) {
            Some(_) => None,
            None => Some(format!(
                "`{named}` is not installed, or is not on this window's PATH. Install it and start Quill again."
            )),
        }
    }
...
mod tests {
    ...
    fn the_two_rows_the_ticket_names_are_command_lines_and_neither_wants_a_key() {
        ...
        // the better half of the feature: there is nothing for Quill to store, read or redact.
