...
//! yields whole events, which is `quill_dap::codec`'s arrangement for `Content-Length` framing and
...
mod tests {
    ...
    fn a_stream_split_at_every_boundary_reads_the_same() {
        ...
        // could enumerate the ways it might. `quill_dap`'s framing has the same test.
