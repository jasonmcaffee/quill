...
use quill_chat::agent::Decoder;
use quill_chat::model::{Conversation, Message, Role};
use quill_chat::provider::Wire;
use quill_chat::session::{Session, State};
...
fn a_recorded_claude_tool_turn_shows_the_call_the_agent_ran_and_the_answer_it_got() {
    ...
    // from its own tool. Quill ran none of it.
    let session = session_from("claude-cli-tool.jsonl", Wire::ClaudeCli);
    assert!(matches!(session.state(), State::Finished { .. }), "{:?}", session.state());
    let calls: Vec<&quill_chat::ToolCall> = session
        .chat
        .messages
        .iter()
