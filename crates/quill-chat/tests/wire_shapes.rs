...
use quill_chat::model::{Conversation, Message, Role, ToolCall};
use quill_chat::provider::{Provider, Wire};
use quill_chat::sse;
use quill_chat::wire::{self, Decoder, Tool};
use quill_chat::Reply;
...
fn the_responses_request_is_a_list_of_items_rather_than_a_list_of_messages() {
    ...
    answering.tools.push(ToolCall::new("call_a", "quill_git", "{}"));
    chat.push(answering);
    let mut results = Message::new(3, Role::Tool);
    let mut answered = ToolCall::new("call_a", "quill_git", "{}");
    answered.answer = Some("clean".to_owned());
    results.tools.push(answered);
    chat.push(results);

    let body = wire::request(&provider(Wire::Responses), &chat, "You are in Quill.", &[], true);
    assert_eq!(body["model"], "gpt-5-codex");
    assert_eq!(body["instructions"], "You are in Quill.");
    ...
    assert_eq!(
        body["store"], false,
        "the transcript is Quill's rather than the server's"
    );
    ...
    assert_eq!(with_tools["tools"][0]["name"], "quill_git");
...
fn an_anthropic_thinking_block_goes_back_up_exactly_as_it_arrived() {
    ...
    answering.tools.push(ToolCall::new("t1", "quill_git", "{}"));
    chat.push(answering);
