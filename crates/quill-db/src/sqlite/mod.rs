...
impl Session {
    ...
    /// rather than a check in Quill — the same arrangement the PostgreSQL session makes with a
...
mod tests {
    ...
    fn a_database(name: &str) -> PathBuf {
        let folder = std::env::temp_dir().join(format!("quill-db-{}-{name}", std::process::id()));
    ...
    fn a_file_that_is_not_there_is_said_rather_than_created() {
        ...
        let missing = std::env::temp_dir().join("quill-db-no-such-file-ever.db");
    ...
    #[test]
    fn a_read_only_source_is_refused_by_sqlite_rather_than_by_quill() {
        // The guarantee is the engine's. Quill also hides the editing controls, but a statement that
