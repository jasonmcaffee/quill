...
//!
//! The sixth crate, and it is arranged the way `quill-dap` and `quill-chat` are: the wire, the values,
...
//! `quill-dap`'s scripted adapters with a socket instead of a pipe.
//!
...
//! connection on it. Quill has no runtime on purpose — `quill_git::Worker`, the terminal's reader,
//! `text_search`, `symbol_index`, `quill-dap` and `quill-chat` are all plain threads with channels,
//! and the workspace `Cargo.toml` says in as many words that "a runtime added for one pane would be a
//! second concurrency model in a program that has one". Shelling out to `psql` is not `quill-git`'s
