; The Windows installer for Unluminate.
;
...
; `tasks/unluminate-installer-tdd.md` records why this is Inno Setup and not WiX, NSIS or a program of our
; own, and why the defaults below are the defaults.
;
; The short version of the defaults: a plain double click installs into %LOCALAPPDATA%\Programs\Unluminate
; with no elevation prompt at all, and the dialog offers all users for anybody who wants Program
; Files. Unluminate offers itself for text and Markdown files without taking the default association for
; any of them. Uninstalling removes everything it wrote and leaves %APPDATA%\Unluminate — the settings,
; the pane sizes, the recent projects and any installed plugins — alone.

#define AppName        "Unluminate"
#define AppPublisher   "Jason McAffee"
#define AppUrl         "https://github.com/jasonmcaffee/unluminate"
#define ExeName        "unluminate.exe"
; The command line, installed beside the editor. `unluminate-cli` looks for `unluminate` next to itself, so
; being in the same folder is what makes `unluminate-cli launch` work with nothing configured, and the
; PATH task below puts both of them on the path together.
#define CliName        "unluminate-cli.exe"
#define ProgId         "Unluminate.Document"
...
AppId={{520016FC-ED64-4B3C-AAAC-D75ABC651C30}
AppName={#AppName}
...
; Unluminate is built for x64 and needs Windows 10 or later.
...
OutputDir={#OutputDir}
OutputBaseFilename=UnluminateSetup-{#AppVersion}-x64
Compression=lzma2/max
SolidCompression=yes
WizardStyle=modern
SetupIconFile=..\icon\unluminate.ico
...
Name: "addtopath"; Description: "Add Unluminate to the PATH, so that ""unluminate"" opens a folder and ""unluminate-cli"" drives it from a terminal"; GroupDescription: "Other:"
Name: "contextfile"; Description: "Add ""Open with Unluminate"" to the right click menu of a file"; GroupDescription: "Other:"
Name: "contextfolder"; Description: "Add ""Open with Unluminate"" to the right click menu of a folder"; GroupDescription: "Other:"
Name: "associate"; Description: "Offer Unluminate in ""Open with"" for text, Markdown and source files"; GroupDescription: "Other:"
...
[Registry]
; ---------------------------------------------------------------------------------------------
; Where Windows looks the program up by name, which is what makes Win+R "unluminate" work whether or not
...
; ---------------------------------------------------------------------------------------------
; "Open with Unluminate" on a file, on a folder, and on the empty space inside a folder. %1 is what was
...
; ---------------------------------------------------------------------------------------------
; The file kinds. Unluminate registers itself as an application that *can* open these and appears in Open
...
{ Whether Path already has Entry in it, compared without regard to case or to a trailing slash. The
  semicolons on both sides are what stop "C:\Unluminate" matching inside "C:\Unluminate Extras". }
...
begin
  ...
  // sizes, the recent projects and any installed plugins live in %APPDATA%\Unluminate and are
