; The Windows installer for Quill.
;
...
; `tasks/quill-installer-tdd.md` records why this is Inno Setup and not WiX, NSIS or a program of our
; own, and why the defaults below are the defaults.
;
; The short version of the defaults: a plain double click installs into %LOCALAPPDATA%\Programs\Quill
; with no elevation prompt at all, and the dialog offers all users for anybody who wants Program
; Files. Quill offers itself for text and Markdown files without taking the default association for
; any of them. Uninstalling removes everything it wrote and leaves %APPDATA%\Quill — the settings,
...
AppId={{07AEDC60-658D-4EE2-B2AD-5EE22A8017A3}
AppName={#AppName}
...
; Quill is built for x64 and needs Windows 10 or later.
ArchitecturesAllowed=x64compatible
ArchitecturesInstallIn64BitMode=x64compatible
MinVersion=10.0

; Upgrading while Quill is open asks to close it through the Restart Manager, rather than failing on
...
OutputDir={#OutputDir}
OutputBaseFilename=QuillSetup-{#AppVersion}-x64
Compression=lzma2/max
SolidCompression=yes
WizardStyle=modern
SetupIconFile=..\icon\quill.ico
...
[Registry]
; ---------------------------------------------------------------------------------------------
; Where Windows looks the program up by name, which is what makes Win+R "quill" work whether or not
...
; ---------------------------------------------------------------------------------------------
; "Open with Quill" on a file, on a folder, and on the empty space inside a folder. %1 is what was
...
; ---------------------------------------------------------------------------------------------
; The file kinds. Quill registers itself as an application that *can* open these and appears in Open
...
{ Whether Path already has Entry in it, compared without regard to case or to a trailing slash. The
  semicolons on both sides are what stop "C:\Quill" matching inside "C:\Quill Extras". }
...
begin
  ...
  // sizes, the recent projects and any installed plugins live in %APPDATA%\Quill and are
