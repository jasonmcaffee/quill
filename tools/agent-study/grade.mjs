...
// how much of the work went through Unluminous rather than round it, and every refusal.
//
...
let total = 0, viaUnluminous = 0, viaOwn = 0, refused = 0, scenarios = 0, bypassed = 0;
...
for (const f of fs.readdirSync(dir).filter(x => x.endsWith('.json') && !x.startsWith('_')).sort()) {
  ...
  const own = calls.filter(s => !/unluminous/i.test(s.tool ?? ''));
  scenarios += 1;
  total += calls.length;
  viaUnluminous += calls.length - own.length;
  viaOwn += own.length;
...
console.log(`  through Unluminous      ${viaUnluminous}  ${pct(viaUnluminous)}`);
console.log(`  the agent's own    ${viaOwn}  ${pct(viaOwn)}   <- the number to drive down`);
console.log(`refused calls        ${refused}`);
console.log(`scenarios that went round Unluminous   ${bypassed} of ${scenarios}`);
