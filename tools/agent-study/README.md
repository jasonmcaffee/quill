# Watching an agent drive Quill

Quill's contract is that everything a person can do, an agent can do too. The tests in the tree prove
...
Quill's own state back, never by believing what the agent said it did.
...
2. **A config that equips the Quill MCP**, pointed at by `OPENCODE_CONFIG`. Use a file of the study's
   ...
   ```json
   {
     ...
     "mcp": { "quill": { "type": "local", "enabled": true,
       "command": ["<repo>/target/release/quill-cli.exe", "mcp", "serve"] } }
   }
   ```

3. **A Quill window open on the sample project**, and `quill-cli` built:

   ```sh
   cargo build --release -p quill-cli -p quill-app
   node tools/agent-study/make-sample-project.mjs
   ./target/release/quill-cli.exe launch _agent_output/agent-study/scratch-project --no-wait
   ```
...
`before` and `after` are `quill-cli` commands run either side of the conversation. They are the point:
...
refused calls        9
scenarios that went round Quill   13 of 23
```

Two things to look at in the transcripts, both of which the first run found:

- **Every refusal.** Each one is an agent guessing a name Quill does not accept. The refusal messages
  are good and it self-corrects, but every one is a wasted round trip that will happen to every agent.
- **Every non-Quill tool call.** Each is a job Quill either cannot do, or can do and did not get
