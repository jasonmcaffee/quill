#!/usr/bin/env bash
# Puts Unluminous and the sample project back to a known state, so one scenario cannot contaminate
...
Q="${UNLUMINOUS_CLI:-$REPO/target/release/unluminous-cli.exe}"
...
git clean -fdq -e NOTES.txt -e .unluminous -e target >/dev/null 2>&1
