#!/usr/bin/env bash
# Puts Unluminate and the sample project back to a known state, so one scenario cannot contaminate
...
Q="${UNLUMINATE_CLI:-$REPO/target/release/unluminate-cli.exe}"
...
git clean -fdq -e NOTES.txt -e .unluminate -e target >/dev/null 2>&1
