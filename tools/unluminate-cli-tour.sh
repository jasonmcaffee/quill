#!/usr/bin/env bash
# Drive a real Unluminate window through every area of the command line, photographing it as it goes.
#
# This is the fourth layer of Unluminate's tests: the screenshot tests render offscreen, and only a real
# run shows that the real window really did what a command asked. Every step writes a PNG into
# `_agent_output/task-1661-unluminate-cli/shots`, and the reply to every command is appended to
# `transcript.txt` beside them, so the pictures and the answers can be read together.
#
#   pwsh/bash tools/unluminate-cli-tour.sh
#
# It leaves the window open at the end, on purpose: the last picture is of a window somebody can
# still look at. `unluminate-cli quit` closes it.

set -u

CLI=${UNLUMINATE_CLI:-./target/release/unluminate-cli.exe}
OUT=_agent_output/task-1661-unluminate-cli
SHOTS=$OUT/shots
mkdir -p "$SHOTS"
: >"$OUT/transcript.txt"
...
echo "Driving the Unluminate at:"
"$CLI" instances
...
say explorer-expand      explorer expand unluminate-cli/src
...
say tab-open-tdd         tab open tasks/unluminate-cli-tdd.md --permanent
say tab-open-source      tab open unluminate-cli/src/catalogue.rs --permanent
...
say modal-new-file       modal open new-file --path _agent_output/task-1661-unluminate-cli
