...
# elevation.
#
# Nothing here touches Quill's settings file. It prints the line to paste, because a script that
...
param(
    ...
    [string] $Into = (Join-Path $env:LOCALAPPDATA 'Quill\adapters'),
...
if ($Remove) {
    ...
    if ($PSCmdlet.ShouldProcess($folder, 'Remove')) {
        ...
        Write-Host 'Take the `debug.lldb` line out of the settings as well, or Quill will look for an adapter that has gone.'
    }
    return
}
...
if (Test-Path $adapter) {
    ...
    Write-Host ''
    Write-Host 'Put this in Quill''s settings file, under Edit -> Settings, or write it by hand:'
...
Write-Host 'Put this in Quill''s settings file:'
Write-Host "  debug.lldb = $adapter"
Write-Host ''
Write-Host 'The real-adapter test finds it through an environment variable of the same shape:'
Write-Host "  `$env:QUILL_LLDB_ADAPTER = '$adapter'; cargo test -p quill-app --test screenshots -- a_real_debugger"
Write-Host ''
