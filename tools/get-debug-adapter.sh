...
#
# **Quill itself fetches nothing** — that is the rule that keeps a document from making a network
...
# rather than as a program, so there is nothing for Quill to look for on PATH and nothing a package
...
#
# Nothing touches Quill's settings file. It prints the line to paste, because a script that edited
...
INTO=${QUILL_ADAPTERS:-$HOME/.local/share/quill/adapters}
...
fetch_node() {
  ...
  if [ "$REMOVE" = yes ]; then
    ...
    rm -rf "$folder"
    echo "Removed $folder."
    echo 'Take the debug.node line out of the settings as well, or Quill will look for an adapter that has gone.'
    return
  fi
  ...
  if [ -f "$adapter" ]; then
    ...
    echo
    echo "Put this in Quill's settings file, under Edit -> Settings, or write it by hand:"
  ...
  echo "Put this in Quill's settings file:"
  echo "  debug.node = $adapter"
  echo
  echo 'The real-adapter test finds it through an environment variable of the same shape:'
  echo "  QUILL_NODE_ADAPTER='$adapter' cargo test -p quill-app --test screenshots -- a_real_node_debugger"
  echo
...
fetch_lldb() {
  ...
  if [ "$REMOVE" = yes ]; then
    ...
    rm -rf "$folder"
    echo "Removed $folder."
    echo 'Take the debug.lldb line out of the settings as well, or Quill will look for an adapter that has gone.'
    return
  fi
  ...
  echo
  echo 'The real-adapter test finds it through an environment variable of the same shape:'
  echo "  QUILL_LLDB_ADAPTER='$adapter' cargo test -p quill-app --test screenshots -- a_real_debugger"
  echo
