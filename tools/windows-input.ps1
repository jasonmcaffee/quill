<#
.SYNOPSIS
  The one way an Unluminous script drives the real window with real keyboard and mouse input, written so
...
.DESCRIPTION
  ...
  drive Unluminous's real window — the screenshot walks, the wheel and pinch reproductions, the
...
Add-Type -Namespace Unluminous -Name Input -MemberDefinition @'
...
function Get-HeldInput {
    ...
    foreach ($name in $script:Modifiers.Keys) {
        if (([Unluminous.Input]::GetAsyncKeyState($script:Modifiers[$name].vk) -band 0x8000) -ne 0) { $held += $name }
    }
    foreach ($name in $script:MouseButtons.Keys) {
        if (([Unluminous.Input]::GetAsyncKeyState($script:MouseButtons[$name].vk) -band 0x8000) -ne 0) { $held += $name }
    }
    ,$held
}
...
function Clear-HeldInput {
    $windowsHeld = ([Unluminous.Input]::GetAsyncKeyState(0x5B) -band 0x8000) -ne 0 -or
                   ([Unluminous.Input]::GetAsyncKeyState(0x5C) -band 0x8000) -ne 0
    if ($windowsHeld) {
        [Unluminous.Input]::keybd_event(0x11, 0, $script:KeyDown, [UIntPtr]::Zero)
        [Unluminous.Input]::keybd_event(0x11, 0, $script:KeyUp,   [UIntPtr]::Zero)
    }
    foreach ($name in $script:Modifiers.Keys) {
        $key = $script:Modifiers[$name]
        $flags = $script:KeyUp
        if ($key.extended) { $flags = $flags -bor $script:Extended }
        [Unluminous.Input]::keybd_event([byte]$key.vk, 0, $flags, [UIntPtr]::Zero)
    }
    foreach ($name in $script:MouseButtons.Keys) {
        [Unluminous.Input]::mouse_event([uint32]$script:MouseButtons[$name].up, 0, 0, 0, [UIntPtr]::Zero)
    }
}
...
function Send-Key {
    ...
    try {
        [Unluminous.Input]::keybd_event([byte]$Vk, 0, $script:KeyDown, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds $HoldMs
    } finally {
        [Unluminous.Input]::keybd_event([byte]$Vk, 0, $script:KeyUp, [UIntPtr]::Zero)
    }
}
...
function Send-Wheel {
    ...
    Invoke-WithKeysHeld $Keys {
        ...
        for ($i = 1; $i -le $turns; $i++) {
            [Unluminous.Input]::mouse_event(0x0800, 0, 0, $step, [UIntPtr]::Zero)
...
function Invoke-Click {
    ...
    [void][Unluminous.Input]::SetCursorPos($X, $Y)
    Start-Sleep -Milliseconds 250
    for ($i = 0; $i -lt $Times; $i++) {
        try {
            [Unluminous.Input]::mouse_event([uint32]$down, 0, 0, 0, [UIntPtr]::Zero)
            Start-Sleep -Milliseconds 40
        } finally {
            [Unluminous.Input]::mouse_event([uint32]$up, 0, 0, 0, [UIntPtr]::Zero)
        }
...
function Invoke-Drag {
    ...
    [void][Unluminous.Input]::SetCursorPos($FromX, $FromY)
    Start-Sleep -Milliseconds 300
    try {
        [Unluminous.Input]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
        ...
        [Unluminous.Input]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    }
    Start-Sleep -Milliseconds 300
}
...
#
# `$UnluminousInputReadOnly` is for the one caller that wants to look without touching: `unstick-keyboard
# -Check` promises to change nothing, and a repair on load would make that a lie.
if (-not $UnluminousInputReadOnly) {
    Clear-HeldInput
