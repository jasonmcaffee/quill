...
Quill speaks the [Model Context Protocol](https://modelcontextprotocol.io), so an AI agent can
...
If you want to drive Quill yourself, read [commands.md](commands.md) instead. This is the same set of
commands with an agent's tool-calling machinery in front of it.

## The quickest way in

`Quill -> Edit -> Settings -> Tools -> MCP`, and press **Install for Claude Code** or **Install for
Codex**. Restart the agent. In Claude Code, `/mcp` will list Quill.

The same thing from a terminal:

```sh
quill-cli mcp install both
quill-cli mcp install claude --scope project   # this folder only, in .mcp.json
quill-cli mcp install codex --remove           # take it out again
```
...
it does whenever anybody runs the command, not something Quill asked for.

For a client with no button of its own, `quill-cli mcp config` prints the block to paste. It is the
same block the Settings page shows.

## What an agent is given

**The tools are generated from `catalogue.rs`** — the one list of commands that `quill-cli` parses
against and the window dispatches on. A command added to Quill is a tool the day it is added, and a
...
`quill-cli mcp tools --count` prints the real figures for both, against the catalogue as it is now:
...
- **A screenshot comes back as a picture.** `window screenshot` writes the PNG and the reply carries
  ...
  shapes.
- **The whole written reference as a resource**, at `quill://commands.md`. It costs nothing until it
  is read, and it cannot go stale: a test fails while it disagrees with the catalogue.

## Which window a call goes to

A project is a window — `File -> New Window` starts a second process — so several Quills may be
running. In order, first answer wins:

1. `instance` on the tool call, `--instance` on `mcp serve`, or `QUILL_INSTANCE` in the environment.
   A process id, a port, or any part of a project's path.
2. Exactly one Quill running: that one.
3. Exactly one running Quill on the folder the server prefers — `CLAUDE_PROJECT_DIR`, which Claude
...
running Quill host it. Either way the endpoint is `http://127.0.0.1:7345/mcp`.

- `POST /mcp` — a JSON-RPC request, answered `application/json`. A notification is `202` with no body.
- `GET /mcp` — `405`. There is no server-initiated stream; Quill has nothing to push.
- `DELETE /mcp` — `405`. There are no sessions.

One server drives every window, because it finds Quills through the instance files rather than
through its own process. So two Quills and one `mcp.port` is not a collision to fix: the second
...
The same names in `settings.conf`, on the Settings page, and in `quill-cli settings set`. A change
takes effect at once: the listener starts, stops or moves in the same frame.

`quill-cli mcp status --json` says what a window is actually doing, which is not always what the
settings say — the port may be held by another Quill, and a Quill started with `--control off` has no
command channel for a server to drive.

## Writing a client of your own

The MCP server is a client of Quill's own control channel, which is a smaller and older thing: a
