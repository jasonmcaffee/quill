//! Where a running Quill says how to reach it, and how the client finds one.
//!
//! A Quill that is listening writes one small file into `<settings folder>/instances`, named after
...
//!
//! Several Quills run at once — a project is a window, and `File -> New Window` starts a second
//! process — so a fixed port would be a fixed collision. Each window asks the operating system for
//! a free port, and the file is how the client is told which one. It is also how `quill-cli
//! instances` can list every running Quill without talking to any of them.
//!
//! ## Why a token
//!
...
//! ## Stale files
//!
//! A Quill that is killed rather than closed leaves its file behind. Nothing sweeps them on a
...
/// What one running Quill advertises.
...
///
/// Inside the folder Quill already keeps its settings in, so there is one place per person that
/// belongs to Quill rather than two. `QUILL_INSTANCES` names another folder, which is what the
/// tests use so that they never see the person's real windows.
pub fn folder() -> PathBuf {
    if let Some(named) = std::env::var_os("QUILL_INSTANCES") {
...
///
/// This is deliberately the same rule as `quill_app::services::store::settings_folder`, and there
/// is a test in `quill-app` that fails if the two ever come to disagree. It is written twice
...
///
/// Nothing is checked here beyond the file parsing. Whether the Quill it names is still running is
...
/// Remove an instance's file, which a Quill does as it stops and the client does when it finds one
...
mod tests {
    ...
    fn the_newest_instance_is_listed_first() {
        let folder = std::env::temp_dir().join("quill-cli-instances-order");
    ...
    fn a_file_that_is_not_an_instance_file_is_ignored() {
        let folder = std::env::temp_dir().join("quill-cli-instances-other");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
