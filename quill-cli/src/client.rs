//! Finding a Quill to talk to, and starting one.
//!
...
/// The Quills that are running, stale files swept as they are found.
///
...
///
/// With one running Quill, that one. With several and nothing named, a refusal that lists them:
...
pub fn choose(wanted: Option<&str>) -> Result<Instance, Unreachable> {
    ...
    let Some(wanted) = wanted else {
        return match alive.len() {
            0 => Err(Unreachable::new(
                code::NOT_RUNNING,
                "No Quill is running. Start one with `quill-cli launch <folder>`.",
            )),
            1 => Ok(alive.into_iter().next().expect("one")),
            _ => Err(Unreachable::new(
                code::SEVERAL,
                format!(
                    "{} Quills are running, so say which one with --instance:\n{}",
                    alive.len(),
                    alive
                        .iter()
                        .map(|instance| format!(
    ...
    match matched.len() {
        0 => Err(Unreachable::new(
            code::NOT_RUNNING,
            format!("No running Quill matches --instance {wanted}."),
        )),
        1 => Ok(matched.into_iter().next().expect("one")),
        _ => Err(Unreachable::new(
            code::SEVERAL,
            format!("--instance {wanted} matches {} running Quills.", matched.len()),
        )),
    }
}
...
pub fn ask(
    ...
    protocol::ask(instance.port, &request, timeout).map_err(|problem| {
            ...
            || problem.kind() == std::io::ErrorKind::TimedOut
        {
            Unreachable::new(
                code::TIMED_OUT,
                format!("Quill did not answer {command} within {} ms.", timeout.as_millis()),
            )
        } else {
            Unreachable::new(
                code::NOT_RUNNING,
                format!(
                    "Could not reach the Quill on port {}: {problem}. It may have just closed.",
                    instance.port
                ),
            )
        }
...
/// Where the `quill` program is.
///
/// Next to this one first, because an installed Quill puts them in the same folder and a build puts
/// them in the same `target` folder, so that answer is right without anything being configured.
/// `QUILL_BIN` overrides it, and the last resort is whatever is on the path.
pub fn quill_program() -> PathBuf {
    if let Some(named) = std::env::var_os("QUILL_BIN") {
        return PathBuf::from(named);
    }
    let name = if cfg!(windows) { "quill.exe" } else { "quill" };
...
/// Start a Quill on `folder`, and wait until it is answering.
///
/// Waiting is the point. A script that starts a window and sends it a command straight away used to
/// race the window's first frame; this returns when the new Quill has written its instance file and
...
pub fn launch(
    ...
    let child = std::process::Command::new(&program)
        ...
        .map_err(|problem| {
            Unreachable::new(
                code::FAILED,
                format!(
                    "Could not start {}: {problem}. Name it with QUILL_BIN if it is somewhere else.",
                    program.display()
                ),
            )
        })?;
    ...
    Err(Unreachable::new(
        code::TIMED_OUT,
        format!(
            "Quill was started but had not begun answering within {} ms. \
             It may still be loading fonts; try `quill-cli instances`.",
            timeout.as_millis()
        ),
    ))
}
...
mod tests {
    ...
    fn the_program_beside_this_one_is_preferred_and_can_be_named() {
        let previous = std::env::var_os("QUILL_BIN");
        std::env::set_var("QUILL_BIN", "/somewhere/else/quill");
        assert_eq!(quill_program(), PathBuf::from("/somewhere/else/quill"));
        match previous {
            Some(value) => std::env::set_var("QUILL_BIN", value),
            None => std::env::remove_var("QUILL_BIN"),
        }
    }

    #[test]
    fn no_running_quill_is_a_refusal_that_says_how_to_start_one() {
        let folder = std::env::temp_dir().join("quill-cli-choose-none");
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
        let previous = std::env::var_os("QUILL_INSTANCES");
        ...
        assert!(problem.message.contains("quill-cli launch"), "{}", problem.message);
        match previous {
            Some(value) => std::env::set_var("QUILL_INSTANCES", value),
            None => std::env::remove_var("QUILL_INSTANCES"),
        }
        std::fs::remove_dir_all(&folder).ok();
    }
}
