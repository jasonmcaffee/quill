//! What goes over the wire between the client and a running Quill.
//!
...
//! Python drive Quill, and `quill-cli` is the friendly face rather than the only way in.
//!
//! ```text
...
/// Something asked of a running Quill.
...
pub mod code {
    /// No Quill is running, or the one named is not answering.
    pub const NOT_RUNNING: &str = "not-running";
    /// Several Quills are running and none was named.
...
/// sends one command and stops, and a program driving Quill in a loop opening a socket per command
...
pub fn ask(port: u16, request: &Request, timeout: Duration) -> std::io::Result<Reply> {
    ...
    if line.trim().is_empty() {
        return Err(std::io::Error::new(
            ...
            "Quill closed the connection without answering",
        ));
    }
    let value: Value = serde_json::from_str(line.trim())
        .map_err(|problem| std::io::Error::new(std::io::ErrorKind::InvalidData, problem))?;
    Reply::from_json(&value).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::InvalidData, "Quill's answer was not a reply")
    })
}
