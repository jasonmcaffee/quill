...
//!
//! [`as_json`] is the same list as data, which is what `quill-cli commands --json` prints. That is
...
pub fn overall() -> String {
    ...
    out.push_str("quill-cli — drive a running Quill window from the command line.\n\n");
    out.push_str("Usage:\n  quill-cli [flags] <area> <verb> [arguments]\n\n");
    ...
    out.push_str(
        "\n`quill-cli commands --json` prints every command as data, which is what to give a\n\
         program or an agent. `quill-cli <area> <verb> --help` explains one command.\n\
         The written documentation is in quill-cli/docs/commands.md.\n",
    );
    out
}
...
pub fn as_json(only: Option<&str>) -> Value {
    ...
    json!({
        "usage": "quill-cli [flags] <area> <verb> [arguments]",
...
mod tests {
    ...
    fn a_commands_help_leads_with_its_examples() {
        ...
        assert!(help.contains("quill-cli terminal send git status"));
    }
