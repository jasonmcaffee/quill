...
//! every command in the catalogue must have a heading of its own in `quill-cli/docs/commands.md`,
...
fn every_command_has_a_section_in_the_written_reference() {
    ...
    assert!(
        missing.is_empty(),
        "quill-cli/docs/commands.md is missing {} command{}:\n{}\n\
...
fn every_commands_usage_line_is_written_out_where_it_is_documented() {
    ...
    assert!(
        wrong.is_empty(),
        "quill-cli/docs/commands.md does not carry the current usage line for:\n{}",
        wrong.join("\n")
    );
}
...
fn nothing_is_documented_that_no_longer_exists() {
    ...
    assert!(
        stale.is_empty(),
        "quill-cli/docs/commands.md documents commands that do not exist: {}",
        stale.join(", ")
    );
}
...
fn the_reference_leads_with_what_an_agent_needs_before_anything_else() {
    ...
    assert!(
        opening.contains("quill-cli commands"),
