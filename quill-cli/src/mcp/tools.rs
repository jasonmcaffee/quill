...
//! should have it and should pay the context for it. `quill-cli mcp tools --count` prints the real
...
fn area_description(area: &'static str, commands: &[&'static Command]) -> String {
    ...
    for command in commands {
        out.push_str("  ");
        out.push_str(command.usage().trim_start_matches("quill-cli "));
        out.push_str("\n      ");
...
///
/// `quill` for the commands with no area, `quill_<area>` for an area, `quill_<area>_<verb>` for one
...
pub fn tool_name(area: &str, verb: &str) -> String {
    let mut name = String::from("quill");
...
pub struct Call {
    ...
    /// Which Quill the caller asked for, if it named one.
    pub instance: Option<String>,
}

/// Turn `tools/call` into the request `quill-cli` would have sent.
///
...
mod tests {
    ...
    fn every_command_is_offered_as_a_tool_in_both_shapes() {
        // The promise of this module. A command added to Quill tomorrow is a tool tomorrow, and
    ...
    fn a_command_tool_carries_its_own_values_and_never_the_instance() {
        let call = resolve(
            Shape::Every,
            "quill_settings_set",
            json!({ "key": "appearance.font.size", "value": 20, "instance": "quill" })
                .as_object()
                .expect("an object"),
        )
        .expect("it resolves");
