// Measure how well a local model drives Quill when it is given only the CLI documentation.
//
//   node quill-cli/agent-assessment/run.js --url http://localhost:8087/v1/chat/completions \
//        --model qwen3.8-27b --out _agent_output/task-1661-quill-cli/assessment
//
...
//   4. runs them against the live Quill;
...
const DOCS = path.join(REPO, 'quill-cli', 'docs', 'commands.md');
...
function systemPrompt(documentation) {
  return `You drive the Quill text editor from a command line tool called quill-cli.

A person tells you what they want in plain English. You answer with the quill-cli command line
...
- Every command begins with the word quill-cli.
...
Here is the complete documentation for quill-cli.
...
function commandsIn(answer) {
  return answer
    ...
    .map((line) => line.replace(/^[-*]\s+/, ''))
    .filter((line) => line.startsWith('quill-cli'))
    .map((line) => line.slice('quill-cli'.length).trim());
}
...
/** Run quill-cli and return the parsed reply, whatever the exit code. */
...
function grade(task, lines) {
  ...
  if (planned.some((plan) => plan === null)) {
    ...
    return { pass: false, why: `line ${at + 1} is not a valid command: quill-cli ${lines[at]}` };
  }
  ...
  for (const line of lines) {
    ...
    if (code !== 0) {
      ...
      return { pass: false, why: `quill-cli ${line} failed: ${why}` };
    }
  }
...
function main() {
  ...
  const out = flag('out', path.join(REPO, '_agent_output', 'task-1661-quill-cli', 'assessment'));
  const only = flag('only', null);
  const folder = flag(
    'project',
    path.join(REPO, '_agent_output', 'task-1661-quill-cli', 'assessment-project'),
  );
  ...
  if (!rooted || !rooted.ok) {
    console.error('could not point the running Quill at the sample project; is one running?');
    process.exit(2);
  }
