...
`quill-db` connection the same questions, so a column typed `not null` in the tree is the column an
...
## Adding a data source, and what Quill will not do with a password
...
`Database` joins Quill's own six menus, after `Agent-Tasks`: showing the pane, opening the workspace,
adding a source, reloading the tree, and submitting whatever is pending — the same five things the
rail button, the toolbar and `quill-cli plugins run database …` all reach, because a menu entry in
Quill needs nothing else to be run from the command line.
...
Everything on this page has a command behind it: `quill-cli plugins run database <command>` —
...
`pending`, `submit`, `read-only`, `confirm` — and `quill-cli plugins view database` answers the whole
pane as data. `query` does not wait, for the reason nothing that runs inside a frame does; `state`
says when it has finished and `result` has the rows. `quill-cli/docs/commands.md` is the full
reference.

```sh
quill-cli plugins run database add-source library C:\path\to\library.db
quill-cli plugins run database connect library
quill-cli plugins run database open album
quill-cli plugins run database set 1 note "Warp Records"
quill-cli plugins run database pending --json
...
Real captures of `quill.exe`, driven the way `documentation/overview.md`'s own pictures were: the
...
than a fixed picture. So the state was built through `quill-cli`, the same command line
`quill-cli/docs/commands.md` documents, against a small SQLite fixture built for this page:
