...
Unluminous has a version — `0.1.0` in the workspace `Cargo.toml` — and no way for a person running it to
see what they are running. `Unluminous -> About Unluminous` writes one line into the status bar and nothing
...
goes up, the build date moves, Unluminous is reinstalled on the dev machine, and a GitHub release is cut
...
1. `Unluminous -> About Unluminous` opens a modal reading exactly:
   ```
...
3. The About modal is reachable, readable and closable from `unluminous-cli`, as every other modal is,
   and the build date is in `unluminous-cli status`.
4. `pwsh tools/release.ps1` takes a finished checkout to: a bumped version, a fresh build, the
   installer built, Unluminous reinstalled on this machine, a `v<version>` tag pushed, and a GitHub
   release with the installer attached.
5. `CLAUDE.md` carries the instruction, at the top, where the rest of the conventions are.

**Non-Goals**

- No update checking, no "a newer version is available", no telephoning home. Unluminous fetches nothing,
...
is not what a person looks for when they choose *About Unluminous* from a menu. Every other application
...
only way to get Unluminous onto another machine today is to build it there.
...
flowchart TD
    subgraph Build["cargo build"]
        CT[Cargo.toml<br/>workspace version] --> BRS[crates/unluminous-app/build.rs]
        CLOCK[the machine clock] --> BRS
        BRS -->|cargo:rustc-env=UNLUMINOUS_BUILD_DATE| BI[build_info.rs<br/>VERSION, BUILD_DATE]
    end
    ...
    subgraph Window["the window"]
        MENU[Unluminous -> About Unluminous] --> ACT[Action::About]
        ACT --> STATE[UnluminousApp::about: Option-About]
        STATE --> ABOUT
        ABOUT --> MODAL[components::modal::show]
    end

    CLI[unluminous-cli modal open about] --> STATE
...
`crates/unluminous-app/build.rs` already exists — it puts the icon and the Windows version block inside
`unluminous.exe`. It gains one job: work out the local date and time, and emit

```
cargo:rustc-env=UNLUMINOUS_BUILD_DATE=2026-08-25 10:45pm
```

**How the local time is read.** Unluminous has no dates library, deliberately: `unluminous_git::blame` computes
...
`unluminous-git` asks `git` rather than reimplementing it:
...
cargo:rerun-if-changed=../../unluminous-cli
cargo:rerun-if-env-changed=UNLUMINOUS_BUILD_DATE
```

so it reruns whenever any source in the workspace changed — which is exactly when `unluminous-app` was
...
the crate's fingerprint, `cargo test` after `cargo build` would recompile `unluminous-app` and relink
every screenshot test for no reason other than the clock. That is minutes a day paid for seconds of
precision.

`UNLUMINOUS_BUILD_DATE` is honoured if it is already set in the environment, which gives a reproducible
build an escape hatch and gives the release script a way to pin the stamp if it ever needs one.

### 2. `build_info`

A new module, `crates/unluminous-app/src/build_info.rs`, is the only place either fact is read:

```rust
pub const VERSION: &str = env!("CARGO_PKG_VERSION");
pub const BUILD_DATE: &str = env!("UNLUMINOUS_BUILD_DATE");
```
...
`About Unluminous`, three lines in the body, and a footer with one `Close` button.
...
`UnluminousApp` holds `pub about: Option<About>`. `Action::About` sets it to `About::current()` after
...
Unluminous` is a menu entry, so `unluminous-cli action run about` already works and needs nothing. What is
added is the modal being a first-class one, which is four small edits in `app/cli.rs`:

- `MODALS` gains `("about", "Who wrote Unluminous, what version this is and when it was built.")`
- `modal_id("about") -> "unluminous-about"`, so `modal move`, `modal size` and `modal reset` work on it
...
<name>` gains `about`, and `unluminous-cli/docs/commands.md` is regenerated with `cargo run -p unluminous-cli
...
3. **Build and install.** `installer/windows/build.ps1 -Install`. That already builds `unluminous.exe` and
   `unluminous-cli.exe`, refuses to package an executable with no version block, compiles the Inno Setup
   installer, closes a running Unluminous politely and installs with every optional task on. The rebuild
   is what moves the build stamp.
4. **Keep the installer.** Copy `installer/dist/UnluminousSetup-<v>-x64.exe` to `releases/`.
5. **Commit and tag.** `Cargo.toml` and `Cargo.lock` only, as `Unluminous <version>`; tag `v<version>`;
   push the branch and the tag.
6. **Publish.** `gh release create v<version> --title "Unluminous <version>" --notes <notes>` with the
   installer attached.
...
`unluminous-0.1.0.dmg` already committed is left exactly where it is — it is not mine to delete — and is
...
> `Cargo.toml`, rebuilds (which moves the build date the About box shows), reinstalls Unluminous on this
...
sequenceDiagram
    ...
    participant M as Unluminous menu
    participant A as UnluminousApp
    participant D as about_dialog
    P->>M: Unluminous -> About Unluminous
    M->>A: Action::About
...
only thing Unluminous wants out of a dates library, and the offset is the only thing arithmetic cannot
...
**Making About a page in the Settings window instead of its own modal.** Rejected — *About Unluminous* is
...
|---|---|---|
| `the_about_box_names_the_developer_the_version_and_the_build_date` | screenshot | opens `Unluminous -> About Unluminous`, finds the three named controls, snapshots with a fixed `About` |
...
installed `%LOCALAPPDATA%\Programs\Unluminous\unluminous.exe` reports the new version block, the running window's
About box shows `0.2.0` and today's date, `unluminous-cli status --json` carries the same two values, and
...
to `~/.claude/CLAUDE.md`). The plan put it in the Unluminous repository's own `CLAUDE.md`, which is right,
and which an agent working out of a terminal rooted at `ai-service` — where Unluminous's tickets are filed —
never reads. One short section carries the rule to wherever the work is being done, and points at the
Unluminous file for the rest.
...
| Installer | `UnluminousSetup-0.2.0-x64.exe`, 7.5 MB, compiled by Inno Setup |
| Installed | `%LOCALAPPDATA%\Programs\Unluminous\unluminous.exe`, version block reads `ProductName Unluminous`, `ProductVersion 0.2.0` |
| Tag and push | `v0.2.0`, and `main` at `3fb3c25` |
| Release | <https://github.com/jasonmcaffee/unluminous/releases/tag/v0.2.0>, asset 7,832,855 bytes, downloads anonymously with HTTP 200 |
| The window | About box on the **installed** binary reads `Developed by Jason McAffee` / `Version: 0.2.0` / `Build Date: 2026-08-25 9:17pm` |
| The command line | `unluminous-cli status --json` carries the same two values; `modal open about`, `modal state`, `modal move/size/reset`, `modal accept`, `modal cancel` and `action run about` all behave |
