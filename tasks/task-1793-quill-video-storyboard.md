# Quill — the product video

**task-1793.** A 4K demo of Quill for the Quill section of jasonmcaffee.com.

This is the script and the shot list. It is also the thing the recording harness reads: every beat
below names the exact `quill-cli` commands that produce it, so the video is *driven* rather than
...
3. **It is AI-first, and that is a definition rather than a slogan.** Everything a person can do in
   ...
   worked by agents in terminals Quill owns.
...
| Composition | full-desktop capture: wallpaper, then Quill inset **72 px** on every edge, at **0.83** background opacity |
...
Quill's own background rather than as the desktop behind it. 72 px on a 2160-tall frame is a hair
...
`_agent_output/task-1793-quill-video/fleet/`.
...
One project exercises **every** language plugin Quill ships (Rust, TypeScript, JavaScript, CSS, HTML,
...
Every beat is one recorded segment. **Every `quill-cli` line below is literal** and is what the
harness sends.
...
**On screen.** The wallpaper alone for 0.6 s. Quill fades up from nothing over 1.2 s, already open on
...
**Setup.**
```
quill-cli settings set appearance.background.opacity 0.83
quill-cli window position --x 72 --y 72
quill-cli window size --width 3696 --height 2016
quill-cli tab open src/telemetry.rs
...
**Script.**
```
quill-cli editor scroll --line 40           # slow, 8 lines a step
quill-cli explorer expand web
quill-cli editor scroll --line 1
```
...
**Script.**
```
quill-cli editor caret --line 63 --column 1
quill-cli editor insert "…"                 # one character per call, 55 ms apart
quill-cli editor complete --stem dur        # shown, then chosen
quill-cli editor complete --stem dur --choose 0
quill-cli tab save
```
...
**Script.**
```
quill-cli tab open src/report.rs
quill-cli editor definition --name Health
quill-cli editor navigate-back
quill-cli editor references --name Flight
quill-cli editor rename --name fleet_health --to fleet_summary
...
quill-cli editor view split
quill-cli editor scroll --line 20
quill-cli editor caret --line 8 --column 1
quill-cli editor insert "…"                 # typed, so the preview updates live
```
...
quill-cli browser open web/index.html
```
...
```
quill-cli run list
quill-cli run select "fleet report"
quill-cli run start
quill-cli run output
```
...
**Script.**
```
quill-cli debug breakpoint add src/report.rs 48
quill-cli debug start
quill-cli debug status
quill-cli debug frames
quill-cli debug variables --expand flights
quill-cli debug hover --name health
quill-cli debug watch add "flights.len()"
quill-cli debug step-over
quill-cli debug step-over
quill-cli debug evaluate "health.uptime_ratio()"
quill-cli debug continue
```
...
**Script.**
```
quill-cli panel dock terminal --side right
quill-cli panel size terminal --width 900
quill-cli panel dock terminal --side bottom
quill-cli pane split
quill-cli tab move --pane 1
quill-cli pane width --fraction 0.62
quill-cli panel zoom explorer --factor 1.15
quill-cli panel reset
```
...
**Script.**
```
quill-cli plugins pane database/explorer --side right
quill-cli plugins tab database/workspace --open
quill-cli plugins run database console --sql "select tail, model, hours from drone order by hours desc limit 12"
```
...
back to Quill Dark. Held on the last one for a beat.
...
**Script.**
```
quill-cli theme set dracula
quill-cli theme set palenight
quill-cli theme set deep-ocean
quill-cli theme set monokai-pro
quill-cli theme set one-dark
quill-cli theme set quill-dark
```
...
> They bring their own tools and their own keys. Quill holds none.

**Script.**
```
quill-cli plugins pane agent-chat/chat --side right
quill-cli plugins run agent-chat send "what does fleet_health do, and where does its data come from?"
quill-cli plugins run agent-chat state --wait
```
...
ticket opens as a modal. A card's play button starts an agent in a terminal Quill owns, and the
...
quill-cli plugins run agent-tasks open QUILL-4
```
...
wrong costs one beat. `_agent_output/task-1793-quill-video/segments/beat-NN.mp4`.

**Driving.** `scripts/drive.mjs` sends the `quill-cli` lines above with the pauses written beside
...
it all with a 2 s fade at each end, and one final scale-safe encode to `quill-demo-4k.mp4`.
...
A new **Quill** section on jasonmcaffee.com, between Apps and Articles: a short description of what
Quill is, the video as the section's centrepiece with a poster frame, and a link to the GitHub
...
did not were both defects in Quill rather than mistakes in the pacing, and both are worth writing
...
written to `.quill/breakpoints.conf` is 2037, which is line 50 in that file, byte for byte. A
...
**A second, separate cause with the identical symptom:** restoring `.quill/breakpoints.conf` from git
...
`restoreProject()` now restores `src` and `web` only, and the demo project stopped tracking `.quill/`.
...
- **Two Quills running makes a bare `quill-cli` ask which one is meant**, which is right and is also
