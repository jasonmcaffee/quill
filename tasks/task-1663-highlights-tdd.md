...
`task-1663` asks for something Quill has not had at all: **a way to mark a passage of text and have
...
Quill already has the two halves this needs and has never joined them. `quill-core` keeps
per-character formatting as spans that grow and shrink with every edit; `services::project_state`
keeps a `.quill` folder beside the project holding what was open. A highlight is a span that has to
...
| 6 | Looking up what is highlighted is cheap enough to do while painting every frame, on a project of any size Quill opens. |
| 7 | `quill-cli highlight` adds, lists and clears them, one at a time and in bulk across many files, and is documented in `quill-cli/docs/commands.md`. |
| 8 | All four test layers stay green, and the new drawing has screenshots a person has looked at. |

**Non-goals**

- **A highlight that follows the text through a change made outside Quill.** A highlight is anchored
  to a byte range. Editing the file *in Quill* moves it, because the editor knows what changed;
...
- **Highlights shared between two people.** `.quill/highlights.txt` is a file in the project as
  `.idea/` is, and whether it is committed is the project's business, not Quill's.
...
There is nowhere to put this. Quill has three things that already know something about a range of
...
alpha either, and its comment says so on purpose: *text in Quill is always fully opaque*. A highlight
...
does for formatting, and it is the reason the answer belongs in `quill-core` rather than in a service
...
```
quill-core                          quill-app                                quill-cli
----------                          ---------                                ---------
highlights::Highlights              services::file_marks::FileMarks          the `highlight`
  ...
  of (range, Rgba), shifted by        highlights, and .quill/highlights.txt    catalogue:
...
Quill that know a range of bytes moved, and they already shift `chars` and `paragraphs` there. A
...
editor's font already follows, for the same reason: what Quill saves is plain text, and a highlight
is not in it.

## Components and interfaces

### 1. `quill_core::highlights` — the set, and the arithmetic
...
the change to `quill-core` outside the new module.

### 3. `services::file_marks` — what Quill remembers about a file
...
**The format**, `.quill/highlights.txt`, in the plain text spirit of the two files beside it:

```
# The highlighted passages in this project. Written by Quill, and safe to delete.
...
`QuillApp::restore_project` and from nowhere else, exactly as the project state is, so a test neither
reads nor writes a `.quill` folder and a screenshot test's sample project is not changed underneath
...
Quill has had no menu on the editing area at all. It gets one, built from the same
...
two strips beside it; the ask says *a colour wheel*, and the style guide says Quill paints its own
...
`quill-cli action run` the day they exist:
...
`QuillApp::highlight_selection`, so there is still one place a highlight is made.
...
### 7. `quill-cli highlight` — one at a time, or a hundred at once

```
quill-cli highlight list [path] [--all]
quill-cli highlight add [path] [--from-line n] [--from-column n] [--to-line n] [--to-column n]
                               [--text <needle>] [--color <name|#rrggbbaa>]
quill-cli highlight clear [path] [--from-line n] [--to-line n] [--all]
quill-cli highlight apply [--from-file <path>] [--json-text <json>]
```
...
through its `Document`. One function, `QuillApp::change_highlights(path, f)`, is where that choice is
...
**A file that changed on disk.** Byte offsets against a file somebody rewrote outside Quill point at
...
**Two Quill windows on one project.** Each writes `.quill/highlights.txt` when its own set changes
...
| **One `.quill/highlights/<path>.txt` per file.** | Six hundred files to open when a project opens, a directory tree mirroring the project, and a rename that has to move a metadata file. One file, read once. |
| **A sqlite database in `.quill`.** | A dependency, a schema, a migration story and a binary file in a folder people put in git, in exchange for indexing a list measured in tens of entries. Everything else Quill remembers is plain text and this should be too. |
...
**Layer 1 — `quill-core`, no window.** The set: adding, cutting an overlap away, clearing, the binary
...
**Layer 2 — `quill-app`, no window.** `FileMarks` round trips through a temporary folder, writes its
paths relative to the project, skips a line it cannot read, and leaves a project with no `.quill`
...
`cargo run --release` on a three file project, driven by `quill-cli` from a terminal:
...
- `highlight apply --from-file marks.json --replace` marked three passages across three files in one
  request, none of which was open, and wrote `.quill/highlights.txt`.
...
- Quill was then **quit and started again**, and `highlight list --all` returned the same three marks
  in the same places. `live-restored.png` is the window after that restart. That is the only thing
  that proves the disk half, because no test is allowed to write a `.quill` folder.
