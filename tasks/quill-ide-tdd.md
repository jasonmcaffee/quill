# Quill as a code editor: line numbers, tabs, git and plugins

A technical design for the six things `task-1649` asks for. It records what was chosen, what was
considered and rejected, and where each piece belongs, so that the work reads like the rest of Quill
rather than like a second editor bolted to the side of it.

Quill today is a good editor for prose. It opens any file holding text, so a `.rs` or a `.ts` file
...
`app::actions::Action` with an arm in `QuillApp::run_action`. Git has twenty-odd entries and the
...
**Nothing in `quill-core` knows about a window.** Syntax highlighting produces spans over text, which
...
|---|---|---|
| `quill-core` | The editor: buffer, formatting, caret, layout, undo, the Markdown parser, **and the syntax tokeniser**. | Any user interface dependency. |
| `quill-terminal` | The terminal. | Any user interface dependency. |
| `quill-git` | **New.** Reading and changing a git repository. | Any user interface dependency, and any decision about what a dialog looks like. |
| `quill-app` | The window: drawing, input, fonts, settings, menus, **the plugin registry**. | Editor behaviour, terminal emulation, or git plumbing. |

`quill-git` is a crate rather than a `quill-app/services/git.rs` for one reason that matters: its
...
bar `quill-core` is held to. A module inside `quill-app` would drag `eframe` into every one of those
runs.
...
Quill already has a palette read out of `design/intial-design-screenshot.png` rather than chosen by
...
`Show Line Numbers`. The gap is where a folding arrow would go if Quill ever grows folding; leaving
...
Quill wraps. One paragraph is several `PlacedLine`s, and a numbered wrapped line would count the
...
rule, because Quill has no notion of a code file that is worth a setting: a `.txt` file with numbers
...
unsaved changes shows the amber dot Quill already uses for that, in place of the cross until the
pointer is over it.
...
Everything that is per document moves out of `QuillApp` and into the tab:
...
`QuillApp::document` becomes `QuillApp::files.active().document`. The laid out `Layout` stays on
`QuillApp` as a cache of the active file only, and switching tabs sets `layout_stale`, exactly as
...
| `Cut` | Puts the path on Quill's file clipboard, marked to move. |
| `Copy` | Puts the path on Quill's file clipboard, marked to copy. |
...
platform (`CF_HDROP` on Windows, `NSFilenamesPboardType` on macOS) and `arboard`, which Quill already
depends on, exposes neither. Copying a path *as text* does go to the system clipboard, because that
is just text. The consequence is stated plainly in the guide: you cannot cut a file in Quill and
...
is one text prompt in Quill rather than six.
...
running Quill has a git that has been configured: a credential helper holding their token, an ssh
agent, `commit.gpgsign`, `core.autocrlf`, hooks, `include` directives, per-repository identity,
`safe.directory`. `git push` from Quill must be the same push they get from their terminal, or the
...
- A **`Commit` / `Shelf` tab strip** at the top. Shelf shows the stash list, because a stash is what
  Quill has and a shelf is IntelliJ's own thing; the tab is named for what it does, `Stashes`.
...
the moment of commit — means Quill's idea of what is staged and git's disagree for as long as the
panel is open, and the person who runs `git status` in Quill's own terminal while the panel is open
...
files are marked in the explorer, and the Git menu grows `Continue` and `Abort`. Quill does not have a
three way merge editor, and this design does not add one — the conflicted file opens with its markers
in it, which is a file holding text and therefore something Quill already edits.
...
Git cannot be verified by looking at a screenshot. `crates/quill-git/tests/` builds real repositories
...
than on Quill's own idea of what happened. A second repository is created under `sample/git-demo/`
...
same flags — so every plugin would have to be rebuilt for every Quill release, and a plugin crash
...
`quill-core/src/syntax.rs`, next to the Markdown parser and for the same reason: it reads text and
...
and `Promise` as a type without Quill understanding a single thing about JavaScript, which is what
...
as modified, for exactly the reasons `Document::set_base_style` gives about the font: what Quill
...
**A theme colours the tokens and not the background.** Dracula's `#282A36` is not used, and Quill's
...
`Quill`, the second built from Quill's own palette for anyone who wants the two to match.

### The registry

`quill-app/src/services/plugins.rs`.
...
impl Plugins {
    pub fn load(store: &Store) -> Self;               // bundled, then %APPDATA%\Quill\plugins
...
fresh Quill has a marketplace with something in it and colours a `.rs` file the first time it opens
one. Pressing `Install` writes the folder to `%APPDATA%\Quill\plugins\<id>\` and reloads it **from
...
A plugin that will not parse is skipped, with its name and the reason in the status bar. Quill
starting with one plugin fewer is better than Quill refusing to start — the same rule `store.rs`
...
about it — the marketplace says these plugins ship with Quill.
...
into the Quill repository. They are then keyed to transparency and scaled to 32x32, which is the size
...
| Piece | Proved by |
|---|---|
| Syntax tokeniser | `quill-core` unit tests: keywords, a keyword inside a string, a keyword inside a comment, an unterminated string, a number, a function call, each of the three languages. |
| Git plumbing | `quill-git` tests against real repositories built in a temporary folder: status of a clean and a dirty tree, staging, committing, amending, a branch, a merge, a conflict, a rollback, a stash, a push to a second folder acting as a remote, blame over a file with two authors. |
| Plugin loading | `quill-app` unit tests: a manifest parses, a bad manifest is skipped rather than fatal, an extension resolves to the right plugin, a disabled plugin resolves to none, installing writes the folder and reloads from it. |
| Tabs, gutter, menus | `quill-app` unit tests for the state (which tab is transient, how wide the number column is, what the context menu holds for a folder against a file), and screenshot tests for the drawing. |
...
3. Tabs. This is the largest change to `QuillApp`'s state and everything after it wants to be built
   on top of tabs rather than retrofitted into them.
4. The context menu, the prompt and the confirmation, which git then reuses.
5. `quill-git`, tested on its own against real repositories, with no window involved.
6. The git window: menu, commit panel, dialogs, status bar, explorer tints, gutter change bars.
7. Blame in the gutter, which needs both the gutter and `quill-git`.
8. The tokeniser in `quill-core`, tested headless.
