# Quill CLI — technical design

`task-1661`

## Introduction

Quill can only be driven by hand. Everything it does — opening a file, starting a terminal, running a
...
This document designs `quill-cli`: a command line that drives a **running** Quill window, a control
channel for it to speak down, and a rule that keeps the two from drifting apart as Quill grows. The
...
| **G3** | Several Quill windows at once, each addressable, since a project is a window. |
| **G4** | An agent given only `quill-cli/docs/commands.md` can drive Quill. Measured, at **≥97%** on a scored assessment. |
...
| | |
|---|---|
| **N1** | Not a headless Quill. It drives a window that exists; it is not a way to edit files without one. `sed` already exists. |
...
Quill's architecture already has the right shape for this and does not use it. `app::actions::Action`
is the one thing a menu or a shortcut can ask for, and `QuillApp::run_action` is the one place an
...
- **An agent cannot use Quill at all.** This is the ticket's actual ask, and there is no partial
...
**One catalogue, two halves.** `quill-cli` is one crate with a library and a binary. The library holds
the command list, the wire format and instance discovery; `quill-app` depends on it. The client parses
against the same list the window dispatches on, so a command the CLI accepts is a command the window
knows. The dependency points one way — the client never depends on the window — so `quill-cli` stays a
small, fast program with no graphics card and no fonts behind it.

**The window answers, not the thread.** The listener never touches `QuillApp`. It queues the request,
...
| Where | What |
|---|---|
| `quill-cli/src/catalogue.rs` | Every command: area, verb, positional arguments, flags, summary, examples. The single list. |
| `quill-cli/src/parse.rs` | argv → a named argument map, against the catalogue. No argument-parsing library: the catalogue already says what each command takes, and a library would mean writing those facts down twice. |
| `quill-cli/src/protocol.rs` | `Request`, `Reply`, the error codes, and `ask()`. |
| `quill-cli/src/instances.rs` | The instance file: its format, where it lives, and reading the folder. |
| `quill-cli/src/client.rs` | Choosing an instance, sweeping stale files, launching a Quill and waiting for it. |
| `quill-cli/src/help.rs` | `--help` and `commands --json`, both printed from the catalogue. |
| `crates/quill-app/src/services/control.rs` | The listener thread, the token, the instance file, and `Pending`. |
| `crates/quill-app/src/app/cli.rs` | `run_cli`: what a command *means*. The only place a request turns into a change. |
| `crates/quill-app/src/app/action_names.rs` | A plain name for every `Action` and `GitAction`, and the test that every menu entry has one. |

### The command surface

`quill-cli <area> <verb>` — the noun first, as `docker container create` and `dotnet tool install`
...
parts of the window, so somebody who can see Quill can guess the area.
...
A Quill that is listening writes `<settings folder>/instances/<pid>.conf` in the same `name = value`
...
A file per window, because a project is a window and several Quills run at once — so there is no
...
|---|---|
| 0 | It worked |
| 1 | Quill refused it (`not-found`, `not-applicable`, `failed`) |
| 2 | The command line was wrong (`usage`, `unknown-command`) |
| 3 | No Quill running / unreachable (`not-running`, `refused`) |
| 4 | Several Quills, none named (`several-instances`) |
| 5 | Reached but too slow (`timed-out`) |

The split is what a script cares about: 2 is the caller's mistake, 1 is Quill's answer, 3–5 are about
...
| `git action --wait` | `quill_git::Worker` | git runs one command at a time on a thread. |
...
**It can be closed.** `quill --control off`, or `QUILL_CONTROL=off`. Open by default, because a
...
| The client depends on `quill-app` | Rejected: drags eframe, wgpu and fonts into a program that should start in milliseconds. |
...
| The client asks the window for the catalogue at startup | Rejected: a round trip before every command, and `--help` would need a running Quill. |
...
Quill's four layers, and where this work lands in each.

**1. Unit tests with no window** — `quill-cli` (43) and the naming rules in `quill-app`:
...
**3. Integration through the real window** (`crates/quill-app/tests/cli.rs`): build a `QuillApp`
...
**4. The real application.** A script that launches `quill.exe`, drives it with `quill-cli`, takes a
**screenshot after each step**, and leaves the pictures under `_agent_output/task-1661-quill-cli/` to
...
run against a live Quill and checked. The bar is **97%**, and the result — after the bar is reached —
goes in `quill-cli/agent-assessment/qwen-38-27B-assessment.md`.
...
| A test opens a port or writes an instance file into the person's settings. | `open_control_channel` is called from `main.rs` and nowhere else, exactly as `load_settings` and `restore_project` are. The control tests point `QUILL_INSTANCES` at a folder of their own and take turns under a mutex, because an instance file is named after the process id. |
| `quill-cli` cannot find `quill`. | It looks beside itself first — where both a build and an installation put them — then at `QUILL_BIN`, then the path, and says which it tried. |
