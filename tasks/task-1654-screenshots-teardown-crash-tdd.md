...
`cargo test --workspace --release` in `C:\jason\dev\unluminous` fails about two runs in five with
...
- Patching `wgpu`, `egui_kittest` or the graphics driver. The change lives in Unluminous's own test setup.
...
Nothing in Unluminous needs that. Every one of the 91 tests wants the same thing: a device to draw
...
### The change, in `crates/unluminous-app/tests/screenshots.rs`
...
that is not a defect in Unluminous.
