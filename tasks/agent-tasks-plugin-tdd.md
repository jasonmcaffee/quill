# agent-tasks-plugin — the task board as an Unluminous plugin
...
The board, its tickets, its terminals and its agents, drawn in `egui` inside Unluminous, over a single
...
most of the time, and the failure looks like a bug in Unluminous. A plugin that needs PostgreSQL, Node and a
daemon is not a plugin somebody installs. And Unluminous already owns real pseudoterminals with a real
emulator in `unluminous-terminal`, so going through a WebSocket to a daemon that spawns `node-pty` would be
...
`unluminous-terminal` already has everything the board needs and it is already tested with no window: a
...
So a ticket's terminal is a `unluminous_terminal::Session` owned by the plugin, and the pane draws it with
the same painter the terminal tile uses. There is no daemon, no WebSocket, and no second emulator.

The cost is the one thing the daemon bought: **a session does not survive Unluminous closing.** The daemon
...
The layout is one crate module with one file per part of the window, which is `unluminous-app`'s own rule.
...
are tested with no window, which is the rule the crate table in `CLAUDE.md` sets for `unluminous-core`,
`unluminous-terminal` and `unluminous-git`. The four files under `components/` draw and decide nothing.

## 4. The database, and the setting that names it

One file, in the folder Unluminous already keeps its settings in.

```
macOS    ~/Library/Application Support/Unluminous/plugins/agent-tasks/board.sqlite3
Windows  %APPDATA%\Unluminous\plugins\agent-tasks\board.sqlite3
Linux    ~/.config/Unluminous/plugins/agent-tasks/board.sqlite3
```
...
file was last written by, so a file from a newer Unluminous is refused with a message rather than opened and
half understood.
...
The description is markdown, and Unluminous already reads and draws markdown. So the description is a
`unluminous_core::Document` and the preview is the one `components::editor_view` draws, which means the
...
`agent.rs` builds the same command lines §1.3 records, from the same three registries the rest of Unluminous
uses for this kind of decision: which agent programs exist is a list in Unluminous, which one a ticket uses
is data on the row, and where it is found on this machine is `services::debuggers`' pattern applied to
agents.

```rust
/// The agents this version of Unluminous can launch. Checked the way `plugins::DEBUGGERS` is checked.
pub const AGENTS: &[&str] = &["claude", "codex"];
```

Launching one is `unluminous_terminal::Session::spawn` in the ticket's project directory, then waiting until
...
**The session id is a UUID Unluminous chooses for Claude, and Codex does not take one.** Claude accepts
...
### 5.4 Resuming a session after Unluminous has closed
...
session started in one Unluminous is resumed by another Unluminous window, because the id is in the database rather
than in the window.

**For Codex it is refused, and the refusal is the honest answer.** Codex names its own sessions, so the id
on the ticket is Unluminous's marker rather than a conversation Codex has heard of. Pressing Start begins a new
...
Reading JIRA is HTTP and Unluminous has no HTTP client, so this is the one part of the plugin that adds a
dependency for a network call: `ureq`, blocking, on the worker thread pattern `unluminous_git::Worker`
...
| A lane's background | `EXPLORER`, which is the panel ground every list in Unluminous sits on |
| A card | `CONTROL`, with `CONTROL_BORDER` round it and the epic's colour down its left edge |
| A card under the pointer | `SELECTED_ROW`, which is the pill every list in Unluminous draws for its chosen row |
...
Every control on the board is reachable from `unluminous-cli plugin run agent-tasks <command>`, and
...
Unluminous's rule is that everything a person can do an agent can do too, through the same code. So each of
...
in the same places Unluminous's are, and the count below is what the design commits to.
...
`cargo run --release -p unluminous-app --example board_cost` is what measures all of it, on a board of 5000
...
phone, and giving it one would mean putting a web server inside Unluminous.
...
**Dictation.** The description editor can dictate through a local speech service the API proxies. Unluminous
...
uploads folder and rewrites the markdown to point at a route that reads it back. Unluminous draws pictures
