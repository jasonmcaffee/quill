...
Two things a person does every day in an editor, and Quill answered both the way a text field
...
- The same two edits are reachable from the command line as `quill-cli editor indent`, with and
  without `--space`, and documented in `quill-cli/docs/commands.md`.
...
- **An indent setting.** Quill has no tab width and no "insert spaces for tabs" preference, and the
  ticket asks for a tab from `Tab` and a space from `Space`, which is the one answer that needs no
  setting. A document Quill saves is plain text, and what the key says is what goes in.
...
- **Block (column) selections.** Quill has no rectangular selection, so there is nothing to answer
  here.
...
a tab from `Tab`, a space from `Space` — which is also what Quill needs no setting to answer.
...
behaves. Quill has no such setting and no reason to grow one for this: the ticket names the two
characters, and a literal tab and a literal space are the only answers that cannot be wrong on a
machine that has not told Quill anything about its house style.

## 5. Design

### 5.1 The command

`quill_core::document` gains one command and one small type:
...
`quill-cli editor indent` is the agent's half, and it is a row in the catalogue like every other
...
`Command` the keys do, in `QuillApp::run_cli`'s one place a command turns into a change, so an
...
- **`quill-core`, no window.** `Indent` over a multi-line selection puts a character at the start of
...
`tools/release.ps1` and the installed window is driven with `quill-cli` — a selection made, `Tab`
