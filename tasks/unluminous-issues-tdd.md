# unluminous-issues — the reported faults, their causes, and what each change is
...
is actually happening in the code, what changes, and what proves it. Unluminous's own rule from
...
and every file under `crates/unluminous-app/src/services/agent_tasks/`,
`crates/unluminous-app/src/components/agent_tasks/` and `crates/unluminous-app/plugins/agent-tasks/` is
...
> When I try to expand a folder with a lot of files, like when unluminous is at /, and I expand dev, unluminous
...
`file_kind::openable` on every file so the row can be drawn dimmed when Unluminous cannot open it.
...
carries on every platform Unluminous runs on, so `Refusal::TooLarge` is still decided without a second
`stat`.
...
The pane on the right of the explorer that holds all the tabs is what Unluminous calls the **editing
area** — `dock::Regions::editor`, with one `components::file_tabs` strip per pane inside it. The
ticket's "file pane" and Unluminous's "editing area" are the same thing, and the name in the window follows
Unluminous, because `panel list`, `dock` and a dozen accepted screenshots already use it.
...
- `Action::ToggleEditor`, on the `View` menu as `Hide Editor` and `Show Editor`, next to
  ...
  to build `unluminous-cli action list`, so this is one entry rather than an entry and a catalogue row.
...
- `UnluminousApp::ui` draws no tab strip and no editing pane when the editing area is hidden.
...
reload. And `unluminous-cli action list` offers `Hide Editor`, which is the agent's way in.
...
a general capability of Unluminous's plugin system, described by `tasks/ui-plugin-architecture.md`, and no
...
`crates/unluminous-app/tests/snapshots/agent_tasks_pane.png` stops being written by any test. It is left on
...
**Which models exist is a list in Unluminous, and it is per agent.** A new `agent::MODELS` answers, keyed by
...
board written by a newer Unluminous, or by hand, must not silently lose a model — so the list the dropdown
...
The board file on this machine proves it. `~/Library/Application Support/Unluminous/plugins/agent-tasks/board.sqlite3`
...
- `check_version` stops being the only thing that writes the version. It refuses a file from a newer
  Unluminous as it does now, and then, **after** `migrate` has run, the version the file is at is written
...
> The Unluminous -> Settings -> Plugins -> Agent-Tasks has a scroll bar but doesn't scroll.
...
four are Unluminous's plumbing described to the person using it. What the connection actually is, is a URL
and a key.
...
**The key is taken from the environment when the keychain has none.** Unluminous launched from a terminal
inherits `ANTHROPIC_API_KEY` from `~/.zshrc`; Unluminous launched from the Dock does not. So
...
from zshrc" means in practice, and it is why a person who launches Unluminous from a terminal has nothing
to type at all.
...
`crates/unluminous-app/plugins/` holds agent-tasks, css, html, javascript, mermaid, rust and typescript.
Markdown is built into `unluminous-core` — `unluminous_core::markdown::render` is what the editor's own preview
...
`unluminous_core::markdown::render` for the source, `unluminous_core::layout` for the lines, and
...
diagrams. `UnluminousApp::refresh_preview` resolves those in two extra passes that decode an image and lay
...
table into a layout with the right number of lines, which is `unluminous_core::markdown`'s own test shape.
...
run tile draw, so it has Unluminous's keyboard, selection, mouse reports, resize and clipboard rules. That
...
happened against Unluminous's own state read back, rather than against what the agent said it did.

**Where it lives.** `crates/unluminous-app/tests/agent_board.rs`, a test file of its own, because it starts
...
1. **A ticket is claimed and worked.** Create a board in a temporary folder, create a ticket whose
   ...
   folder with a given line in it — and start the agent. Then wait, with a ceiling, on Unluminous's own
...
**What they do not do.** They do not assert on the agent's wording. Every assertion is a state Unluminous
...
`~/.claude.json` on their behalf is Unluminous silently granting an agent the run of a folder. What the board
...
Code session, so the agent Unluminous spawned was that session's grandchild. An Unluminous started from the Dock or a
...
```
UNLUMINOUS_TEST_PROJECT=<a folder you have already opened Claude Code in> \
...
Every test in `crates/unluminous-app/tests/screenshots.rs` fails in this session with
`Failed to create render state: RequestDeviceError(RequestDeviceError { inner: Core(Device(Lost)) })`,
including tests this work never touched. The machine's graphics card is fine and Unluminous itself is running
...
```
cargo test -p unluminous-app --test screenshots
```
...
An attempt to run the screenshot tests inside the running Unluminous's own terminal, through the MCP control
channel, found that Unluminous did not answer at all: `terminal list` timed out with
