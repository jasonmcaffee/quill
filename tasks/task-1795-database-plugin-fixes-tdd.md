...
plugin at all — it is in every field in Quill — so it is the first section here.

## 1. A field only takes a click in the middle of it, and that is two of the reported faults

`controls::field_text_rect` is the function every field in Quill hands its rectangle to, and it was
...
Applied to every input in Quill: `modal::field`, `controls::search_field_over`, the console's SQL
...
- **Every user-visible string**: `plugins/database/plugin.conf`'s `description` and `limitations`,
  ...
  `quill-cli`'s own documentation for the database commands.
- **Every source comment** in `crates/quill-db`, `crates/quill-app/src/services/database`,
  `crates/quill-app/src/components/database` and `quill-cli`. A comment that names it is a comment
...
`cargo run --example plugin_icon -- <source> crates/quill-app/plugins/<id>`, which keys the flat
...
`quill-database-<source name>`, and the settings file records `password.keychain = <entry>` — the
...
**Windows has a credential store, and Quill can now write to it.** `services::agent_tasks::keychain`
...
given, that it *"cannot be tested from this machine"*, has not been true since Quill grew a Windows
build. It is `CredWriteW`, `CredReadW` and `CredDeleteW` in `Win32::Security::Credentials`, which is
one feature flag on the `windows-sys` dependency Quill already has — the precedent `services::recycle`
...
- **`NULL`** is what an empty box means only when the cell was NULL before; otherwise an empty box is
  ...
  empty box means, and `quill_db::Value` keeps them apart all the way from the wire.
...
`quill_db::sql::create_table` and redrawn as the form is typed into — the rule Preview already keeps,
...
`crates/quill-app/tests/screenshots.rs`, except where it says otherwise.
