...
- No change to how Quill's own two-way focus works — `Focus::Editor` and `Focus::Terminal` stay as
...
Its only guard is `has_focus`, which `app::QuillApp::show_editor` computes as
...
The same is true of the menu shortcut watcher in `QuillApp::show`, which walks `input.events` looking
...
flowchart TD
    ...
    RAW --> SC[Menu shortcut watcher in QuillApp show]
    ...
    Q2 -->|no| DOC[quill_core Command on the document]
...
`memory.focused().is_some()`, which is true of **any** focusable widget, and every control Quill
...
for focus when clicked, and Quill has no `DragValue`.
...
**Ordering is already right and does not need arranging.** `QuillApp::show` draws the explorer before
...
The watcher in `QuillApp::show` asks the question once per frame and skips those three when the
answer is yes.
...
| Option | Pros | Cons |
|---|---|---|
| **Ask `text_edit_focused` in one shared place** (chosen) | One question, three readers; covers every text box now and later; the terminal moves on to the same answer | Depends on an egui predicate rather than something Quill owns |
| Ask `egui_wants_keyboard_input`, which is `focused().is_some()` | What the terminal used; slightly cheaper | True for any focusable widget, and every Quill control is focusable, so tabbing to a button would stop the document taking typing |
...
| Add a `Focus::TextBox` variant to Quill's own enum | Fits the existing idea of focus | Quill would have to be told which box, and by whom, and would then be tracking what egui already tracks correctly |
...
Functional, through the real window, in `crates/quill-app/tests/screenshots.rs`, driving the same
...
- **`SendKeys` and a real mouse click go to whatever window is in front.** The first attempt typed
  `note` into a terminal that happened to be over Quill, and reported the document unchanged, which
...
window is Quill's. `MainWindowHandle` is no good either — it comes back with a zero sized client
...
Before the fix: the filter box reads `note`, line one reads `note# Quill`, the tab, the title bar and
the status bar all carry the unsaved mark and the caret is at `Ln 1, Col 5`. After it: the filter box
reads `note`, line one reads `# Quill`, there is no unsaved mark anywhere and the caret is at
