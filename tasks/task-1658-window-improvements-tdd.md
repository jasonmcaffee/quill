...
**The fault.** Unluminous's window is created with `with_decorations(false)`, because rounded corners and a
...
One line in `UnluminousApp::show_editor`: `CursorIcon::Text` while the editing area is hovered.
...
reference capture of IntelliJ puts it. Narrower than IntelliJ's forty, because Unluminous's holds three
...
called `Terminal`. `tile` is what the rest of Unluminous already calls the thing along the bottom.

**The look.** A button that is on is the pill every list in Unluminous draws for its chosen row —
...
written down in the style guide and was settled in `task-1657` for the `F`: every icon in Unluminous is
...
`services/project_state.rs`, and a `.unluminous` folder **inside the project**, beside `.idea` and
...
| File | What is in it |
|---|---|
| `.unluminous/workspace.conf` | `explorer.visible`, `terminal.visible`, `terminal.tabs`, `files.active` |
...
**Reading and writing are turned on by the released binary only.** `UnluminousApp::restore_project` is
called from `main.rs` and by nothing else, exactly as `load_settings` is, so a test neither reads nor
writes a `.unluminous` folder — and a `.unluminous` written into a screenshot test's own sample project would
...
project's state going out to `.unluminous` and coming back.

Four of the seven cannot be proved that way, because they are the operating system's answer rather than
Unluminous's, so they were driven against the real `unluminous.exe` on Windows:
...
| Open Folder | A second `unluminous.exe` started on the chosen folder; the first window kept its project |
