...
- Persisting undo history across Quill restarts.
...
Quill restores whole snapshots, so a monotonic history revision carried by each snapshot is the
smallest version of this pattern. It is safer than a raw vector index because Quill caps history and
...
a hypothetical word instead of inserting it. Regenerate `quill-cli/docs/commands.md` from the
...
| Store only an undo vector index | Familiar `savedIndex` shape | Quill caps the vector and branches by clearing redo; a reused numeric index can name different content. |
...
- Run the focused Rust tests for the changed crates and command path, build release artefacts, then
  drive the installed Quill through the real CLI round trip before publishing the patch release.
