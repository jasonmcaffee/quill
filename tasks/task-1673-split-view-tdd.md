...
So `quill_core::markdown::Preview` gained a fourth structure beside the text, the spans and the
paragraph styles:
...
`quill_core::scroll_sync` is two pure functions and no state:
...
Both functions are pure and take laid out pages, so the whole of this is tested in `quill-core` with
...
`QuillApp::scroll_the_two_halves_together` compares where both halves are **before** the frame draws
...
**The command line needed the rule split out.** `quill-cli editor scroll` is applied at the top of a
...
| | What it costs |
|---|---|
| A drag payload in egui's `DragAndDrop` memory | egui's own drag-and-drop wants each drop zone to be a widget it knows about, and Quill's panes are painted rectangles rather than widgets. It would also put the state somewhere the window cannot see while it is deciding. |
...
The third is what every other component in Quill already does — "components take a rectangle and
...
`OpenFiles::drag_tab` does the move, and it is the same call `quill-cli tab move` makes, so a
...
Every other component in Quill is one function that interacts and draws. This one cannot be, and both
...
`QuillApp::save`, which is why it never made a file. Making a file is on the right click menu, which
...
`task-1661` asks that every feature be reachable from `quill-cli`, so both new capabilities are:

- `quill-cli tab move <position> [--tab] [--pane]` — what dragging a tab does, through the same
  `OpenFiles::drag_tab`.
- `quill-cli editor scroll [--line|--to|--top|--bottom] [--preview]` — reads how far through the file
...
**No horizontal scrollbar.** Text in Quill wraps, so there is nothing to scroll sideways. A picture is
