...
variables tree, inline values, thirteen `quill-cli debug` verbs. None of that is what failed. What
...
2. **`cargo run` is refused.** It is the configuration Quill's own project suggests, and pressing
...
- One command — `quill-cli debug adapters` — tells an agent the whole state of the world: which
...
Nothing in the protocol layer changes. `quill-dap` is untouched; every change is in
...
icons, the bug drawn by `theme::icon` like every other icon in Quill rather than lettered.

**When the Debug button is there.** Quill's rule is that a control is absent when it cannot apply,
...
`QuillApp::run_widget_state` — the widget itself still knows nothing and still returns an `Action`,
so the button, the menu, `Shift+F9` and `quill-cli debug start` remain one path.
...
easy to hit: debug a Node server while reading `README.md` and Quill says the file's language has not
...
| `cargo run -p quill-app --bin quill` | `cargo build -p quill-app --bin quill --message-format=…` | |
...
**Where it runs.** On a thread, the way `quill-git` runs `git` on a thread, with the window woken
when it finishes — never on the UI thread, because a cold `cargo build` of Quill is minutes. While it
runs the debug tile says `Building quill…` and the status bar says the same; the button does not
...
- **As a button in the debug tile**, which opens instead of staying empty:
  ...
  `Copy command`.
- **In `quill-cli debug adapters`**, as a field, so an agent does not parse prose.
- **As `quill-cli debug install lldb`**, which is the button's own path.
...
Two verbs, because Quill enforces that everything reachable by hand is reachable from the command
...
1. Open Quill's own project, press Debug on `cargo run`. Expect the install offer.
2. Press Install. Expect winget in the run tile and `lldb-dap.exe` on the machine.
3. Press Debug again. Expect a build, then a session, then a stop on a breakpoint in Quill's own
...
versioned separately; Quill's installer is a folder of its own with the binary and its plugins in it.
Rejected on size and on what it would commit Quill to shipping.
...
which is what `quill-git` already does with `git`.
...
1. **js-debug binds `::1`.** Its own banner says `Debug server listening at ::1:8123`, and Quill
...
2. **The program runs in a child session.** js-debug answers `launch` on the parent and then sends a
   `startDebugging` reverse request whose `configuration` carries a `__pendingTargetId`. Quill
...
`%LOCALAPPDATA%\Quill\adapters`, which `well_known` now looks in — so `tools/get-debug-adapter.ps1`'s
...
```
quill-cli debug adapters                     lldb found …\Quill\adapters\codelldb…, node missing + the command
quill-cli debug install node                 fetched and unpacked; adapters then reports node found
quill-cli debug breakpoint add quill-cli/src/main.rs 58
quill-cli debug start "cli status" --wait-for-pause     cli status is paused at main.rs:58
quill-cli debug frames                       20 frames
quill-cli debug variables                    the frame's locals
quill-cli debug step-over --wait-for-pause   cli status is paused at main.rs:59
```
