...
> create a new plugin for html in unluminous.
>
...
to `unluminous_core::syntax` with the manifest written against them. This is that ticket again with a
...
`unluminous_core::syntax` where it can be tested with no window. What follows is the design of the states
...
digit, another `<` — and the browser treats it as literal text, so Unluminous does too. This is why
...
one place in HTML where Unluminous knows what a word is without having to be told.
...
positional certainty and this one does. Unluminous's honesty rule is to show what is known and not guess
...
- **Anything else is plain text**, and here that *is* the CSS rule, deliberately. There is one tag
  ...
  wall of colour and would say something untrue about names Unluminous has never heard of.
...
The list is data, not a list of element names inside Unluminous, for the reason `language.definers`
exists: a list of languages inside Unluminous is a list a plugin somebody writes later can never join. A
...
and it is the one registry-shaped key in Unluminous that is not validated against a list — because the
...
good one. Unluminous already has the seam for this and it is `CodeHighlighter`: *`unluminous-core` holds no
...
than six that almost agree. The mapping is **Unluminous's per-token one rather than Dracula's
...
| attribute name | `Builtin` | `#BD93F9` | `entity.other.attribute-name`; purple is what `builtin` is everywhere else in Unluminous |
...
**absent** for a `.html` file — Unluminous's rule for a control that can never apply, and the same
...
- **A rendered preview.** `language.renders = html` would mean a box model, an inline layout engine,
  a cascade and a network story, which is a browser. `unluminous-core` draws Mermaid because a diagram is
  arithmetic; a web page is not. The three view mode buttons are absent for a `.html` file, which is
  Unluminous's rule for a control that can never apply.
...
- **Case-insensitive element names.** `<DIV>` is valid HTML and is drawn as a custom element here,
  ...
  byte of every file in Unluminous. Nobody has written uppercase tags since 1999 except in `<!DOCTYPE>`,
...
1. **`unluminous-core`, no window.** The state machine's own tests: `5 < 3` in prose is prose, an
...
2. **`unluminous-app` unit tests.** The manifest parses, every key reaches the grammar, the older plugins
...
4. **The real window, through `unluminous-cli`.** Open a real `.html` file, read `editor text`, take a
...
| Where | What |
|---|---|
| `unluminous-core/src/syntax.rs` | `Grammar::markup`, `Grammar::raw_text`, the five states in `scan`, `Embedded`, `scan_with_embedded`, `tags`, `markup_position` |
| `unluminous-core/src/folding.rs` | `tag_regions` for a markup grammar, `Tokens::put_in_order` |
| `unluminous-app/src/services/plugins.rs` | `language.markup` and `language.raw_text` read into the grammar |
| `unluminous-app/src/app/mod.rs` | `colour_the_file` and `PluginHighlighter` colour the embedded regions |
| `unluminous-app/src/app/completion.rs` | the language's words are offered by position in a markup file |
| `unluminous-app/plugins/html/` | the manifest, the icon and `icon.md` |
| `crates/unluminous-app/tests/` | the new screenshot and the accepted `plugins_page` |
