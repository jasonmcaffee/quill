...
`task-1659` asks for three things Unluminous has not had, all of them modelled on IntelliJ: a **Go to
...
Three of the four are new user interface over machinery Unluminous already has — a file tree, a text
buffer, a layout engine, a modal shape. The fourth, pictures in the preview, is the only one that
reaches into `unluminous-core`: the layout engine places glyphs and has no notion of a line that is taller
...
| 5 | Every modal in Unluminous can be dragged by its header and resized from any of its four edges and four corners. |
...
- The rest of IntelliJ's `Search Everywhere`: tabs for classes, symbols and actions. Unluminous has no
...
- Regular expressions in the search box. `Match case` is offered because it is one line of code and
  ...
  performance story.
- Remote images (`![](https://…)`) in the preview. Unluminous makes no network requests, and a preview
...
- An index. Both searches read the project when they are asked to. An index is what a project of a
  million files needs and is a cache to keep correct; the thread and the cap answer the sizes Unluminous
  is used at. Measured on Unluminous's own repository: 618 files, and a search of all of them in 20 ms.
...
**Finding text means leaving Unluminous.** Nothing in the editor searches more than the open file — in
...
**The Markdown preview shows `alt text` where a picture should be.** `unluminous-core::markdown` reads
...
flowchart TB
    ...
    subgraph core["unluminous-core (no user interface)"]
        md["markdown::render\nPreview + images"]
...
the Unluminous window carries a dragged modal along with the middle it was dragged from. They live in
egui's own memory under the modal's id rather than in `UnluminousApp`: the window has no decision to make
...
as `unluminous_git::Worker` and the terminal already are, with a waker that asks the window to draw again.
...
The modal is two panes with a `components::splitter` between them, because every pane in Unluminous is
...
Opening a result goes through `UnluminousApp::open_the_match`, which opens the file in a tab of its own,
...
This is the only part that reaches into `unluminous-core`. The preview is not a second renderer: markdown
...
`unluminous-core`, and that crate's whole point is that its tests run with no window, no graphics card and
no fonts.
...
words, exactly as `unluminous-git` reports git's.

## Alternatives considered

**Modal geometry in `UnluminousApp` rather than in egui's memory.** Rejected: it is state no other part of
the window has a decision to make about, and every dialog would have to thread a field through.
`gutter_menu` is held in `UnluminousApp` for a reason that does not apply here — a screenshot test cannot
...
to keep correct, it costs memory proportional to the project, and the thread answers the sizes Unluminous
is used at in well under a second. If a project ever makes it feel slow, this is where to go.

**Images through a new `Command`/`Document` concept in `unluminous-core`.** Rejected: the preview is not a
...
**`unluminous-core` (no window).** `markdown::render` produces an image entry for a line that is only an
...
**`unluminous-app` services (no window).** `file_search`: subsequence matching, name beating folder,
...
**The real application.** `cargo run --release` on Unluminous's own repository — a project with a few
