...
The ask is one sentence: on Windows, with the Quill project open, a few tabs open and the background
...
was not a fix but a measurement: `crates/quill-app/examples/frame_cost.rs`, which opens a real file
...
```
crates/quill-app/src/app/mod.rs: 168990 bytes, 3556 lines, laid out at 900 points wide
...
`Document::apply(Command::PlaceCaret)` bumps the document's revision. `QuillApp::colour_the_file`
and `QuillApp::refresh_layout` are both keyed on that revision. So **every frame in which the caret
...
half frames a second. Quill was not a bit slow. It was doing, sixty times a second, work that should
...
| 4 | Dragging the window is not made jagged by anything Quill does inside a frame. |
...
- **A different text engine.** Quill's layout is its own, deliberately
  (`quill-technical-design-document.md` §3). Nothing here replaces it; everything here makes it stop
  doing work twice.
...
|---|---|---|---|
| 1 | `QuillApp::colour_the_file` | Keyed on `Document::revision()`, which a caret move bumps. | The whole file re-tokenised and re-coloured on every frame of a drag. |
...
| 3 | `QuillApp::refresh_layout` | The same `revision()` fault. | The whole document laid out again on every frame of a drag: **82 ms**. |
...
| 6 | `quill_core::layout` | Three heap allocations for every grapheme cluster: the cluster's text twice, and a `CharStyle` cloned only to be compared. | Roughly 350k allocations per layout of this file. |
...
at all happen" still reads it — whether a file's marked passages need writing to `.quill`, whether
...
altogether, which is tempting, but `CharStyle` is a public value in `quill-core` that tests build by
...
`quill_core::relayout` takes the previous layout and the document as it is now, and:
...
`QuillApp::follow_the_open_file` records for deriving the reveal from the state rather than firing it
...
**Laying out on a thread.** `quill-git` and the text search both run on one, and the same shape would
...
lands on a line that has since moved. If a future Quill opens files where a *first* layout is slow — a
...
`cargo run --release -p quill-app --example frame_cost -- crates/quill-app/src/app/mod.rs 900`, on the
...
- **`quill-core`**, with no window: the two-revision invariant, `set_many` against a loop of `set`,
...
- **`quill-app`**, with no window: `paint_text` reporting how many glyphs it placed, the font memo
...
- **The real window**: `cargo run --release`, with the Quill project open and several tabs, dragging a
...
|---|---|---|
| `moving_the_caret_is_not_a_change_to_the_text` | `quill-core`, `document.rs` | a caret move counting as a change to the text |
| `a_layout_that_changed_means_the_text_revision_moved` | `quill-core`, `document.rs` | a command that changes the layout and forgets to say so |
| `an_edit_measures_the_paragraph_it_touched_and_not_the_document` | `quill-core`, `layout.rs` | `relayout` quietly laying the whole document out |
| `painting_a_long_document_costs_a_screenful` | `quill-app`, `editor_view.rs` | the painter collecting every glyph in the file |
| `dragging_a_selection_lays_nothing_out_again_and_colours_nothing_again` | `quill-app`, `screenshots.rs` | the whole fault, end to end, through a real pointer drag |
