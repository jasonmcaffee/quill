# Unluminate — the product video

**task-1793.** A 4K demo of Unluminate for the Unluminate section of jasonmcaffee.com.

This is the script and the shot list. It is also the thing the recording harness reads: every beat
below names the exact `unluminate-cli` commands that produce it, so the video is *driven* rather than
...
3. **It is AI-first, and that is a definition rather than a slogan.** Everything a person can do in
   ...
   worked by agents in terminals Unluminate owns.
...
| Composition | full-desktop capture: wallpaper, then Unluminate inset **72 px** on every edge, at **0.83** background opacity |
...
Unluminate's own background rather than as the desktop behind it. 72 px on a 2160-tall frame is a hair
...
`_agent_output/task-1793-unluminate-video/fleet/`.
...
One project exercises **every** language plugin Unluminate ships (Rust, TypeScript, JavaScript, CSS, HTML,
...
Every beat is one recorded segment. **Every `unluminate-cli` line below is literal** and is what the
harness sends.
...
**On screen.** The wallpaper alone for 0.6 s. Unluminate fades up from nothing over 1.2 s, already open on
...
**Setup.**
```
unluminate-cli settings set appearance.background.opacity 0.83
unluminate-cli window position --x 72 --y 72
unluminate-cli window size --width 3696 --height 2016
unluminate-cli tab open src/telemetry.rs
...
**Script.**
```
unluminate-cli editor scroll --line 40           # slow, 8 lines a step
unluminate-cli explorer expand web
unluminate-cli editor scroll --line 1
```
...
**Script.**
```
unluminate-cli editor caret --line 63 --column 1
unluminate-cli editor insert "…"                 # one character per call, 55 ms apart
unluminate-cli editor complete --stem dur        # shown, then chosen
unluminate-cli editor complete --stem dur --choose 0
...
**Script.**
```
unluminate-cli tab open src/report.rs
unluminate-cli editor definition --name Health
unluminate-cli editor navigate-back
unluminate-cli editor references --name Flight
unluminate-cli editor rename --name fleet_health --to fleet_summary
...
unluminate-cli fold expand --line 12
```
...
unluminate-cli editor view split
unluminate-cli editor scroll --line 20
unluminate-cli editor caret --line 8 --column 1
unluminate-cli editor insert "…"                 # typed, so the preview updates live
```
...
unluminate-cli browser open web/index.html
```
...
unluminate-cli run select "fleet report"
unluminate-cli run start
unluminate-cli run output
```
...
**Script.**
```
unluminate-cli debug breakpoint add src/report.rs 48
unluminate-cli debug start
unluminate-cli debug status
unluminate-cli debug frames
unluminate-cli debug variables --expand flights
unluminate-cli debug hover --name health
unluminate-cli debug watch add "flights.len()"
unluminate-cli debug step-over
unluminate-cli debug step-over
unluminate-cli debug evaluate "health.uptime_ratio()"
unluminate-cli debug continue
```
...
**Script.**
```
unluminate-cli panel dock terminal --side right
unluminate-cli panel size terminal --width 900
unluminate-cli panel dock terminal --side bottom
unluminate-cli pane split
unluminate-cli tab move --pane 1
unluminate-cli pane width --fraction 0.62
unluminate-cli panel zoom explorer --factor 1.15
unluminate-cli panel reset
```
...
**Script.**
```
unluminate-cli plugins pane database/explorer --side right
unluminate-cli plugins tab database/workspace --open
unluminate-cli plugins run database console --sql "select tail, model, hours from drone order by hours desc limit 12"
```
...
back to Unluminate Dark. Held on the last one for a beat.
...
**Script.**
```
unluminate-cli theme set dracula
unluminate-cli theme set palenight
unluminate-cli theme set deep-ocean
unluminate-cli theme set monokai-pro
unluminate-cli theme set one-dark
unluminate-cli theme set unluminate-dark
```
...
> They bring their own tools and their own keys. Unluminate holds none.

**Script.**
```
unluminate-cli plugins pane agent-chat/chat --side right
unluminate-cli plugins run agent-chat send "what does fleet_health do, and where does its data come from?"
unluminate-cli plugins run agent-chat state --wait
```
...
ticket opens as a modal. A card's play button starts an agent in a terminal Unluminate owns, and the
...
unluminate-cli plugins run agent-tasks open UNLUMINATE-4
```
...
wrong costs one beat. `_agent_output/task-1793-unluminate-video/segments/beat-NN.mp4`.

**Driving.** `scripts/drive.mjs` sends the `unluminate-cli` lines above with the pauses written beside
...
it all with a 2 s fade at each end, and one final scale-safe encode to `unluminate-demo-4k.mp4`.
...
A new **Unluminate** section on jasonmcaffee.com, between Apps and Articles: a short description of what
Unluminate is, the video as the section's centrepiece with a poster frame, and a link to the GitHub
...
did not were both defects in Unluminate rather than mistakes in the pacing, and both are worth writing
...
written to `.unluminate/breakpoints.conf` is 2037, which is line 50 in that file, byte for byte. A
...
**A second, separate cause with the identical symptom:** restoring `.unluminate/breakpoints.conf` from git
...
`restoreProject()` now restores `src` and `web` only, and the demo project stopped tracking `.unluminate/`.
...
- **Two Unluminates running makes a bare `unluminate-cli` ask which one is meant**, which is right and is also
