...
> opacity into a Settings modal, open any text file, make every pane draggable, allow several Unluminous
> instances with a recent projects list, add a terminal with tabs, put the menus in the macOS menu bar
> with Unluminous first, and break the code up into folders.
> Spec: `tasks/improvements.md`, plus the new terminal design document `tasks/unluminous-terminal-tdd.md`.
...
- `Settings -> Appearance -> Font` sets the editor's font family and size for the whole document rather
  than for the selection. A new `Document::set_base_style` in `unluminous-core` applies it without pushing an
...
- The terminal uses `alacritty_terminal` for the escape sequence emulation and its pseudoterminal, with
  ...
  options considered and the numbers behind the choice are in `tasks/unluminous-terminal-tdd.md`.
...
- Several Unluminous windows means several processes, one project each, started with the folder as the first
  argument. `services/launcher.rs` records why.

## Steps

[x] ✅ - write the terminal technical design document `tasks/unluminous-terminal-tdd.md`: what a terminal has to
...
[x] ✅ - reorganise `unluminous-app` into `app/`, `components/`, `services/` and `theme/`, moving every existing
      module into its folder and updating the imports and the tests, with no behaviour change
[x] ✅ - add `Document::set_base_style` to `unluminous-core` with unit tests: it changes the family and size over
...
[x] ✅ - add `app/actions.rs` and `components/menu_bar.rs`: one `Action` enum, one list of menus, the
      in-window bar with `Unluminous` first and then `File`, `Edit`, `View`, and one dispatcher
...
[x] ✅ - add `services/launcher.rs` and `File -> New Window`: a second Unluminous process on a folder of its own
[x] ✅ - add `File -> Recent Projects`: the list, opening one in a new window, and writing the list on every
      folder that is opened
[x] ✅ - new crate `unluminous-terminal`: the session over a pseudoterminal, the screen snapshot, the colour
...
[x] ✅ - unit tests for `unluminous-terminal`: the key encoding, the palette, the screen snapshot after feeding
...
[x] ✅ - launch the real application: it starts with the macOS menu bar installed and the terminal open,
      ...
      `unluminous --print-menus` prints what went into the menu bar. A screen capture of the bar itself is the one
...
[x] ✅ - update `README.md`, `tasks/unluminous-technical-design-document.md` and add `CLAUDE.md` recording the
...
1. **An opened file drew nothing.** The layout cache compared the document's revision number, and a revision
   ...
   found it. `UnluminousApp::forget_layout` now throws the layout away when the document is replaced, and two tests
...
Two smaller things came from reading the ask again rather than from a fault: `Settings` is in the `Unluminous` menu
...
reports nothing on the error output, `unluminous --print-menus` prints what went into it, the process is named
`Unluminous` so the application menu says `Unluminous` rather than `unluminous`, and the same list of menus is clicked through
