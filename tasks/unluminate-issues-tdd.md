# unluminate-issues — the reported faults, their causes, and what each change is
...
is actually happening in the code, what changes, and what proves it. Unluminate's own rule from
...
and every file under `crates/unluminate-app/src/services/agent_tasks/`,
`crates/unluminate-app/src/components/agent_tasks/` and `crates/unluminate-app/plugins/agent-tasks/` is
...
> When I try to expand a folder with a lot of files, like when unluminate is at /, and I expand dev, unluminate
...
`file_kind::openable` on every file so the row can be drawn dimmed when Unluminate cannot open it.
...
carries on every platform Unluminate runs on, so `Refusal::TooLarge` is still decided without a second
`stat`.
...
The pane on the right of the explorer that holds all the tabs is what Unluminate calls the **editing
area** — `dock::Regions::editor`, with one `components::file_tabs` strip per pane inside it. The
ticket's "file pane" and Unluminate's "editing area" are the same thing, and the name in the window follows
Unluminate, because `panel list`, `dock` and a dozen accepted screenshots already use it.
...
- `Action::ToggleEditor`, on the `View` menu as `Hide Editor` and `Show Editor`, next to
  ...
  to build `unluminate-cli action list`, so this is one entry rather than an entry and a catalogue row.
...
- `UnluminateApp::ui` draws no tab strip and no editing pane when the editing area is hidden.
...
reload. And `unluminate-cli action list` offers `Hide Editor`, which is the agent's way in.
...
a general capability of Unluminate's plugin system, described by `tasks/ui-plugin-architecture.md`, and no
...
`crates/unluminate-app/tests/snapshots/agent_tasks_pane.png` stops being written by any test. It is left on
...
**Which models exist is a list in Unluminate, and it is per agent.** A new `agent::MODELS` answers, keyed by
...
board written by a newer Unluminate, or by hand, must not silently lose a model — so the list the dropdown
...
The board file on this machine proves it. `~/Library/Application Support/Unluminate/plugins/agent-tasks/board.sqlite3`
...
- `check_version` stops being the only thing that writes the version. It refuses a file from a newer
  Unluminate as it does now, and then, **after** `migrate` has run, the version the file is at is written
...
> The Unluminate -> Settings -> Plugins -> Agent-Tasks has a scroll bar but doesn't scroll.
...
four are Unluminate's plumbing described to the person using it. What the connection actually is, is a URL
and a key.
...
**The key is taken from the environment when the keychain has none.** Unluminate launched from a terminal
inherits `ANTHROPIC_API_KEY` from `~/.zshrc`; Unluminate launched from the Dock does not. So
...
from zshrc" means in practice, and it is why a person who launches Unluminate from a terminal has nothing
to type at all.
...
`crates/unluminate-app/plugins/` holds agent-tasks, css, html, javascript, mermaid, rust and typescript.
Markdown is built into `unluminate-core` — `unluminate_core::markdown::render` is what the editor's own preview
...
`unluminate_core::markdown::render` for the source, `unluminate_core::layout` for the lines, and
...
diagrams. `UnluminateApp::refresh_preview` resolves those in two extra passes that decode an image and lay
...
table into a layout with the right number of lines, which is `unluminate_core::markdown`'s own test shape.
...
run tile draw, so it has Unluminate's keyboard, selection, mouse reports, resize and clipboard rules. That
...
happened against Unluminate's own state read back, rather than against what the agent said it did.

**Where it lives.** `crates/unluminate-app/tests/agent_board.rs`, a test file of its own, because it starts
...
1. **A ticket is claimed and worked.** Create a board in a temporary folder, create a ticket whose
   ...
   folder with a given line in it — and start the agent. Then wait, with a ceiling, on Unluminate's own
...
**What they do not do.** They do not assert on the agent's wording. Every assertion is a state Unluminate
...
`~/.claude.json` on their behalf is Unluminate silently granting an agent the run of a folder. What the board
...
```
UNLUMINATE_TEST_PROJECT=<a folder you have already opened Claude Code in> \
...
Every test in `crates/unluminate-app/tests/screenshots.rs` fails in this session with
`Failed to create render state: RequestDeviceError(RequestDeviceError { inner: Core(Device(Lost)) })`,
including tests this work never touched. The machine's graphics card is fine and Unluminate itself is running
...
```
cargo test -p unluminate-app --test screenshots
```
...
An attempt to run the screenshot tests inside the running Unluminate's own terminal, through the MCP control
channel, found that Unluminate did not answer at all: `terminal list` timed out with
