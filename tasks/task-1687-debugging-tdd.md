...
> We want our unluminous editor to support line breaks and debugging for the languages we support,
...
variables while the program is paused — behaving the way IntelliJ behaves, in the languages Unluminous's
plugins already claim.

This document is the design. `task-1689` is the implementation, and it is built: the whole of §4 to
§12 shipped in Unluminous 0.14.0.
...
- **§2.2's lifecycle summary has `setBreakpoints` before `launch`, and it has to be the other way.**
  ...
  js-debug. `unluminous_dap::session`'s own comment records it.
...
- **`runInTerminal` is answered with `success` and no process id.** §7.2 says Unluminous replies with the
...
`verified` — an honest protocol, which suits Unluminous.

Two details matter to this design. The adapter is started as a child process and spoken to over
**stdio**, or is a server the client connects to on a **port** — both are common, and Unluminous needs
both. And the protocol has a reverse request, **`runInTerminal`**, by which the adapter asks the
*editor* to run the debuggee in the editor's own terminal — which is exactly what Unluminous's run tile
is.
...
client is a bounded piece of work for an editor of Unluminous's size, nothing like the open-ended cost
...
**Writing our own debuggers.** Unluminous draws its own Mermaid because a diagram is arithmetic.
A debugger for one language is a career; for three it is not a serious option.

## 3. The shape of the design

A new crate, **`unluminous-dap`**, speaks the protocol: the framing, the typed messages, the session
lifecycle, and a worker thread arranged exactly as `unluminous_git::Worker` is — requests down a
...
Unluminous** — the rule `language.renders` and `run.project` already follow. A manifest names
`debug.adapter = lldb`; the name is checked against a built-in registry and an unknown one is
refused with a message; Unluminous's own code knows how to find and start each adapter it ships
...
Breakpoints live where highlights live — in `unluminous_core`, inside the `Document`, as offsets that
move with the text — and persist beside the project in `.unluminous`, owned by the open `Document` or by
the store under the one rule every awkward case in Unluminous is already settled by.

Everything is an `Action` and a row in the CLI catalogue, so the whole feature is reachable from
the menus, the keyboard, `unluminous-cli` and the MCP server on the day it lands — which for this
...
`unluminous-dap` implements the base protocol: ASCII headers, `Content-Length: N`, `\r\n\r\n`, then N
...
Messages are typed with `serde`, and only the messages Unluminous uses are typed: `initialize`,
...
Unluminous does not read are not modelled — the protocol is large and additive, and `serde` ignores
...
`supportsConditionalBreakpoints`, `supportsLogPoints`, `exceptionBreakpointFilters` — so Unluminous
...
not die, kill it — it is a child process Unluminous owns, the same as any run.

### 4.3 The thread

The window never blocks on an adapter. `unluminous_dap::Client` is arranged as `unluminous_git::Worker` is:

- `start(adapter: AdapterCommand, waker: Waker) -> Client` spawns the adapter process (with
  `CREATE_NO_WINDOW` on Windows, as `unluminous-git` runs git) or connects to its port, and starts one
...
// debug.adapter is `gdb`, and this version of Unluminous drives lldb, node
```
...
Unluminous fetches nothing, ever — the preview's rule, kept here. Each registry entry knows the
...
as git's stderr is shown, because a debugger explains itself better than Unluminous could.
...
**What about Python?** Unluminous ships no Python plugin today, so the registry ships no `python`
...
`unluminous_core::breakpoints`, inside the `Document`, built as `highlights` is built: a sparse set
...
really landed and `verified`. Unluminous draws the adapter's answer, not its own hope: a breakpoint the
...
carries both fields — so the adapter does the evaluating and the logging, and Unluminous's cost is two
...
and sent back through `setExceptionBreakpoints`. Unluminous holds no list of its own; an adapter that
offers none gets no control.

### 6.5 The file beside the project

`.unluminous/breakpoints.conf`, in the numbered form `run-configurations.conf` established, written by
...
owned by its `Document`, and every other file is owned by the store.** `UnluminousApp::change_highlights`
...
`set_highlights` clamps, so a file rewritten outside Unluminous gives a misplaced dot rather than a
...
that knowledge is Unluminous's, in the registry, never the plugin's). `Debug Current File` exists
...
— lldb-dap, js-debug and debugpy can all ask for it — Unluminous answers it by starting the requested
command through the run tile's own path (`RunPanel::start`'s machinery, a real
`unluminous_terminal::Session` at the size `run_grid_size()` already computes), and replies with the
...
On `stopped`, Unluminous fetches the stack, opens the top frame's file if it is not open
...
Unluminous already owns the machinery: `FileSymbols::read` has the file's identifier list sorted by
...
A `debug` area in `unluminous-cli/src/catalogue.rs`, one row per verb, and therefore — with no further
...
driving Unluminous can now observe a program's actual state instead of reasoning about it.

## 11. Where the state lives

- `.unluminous/breakpoints.conf` — the project's breakpoints (§6.5). Shared, written by the released
  binary only.
- `.unluminous/workspace.conf` — `debug.visible`, `debug.watches` (per-person, beside `run.selected`).
- `UnluminousApp` — one `Option<DebugState>` (in `app/debug.rs`, as `app/git.rs` holds `GitState`):
  the `unluminous_dap::Client`, the session state machine, the fetched stack and variable rows, the
  watch results, the inline-value cache. All of it dies with the session; none of it is written
  anywhere.
- `unluminous_core::breakpoints` — the open documents' sets, riding the document exactly as highlights
  ride it.
...
1. **`unluminous-dap`, no window, no process.** The codec against byte transcripts, including torn
   ...
   `unluminous_core::breakpoints`: offsets shift under `insert`/`remove_range`, ride the undo snapshot,
   clamp on load — the highlights tests, re-asked.
2. **`unluminous-app` units.** `breakpoints.conf` round-trips through `store::Values`; a block missing
...
4. **The real thing.** A fixture program per adapter (a ten-line Rust binary built by the test, a
   `hello.js`), driven end to end through `unluminous-cli`: breakpoint add, `debug start
   ...
   one that lies about Unluminous). And the real window: a person sets a breakpoint in a real project,
...
- **Debugging a Cargo/npm configuration by deriving the binary** — Zed's locators. Right and
  ...
  then the registry's refusal sentence keeps Unluminous honest (§7.1).
...
- **Downloading adapters.** Zed fetches them; Unluminous fetches nothing — the rule that keeps a
...
- **A DAP server for Unluminous itself.** Out of scope and out of character; Unluminous is the client.
