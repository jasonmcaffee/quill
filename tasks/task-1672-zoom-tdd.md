...
Quill's editing area is not an `egui::ScrollArea`. Each tab holds `OpenFile::scroll`, **a number of
...
(`QuillApp::zoom_the_text`), the keyboard (`Action::ChangeFontSize`, `Action::ResetFontSize`), and the
...
**Remember the line and how far down it the point was.** What shipped. `quill_core::Anchor` is two
numbers:
...
`Layout::anchor_at_y` takes one and `Layout::y_of_anchor` puts it back, and both are `quill-core`
...
Nine tests were added and all of Quill's 210 screenshot tests still pass:

- four in `quill-core` for the anchor itself, including the wrapped line and the empty document;
- five in `quill-app` driving the real window through `egui_kittest` — the pinch, the keyboard, a tab
...
is not in `.quill/`. What survives a restart is the font size, exactly as it did before.

**The horizontal position is not anchored.** Quill's editing area wraps rather than scrolling
