...
The data model is the interesting part, because it is the same shape Quill's is. `foldState` is a
...
That is the one place Quill's data model has to be *deliberate* rather than *lazy*. Quill's fold state
...
`quill_core::folding` rather than left to the set's natural behaviour.
...
**`quill-core`, with no window.** A nested file — a function holding a `for` holding an `if`, the
...
**`quill-app`, with no window.** Over a nested document:
