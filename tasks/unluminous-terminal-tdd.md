# Unluminous Terminal Technical Design Document
...
Unluminous gets a terminal along the bottom of the window with several tabs, the way IntelliJ has one. Each
...
The original ticket for Unluminous asked for the editor to be written here rather than taken from a library,
and `tasks/unluminous-technical-design-document.md` section 3 records where that line was drawn: the text
...
- The drawing: one background rectangle per run of cells that share a background colour, one glyph per
  cell out of Unluminous's own glyph atlas, the cursor, and the rules for underline and strikethrough.
...
covers 63 per cent of the crate, and the interface changes between versions. It is Apache-2.0 while Unluminous
...
Rust process holding the pseudoterminal would work. It also means shipping a web engine inside Unluminous,
...
encoding and the resizing, is written in a new crate `unluminous-terminal` and in
`unluminous-app/src/components/terminal_panel.rs`.
...
3. `renderable_content` hands over exactly what a painter needs and nothing else, so Unluminous's own drawing
   stays ours and stays small.
4. The parts most likely to need changing for Unluminous, which are the palette, the key encoding and the
...
flowchart TD
    ...
    subgraph term["unluminous-terminal: no user interface dependencies"]
...
`unluminous-terminal` has no user interface dependency, for the same reason `unluminous-core` has none: its tests
...
2. The panel draws the `Screen`: one rectangle per run of cells sharing a background colour, then one
   glyph per cell from Unluminous's glyph atlas, then the underline and strikethrough rules, then the cursor.
...
The waking is a function the session is given when it is built, so `unluminous-terminal` never learns what
...
- Indices 0 to 15 are the named colours. They are set to the values in Unluminous's own palette so the
...
### Layer 1: unit tests in `unluminous-terminal`, with no window and no shell
...
and can be a comparison baseline like every other Unluminous snapshot. They cover the tile with output in it,
...
- Ligatures and shaped text in the terminal. A terminal grid puts one character in one cell, which is
  what makes it a grid. `unluminous-core`'s layout is not used here at all.
...
- The Kitty keyboard protocol, which reports key releases and modifiers a terminal cannot otherwise
  express. `alacritty_terminal` supports it and it is left switched off, because nothing Unluminous runs needs
...
Pulled into `~/dev/unluminous/reference` and read. Nothing was copied.

| What | Version or commit | What was taken from it |
|---|---|---|
| [`alacritty_terminal`](https://crates.io/crates/alacritty_terminal) | 0.26.0 as published | The interface a host application drives: `Term::new`, `tty::new`, `EventLoop::new`, `Notifier`, `renderable_content`, and the shape of `Cell` and its flags. Also compiled into Unluminous. |
...
|---|---|
| `alacritty_terminal`'s interface changes between versions | It is used from one file, `unluminous-terminal/src/session.rs`, and everything above it works on our own `Screen`. A version bump touches that one file. |
...
| The terminal's font is not monospaced because the system has none of the families Unluminous offers | The renderer falls back to the first family it has, and a proportional face in a grid is legible but uneven. Stated rather than hidden. |
