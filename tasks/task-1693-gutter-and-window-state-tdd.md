...
`quill_core::layout` gives every `PlacedLine` four numbers: `y`, `height`, `baseline` and the pair
...
`Action::NewFolder(_)` is named `new-folder`, so `quill-cli action` reaches it the day it exists,
...
`mouse_event`, the window brought to the front first — a freshly started Quill resizes correctly from
**all four edges**:
...
freshly started Quill worked again immediately. Two separate instances behaved the same way.

It also explains the shape of the report. One refused drag is enough, it is silent, and afterwards
nothing about moving or resizing works until Quill is restarted — so which edge a person notices
first is a matter of which one they happened to try.

### What is done

**A grip is not added when the window is maximised.** That is Quill's own rule for a control that can
...
## 4. What Quill comes back as

The ticket asks for four things at once — *the windows/projects I had open should all open, and be in
the same location and state, including open files, scroll position, terminal windows* — and Quill
...
Three additions to `.quill/workspace.conf`, in the comma-list shape `files.panes` already uses so
that a Quill which has never heard of them reads the file unchanged:
...
settings file, because Quill's windows are one per project: remembering it per person would mean the
...
A Quill window is a process, which `services::launcher` records as a deliberate decision and which
...
- Starting Quill **with no folder named** — which is the shortcut case, and is exactly the condition
  `starting_folder` already tests for by asking whether the current directory is the folder
  `quill.exe` lives in — opens the first line itself, starts a process for each of the others through
  `launcher::open_window`, and **rewrites the file to exactly that list**, dropping any folder that
  is no longer there.
- Starting Quill **with a folder named** — `quill .`, a file dropped on the icon, `quill-cli launch`
  — restores nothing and only adds itself.
- **A project that already has a window is skipped**, live windows being what
  `quill_cli::client::running` answers. Two Quills on one folder would be two processes writing one
  `.quill` folder and the last one to write would win, which is the same reason `OpenFiles::open`
...
many words, and it is the only rule available: Quill has no application-wide quit to hang the
...
person's own settings folder, which is where every other list Quill keeps already is.
...
`FileTree` is read when Quill is told to read it — opening a project, a menu entry, a file operation
Quill itself did. An agent writing a file with its own tools is none of those, so the row never
appears.
...
expensive part. Quill does not need to watch a tree. It needs to notice that a **folder somebody can
...
rather than absent. That is deliberately the other half of Quill's own rule: absent is for a control
...
- `quill-core`: nothing changes. The gutter reads the numbers `PlacedLine` already carries.
- `quill-app` unit tests: the gutter's band arithmetic; the two ratios giving exactly today's sizes at
...
All of this against `target/release/quill.exe`, driven through `quill-cli` and, where a pointer was
...
- **The eight grips.** A freshly started window resized from all four edges: top `H 720 -> 660`,
  ...
  **maximised** window, every later drag — the edges *and* the title bar — did nothing for the rest
  of that process's life, and a freshly started Quill worked at once. Twice, in two processes.
- **Two windows, quit and brought back.** Projects A and B, each closed with `close-window`, then one
  launch from the folder `quill.exe` lives in: both windows came back, A at 500,200 sized 1300x850
...
- **A file made outside Quill.** `from-an-agent.md` and `new-folder-outside/` written from a shell:
...
- **The gutter at 30 points** in the real window, over a real desktop:
  `_agent_output/task-1693-quill-gutter-and-misc/live-large-font.png`.
