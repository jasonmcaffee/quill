...
are the first things Unluminous's own renderer is pointed at, so a document about drawing Mermaid is
...
```mermaid
flowchart LR
  subgraph core["unluminous-core (no window, no fonts, no card)"]
  ...
  subgraph app["unluminous-app (the window)"]
...
A diagram's height depends on how wide the pane is, and `unluminous-core` has no pane. This is the same
shape `task-1659` used for pictures, and it is reused rather than reinvented.

```mermaid
sequenceDiagram
  participant W as UnluminousApp
...
| It has the same raw / side by side / preview toggle a `.md` file has | The three buttons are found **by name** on a `.mmd` file, each switches the mode, and `unluminous-cli editor view side` does the same thing from a script. |
...
- **Not a network client.** There is no path through any of this that opens a socket. This is the one
  ...
  nothing in `unluminous-core` can make a request, and the app never hands a diagram's text to anything
  but its own painter.
...
| Mermaid's syntax moves under us | The parsers are permissive: an unknown statement inside a known diagram is skipped rather than fatal, so a file using a newer flowchart feature still draws the parts Unluminous understands. |
...
## 2. Four ways to draw it, and why Unluminous writes its own
...
hundred megabytes that Unluminous does not otherwise need, that the installer would have to acquire, and
...
Rejected, and for Unluminous more firmly than for most applications. The whole character of this window is
...
The argument is the same one `unluminous_core::markdown` already made, and it is worth restating because
it is the reason this fits at all. **Unluminous is not short of a way to draw.** It has a layout engine, a
...
curves are polylines rather than splines, the fonts are Unluminous's, the colours are Unluminous's palette
rather than Mermaid's default theme, and where two layouts are both reasonable Unluminous may choose the
...
| Where | What |
|---|---|
| `unluminous-core/src/mermaid/` | Reading the source and working out where everything goes. No user interface dependency; measured through `FontMetrics`; tested with `FixedMetrics`, so the expected numbers are arithmetic a reader can check and are the same on every machine. |
| `unluminous-app/src/components/diagram_view.rs` | Painting a laid-out diagram, and the scrolling and zooming over it. |
| `unluminous-app/src/services/mermaid_scene.rs` | Keeping a laid-out diagram between frames, so a preview redrawn sixty times a second lays a diagram out once. |
| `unluminous-app/plugins/mermaid/` | The plugin: the manifest, the grammar for colouring `.mmd` source, the colour scheme and the icon. |

`unluminous-core::mermaid` is laid out as one file for the shared parts and one for each family of
diagram:
...
`unluminous-core` cannot name an `egui` type, and it should not want to: what a diagram is, once it has
...
`unluminous-core` where it can be tested. An arrowhead is a filled `Polygon` of three points. A pie slice
is a `Polygon` whose arc has been flattened into segments. A crow's foot is three `Line`s. A dashed
relationship is a `Line` with a `Dash`. The painter in `unluminous-app` is then about a hundred lines with
no diagram knowledge in it at all, which means a diagram type added later needs no change there.

`Paint` is a colour and an alpha. `unluminous_core::style::Color` has no alpha, deliberately — text in
Unluminous is always opaque — and diagrams do want a wash behind a subgraph, so `Paint` carries its own
...
- **YAML front matter** — a `---` block at the top. Skipped, except `title:`, which becomes the
  diagram's title. Mermaid also carries `config:` there; Unluminous has one theme and ignores it.
...
A diagram type Unluminous has no renderer for is not an error, and is not drawn as a broken diagram
either. It is a panel naming the type — `Unluminous does not draw a wardley diagram yet` — above the
...
keyboard's command-1/2/3, and `unluminous-cli editor view`.
...
means everywhere else in Unluminous and what `services::picture` already decided for a picture in a tab.
...
same reason: `unluminous-core::markdown` cannot know how wide the pane is, and until it knows that it
...
`language.renders` names a renderer that is **built into Unluminous**. It is read, checked against the
list of renderers this version has, and refused with a message if it names one Unluminous does not know —
...
**`unluminous-core`, with no window.** The bulk of it. Every diagram type has a parse test asserting on
...
**`unluminous-app`, with no window.** The plugin manifest parses and claims `.mmd`; `language.renders` is
...
and wrong at the same time**, and text is where the two come apart, because `unluminous-core` cannot
...
- **No Mermaid theme.** Unluminous has one palette, read out of the design, and a diagram is drawn in it.
...
- **No `click`, no `href`, no callbacks.** A diagram in Unluminous is a picture, not a control surface.
...
- **No icons or images inside nodes.** `fa:` and `@{ img: ... }` fall back to their text. Fetching is
  refused everywhere in Unluminous and a font of icons is a dependency for a rare case.
