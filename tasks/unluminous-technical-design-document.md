# Unluminous Technical Design Document
...
Board ticket: task-14 "Dev Unluminous Project" on the local Tasks and Remote Control board
Later work: the improvements asked for in `tasks/improvements.md` are implemented as well, and are described
in sections 10.4 to 10.9. The terminal has a design document of its own, `tasks/unluminous-terminal-tdd.md`,
because the decision behind it needed its own set of options and numbers.

## 1. What Unluminous is

Unluminous is a desktop text editor for macOS and Windows, written in Rust, that opens and saves `.md` and
...
behind Unluminous shows through, while the text stays fully opaque and readable.

Unluminous exists so that a writer can keep a reference window, a terminal, or a design open behind the
...
| Any text file | A file Unluminous has no special handling for, such as a `.js` or a `.rs` file, opens as plain text |
| Draggable panes | Every pane's size is set by dragging its edge, and a later pane must work the same way |
| Several windows | Several Unluminouss at once, each with its own project, and a list of recent projects to open one from |
...
| `Unluminous` first | The application's own menu is the first thing in the top bar, so it reads `Unluminous  File  Edit  View` |
...
ranges over the text, which is the same model Unluminous needs.

The problem is that `parley` does the job the ticket asks us to do. Adopting it means the interesting
part of Unluminous, laying out styled text and breaking it into lines, comes from a library. `parley` is
...
first at embedded devices, and it is good at that. For Unluminous the markup language is a poor fit,
...
```mermaid
flowchart TD
    subgraph app["unluminous-app: the shell, depends on egui"]
    ...
    subgraph core["unluminous-core: the editor, no interface dependencies"]
        ROPE["rope.rs\nB-tree text buffer"]
    ...
    subgraph term["unluminous-terminal: the terminal, no interface dependencies"]
...
The folders in `unluminous-app` are the ones `tasks/improvements.md` asks for, and what belongs in each is
...
The important edge is the dashed one. `unluminous-core` never asks how a glyph is drawn. It asks a
...
2. `editor_view.rs` turns it into a `unluminous_core::Command`, for example `InsertText`, `MoveCaret` or
   `ToggleBold`.
...
Ours is a plain sorted vector, because a document of the size Unluminous targets never has enough spans
...
at most 256 states, and Unluminous opens plain text files rather than very large ones. If a file ever turns
...
| `alacritty_terminal` | 0.26 | The terminal's escape sequence emulation and its pseudoterminal | Several hundred escape sequences and two platform pseudoterminal APIs. `tasks/unluminous-terminal-tdd.md` records the decision |
...
`unluminous-core` depends on `unicode-segmentation` and nothing else. That is deliberate: the editor
compiles and its tests run with no window, no graphics card and no fonts. `unluminous-terminal` follows the same
...
All of this lives in `unluminous-app/src/services/windows_transparency.rs` behind `#[cfg(windows)]`, with
...
so opening Unluminous on a large folder is not slow. Directories sort before files and both sort by name.
Only `.md` and `.txt` files are listed, because those are the file types Unluminous can open, and showing
...
`crates/unluminous-app/examples/sample_design.rs` reports, for each region of the image, the colour covering
...
Unluminous draws its own title bar. The window is created with no operating system decorations, because
...
styling only brightens the colour. `theme::install_fonts` hands egui the same family Unluminous is using, with
...
for a font family that has not been bound yet panics inside egui. `UnluminousApp::prepare` does it, and both the
...
as the space the font asks for between one line and the next, and Unluminous's renderer asks for more. Putting
it there leaves `unluminous-core`'s arithmetic exact and its layout tests independent of any platform.
...
Those cannot both be true. Unluminous keeps Helvetica, so that the labels in the window match what is actually
drawn.

## 10.2 Opening a folder, and listing what is in it

The `File` menu in the title bar holds `Open Folder`, `Open File`, `Save` and `Save As`. Unluminous draws its own
...
The explorer lists every file, not only the ones Unluminous can open. Version one listed `.md` and `.txt` only,
reasoning that showing a file that cannot be opened is worse than not showing it. That was wrong: an
explorer that hides most of a folder does not tell you what is in the folder. Files Unluminous cannot open are
...
The parser is ours, in `unluminous-core/src/markdown.rs`. It is the kind of thing the ticket's instruction to
write our own was aimed at, and it fits the existing design exactly: Unluminous already has a styled text model,
...
The preview is then laid out by `unluminous_core::layout` and painted by `editor_view::paint_text`, which is the
...
text, reference style links, nested block quotes and HTML. A table needs layout Unluminous does not have. The
...
one worth stating: what Unluminous saves is plain text and carries no formatting, so changing the font does not
change the file.
...
## 10.6 What files Unluminous opens
...
text, and a file larger than sixteen megabytes, which is more than Unluminous can read into the editor without
...
`File -> New Window` starts another Unluminous on the same folder, and `File -> Recent Projects` starts one on a
...
it stops it takes nothing with it. Unluminous already takes the folder to open as its first argument, which is all
...
`Unluminous`, `File`, `Edit` and `View`, in that order. On macOS they are in the bar along the top of the screen,
built with `muda`, because that is where macOS puts menus and `tasks/improvements.md` asks for it. On Windows
they are drawn at the left of Unluminous's own title bar, starting with `Unluminous`, and the three window buttons move
to the right hand end, where Windows puts them. Either way `Unluminous` is the first thing in the top bar.

Both bars are built from one list, `app/actions.rs`, and everything either of them can ask for is an
`Action`. `UnluminousApp::run_action` is the only place an action turns into a change, so the two bars and the
...
tests ask for it and click through it, which exercises the same list of menus. And `unluminous --print-menus`
...
It has a design document of its own, `tasks/unluminous-terminal-tdd.md`, because the decision behind it needed its
...
the screen snapshot the painter reads, and the resizing. They live in `unluminous-terminal`, which has no user
interface dependency, so the key encoding and the palette are tested by value with no window, and in
`unluminous-app/src/components/terminal_panel.rs`, which draws the grid out of the same glyph atlas the editor
draws from.
...
### Layer 1: unit tests on unluminous-core and unluminous-terminal
...
`unluminous-terminal` is tested the same way and for the same reason. The key encoding and the colour palette are
...
Unluminous did not. It does now, in `services::text_renderer`, and the family it draws the terminal in is Menlo
...
Pulled into `~/dev/unluminous/reference` at these commits. None of it is compiled into Unluminous; all of it
was read.
...
| [ab-glyph](https://github.com/alexheretic/ab-glyph) | `f02491a1bd5dda92263acb0af2e326b73ca28978` | Glyph metrics and rasterising. Also compiled into Unluminous. |
...
|---|---|
| The framework choice turns out wrong | `unluminous-core` has no interface dependencies, so replacing the shell does not touch the editor |
...
dividers between the panes were left are kept in `~/Library/Application Support/Unluminous` on macOS and
`%APPDATA%\Unluminous` on Windows, in two plain text files that can be read and corrected by hand. A file that
cannot be read is treated as a file that is not there, because Unluminous starting with its defaults is better
than Unluminous refusing to start over a stray line.

## 15. Milestones

1. The workspace, with `unluminous-core` and `unluminous-app` both compiling on macOS and Windows.
