...
It was rejected for this task, for reasons that are Unluminous's own rather than the protocol's:
...
- **The feature dies on most machines.** A server is a separate program per language —
  ...
  network. Unluminous fetches nothing, ever.
- **The costs are out of scale with the product.** rust-analyzer typically holds one to four
  ...
  twenty-second stalls while a monorepo loads. Unluminous's whole editor budget is a frame at sixty a
  second.
- **Nothing about it can be a screenshot test.** When a server answers, and with what, depends on
  the machine, the server version and the phase of its indexing. Every layout in Unluminous is
...
which is a runtime Unluminous deliberately does not have). A tree-sitter grammar is code, and Unluminous's
...
The third mechanism is the one Unluminous already half-has. Sublime Text's goto-definition is its
...
Unluminous's version of it: **definitions are read from the token stream the tokeniser already
...
### 3.1 `unluminous_core::symbols` — definitions are read from the tokens
...
simply absent for a `.css` file, which is Unluminous's rule for a control that cannot apply.
...
- **Cost, measured against what exists**: tokenising is 1.4 ms for a coloured 170 KB file and the
  whole-project text search reads 618 files in 20 ms on Unluminous's own repository, so a full index
...
- **One rule decides staleness, and it is the highlights' rule**: *a file that is open is owned by
  ...
  disk-owned side is refreshed per file when Unluminous saves one, and rebuilt when a project opens.
  A file changed outside Unluminous can therefore be briefly stale, and §4.2 shows why that cannot
...
it costs the one thing the search never has to pay: invalidation. A file edited outside Unluminous, a
...
`UnluminousApp::open_the_match` already does everything a jump needs: opens the file as a real tab,
...
`run_action` — which puts them in both menu bars and, through `unluminous-cli action list`, on the
...
IntelliJ's keys, because Unluminous's search modals already chose IntelliJ's. The existing menu test
...
dialog and VS Code hides it behind `Shift+Enter`; Unluminous has one modal where the preview *is* the
...
front so earlier ranges never shift later ones. Undo in Unluminous restores a snapshot, so the whole of
...
And the measuring instrument comes with it: `crates/unluminous-app/examples/symbol_cost.rs`, the
...
arms, documented in `unluminous-cli/docs/commands.md` under the existing doc-or-fail test:
...
The CLI path goes through `UnluminousApp::run_cli` into the same functions the modal uses — the rule
...
scenarios, each with its expected behaviour and the test layer that holds it (1 = unluminous-core unit
test, 2 = unluminous-app unit test, 3 = screenshot test, 4 = real window / CLI). "The mechanism works
...
- **Non-destruction**: for every rename applied to a buffer or file, the result equals the input
  ...
  test.
- **Isolation**: no function in `unluminous_core::symbols` performs I/O; the app-side workers touch
...
| Piece | Crate / place |
|---|---|
| `symbols` module: definitions, `identifier_at`, occurrences, ranking, `ReplaceMany` | `unluminous-core` (unit tests, no window) |
| `Grammar::definers`, `brace_definitions` + manifest parsing | `unluminous-core` / `services::plugins` |
| `symbol_index` worker, references search mode | `unluminous-app/services` |
| references + rename modal | `unluminous-app/components/references.rs` |
...
| `symbol_cost` example | `unluminous-app/examples` |
