...
The convention this settles on is tmux's, in Quill's spelling: **`--tab <index>`**, counting from 0,
...
`quill_terminal::Tabs` gains `at(index)` and `at_mut(index)`, which answer `Option<&Session>` and
...
3. The unit tests in `quill-terminal` for `at` and `at_mut`, beside the ones that already cover
   `active`.
4. A real window with two terminal tabs running different things, driven by `quill-cli`: a command
