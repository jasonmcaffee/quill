...
it holds.** Today the only way to read a value in Unluminous is to find the row in the variables tree at
...
- **No `toString()`/custom renderers.** IntelliJ's Data Views can call a method on an object to
  render it. That is the debugger's business, and both adapters Unluminous drives already render their own
  values.
...
| IntelliJ | What it does | Unluminous |
|---|---|---|
...
| **On-demand tooltip (Alt held)** | A 2009 option making the tooltip appear only with Alt held. | Not copied. Unluminous already spends the modifier: `Ctrl/Cmd`-hover is Go to Definition. §5.3. |
| **Inline values** | Values at the ends of lines, change-highlighted. | Already built, `task-1687` §8.5. Unchanged. |

The one thing worth naming that IntelliJ does **not** do, and Unluminous will not either: it does not keep
...
flowchart TD
    ...
    Q -->|no| X[nothing]
    Q -->|yes| E[unluminous_core::expressions::at<br/>the identifier plus its field path]
...
- **`unluminous_core::expressions`** — what text a point in a file is a question about. No window, no
  debugger, no allocation beyond the answer.
- **`unluminous_dap`** — the `hover` context on `evaluate`, the `setExpression` request, and the two
...
grammar Unluminous ships, so the walk needs no plugin key. This is deliberately **not** a new manifest
...
let people turn a *distraction* down and Unluminous's tick box already turns the distraction off. One
...
`supportsEvaluateForHovers`, a capability `unluminous_dap::Capabilities` has read since `task-1689` and
...
resumes, along with every `variablesReference` in it — `unluminous_dap::Request::resumes`' rule, which
...
So `unluminous_dap` gains `Request::SetExpression { expression, value, frame }` and
`Capabilities::set_expression` from `supportsSetExpression`, and the root's value is editable only
when the adapter offers it. **A control that can never apply is absent**, which is Unluminous's rule: with
...
`setVariable` support from adapters entirely; Unluminous's tile already sends `setVariable` and has been
...
being the seventh field in Unluminous to put its words against its own top edge. `Enter` applies and
`Escape` cancels.
...
`unluminous-cli debug hover` work either way, so there is no third value. It goes on the **Editor** page
...
```
unluminous-cli debug hover <path> <line> <column> [--expand <row>]
...
that word is. It answers with the expression Unluminous read, the value, the type, and the rows — the same
...
left is a panel, and Unluminous has a panel for variables that is one keystroke away and already docks to
...
1. **`unluminous-core`, no window.** §5.1's reading: the bare word, the field path, the `->` spelling, the
...
2. **`unluminous-dap`, scripted adapters.** The `hover` context reaches the wire; `setExpression` is
...
3. **`unluminous-app`, no window.** §8.1's liveness rule as a pure function of two rectangles and a point;
...
inside Unluminous and the exact thing `language.definers` and `task-1680`'s nine keys exist to prevent. So
...
`setExpression`, which would have meant that on the adapter Unluminous's own registry prefers, the
...
`unluminous-cli debug set-expression` takes the same two ways, so the command line and the popup cannot
disagree.
