# task-1695 — what happened when an agent drove Unluminous

A supervised observation of **Qwen 3.8 27B** (`Qwen3.8-27B-IQ4_XS`, llama.cpp, 96k window) driving a
real Unluminous window through the MCP server, across 23 scenarios covering every area the server offers.

Each scenario is phrased the way a person would say it — *"I want main.rs on the left and shapes.rs on
the right"* — rather than as a command. Unluminous is reset to a known state between scenarios, and each
one's result is checked by reading Unluminous's own state back through `unluminous-cli` rather than by believing
...
| Scenarios | 23 |
| Tool calls | 126 |
| …through Unluminous's MCP | 96 |
| …through the agent's own `bash`/`grep`/`read`/`edit` | **30 (24%)** |
| Refused calls | 9 |
| Scenarios where the agent used its own tools instead of Unluminous's | **13 of 23** |

## The headline: reachable is not the same as reached

Asked cold what it could do with Unluminous, the agent answered accurately and without a single call — the
...
- **git** — ran `bash git status` and `bash git diff`. Unluminous has `git status` and the whole Git menu.
- **find every use of a name** — ran its own `grep`. Unluminous has `editor references`, which classifies
  each hit as code, comment or string, and reads open tabs as they stand rather than as the disk has
  them.
- **go to a definition** — ran `grep '(struct|class|type|interface|typealias)\s+Rect'`. Unluminous has
  `editor definition`.
- **make a folder and a file** — ran `bash mkdir -p tests && touch tests/smoke.rs`, then
  `bash find .` to show the tree. Unluminous has `explorer new-folder`, `new-file` and `tree`.
- **rename a symbol across the project** — grepped, read three files, and ran three
  `edit --replaceAll`. Unluminous has `editor rename`, which is one undo step per file, leaves comments
...
`README.md`, which Unluminous's rename would have left alone by default. It wrote three files behind the
back of a window that had two of them open. And it was not one undo step. **Unluminous's most valuable
refactoring feature was bypassed and the result was worse than Unluminous's would have been.**
...
occurrence first, and having grepped it no longer needs Unluminous.

The other cause is that nothing in the tool descriptions says what Unluminous knows that `grep` does not.
...
— all through Unluminous, all correctly — and then answered:
...
`total = 35.5` was there, behind nineteen stack frames, fourteen of which Unluminous itself marks
...
its own repository after the window opened, Unluminous reported:

```
root: C:/jason/dev/unluminous   branch: main   changed: 0
```

…while the project itself was `scratch-project`, on `master`, with nine changes. Unluminous was answering
...
`git action commit` in that state would have committed to the wrong repository. Unluminous already has the
...
model writing JSON reaches for camelCase by default. Unluminous already decided that "a name written with
...
In s07 the agent read `.unluminous/run-configurations.conf` directly — Unluminous's own state file, whose
...
| | |
|---|---|
| `task-1699` | Make Unluminous's own answer the one an agent reaches for — `editor definition [name]`, and descriptions that say what Unluminous knows that `grep` does not |
...
unluminous-cli launch _agent_output/agent-study/scratch-project --no-wait
...
of tool calls that went round Unluminous rather than through it: **24% when this was written.**
