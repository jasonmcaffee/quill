...
3. There is no way to change the size without opening Settings. **Pinch** on a trackpad and
   **Ctrl/Cmd with plus, minus and zero** change it in place, and what they change is the same global
   setting, so it is still there next time Quill starts.
...
- **Not** a per-file font. IntelliJ has one editor font; so does Quill.
- **Not** egui's own zoom. `Ctrl+=` in egui scales the whole interface, menus and all. That is not
  ...
  switched off and the key presses become Quill's own actions.
...
Quill draws a 44-point strip under the title bar holding bold, italic, underline, strikethrough, five
...
For a code file every one of those controls is meaningless. Quill saves plain text and carries no
...
`QuillApp::apply_settings` applies the change to `self.document_mut()`, which is the **active tab
only**. Open three files, change the font, and the two tabs that were not showing keep the old one
until Quill is restarted. The Markdown preview keeps it too, because `self.preview` is rebuilt from
...
```mermaid
flowchart TD
  subgraph app["quill-app :: app"]
    A["QuillApp::ui"]
    R["QuillApp::run_action"]
    S["apply_settings"]
    Z["zoom_from_the_pointer"]
  end

  subgraph comp["quill-app :: components"]
    T["toolbar::show"]
  ...
  subgraph svc["quill-app :: services"]
...
///
/// Quill saves plain text and carries no formatting to disk, so formatting is how a document is
...
is very often the beginning of a Markdown file, and is the one Quill starts with. The `View` menu's
...
here so it is not re-proposed: every other icon in Quill is vector strokes tinted at the point of
...
`QuillApp::ui` currently reserves `size::TOOLBAR` unconditionally. It becomes:
...
/// nothing onto any document's undo history and marks no file as changed, because what Quill
...
| `Reset Font Size` | none | back to 16, the size a new Quill has |
...
| **A generated `F` image from Ideogram.** | A raster cannot be tinted for hover or drawn at the window's scale without resampling, and would be the one icon in Quill that is not vector strokes. Recorded above so it is not proposed again. |
...
**`quill-app` screenshots, through the real window:**
...
- **The tick on a menu row was an empty box.** `controls::menu_row` drew the character at U+2713, and
  no font in the stack Quill hands egui has a shape for it. It is the exact fault the style guide
  already records for the shift symbol at U+21E7, and it was on `Raw Markdown` on the `View` menu in
  every capture of it. It uses `icon::tick` now, which is the tick every tick box in Quill draws.
