# Shipping Unluminate: an installer for Windows and for macOS
...
Until now Unluminate is run with `cargo run --release`. That is fine for the person writing it and no use
...
exception is deliberate and is argued for in section 3: `unluminate.exe` gains an icon and a version
...
**Nothing is invented that the machine already knows.** The same argument `unluminate-git` makes for
shelling out to `git`: the platform has an installer format, a shortcut format, an uninstall
registry and a bundle layout, and Unluminate should produce exactly those rather than something adjacent
to them.
...
installer/
  ...
  icon/                the Unluminate mark: the generator, and its output
    ...
    unluminate.ico          committed, so a build needs no generator
    unluminate.icns         committed, for the same reason
    Unluminate.iconset/     the PNG set, in the names `iconutil` on a Mac reads
  windows/
    unluminate.iss          the Inno Setup script
    license.txt        shown on the licence page
    build.ps1          cargo build -> ISCC -> dist/UnluminateSetup-<version>-x64.exe
  macos/
    Info.plist         the bundle's manifest, with the version substituted in
    build.sh           cargo build -> Unluminate.app -> dist/Unluminate-<version>.dmg
...
`build-git-demo.ps1` — and rejected: `tools/` is scripts that help while working on Unluminate, and this
is how Unluminate is delivered.

---

## 2. The mark

Unluminate has a palette read out of a screenshot and a rule that icons are drawn rather than lettered,
...
`build.sh` still prefers `iconutil` when it is on the machine, feeding it `icon/Unluminate.iconset`, whose
file names are Apple's own `icon_<size>x<size>[@2x].png`, and falls back to the committed
`unluminate.icns`. Apple's tool is the definition of the format; the hand-written file is there so that the
icon can be rebuilt on a machine that is not a Mac, and so that a checkout with no generator run still
has one.

**The output is committed.** `unluminate.ico` is a build input for `unluminate.exe` itself (section 3) and the
generator is a second workspace with its own dependencies. A checkout must build without running it.

---

## 3. `unluminate.exe` carries its own icon and version
...
button and the Alt-Tab entry all read the exe. So `unluminate.exe` gets one.

`crates/unluminate-app/build.rs`, active only on Windows, uses `winresource` to compile a resource script
...
`installer/icon/unluminate.ico` and fills in `ProductName`, `FileDescription`, `CompanyName`,
...
Windows SDK still produces a working `unluminate.exe`, just an unlabelled one; only the installer needs
...
**Inno Setup**, then, at 6.7. What `unluminate.iss` says, and why:
...
- **Per-user by default, per-machine offered.** `PrivilegesRequired=lowest` with
  ...
  `%LOCALAPPDATA%\Programs\Unluminate` with no elevation prompt at all, and the dialog offers all users
...
- **`CloseApplications=yes`**, so upgrading while Unluminate is open asks to close it through the Restart
  Manager rather than failing on a locked file.
- **Four optional tasks**, all offered on one page: a desktop icon, `unluminate` on the `PATH`, an *Open
  with Unluminate* entry on the right click menu of a folder and of a file, and the file associations.
- **Associations are polite.** Unluminate is registered as an application that *can* open `.md`,
...
- **Everything it writes, it removes.** The `PATH` entry, the shell keys and the association entries
  ...
  does **not** touch is `%APPDATA%\Unluminate` — the settings, the pane sizes, the recent projects and any
...
**Unluminate does not answer it**. The Restart Manager sends a running program a request to shut down;
Unluminate's window ignores it, so setup reports "Setup was unable to automatically close all
...
`CloseApplications=force` would have the Restart Manager terminate Unluminate outright. **No**: it would
throw away unsaved changes in a text editor, silently, to save a person one click.

Teaching Unluminate to answer would be the real fix, and it is a change to the application rather than to
its packaging — it belongs in `unluminate-app`, not here, and it also matters for a Windows restart, which
today force-closes Unluminate for exactly the same reason. It is written down here as the follow-up it is.
...
`/Applications` — and Unluminate in particular asks the operating system for a transparent, undecorated
...
`build.sh` builds the layout by hand — `Contents/MacOS/unluminate`, `Contents/Resources/Unluminate.icns`,
...
The plist declares `CFBundleIdentifier` `com.jasonmcaffee.unluminate`, the version from `Cargo.toml` in
...
will not run and it is not obvious which of the two changes did it. Unluminate needs no entitlements under it:
...
application *inside* the image said `Unluminate.app does not have a ticket stapled to it`. It was accepted
...
The disk image is `hdiutil create` over a staging folder holding `Unluminate.app` and a symlink to
`/Applications`, converted to a compressed read-only `UDZO` image.

It is written to `releases/unluminate-<version>.dmg` rather than left in `installer/dist/`. Those are two
...
`build.sh` went from a checkout to `Unluminate.app` and `Unluminate-0.1.0.dmg` on the first run. Both of the
...
`unluminate.icns` being used.

What that run showed, beyond the two files existing. `mdls` reports the bundle's display name as
`Unluminate` and its version as `0.1.0`, which is what the Dock and the menu bar read, so the bundle does
the job it was added for. `codesign --verify --deep --strict` passes on the bundle, on the copy in
`/Applications` and on the copy inside the mounted image. The image mounts with `Unluminate.app` beside an
...
1. `build.ps1` produces `installer/dist/UnluminateSetup-<version>-x64.exe`.
2. It is installed silently on this machine, and then: the Start Menu has **Unluminate**, the desktop has
   **Unluminate**, `unluminate --print-menus` answers from a new shell, *Open with* on a `.md` file lists Unluminate,
   right clicking a folder offers *Open with Unluminate*, and Add or Remove Programs has one entry with
   the icon, the version and a publisher.
3. `unluminate.exe` is run from the installed location, on a real folder, and the window comes up — which
...
4. It is uninstalled, and the install directory, the shortcuts, the `PATH` entry and the registry
   keys are all checked to be gone, while `%APPDATA%\Unluminate` is checked to still be there.
...
1. `installer/macos/build.sh` produces `installer/dist/Unluminate.app` and `installer/dist/Unluminate-<version>.dmg`.
...
3. The image is mounted and holds `Unluminate.app` beside an alias to `/Applications`, with the signature
