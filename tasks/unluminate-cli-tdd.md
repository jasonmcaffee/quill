# Unluminate CLI — technical design

`task-1661`

## Introduction

Unluminate can only be driven by hand. Everything it does — opening a file, starting a terminal, running a
...
This document designs `unluminate-cli`: a command line that drives a **running** Unluminate window, a control
channel for it to speak down, and a rule that keeps the two from drifting apart as Unluminate grows. The
...
| **G3** | Several Unluminate windows at once, each addressable, since a project is a window. |
| **G4** | An agent given only `unluminate-cli/docs/commands.md` can drive Unluminate. Measured, at **≥97%** on a scored assessment. |
...
| | |
|---|---|
| **N1** | Not a headless Unluminate. It drives a window that exists; it is not a way to edit files without one. `sed` already exists. |
...
Unluminate's architecture already has the right shape for this and does not use it. `app::actions::Action`
is the one thing a menu or a shortcut can ask for, and `UnluminateApp::run_action` is the one place an
...
- **An agent cannot use Unluminate at all.** This is the ticket's actual ask, and there is no partial
...
flowchart LR
  ...
  subgraph app["unluminate.exe — the window"]
    listen["services::control<br/>listener thread"]
    queue(["queue of Pending"])
    frame["UnluminateApp::ui — a frame"]
...
**One catalogue, two halves.** `unluminate-cli` is one crate with a library and a binary. The library holds
the command list, the wire format and instance discovery; `unluminate-app` depends on it. The client parses
against the same list the window dispatches on, so a command the CLI accepts is a command the window
knows. The dependency points one way — the client never depends on the window — so `unluminate-cli` stays a
small, fast program with no graphics card and no fonts behind it.

**The window answers, not the thread.** The listener never touches `UnluminateApp`. It queues the request,
...
| Where | What |
|---|---|
| `unluminate-cli/src/catalogue.rs` | Every command: area, verb, positional arguments, flags, summary, examples. The single list. |
...
| `crates/unluminate-app/src/app/action_names.rs` | A plain name for every `Action` and `GitAction`, and the test that every menu entry has one. |

### The command surface

`unluminate-cli <area> <verb>` — the noun first, as `docker container create` and `dotnet tool install`
...
parts of the window, so somebody who can see Unluminate can guess the area.
...
A file per window, because a project is a window and several Unluminates run at once — so there is no
...
|---|---|
| 0 | It worked |
| 1 | Unluminate refused it (`not-found`, `not-applicable`, `failed`) |
| 2 | The command line was wrong (`usage`, `unknown-command`) |
| 3 | No Unluminate running / unreachable (`not-running`, `refused`) |
| 4 | Several Unluminates, none named (`several-instances`) |
| 5 | Reached but too slow (`timed-out`) |

The split is what a script cares about: 2 is the caller's mistake, 1 is Unluminate's answer, 3–5 are about
...
```mermaid
sequenceDiagram
  participant U as unluminate-cli
...
| `git action --wait` | `unluminate_git::Worker` | git runs one command at a time on a thread. |
...
**It can be closed.** `unluminate --control off`, or `UNLUMINATE_CONTROL=off`. Open by default, because a
...
| The client depends on `unluminate-app` | Rejected: drags eframe, wgpu and fonts into a program that should start in milliseconds. |
...
| The client asks the window for the catalogue at startup | Rejected: a round trip before every command, and `--help` would need a running Unluminate. |
...
Unluminate's four layers, and where this work lands in each.

**1. Unit tests with no window** — `unluminate-cli` (43) and the naming rules in `unluminate-app`:
...
**3. Integration through the real window** (`crates/unluminate-app/tests/cli.rs`): build a `UnluminateApp`
...
**4. The real application.** A script that launches `unluminate.exe`, drives it with `unluminate-cli`, takes a
**screenshot after each step**, and leaves the pictures under `_agent_output/task-1661-unluminate-cli/` to
...
run against a live Unluminate and checked. The bar is **97%**, and the result — after the bar is reached —
goes in `unluminate-cli/agent-assessment/qwen-38-27B-assessment.md`.
...
| A test opens a port or writes an instance file into the person's settings. | `open_control_channel` is called from `main.rs` and nowhere else, exactly as `load_settings` and `restore_project` are. The control tests point `UNLUMINATE_INSTANCES` at a folder of their own and take turns under a mutex, because an instance file is named after the process id. |
| `unluminate-cli` cannot find `unluminate`. | It looks beside itself first — where both a build and an installation put them — then at `UNLUMINATE_BIN`, then the path, and says which it tried. |
