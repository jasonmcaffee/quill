# Quill MCP — technical design

`task-1679`

## Introduction

`task-1661` gave Quill a command line and, with it, a way for an agent to drive the editor. The
measurement at the end of that ticket is the reason this one exists: the local Qwen 3.8 27B, handed
only `quill-cli/docs/commands.md`, carried out 64 instructions phrased as a person would say them
...
server: what it exposes, how it stays in step with the CLI as Quill grows, where it runs, how it is
switched on, and how it is installed into the two agents on this machine.

The one thing that must not happen is a second list of what Quill can do. `catalogue.rs` is one
...
| **G2** | An agent can *discover* what Quill does from the tool list alone, without being handed a document first. |
...
| **G6** | No new way into the window's state. Every tool call goes down the control channel that already exists, so `QuillApp::run_cli` stays the one place a command turns into a change. |
...
| **N3** | Not a headless Quill. It drives a window that exists. |
| **N4** | Not a second protocol into the window. The MCP server is a **client** of the control channel, not a peer of it. |
| **N5** | No prompts capability, and no tool that summarises a file or writes code. Quill's job here is to be operated, not to think. |

## 1. What the research says, and what it changed

### 1.1 The protocol moved twice while Quill was being written
...
**The figures against the code as built**, which are what `quill-cli mcp tools --count` prints and
...
- **`grouped` is the default.** One tool an area — `quill_tab`, `quill_editor`, `quill_git` — with
  the area's verbs as an `enum` on a `command` property and every verb's usage line and summary in
  the description. The agent still sees everything Quill can do; it sees it as fourteen paragraphs
  instead of ninety-seven schemas.
- **`every` is there for permissions.** Claude Code allows a tool to be permitted by name —
  `mcp__quill__quill_tab_open` — and that is only useful if `tab open` is a tool of its own. Somebody
  who wants "may open tabs, may not quit" needs this shape and should pay the context for it.

`mcp.tools` chooses, and the number of tools each produces is printed by `quill-cli mcp status` so
...
flowchart LR
  ...
  subgraph server["quill-cli mcp serve"]
  ...
  subgraph quill["A running Quill"]
    ctrl["services::control<br/>loopback, token, one JSON object a line"]
    cli["QuillApp::run_cli<br/>the one place a command becomes a change"]
    ctrl --> cli
  end
  cc -- stdio --> io1
  cx -- stdio --> io1
  rpc -- "the same request quill-cli sends" --> ctrl
  quill -. "hosts, when mcp.enabled" .-> io2
```
...
into exactly the request `quill-cli` would have sent — the same wire name, the same arguments
...
**It lives in `quill-cli`, not in `quill-app`.** The dependency already points that way —
`quill-app` depends on `quill-cli` and never the reverse — so putting the server in the CLI crate
...
**One server can drive every window.** Because it addresses Quills through the instances folder
rather than through its own process, the server hosted by one window can drive the one next to it.
That turns the obvious port collision — two Quills, one `mcp.port` — from a bug into the intended
...
| File | What is in it |
|---|---|
| `quill-cli/src/mcp/mod.rs` | What an MCP server is for, and the module comment carrying the decisions in §1. |
| `quill-cli/src/mcp/tools.rs` | The catalogue turned into tool definitions. `Shape::Grouped` and `Shape::Every`. Pure — no socket, no process, no window. |
| `quill-cli/src/mcp/server.rs` | The JSON-RPC methods, and turning a tool call into a `protocol::Request` and a `protocol::Reply` back into an MCP result. Takes a `Driver` so it can be tested without a Quill. |
| `quill-cli/src/mcp/stdio.rs` | Lines in on stdin, lines out on stdout. |
| `quill-cli/src/mcp/http.rs` | The Streamable HTTP endpoint: a small HTTP/1.1 reader, `POST /mcp`, `GET`/`DELETE` → 405, loopback bind, origin checking. |
| `quill-cli/src/mcp/install.rs` | The Claude Code and Codex configuration: what it should say, how to write it, and how to read back whether it is there. |
| `quill-cli/src/mcp/base64.rs` | Twenty lines, so a screenshot can be returned as an image. Written rather than depended on, which is the same decision `protocol.rs` made about serde's derive macros. |
| `quill-cli/docs/mcp.md` | What a person needs to connect a client, alongside `docs/protocol.md`. |
| `crates/quill-app/src/services/mcp.rs` | The window's half: start and stop the HTTP listener as the settings change, and report what it is doing. |
| `crates/quill-app/src/components/mcp_page.rs` | `Settings → Tools → MCP`. |
...
```json
{
  ...
  "inputSchema": {
    ...
    "properties": {
      ...
      "instance": { "type": "string", "description": "Which running Quill: its process id, its port, or part of its project path. Only needed when several are running." }
    },
    "required": ["command"],
...
One tool per command, named `quill_<area>_<verb>` with hyphens turned into underscores, since
...
both shapes and there is no fifteenth tool that duplicates `quill_window`.

### 4.4 One resource

`quill://commands.md` serves the whole of `quill-cli/docs/commands.md`, embedded at compile time
...
### 4.5 Which Quill a call goes to

In order, first answer wins:

1. `instance` on the tool call, or `--instance` on `mcp serve`, or `QUILL_INSTANCE` in the
...
3. Exactly one running Quill whose project folder is the server's own preference — its working
...
`quill-cli mcp serve`. The client launches it as a subprocess and speaks newline-delimited JSON-RPC
...
`quill-cli mcp serve --transport http --port <n>`, and the same server hosted inside Quill when
...
|---|---|---|---|
| `mcp.enabled` | true or false | **false** | Whether this Quill hosts the MCP endpoint over HTTP. |
...
`quill-cli settings set` takes, which is the rule the other eleven settings already follow.
...
- **The port is already in use.** Almost always the Quill in the next window. One server drives every
...
| Command | |
|---|---|
| `mcp serve` | Run the server. `--transport stdio\|http`, `--port`, `--tools`, `--instance`. Local — it needs no Quill to start, only to be useful. |
| `mcp status` | What this Quill is doing about MCP: enabled, the port, the shape, how many tools, and whether it is actually listening. Not local — the window is the only thing that knows. |
...
`cargo run -p quill-cli --example reference`, and each carries an example that a test parses and
...
| **T4** | A `tools/call` becomes the right `Request`: `quill_tab {command:"open",arguments:{path:"README.md"}}` sends `tab.open` with `{"path":"README.md"}`. Against a stub driver, no Quill needed. |
...
| **T13** | End to end, by hand and recorded on the ticket: a real Claude Code and a real Codex connected to a real Quill, listing the tools and driving the window. |
...
never designed to be operated by a language model. Quill's catalogue was: `task-1661` designed it
...
**A second binary, `quill-mcp`.** More to install, more to keep on the path, and a second program to
be out of date. `quill-cli mcp serve` is one binary that is already installed next to `quill.exe`.

**A session and an SSE stream.** Streamable HTTP allows both and needs neither. Quill has nothing to
...
3. `mcp::stdio`, and connect a real Claude Code to a real Quill by hand.
...
Quill when the other Quill was itself a moment earlier. `Drop` now joins the accept thread, which
...
ask for it, and `current_exe` in the window is `quill.exe`. `install::quill_cli_program` works it
out the way `client::quill_program` does in the other direction — this program if it is already the
client, then the one beside it, with `QUILL_CLI_BIN` overriding — which is also what lets the
