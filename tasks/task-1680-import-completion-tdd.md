...
way `language.definers` already is, and Quill's other three code plugins have to be able to ask for
it.
...
asks by hand and the `quill-cli editor complete` that prints the same list. **None of that changes.**
...
### 2.3 The syntactic tier, which is the one Quill is on
...
follows exactly — a language describes its imports as data, and Quill does the arithmetic.
...
| "classes, functions, etc that I can import" | the target file's definitions, from the same `quill_core::symbols` reading that `Go to Definition` uses, filtered to the exported ones | the same tier as `Go to Definition`: a syntactic reading, and it says so |
...
costs: leaving `target`, `node_modules` and `__pycache__` out took Quill's own project from 2,022
...
same text `quill_core::syntax` already reads, and it would drift from it, which is the exact fault
...
Three pieces, in the three places Quill already puts these things:

- `quill_core::imports` — *what the caret is in the middle of*. Pure: a `&str`, an offset and a
  `Grammar` in, a `Context` out. No disk, no window, tested with no fonts. The sibling of
  `quill_core::completion`, which says what a stem is, and of `quill_core::symbols`, which says
  what a definition is.
- `quill_app::services::imports` — *what that context could become*. This is where the disk is:
...
- `quill_app::app::completion` — one new branch, four lines long: ask `quill_core::imports` first,
...
accepted by the same `Command::ReplaceMany`, and printed by the same `quill-cli editor complete`.
...
```
```rust
use quill_core::completion::{Candidate, Row};   //  the module is a path of segments
```
...
`use quill_core::completion::{Candidate, R│ow};`
...
`path_roots` is checked against the three meanings Quill has, exactly as `language.definers` is
...
alternative is a list of languages inside Quill — a `match` on the extension saying that `.ts`
...
detail is the path relative to the project root — `crates/quill-core/src/completion.rs` — which is
...
Accepting does **not** add a closing quote, a comma, a `from` clause or a `;`. Quill's editing area
...
first because `services::preview_images` already established that nothing in Quill fetches, the
...
```
crate::app::actions        from  crates/quill-app/src/components/editor_view.rs
  crate     ->  the source root above this file      ->  crates/quill-app/src
  app       ->  a folder or an `app.rs`              ->  crates/quill-app/src/app
  actions   ->  a folder or an `actions.rs`          ->  crates/quill-app/src/app/actions.rs
```

```
quill_core::completion
  quill_core  ->  a package folder whose name reads `quill_core`, and its source root
                                                    ->  crates/quill-core/src
  completion  ->                                    ->  crates/quill-core/src/completion.rs
```
...
repository**, measured by `cargo run --release -p quill-app --example completion_cost`. Import
...
importable file in the project — 618 files on Quill's own repository, and a `.ts` project of that
...
second copy of a subset of names the table already holds, and on Quill's own repository that is
...
`cargo run --release -p quill-app --example completion_cost`, run against Quill's own repository
after the implementation:

| Inside an import | offered | total |
|---|---|---|
| `use ` | 12 | 0.345 ms |
| `use quill_core::` | 14 | 0.874 ms |
| `use quill_core::completion::` | 12 | 0.530 ms |
| `use quill_core::completion::Can` | 1 | 0.579 ms |
...
Without it, `use quill_core::│` would need one more keystroke before the list came back. It changes
...
**The command line.** `quill-cli editor complete` prints whatever the popup would show, and gains
...
19. `use quill_core::comp│` is a `Segment` with segments `[quill_core]` and stem `comp`.
20. `use │` is a `Segment` with no segments and no stem.
21. `use quill_core::│` is a `Segment` with segments `[quill_core]` and no stem.
22. `use quill_core::completion::{Candidate, R│}` has segments `[quill_core, completion]`.
23. `use quill_core::{a, b::c│}` has segments `[quill_core, b]` — the sibling list is stepped over.
24. `pub use quill_core::comp│` reads the same as `use` does.
...
35. `crate::app::actions` resolves from a file under `crates/quill-app/src`.
36. `quill_core::completion` resolves through the package folder whose name reads `quill_core`.
...
| Where | What |
|---|---|
| `quill-core/src/imports.rs` | new. `Context`, `context_at`, and the two backwards walks. |
| `quill-core/src/syntax.rs` | `Grammar` gains the nine import fields and `completes_imports`. |
| `quill-core/src/symbols.rs` | `Definition` gains `exported`, set by `language.export_keyword`. |
| `quill-core/src/completion.rs` | `Source::Module`; `rank_all` beside `rank`. |
| `quill-app/src/services/plugins.rs` | reads the nine keys; refuses an unknown `path_roots` meaning. |
| `quill-app/src/services/imports.rs` | new. Resolving a specifier and a segment path, and listing what each could become. |
| `quill-app/src/services/symbol_index.rs` | `Entry` gains `exported`; `Index` gains `exports_of`. |
| `quill-app/src/app/completion.rs` | the import branch, the trigger changes, and the accept range. |
| `quill-app/src/app/cli.rs` | the one guard that has to allow an empty stem with rows. |
| `quill-app/plugins/*/plugin.conf` | TypeScript, JavaScript, CSS and Rust say how their imports look. |
| `quill-app/tests/screenshots.rs` | a TypeScript fixture project and two pictures. |
| `quill-app/examples/completion_cost.rs` | an import arm, so §7's number is measured. |
