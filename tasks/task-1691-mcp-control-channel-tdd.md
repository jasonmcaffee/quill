...
An agent was asked to drive an Unluminous window entirely through the Model Context Protocol: write a
...
With the window idle, every tool call and every `unluminous-cli` command fails with `timed-out: Unluminous did
...
four `unluminous-control-connection` threads each parked on a semaphore inside `control::serve`, and the
...
not a reliable way to wake a window. Two mechanisms lose it, and both are in code Unluminous does not own:
...
and the two places the request is dropped are both downstream of the proxy. Unluminous would have to stop
...
loop {
    match wait.recv_timeout(NUDGE) {
        ...
        Err(RecvTimeoutError::Disconnected) => return /* Unluminous is closing */,
...
`.unluminous/run-configurations.conf` afterwards, `run remove bogus` was gone afterwards, and `run rerun
primes` had run.
...
```
Unluminous did not answer run.list within 15000 ms. It has not drawn a frame for 65.0 s and has 4
...
```
Unluminous did not answer git.action within 15000 ms. It drew a frame 16 ms ago, so it is busy rather
...
The first attempt at running the prime numbers file used `node primes.js`. Unluminous spawns without a
...
`isError` false. The same command through `unluminous-cli` prints `Unluminous could not start node: Failed to
...
`unluminous-cli`'s exit code right at the same time.
...
list` fails in half a second. The same rule is applied to `unluminous-cli`'s own `client_timeout`, because
...
the wire is exactly what `unluminous-cli` would have sent — the rule `mcp::driver` is built on.
...
the paragraph reads well. `unluminous-cli mcp tools --count` is how it is measured again.

## 6. What is deliberately not here

- **Leaving `eframe`.** §1.3. The channel is fixed from outside the event loop, which is where Unluminous
  sits.
...
- `a_command_that_does_not_wait_is_given_exactly_the_deadline_it_asked_for` and
  ...
  in `unluminous-cli`'s `client_timeout`.
...
by itself — Windows delivers a redraw to a covered window, so an idle Unluminous here answers `tab list`
...
```
Unluminous did not answer tab.list within 3000 ms. It has not drawn a frame for 21.2 s and 1 request
...
`NtSuspendProcess` on the **whole process** is the harder shape, where nothing in Unluminous can run at
...
And the other three, through the MCP server over stdio and through `unluminous-cli`:

```
tools/list                        18 tools, 0 without a timeout property
tools/call unluminous_tab timeout 800  754 ms          (the fifteen second floor is gone)
run start ghost                   isError true, "failed: Unluminous could not start
