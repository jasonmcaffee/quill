...
scripts can be started from inside Unluminous. Clickable from the top right of the title bar. A run panel
...
Zed's tasks are the closest existing thing to Unluminous's temperament: a `tasks.json` of plain data —
...
with arbitrary UI. Unluminous's plugins are data and nothing in one is executed; a typed form per
language would mean either widening `plugin.kind` to code — the seam `services::plugins` names and
deliberately leaves empty — or hard-coding a form per language into Unluminous, which is the same
...
`unluminous_terminal::Session` already runs an arbitrary program in a pseudoterminal —
`SessionSettings { shell, args, working_directory }` — and already receives the child's exit code
(`Event::ChildExit`, currently discarded). The output pane is the terminal stack Unluminous has.
...
- Running one **spawns a `unluminous_terminal::Session`** with the program in place of the shell, so
  output is a real terminal and stopping is killing a process Unluminous owns.
...
- Configurations are **stored in `.unluminous/run-configurations.conf`**, beside the project state that
...
- Everything is reachable from **`unluminous-cli run`**, which also makes every part of it an MCP tool an
...
space in it is written in quotes. The split is done by Unluminous and the arguments are passed to the
...
`.unluminous/open-files.txt` follows, for the same reason: the project may move. The program is found the
...
environment Unluminous itself started with — never replacing it, because a child with no `PATH` is a bug
...
`.unluminous/run-configurations.conf`, read by `services::store::Values` like every other file Unluminous
...
chose one file for the same reason. It lives in `.unluminous` because a run configuration belongs to the
*project* — the command that starts this server is a fact about this folder — which is the decision
IntelliJ's *Store as project file* tick lets you make and Unluminous simply makes. What is per-person
...
written to disk**; IntelliJ writes its temporaries into `workspace.xml` and Unluminous deliberately does
...
program as Ctrl+C — the encoding `unluminous_terminal::keys` already owns. A program still alive after a
...
zero. The grid is never written into by Unluminous: IntelliJ prints its epilogue into the console, but
...
- **Stop**, drawn only while the selected configuration runs — a control absent when it cannot
  apply, Unluminous's rule — killing by §5.3.
...
`Done`. Changes are written to `.unluminous/run-configurations.conf` when the dialog closes, by the
...
and Unluminous already has a JavaScript plugin. A separate Node plugin would be a plugin with no
...
- **`run.project = <name>`** — names a **project detector built into Unluminous**, checked against
...
the flyout and the dialog. The parsing of `package.json` and `Cargo.toml` is Unluminous's own code,
...
A new area `run` in `unluminous-cli/src/catalogue.rs`, one arm each in `app/cli.rs`:
...
temporary folder and no window. `UnluminousApp` holds a `RunPanel` the way it holds the
...
1. **`unluminous-terminal`**: `SessionSettings.env` reaches the child; a spawned `cmd /c exit 3` (and
   `sh -c 'exit 3'` on macOS) ends with `running` false and `exit_code == Some(3)`.
2. **`unluminous-app` units**: the conf file round-trips, a block missing its command is dropped whole,
...
4. **The real window**: run a node server from the widget, watch it in the tile, stop it; then the
   same three from `unluminous-cli run` — which also exercises `run output` for the agent case.
...
- **Before-launch tasks and compound configurations.** Cargo builds before it runs and npm scripts
  ...
  Unluminous configurations, and a `run.N.before` key slots into the format without migration.
...
- **A Services window.** Aggregation pays at a dozen simultaneous runs; the tile's tabs are that
  window at Unluminous's size.
- **Allow-multiple-instances.** Two names make two runs (§5.2).
- **Folders, favourites and pinned tabs** — organisation for a hundred configurations a `.unluminous`
