...
Two of them also need something Quill has never had, and it is the same thing: **the explorer has to
...
defensible, different answer, and it is the one Quill is better placed to give than the editors
above, because Quill saves plain text and nothing else. There is no format conversion, no "save as"
...
2. **There is no single right spelling of a specifier**, which is why VS Code makes it a setting.
   Quill already decided this in `task-1680`: a relative specifier is the one that is always right.
3. **Say what tier you are on.** VS Code's is a type checker. rust-analyzer's is a compiler front end
   that still cannot do the whole job. Quill's is neither, and §4 says exactly what that buys and
   what it costs.
...
| A modified tab is written when it is closed | `QuillApp::close_tab` |
| A row is dragged onto a folder | `components::explorer` reports it; the window settles it |
| Where a file's references are and what they should say | `quill_core::imports` reads, `services::file_move` decides |
| Doing it | `QuillApp::move_path`, which `Rename...` now goes through too |
...
built from the token stream Quill already produces, and chose the third. Every reason holds here and
...
already exist in `quill_core`:

- For the **quoted** family — TypeScript, JavaScript, CSS — a module specifier is a string, and
  whether a given string *is* one is `quill_core::imports`' existing question, asked forwards over
...
`quill-cli explorer move --dry-run` prints the whole change set without touching anything, which is
...
`crates/quill-app/src/services/file_clipboard.rs` is `quill_app` + `["services", "file_clipboard"]`,
and `crates/quill-app/src/services/mod.rs` is `quill_app` + `["services"]`.
...
`Terminal`, and `QuillApp::selected` holds the path the explorer's own cursor is on.
...
`QuillApp::close_tab` is the one place a tab is closed — the tab's cross, `Ctrl+W`, the tab menu, the
...
**It asks first**, through the one confirmation Quill already has. To do that, `QuillApp::Confirmation`
...
- On **Windows** it is the Recycle Bin, through `SHFileOperationW` with `FOF_ALLOWUNDO`. That is one
  feature flag on the `windows-sys` dependency Quill already has for the window's transparency, and
  no new crate.
...
no new crate and cannot be compiled, let alone tested, on the machine Quill is built on.
...
**The component reports and decides nothing**, which is Quill's rule for a component and is the same
...
**What the window does with it** is `QuillApp::move_path(from, to)`, which is §4 and which
...
|---|---|
| `quill-cli explorer select [path]` | Set the explorer's selection, or print it |
| `quill-cli explorer delete <path>` | Delete, without the question — a command line *is* the deliberate act |
| `quill-cli explorer move <path> <folder>` | Move it, and rewrite what refers to it |
| `quill-cli explorer move <path> <folder> --dry-run` | Print the whole change set and change nothing |
| `quill-cli explorer move <path> <folder> --no-refactor` | Move the bytes and leave the references alone |
| `quill-cli tab close --discard` | Close without the save §6 added |
...
**Pure, with no window** — `quill-core`:
...
**With a window** — `crates/quill-app/tests/screenshots.rs`:
...
- a move through `quill-cli explorer move` rewrites a real importing file on disk and leaves an open
...
file in the project that has an import-capable grammar. `quill-cli explorer move --dry-run` prints
the elapsed time, so it stays measured rather than assumed.

Measured against Quill's own repository — 1,001 files in the walk — planning the move of a Rust
...
- **Saving every modified tab when the window closes.** It is the same rule as §6 and it belongs with
  a decision about what Quill does on quit generally, which is a ticket of its own.
