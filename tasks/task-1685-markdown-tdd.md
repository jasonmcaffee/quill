...
> Unluminous markdown preview has a few issues. It's not rendering md tables correctly. I can't select
...
So this is a rewrite of `unluminous_core::markdown` into the two phases every conforming Markdown
...
this table was checked against the code in `crates/unluminous-core/src/markdown.rs` as it stands.
...
`tasks/unluminous-mermaid-plugin-tdd.md` §2 gives about `mermaid.js` and `task-1675` §2 gives about a
language server: it would answer a different question from the one Unluminous is asking.

A Markdown crate produces **events for HTML** — `Start(Tag::Table)`, `Start(Tag::TableCell)`,
`Html`, `SoftBreak`. Unluminous has no HTML, no box model and no inline layout: it has a rope, a list of
...
**Raw HTML** is *removed* rather than shown. Unluminous cannot render it, and a reader seeing `<sub>`
...
A table needs columns of equal width, and Unluminous's layout engine places one glyph after another with
...
`rich`, `mdcat` and `bat`. The precedent is inside Unluminous too: the horizontal rule has been drawn as
...
where the pictures and the diagrams get their room. `unluminous-core` still knows nothing about drawing:
it says which paragraphs are code and the window decides what a code background looks like.

**Colour from the grammar.** The fence's language is looked up in the plugins Unluminous already has, and
the code inside is coloured with `unluminous_core::syntax::highlight` and the plugin's own theme — the
...
`unluminous-core` holds no plugin registry and must not learn about one, and the window implements the
...
One field, because `unluminous_core::Selection` is already an anchor and a head and already knows how to
...
`unluminous-cli` gets **one** command rather than two — `editor preview-select`, with `--from`, `--to`,
...
| File | Change |
|---|---|
| `unluminous-core/src/markdown/mod.rs` | The public shape: `render`, `Options`, `Preview`, `PreviewPanel`, `PreviewTable`, `CodeHighlighter` |
| `unluminous-core/src/markdown/blocks.rs` | The block parser and its stack |
| `unluminous-core/src/markdown/inline.rs` | The delimiter stack, code spans, links, autolinks, entities |
| `unluminous-core/src/markdown/entity.rs` | Named and numeric character references |
| `unluminous-core/src/markdown/table.rs` | Pipe tables, column fitting, box drawing |
| `unluminous-core/src/markdown/build.rs` | Blocks to text, spans and paragraph styles |
| `unluminous-app/src/app/mod.rs` | `refresh_preview` passes metrics and a highlighter; panels and selection are painted; the preview's pointer is read |
| `unluminous-app/src/app/files.rs` | `preview_selection` on the tab |
| `unluminous-app/src/services/plugins.rs` | `Plugins::for_language` |
| `unluminous-app/src/components/editor_view.rs` | Reading the pointer in a page that cannot be typed into, and painting the box-drawing characters rather than lettering them |
| `unluminous-app/src/app/cli.rs`, `unluminous-cli` | The two new commands and their documentation |
...
**A battery in `unluminous-core`, with no window.** One test per row of the table in §1, asserting on the
...
person sees on first opening Unluminous is what the ticket asked for.
...
- **Inline HTML rendered as HTML.** Unluminous has no HTML engine and will not grow one; §4 says what
  happens instead.
