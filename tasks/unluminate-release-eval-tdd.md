# Unluminate: what stands between it and a general release
...
the frame costs timed, the defects reproduced through `unluminate-cli` driving a real window. Where a
...
Unluminate is much closer to a release than a project eleven days old has any right to be. 180,308
...
| | Why it comes here |
|---|---|
| **A. The seven defects** (§7) | Two of them corrupt files. They are cheap and they are the only items that make Unluminate worse than a text editor from 1995. |
...
| | Command | Result |
|---|---|---|
| Whole workspace, non-UI crates | `cargo test --workspace --exclude unluminate-app` | **1,295 passed, 0 failed**, ~15 s of test time |
| `unluminate-app`, no window | `cargo test -p unluminate-app --lib --bins` | **907 passed, 0 failed**, 2.32 s |
| Screenshot layer, through `wgpu` | `cargo test -p unluminate-app --test screenshots` | **476 passed, 0 failed**, 134.40 s |
...
| Cold start | `unluminate-cli launch`, polled until `instances` answered | **969 ms** |
| Memory, one window on this repo | `Get-Process` after the index settled | 234 MB working set, 422 MB private, 35 threads |
| MCP preamble | `unluminate-cli mcp tools --count` | 24 tools / 17,275 tokens; 164 tools / 42,753 tokens |

Everything in §7 was reproduced by driving a real window with `unluminate-cli`, not by reading the
source. The evidence files are in
`ai-service/_agent_output/task-1803-unluminate-release-eval/`.
...
- **The documentation is a test.** `unluminate-cli/src/documentation.rs` fails while a command has no
...
**The six end-to-end agent-board tests never run.** `crates/unluminate-app/tests/agent_board.rs`
...
- And `tools/agent-study/` drives a **real local model** — Qwen 3.8 27B on llama.cpp — through 23
  scenarios phrased the way a person speaks, grading what happened by reading Unluminate's own state
...
`action list --menu` added to narrow an answer, narrow semantic tools added so a dedicated Unluminate
...
`cargo run --release -p unluminate-app --example frame_cost -- <file> [width]` produced every number
...
`unluminate.iss` has no signing directive. Every download therefore meets a SmartScreen warning
...
**There is no first-run experience.** Nothing greets a person who opens Unluminate for the first
...
Each of these was reproduced through `unluminate-cli` against a running window.
...
```
$ unluminate-cli --json tab open latin1.txt
{ "command": "tab.open", "ok": true, "tab": 0,
  "message": "Opened ...latin1.txt in tab 0" }          exit 0
$ unluminate-cli --json tab list
  ...no such tab...
$ unluminate-cli window message
  Unluminate could not open ...latin1.txt: stream did not contain valid UTF-8
```
...
```
$ unluminate-cli --json editor definition relayout
'relayout' has 2 candidate definitions, both "sure":
  crates/unluminate-core/src/layout.rs
...
### 7.7 Four dead-code warnings in `unluminate-app`
...
and `unluminate-cli` depending on none of them so the client stays a small program with no window
behind it. `unluminate-app`'s four folders — `app/`, `components/`, `services/`, `theme/` — are clear
...
`ai-service/_agent_output/task-1803-unluminate-release-eval/` alongside the test logs and the
