//! Writing Unluminate into an agent's own configuration.
//!
...
//! `unluminate-cli`, and the arguments that make it a server. There is no port, nothing is listening,
//! and the process lives for exactly as long as the agent's conversation.
//!
//! ```json
//! { "type": "stdio", "command": "C:\\Program Files\\Unluminate\\unluminate-cli.exe", "args": ["mcp", "serve"] }
//! ```
//!
...
pub const DEFAULT_NAME: &str = "unluminate";
...
pub struct Wanted {
    ...
    pub port: u16,
    pub scope: Scope,
    /// The program to launch, for stdio. This `unluminate-cli`, unless a caller knows better.
    pub program: PathBuf,
...
impl Default for Wanted {
    fn default() -> Self {
        Self {
            ...
            port: DEFAULT_PORT,
            scope: Scope::default(),
            program: unluminate_cli_program(),
...
/// Put Unluminate into a client's configuration.
...
fn write_json_server(file: &Path, name: &str, entry: Option<&Value>) -> Result<(), String> {
    ...
    if !existing.is_empty() {
        let backup = file.with_extension("unluminate-backup");
...
fn write_toml_table(file: &Path, table: &str, contents: Option<&str>) -> Result<(), String> {
    let existing = std::fs::read_to_string(file).unwrap_or_default();
    if !existing.is_empty() {
        let backup = file.with_extension("toml.unluminate-backup");
        std::fs::write(&backup, &existing)
            .map_err(|problem| format!("could not write {}: {problem}", backup.display()))?;
    }
...
/// The `unluminate-cli` an agent should be told to launch.
///
...
/// it are `unluminate-cli` itself and **the window**, and `current_exe` in the window is `unluminate.exe`.
/// So: this program when this program is already it, then the one beside it — an installed Unluminate
/// puts them in the same folder and a build puts them in the same `target` folder, which is right
/// with nothing configured — and `UNLUMINATE_CLI_BIN` overrides both. It is the mirror of
/// `client::unluminate_program`, which finds the window from the client.
pub fn unluminate_cli_program() -> PathBuf {
    let name = if cfg!(windows) { "unluminate-cli.exe" } else { "unluminate-cli" };
    if let Some(named) = std::env::var_os("UNLUMINATE_CLI_BIN") {
        return PathBuf::from(named);
    }
    let Ok(running) = std::env::current_exe() else {
        return PathBuf::from(name);
    };
    if running.file_stem().map(|stem| stem == "unluminate-cli").unwrap_or(false) {
        return running;
    }
...
fn home() -> PathBuf {
    if let Some(home) = std::env::var_os("UNLUMINATE_HOME") {
        // Named so a test can install into a folder of its own rather than into the real settings
        // of the person running it — the rule `Store::at` already keeps for Unluminate's own settings.
...
mod tests {
    ...
    fn a_home(name: &str) -> Home {
        ...
        let folder = std::env::temp_dir().join(format!("unluminate-mcp-install-{name}"));
        std::fs::remove_dir_all(&folder).ok();
        std::fs::create_dir_all(&folder).expect("make the folder");
        std::env::set_var("UNLUMINATE_HOME", &folder);
    ...
    fn wanted(home: &Home) -> Wanted {
        Wanted {
            program: PathBuf::from("C:\\Program Files\\Unluminate\\unluminate-cli.exe"),
    ...
    fn the_stdio_entry_names_this_program_and_carries_no_port() {
        let entry = Wanted { program: PathBuf::from("/opt/unluminate/unluminate-cli"), ..Wanted::default() }
            .entry();
        assert_eq!(entry["type"], json!("stdio"));
        assert_eq!(entry["command"], json!("/opt/unluminate/unluminate-cli"));
    ...
    fn a_windows_path_survives_being_written_as_toml() {
        ...
        .toml_table();
        assert!(table.contains("[mcp_servers.unluminate]"), "{table}");
        assert!(
            table.contains(r#"command = "C:\\Program Files\\Unluminate\\unluminate-cli.exe""#),
    ...
    fn installing_into_claude_keeps_everything_else_in_the_file() {
        ...
        .expect("write");
        write_json_server(&file, "unluminate", Some(&wanted(&home).entry())).expect("it writes");
        let back: Value =
        ...
        assert_eq!(back["mcpServers"]["unluminate"]["args"], json!(["mcp", "serve"]));
        assert!(
            file.with_extension("unluminate-backup").is_file(),
    ...
    fn installing_into_claude_twice_leaves_one_entry_and_removing_leaves_none() {
        ...
        write_json_server(&file, "unluminate", Some(&wanted.entry())).expect("once");
        write_json_server(&file, "unluminate", Some(&wanted.entry())).expect("twice");
        assert!(installed(Client::Claude, &wanted).is_some());
        write_json_server(&file, "unluminate", None).expect("removed");
    ...
    fn a_claude_file_that_is_not_json_is_refused_rather_than_overwritten() {
        ...
        let problem = write_json_server(&file, "unluminate", Some(&wanted(&home).entry()))
    ...
    fn installing_into_codex_leaves_every_other_table_and_every_comment_alone() {
        ...
        write_toml_table(&file, "mcp_servers.unluminate", Some(&wanted(&home).toml_table()))
            .expect("it writes");
        ...
        assert!(after.contains("[mcp_servers.unluminate]"), "{after}");
    ...
    fn installing_into_codex_twice_leaves_one_table_and_removing_leaves_none() {
        ...
        let wanted = wanted(&home);
        for _ in 0..2 {
            write_toml_table(&file, "mcp_servers.unluminate", Some(&wanted.toml_table())).expect("write");
        }
        let after = std::fs::read_to_string(&file).expect("read");
        assert_eq!(after.matches("[mcp_servers.unluminate]").count(), 1, "{after}");
        assert!(installed(Client::Codex, &wanted).is_some());
        write_toml_table(&file, "mcp_servers.unluminate", None).expect("remove");
        let after = std::fs::read_to_string(&file).expect("read");
        assert!(!after.contains("[mcp_servers.unluminate]"), "{after}");
    ...
    fn a_table_is_replaced_rather_than_the_one_after_it_being_eaten() {
        let text = "[a]\nx = 1\n\n[mcp_servers.unluminate]\ncommand = \"old\"\n\n[b]\ny = 2\n";
        let without = remove_table(text, "mcp_servers.unluminate");
    ...
    fn the_copyable_example_is_what_each_client_actually_reads() {
        let wanted = Wanted { program: PathBuf::from("/opt/unluminate-cli"), ..Wanted::default() };
        let claude: Value =
            serde_json::from_str(&wanted.example(Client::Claude)).expect("valid JSON");
        assert_eq!(claude["mcpServers"]["unluminate"]["type"], json!("stdio"));
        let codex = wanted.example(Client::Codex);
        assert!(codex.starts_with("[mcp_servers.unluminate]"), "{codex}");
    }

    #[test]
    fn the_program_an_agent_is_told_to_launch_can_be_named() {
        // The window asks for this too, and `current_exe` there is `unluminate.exe` rather than the
        // client, so it has to be worked out rather than assumed.
        let previous = std::env::var_os("UNLUMINATE_CLI_BIN");
        std::env::set_var("UNLUMINATE_CLI_BIN", "/somewhere/else/unluminate-cli");
        assert_eq!(unluminate_cli_program(), PathBuf::from("/somewhere/else/unluminate-cli"));
        ...
        let worked_out = unluminate_cli_program();
        assert!(
            worked_out.file_stem().map(|stem| stem == "unluminate-cli").unwrap_or(false),
            "it should name unluminate-cli, not {}",
            worked_out.display()
        );
        if let Some(value) = previous {
            std::env::set_var("UNLUMINATE_CLI_BIN", value);
        }
    }
