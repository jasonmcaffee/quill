//! The Model Context Protocol server: how an AI agent discovers and drives Unluminate.
//!
//! `task-1661` gave Unluminate a command line and then measured an agent using it — the local Qwen 3.8
...
//! does, and still names every command Unluminate has.
//!
...
//! exactly the request `unluminate-cli` would have sent, down the same loopback socket with the same
//! token, so `UnluminateApp::run_cli` stays the one place a command becomes a change.
//!
//! ## Two transports
//!
//! [`stdio`] is what an agent launches: `unluminate-cli mcp serve`, one JSON message a line over the
...
//! rather have a URL — and it is what a running Unluminate hosts when `mcp.enabled` is on. It is off by
...
//!
//! [`install`] writes Unluminate into Claude Code's and Codex's own configuration, which is the same
...
pub enum Transport {
    /// The agent launches `unluminate-cli mcp serve` and talks down its pipes. No port.
    #[default]
    Stdio,
