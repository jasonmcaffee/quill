...
//!
//! MCP has moved twice while Unluminate has been written. `2025-06-18` is what every shipping client
...
//!
//! A tool call becomes exactly the request `unluminate-cli` would have sent — the same wire name, the
//! same arguments object, the same token from the same instance file, to the same port. Nothing new
//! reaches the window's state, so `UnluminateApp::run_cli` stays the one place a command becomes a
//! change and a command run by an agent is the same command a person types. [`Driver`] is that
//! step, behind a trait, so the protocol can be tested with no Unluminate running.
...
pub const SERVER_NAME: &str = "unluminate";

/// The whole of `docs/commands.md`, as a resource.
///
/// Embedded rather than read from disk: the installed `unluminate-cli` is one file and the documentation
...
/// Why no Unluminate could be reached.
...
impl<D: Driver> Server<D> {
    ...
    pub fn answer(&self, message: &Value) -> Option<Value> {
        ...
        match method {
            ...
            other => Some(error(
                &id,
                -32601,
                format!("Unluminate's MCP server has no `{other}` method."),
            )),
        }
    }
    ...
    fn read(&self, id: &Value, parameters: &Value) -> Value {
        match parameters.get("uri").and_then(Value::as_str) {
            ...
            Some(other) => error(id, -32602, format!("Unluminate has no resource at `{other}`.")),
...
Unluminate is a text editor. These tools drive a window that is already open on this machine: they open \
...
your working directory. A project is a window, so several Unluminates may be running and `instance` says \
...
fn resource() -> Value {
    json!({
        "uri": COMMANDS_URI,
        "name": "commands.md",
        "title": "Unluminate's command reference",
        "description": "Every command Unluminate has, with its arguments, its flags and its examples. The same document a person reads.",
        "mimeType": "text/markdown",
    })
}
...
/// is the client's own, so what an agent sees and what `unluminate-cli` prints are the same thing.
...
mod tests {
    ...
    fn tools_are_listed_and_called_with_no_initialize_first() {
        ...
        assert!(names.contains(&"unluminate"), "no tool for the commands with no area: {names:?}");
        ...
        let called = ask(
            &server,
            "tools/call",
            json!({ "name": "unluminate_tab", "arguments": { "command": "open", "arguments": { "path": "README.md" } } }),
        );
    ...
    fn a_refusal_from_the_window_is_a_tool_error_and_not_a_protocol_error() {
        ...
        let back = ask(
            &server,
            "tools/call",
            json!({ "name": "unluminate_tab", "arguments": { "command": "open", "arguments": { "path": "nope.md" } } }),
        );
    ...
    fn a_tool_that_does_not_exist_is_a_protocol_error() {
        let server = a_server();
        let back = ask(&server, "tools/call", json!({ "name": "unluminate_nonsense" }));
    ...
    fn a_listing_and_a_document_are_read_the_way_the_client_prints_them() {
        ...
        let back = ask(
            &server,
            "tools/call",
            json!({ "name": "unluminate_tab", "arguments": { "command": "list" } }),
        );
        let text = back["result"]["content"][0]["text"].as_str().expect("text");
    ...
    fn a_screenshot_comes_back_as_a_picture_and_nothing_else_does() {
        ...
        let back = ask(
            &server,
            "tools/call",
            json!({ "name": "unluminate_window", "arguments": { "command": "screenshot", "arguments": { "file": "after.png" } } }),
        );
    ...
    fn the_written_reference_is_offered_as_a_resource_and_can_be_read() {
        ...
        assert!(text.contains("unluminate-cli commands"), "it should be the real document");
