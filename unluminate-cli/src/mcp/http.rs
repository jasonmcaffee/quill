...
//! running Unluminate hosts when `mcp.enabled` is on. `POST` carries a JSON-RPC message and gets one
...
//! server with something to say unprompted. Unluminate has nothing: every answer is the answer to a
...
impl Endpoint {
    ...
    /// usually by the Unluminate in the next window, which is not an error so much as a fact the window
    /// should report.
    pub fn start<D: Driver + Send + Sync + 'static>(
        ...
        let thread = std::thread::Builder::new()
            .name("unluminate-mcp-http".to_owned())
...
impl Drop for Endpoint {
    ...
    /// port as taken by another Unluminate when the other Unluminate was itself a moment ago. One poll is the
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                let _ = std::thread::Builder::new()
                    .name("unluminate-mcp-connection".to_owned())
...
pub fn respond<D: Driver>(request: &Incoming, server: &Server<D>) -> String {
    ...
    if request.path.split('?').next().unwrap_or_default() != PATH {
        return response(404, "text/plain", &format!("Unluminate's MCP server is at {PATH}."));
    }
...
pub fn refuse_a_browser(request: &Incoming) -> Option<String> {
    if let Some(origin) = &request.origin {
        if !is_loopback_origin(origin) {
            return Some(response(
                403,
                "text/plain",
                "Unluminate's MCP endpoint only answers requests from this machine.",
            ));
        }
    }
    match request.fetch_site.as_deref() {
        ...
        Some(_) => Some(response(
            403,
            "text/plain",
            "Unluminate's MCP endpoint does not answer requests made by a web page.",
        )),
    }
}
