//! What goes over the wire between the client and a running Unluminate.
//!
...
//! Python drive Unluminate, and `unluminate-cli` is the friendly face rather than the only way in.
//!
//! ```text
...
/// Something asked of a running Unluminate.
...
/// What a running Unluminate answered.
#[derive(Debug, Clone, PartialEq)]
pub struct Reply {
    pub ok: bool,
    pub command: String,
...
pub mod code {
    /// No Unluminate is running, or the one named is not answering.
    pub const NOT_RUNNING: &str = "not-running";
    /// Several Unluminates are running and none was named.
...
/// sends one command and stops, and a program driving Unluminate in a loop opening a socket per command
...
pub fn ask(port: u16, request: &Request, timeout: Duration) -> std::io::Result<Reply> {
    ...
    if line.trim().is_empty() {
        return Err(std::io::Error::new(
            ...
            "Unluminate closed the connection without answering",
        ));
    }
    let value: Value = serde_json::from_str(line.trim())
        .map_err(|problem| std::io::Error::new(std::io::ErrorKind::InvalidData, problem))?;
    Reply::from_json(&value).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::InvalidData, "Unluminate's answer was not a reply")
    })
}
