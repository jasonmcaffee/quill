//! `unluminate-cli`: the program a person or an agent types at.
//!
//! It does four things and no more: read the command line against the catalogue, find the Unluminate to
...
//! `unluminate-cli editor text > file.md` writes the document and not a sentence about it. With `--json`
...
//!
//! The split is the one a script cares about: 2 is the caller's mistake, 1 is Unluminate's answer, and
...
use unluminate_cli::catalogue::Command;
use unluminate_cli::client::{self, Unreachable, DEFAULT_LAUNCH_TIMEOUT, DEFAULT_TIMEOUT};
use unluminate_cli::instances::Instance;
use unluminate_cli::parse::{self, Global, Typed};
use unluminate_cli::protocol::{code, Reply};
use unluminate_cli::mcp;
use unluminate_cli::{help, VERSION};
...
fn version(global: &Global) -> i32 {
    if global.json {
        say(&json!({
            "ok": true,
            "command": "version",
            "message": format!("unluminate-cli {VERSION}"),
            "result": { "version": VERSION },
        }));
    } else if !global.quiet {
        println!("unluminate-cli {VERSION}");
    }
    OK
}
...
///
/// `clig.dev` asks for a `--dry-run` on anything that changes something, and this is Unluminate's: it
...
/// what the agent assessment uses it for — and it needs no running Unluminate, so it can be used to
...
/// The commands the client answers on its own, with no Unluminate involved.
    ...
    match command.wire().as_str() {
        ...
        "commands" => {
            ...
            if let Some(name) = only {
                if unluminate_cli::catalogue::find(name).is_none() {
            ...
            if typed.global.json {
                ...
                match only.and_then(unluminate_cli::catalogue::find) {
        ...
        "instances" => {
            ...
            if typed.global.json {
                ...
                println!("No Unluminate is running.");
            } else {
                ...
                Ok((instance, pid)) => {
                    ...
                    if typed.global.json {
                        ...
                        match &instance {
                            ...
                            None => println!("Unluminate was started as process {pid}"),
                        }
                    }
                    OK
                }
...
/// Run the MCP server, which is how an AI agent drives Unluminate.
///
    ...
    match transport {
        mcp::Transport::Stdio => match mcp::stdio::serve(&server) {
            ...
            Err(problem) => {
                ...
                eprintln!("unluminate-cli mcp serve: {problem}");
                REFUSED
            }
        },
        mcp::Transport::Http => {
            ...
            let endpoint = match mcp::http::Endpoint::start(port, server) {
                ...
                Err(problem) => {
                    complain(
                        &format!(
                            "Could not listen on port {port}: {problem}. Another Unluminate or another \
            ...
            eprintln!("Unluminate's MCP server is at {}", mcp::endpoint(endpoint.port()));
...
/// Write Unluminate into an agent's own configuration, or take it out again.
...
fn clients_from(named: Option<&str>) -> Result<Vec<mcp::install::Client>, String> {
    match named.map(str::trim) {
        ...
        Some(name) => mcp::install::Client::parse(name).map(|client| vec![client]).ok_or_else(|| {
            format!("`{name}` is not a client Unluminate can write to. It is `claude`, `codex`, or `both`.")
        }),
    }
}
...
mod tests {
    ...
    #[test]
    fn help_and_version_work_with_no_unluminate_running() {
