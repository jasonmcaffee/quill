...
//!
//! This is the single list. The client parses against it, `--help` is printed from it, `unluminate-cli
...
//! `unluminate-cli/docs/commands.md`. A command that is not here does not exist, and a command that is
//! here is documented.
//!
//! ## How a command is named
//!
//! `unluminate-cli <area> <verb>`, in that order — the noun first, as `docker container create` and
...
//! is the action. Areas are what the window is made of, so somebody who can see Unluminate can guess the
...
//! or about a whole Unluminate: `status`, `instances`, `launch`, `quit`, `commands` and `version`.
//!
...
impl Command {
    ...
    pub fn usage(&self) -> String {
        let mut line = format!("unluminate-cli {}", self.typed());
...
///
/// Both spellings are accepted from both sides, so `unluminate-cli tab.open` works and a program that
...
pub fn area_title(area: &'static str) -> &'static str {
    match area {
        ...
        "browser" => "browser — rendered web pages in Unluminate tabs",
        ...
        "plugins" => "plugins — the languages Unluminate colours, and the panes it draws",
        ...
        "mcp" => "mcp — the server an AI agent drives Unluminate through",
        other => other,
    }
}
...
pub fn area_note(area: &'static str) -> &'static str {
    match area {
        ...
        "explorer" => "For requests to create, move, delete or list project paths, start here instead of using shell file operations. Unluminate updates its live tree immediately, and `new-file` also opens the file in a tab. `explorer files` leaves out `target`, `node_modules` and `__pycache__`.",
        "modal" => "One set of commands drives all of them: open it, type in it, read its results, choose a row, accept or cancel. A modal added to Unluminate later is driven with these same commands.",
        "settings" => "The names are the ones in Unluminate's own `settings.conf`, so there is one vocabulary rather than two. A change takes effect at once, in every tab, and is written to the file.",
        "theme" => "A theme says what every name in Unluminate's own palette means, which drawn icon set the rail and the explorer use, and — when it names all nine token colours — how code is coloured in every language at once. Unluminate's own theme names none of the nine, so each language plugin's scheme is what colours its files until a theme is chosen. `settings set appearance.theme` reaches the same code; these exist because a setting cannot say what themes there are.",
        ...
        "action" => "The escape hatch, and the guarantee: every entry on every menu has a name here, and the list is built by walking the real menus, so a menu entry added to Unluminate tomorrow can be run from the command line tomorrow.",
        "project" => "A project is a window. Opening a second project is `unluminate-cli launch <folder>`, which starts a second Unluminate; `project open` changes the folder this window is showing.",
        "mcp" => "The Model Context Protocol server, which is how an AI agent discovers and drives Unluminate without being handed a document first. Its tools are generated from this same catalogue, so a command added to Unluminate is a tool the day it is added.",
        _ => "",
    }
}
...
pub const COMMANDS: &[Command] = &[
    // ---------------------------------------------------------------- the CLI and a whole Unluminate
    Command {
        area: "",
        verb: "status",
        ...
        examples: &["unluminate-cli status --json", "unluminate-cli status --section panes --json"],
        local: false,
    },
    Command {
        area: "",
        verb: "instances",
        summary: "The Unluminate windows that are running, with the port and the project of each. Answered without talking to any of them.",
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli instances --json"],
        local: true,
    },
    Command {
        area: "",
        verb: "launch",
        summary: "Start another Unluminate on a folder and wait until it answers.",
        ...
        examples: &["unluminate-cli launch C:\\jason\\dev\\unluminate", "unluminate-cli launch . --timeout 40000"],
        local: true,
    },
    Command {
        area: "",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli quit"],
        local: false,
    },
    Command {
        area: "",
        verb: "commands",
        summary: "Every command this CLI has, as data: the areas, the arguments, the flags and the examples. This is what to read first when a program or an agent is driving Unluminate.",
        arguments: &[argument("name", false, "One command, such as `terminal send`, instead of all of them.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli commands --json", "unluminate-cli commands \"modal open\" --json"],
        local: true,
    },
    Command {
        area: "",
        verb: "version",
        summary: "What version this command line tool is. The version and build date of the Unluminate editor it is talking to are in `status`, and `modal open about` shows them in the window.",
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli version"],
        local: true,
    },
    // ------------------------------------------------------------------------------- the window
    Command {
        ...
        examples: &["unluminate-cli window screenshot _agent_output/after.png"],
        local: false,
    },
    Command {
        area: "window",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli window focus"],
        local: false,
    },
    Command {
        area: "window",
        ...
        examples: &["unluminate-cli window size", "unluminate-cli window size --width 1100 --height 720"],
        local: false,
    },
    Command {
        area: "window",
        ...
        examples: &["unluminate-cli window position --x 40 --y 40"],
        local: false,
    },
    Command {
        area: "window",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli window message", "unluminate-cli window message Ready for the next step"],
        local: false,
    },
    // ------------------------------------------------------------------------------ the browser
    Command {
        area: "browser",
        verb: "open",
        summary: "Render an HTTP address or local HTML file in a new Unluminate tab.",
        arguments: &[rest("address", true, "An HTTP or HTTPS address, or an HTML path relative to the project folder.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli browser open https://example.com", "unluminate-cli browser open examples/site/index.html"],
        local: false,
    },
    Command {
        area: "browser",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli browser status --json"],
        local: false,
    },
    Command {
        area: "browser",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli browser back"],
        local: false,
    },
    Command {
        area: "browser",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli browser forward"],
        local: false,
    },
    Command {
        area: "browser",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli browser reload"],
        local: false,
    },
    // --------------------------------------------------------------------------------- the tabs
    Command {
        ...
        examples: &["unluminate-cli tab open README.md", "unluminate-cli tab open design/style-guide.md --permanent"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli tab list --json"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli tab show 2", "unluminate-cli tab show README.md"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        examples: &["unluminate-cli tab close", "unluminate-cli tab close notes.md", "unluminate-cli tab close --discard"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli tab next"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli tab previous"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        examples: &["unluminate-cli tab move 0", "unluminate-cli tab move 0 --tab notes.md --pane 1"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli tab save"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli tab save-as notes/copy.md"],
        local: false,
    },
    Command {
        area: "tab",
        ...
        examples: &["unluminate-cli tab reload", "unluminate-cli tab reload --discard"],
        local: false,
    },
    // -------------------------------------------------------------------------------- the panes
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli pane list --json"],
        local: false,
    },
    Command {
        area: "pane",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli pane split"],
        local: false,
    },
    Command {
        area: "pane",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli pane move right", "unluminate-cli pane move left"],
        local: false,
    },
    Command {
        area: "pane",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli pane focus 1"],
        local: false,
    },
    Command {
        area: "pane",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli pane width 0 0.35"],
        local: false,
    },
    Command {
        area: "pane",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli pane unsplit"],
        local: false,
    },
    Command {
        area: "pane",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli pane unsplit-all"],
        local: false,
    },
    // ------------------------------------------------------------------------------- the editor
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor status --json"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        examples: &["unluminate-cli editor text", "unluminate-cli editor text --from-line 1 --to-line 20"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        examples: &["unluminate-cli editor set-text # Notes", "unluminate-cli editor set-text --from-file draft.md"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor insert Hello", "unluminate-cli editor insert \"one\\ntwo\""],
        local: false,
    },
    Command {
        area: "editor",
        ...
        examples: &["unluminate-cli editor caret", "unluminate-cli editor caret --line 42 --column 5"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        examples: &["unluminate-cli editor select --all", "unluminate-cli editor select --from-line 3 --to-line 6"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        examples: &["unluminate-cli editor indent", "unluminate-cli editor indent --space"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        examples: &["unluminate-cli editor dedent", "unluminate-cli editor dedent --space"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor undo"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor redo"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor view preview", "unluminate-cli editor view side"],
        local: false,
    },
    Command {
        area: "editor",
    ...
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor preview --json"],
        local: false,
    },
    Command {
        area: "editor",
    ...
    Command {
        area: "editor",
        verb: "definition",
        summary: "Where a name is defined, from Unluminate's live open tabs and project symbol index. Give the name directly or leave it out for the word at the caret; every candidate is printed best first and --open navigates through the editor.",
    ...
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor navigate-back"],
        local: false,
    },
    Command {
        area: "editor",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli editor navigate-forward"],
        local: false,
    },
    // ------------------------------------------------------------------ the marked passages
    Command {
        ...
        examples: &["unluminate-cli highlight list --json", "unluminate-cli highlight list --all --json"],
        local: false,
    },
    Command {
        area: "highlight",
    ...
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli fold list --json"],
        local: false,
    },
    Command {
        area: "fold",
        ...
        examples: &["unluminate-cli fold toggle", "unluminate-cli fold toggle --line 42", "unluminate-cli fold toggle --regions --json"],
        local: false,
    },
    Command {
        area: "fold",
        ...
        examples: &["unluminate-cli fold collapse --all", "unluminate-cli fold collapse --line 42", "unluminate-cli fold collapse --line 42 --recursive", "unluminate-cli fold collapse --all --regions --json"],
        local: false,
    },
    Command {
        area: "fold",
        ...
        examples: &["unluminate-cli fold expand --all", "unluminate-cli fold expand --line 42", "unluminate-cli fold expand --line 42 --recursive", "unluminate-cli fold expand --all --regions --json"],
        local: false,
    },
    Command {
        area: "fold",
        ...
        examples: &["unluminate-cli fold others", "unluminate-cli fold others --selection", "unluminate-cli fold others --regions --json"],
        local: false,
    },
    // -------------------------------------------------------------------------------- the panels
    Command {
    ...
    Command {
        area: "panel",
        verb: "reset",
        summary: "Put every panel back where a new Unluminate has it: the explorer down the left, the three tiles along the bottom, each at its starting size.",
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli panel reset"],
        local: false,
    },
    // ----------------------------------------------------------------------------- the terminal
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli terminal show"],
        local: false,
    },
    Command {
        area: "terminal",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli terminal hide"],
        local: false,
    },
    Command {
        area: "terminal",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli terminal toggle"],
        local: false,
    },
    Command {
        area: "terminal",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli terminal new"],
        local: false,
    },
    Command {
        area: "terminal",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli terminal list --json"],
        local: false,
    },
    Command {
        area: "terminal",
        ...
        examples: &["unluminate-cli terminal select 1", "unluminate-cli terminal select --tab 1"],
        local: false,
    },
    Command {
        area: "terminal",
        ...
        examples: &["unluminate-cli terminal close", "unluminate-cli terminal close --tab 1"],
        local: false,
    },
    Command {
        area: "terminal",
    ...
    Command {
        ...
        examples: &["unluminate-cli terminal move 0", "unluminate-cli terminal move --tab 2 0"],
        local: false,
    },
    Command {
        area: "terminal",
    ...
    Command {
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli terminal height 400"],
        local: false,
    },
    ...
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli run list --json"],
        local: false,
    },
    Command {
        area: "run",
    ...
    Command {
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli run remove \"Dev server\""],
        local: false,
    },
    Command {
        area: "run",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli run start", "unluminate-cli run start \"Dev server\""],
        local: false,
    },
    Command {
        area: "run",
        ...
        arguments: &[argument("name", false, "The configuration. The chosen one when it is left out.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli run stop", "unluminate-cli run stop \"Dev server\""],
        local: false,
    },
    Command {
        area: "run",
        ...
        arguments: &[argument("name", false, "The configuration. The chosen one when it is left out.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli run rerun"],
        local: false,
    },
    Command {
        area: "run",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli run select \"Dev server\""],
        local: false,
    },
    Command {
        area: "run",
    ...
    Command {
        ...
        arguments: &[argument("name", false, "The configuration. The chosen one when it is left out.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli run status --json", "unluminate-cli run status \"Dev server\" --json"],
        local: false,
    },
    // ---------------------------------------------------------------------------- the debugger
    Command {
    ...
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli debug stop"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug continue --wait-for-pause"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug step-over --wait-for-pause"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug step-into --wait-for-pause"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug step-out --wait-for-pause"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug run-to src/main.rs 42 --wait-for-pause"],
        local: false,
    },
    Command {
        area: "debug",
        verb: "breakpoint",
        summary: "Where the program is to stop. `add` and `remove` take a file and a line; `list` prints every one in the project, with what the debugger said about it while a session is running. Breakpoints are kept in .unluminate/breakpoints.conf and move with the text as the file is edited.",
        arguments: &[
    ...
    Command {
        ...
        examples: &["unluminate-cli debug frames --json", "unluminate-cli debug frames --include-subtle"],
        local: false,
    },
    Command {
        area: "debug",
    ...
    Command {
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli debug set-value Locals/count 7"],
        local: false,
    },
    Command {
        area: "debug",
    ...
    Command {
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli debug set-expression self.items.count 7"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug evaluate items.len()"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli debug watch add attempts", "unluminate-cli debug watch list --json"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug output --tail 20"],
        local: false,
    },
    Command {
        area: "debug",
        ...
        examples: &["unluminate-cli debug status --json", "unluminate-cli debug status --wait-for-pause"],
        local: false,
    },
    Command {
        area: "debug",
        verb: "adapters",
        summary: "Which debuggers this Unluminate drives, where each one is on this machine, what is missing, and the command that installs it. The first thing to run when a debug session will not start: it answers in fields under --json, so nothing has to be guessed at from a refusal.",
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli debug adapters", "unluminate-cli debug adapters --json"],
        local: false,
    },
    Command {
        area: "debug",
        verb: "install",
        summary: "Install a debug adapter by running its own install command in the run tile, where it can be watched with `run output` and stopped. Unluminate itself downloads nothing: what runs is a package manager, or an editor's extension installer, named by `debug adapters`.",
        arguments: &[argument("adapter", true, "Which debugger: lldb or node.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli debug install lldb"],
        local: false,
    },
    // ----------------------------------------------------------------------------- the explorer
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer show"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer hide"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer toggle"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer width 320"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer filter tdd", "unluminate-cli explorer filter"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer expand crates/unluminate-app/src"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer collapse crates", "unluminate-cli explorer collapse"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        examples: &["unluminate-cli explorer tree --json"],
        local: false,
    },
    Command {
        area: "explorer",
        verb: "files",
        summary: "Every file in the project that Unluminate searches, which leaves out what a build wrote: target, node_modules and __pycache__.",
        arguments: NO_ARGUMENTS,
        flags: &[option("limit", "number", "At most this many paths. 500 by default.")],
        examples: &["unluminate-cli explorer files --limit 20 --json"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer select-open-file"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer select README.md", "unluminate-cli explorer select --json"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer delete notes/old.md"],
        local: false,
    },
    Command {
        area: "explorer",
    ...
    Command {
        area: "explorer",
        verb: "new-file",
        summary: "Make an empty file, create its parent folders, update Unluminate's live tree and open it in a tab. The same thing New -> File on the explorer's right click menu does, without the dialog.",
        arguments: &[argument("path", true, "Where the file goes, relative to the project or absolute.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer new-file notes/today.md"],
        local: false,
    },
    Command {
        area: "explorer",
        verb: "new-folder",
        summary: "Make a folder and every folder above it, updating Unluminate's live tree immediately. The same thing New -> Folder on the explorer's right click menu does, without the dialog.",
        arguments: &[argument("path", true, "Where the folder goes, relative to the project or absolute.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer new-folder src/services"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer reload"],
        local: false,
    },
    Command {
        area: "explorer",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli explorer reveal README.md"],
        local: false,
    },
    // ------------------------------------------------------------------------------- the modals
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli modal list --json"],
        local: false,
    },
    Command {
        area: "modal",
    ...
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli modal state --json"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        examples: &["unluminate-cli modal type unluminate-cli", "unluminate-cli modal type --match-case Unluminate"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        examples: &["unluminate-cli modal results --limit 10 --json", "unluminate-cli modal results --wait 5000 --json"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli modal choose 2"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli modal accept", "unluminate-cli modal accept 0"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli modal cancel"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        examples: &["unluminate-cli modal move --x 60 --y 60"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        examples: &["unluminate-cli modal size --width 900 --height 600"],
        local: false,
    },
    Command {
        area: "modal",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli modal reset"],
        local: false,
    },
    // ----------------------------------------------------------------------------- the settings
    Command {
        area: "settings",
        verb: "list",
        summary: "Every setting, with its value, what it means and what it will accept. The names are the ones in Unluminate's own settings file.",
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli settings list --json"],
        local: false,
    },
    Command {
        area: "settings",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli settings get appearance.font.size"],
        local: false,
    },
    Command {
        area: "settings",
    ...
    Command {
        ...
        examples: &["unluminate-cli settings fonts --json"],
        local: false,
    },
    // ----------------------------------------------------------------------------------- themes
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli theme list --json"],
        local: false,
    },
    Command {
        area: "theme",
        verb: "show",
        summary: "One theme in full: every colour in Unluminate's palette by name, the nine token colours, and which drawn icon set it uses. The one the window is painted in when no theme is named.",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli theme show --json", "unluminate-cli theme show \"Monokai Pro\" --json"],
        local: false,
    },
    Command {
        area: "theme",
    ...
    Command {
        area: "plugins",
        verb: "list",
        summary: "The plugins Unluminate has, which of them are switched on, and what each one claims: a language it colours, or a pane, a tab, a menu and a Settings page it contributes. They ship with Unluminate; nothing is fetched.",
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli plugins list --json"],
        local: false,
    },
    Command {
        area: "plugins",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli plugins install rust"],
        local: false,
    },
    Command {
        area: "plugins",
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli plugins enable rust"],
        local: false,
    },
    Command {
        area: "plugins",
        ...
        arguments: &[argument("id", true, "The plugin's id.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli plugins disable rust"],
        local: false,
    },
    Command {
        area: "plugins",
        ...
        arguments: &[argument("id", true, "The plugin's id, as `plugins list` gives it.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli plugins show agent-tasks --json"],
        local: false,
    },
    Command {
        area: "plugins",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli plugins reload --json"],
        local: false,
    },
    Command {
        area: "plugins",
    ...
    Command {
        ...
        examples: &["unluminate-cli plugins tab agent-tasks/board --open"],
        local: false,
    },
    Command {
        area: "plugins",
    ...
    Command {
        ...
        arguments: &[argument("id", true, "The plugin's id.")],
        flags: NO_FLAGS,
        examples: &["unluminate-cli plugins view agent-tasks --json"],
        local: false,
    },
    // ---------------------------------------------------------------------------------- the git
    Command {
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli git status --json"],
        local: false,
    },
    Command {
        area: "git",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli git actions --json"],
        local: false,
    },
    Command {
        area: "git",
    ...
    Command {
        ...
        examples: &["unluminate-cli action list --json", "unluminate-cli action list --menu view --json"],
        local: false,
    },
    Command {
        area: "action",
        ...
        examples: &["unluminate-cli action run toggle-line-numbers", "unluminate-cli action run about"],
        local: false,
    },
    // ------------------------------------------------------------------------------ the project
    Command {
        ...
        flags: NO_FLAGS,
        examples: &["unluminate-cli project open C:\\jason\\dev\\unluminate"],
        local: false,
    },
    Command {
        area: "project",
        ...
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli project recent --json"],
        local: false,
    },
    // ---------------------------------------------------------------------------------- the MCP server
    Command {
        area: "mcp",
        verb: "serve",
        summary: "Run the Model Context Protocol server, which is how an AI agent drives Unluminate. Over stdin and stdout by default, which is what an agent that launches it wants; over HTTP with `--transport http`.",
        arguments: NO_ARGUMENTS,
        flags: &[
            ...
            option("instance", "which", "Which running Unluminate to drive, when several are running."),
        ],
        examples: &["unluminate-cli mcp serve", "unluminate-cli mcp serve --transport http --port 7345"],
        local: true,
    },
    Command {
        area: "mcp",
        verb: "status",
        summary: "What this Unluminate is doing about MCP: whether it is serving over HTTP, on which port, in which tool shape, how many tools that is, and where an agent's configuration should point.",
        arguments: NO_ARGUMENTS,
        flags: NO_FLAGS,
        examples: &["unluminate-cli mcp status --json"],
        local: false,
    },
    Command {
        area: "mcp",
        verb: "install",
        summary: "Write Unluminate's MCP server into an agent's own configuration, so it is there next time the agent starts.",
        ...
        flags: &[
            ...
            option("name", "name", "What the server is called in the agent's configuration. `unluminate` by default."),
            switch("remove", "Take it out again rather than putting it in."),
        ],
        examples: &["unluminate-cli mcp install both", "unluminate-cli mcp install claude --scope project"],
        local: true,
    },
    Command {
        area: "mcp",
        ...
        flags: &[
            ...
            option("name", "name", "What to call the server. `unluminate` by default."),
        ],
        examples: &["unluminate-cli mcp config", "unluminate-cli mcp config codex --transport http"],
        local: true,
    },
    Command {
        area: "mcp",
...
mod tests {
    ...
    fn every_command_has_a_summary_and_an_example() {
        for command in COMMANDS {
            ...
            for example in command.examples {
                assert!(
                    example.starts_with("unluminate-cli "),
    ...
    fn the_usage_line_shows_required_and_optional_apart() {
        let open = find("tab open").expect("tab open");
        assert_eq!(open.usage(), "unluminate-cli tab open <path> [--permanent]");
        let close = find("tab close").expect("tab close");
        assert_eq!(close.usage(), "unluminate-cli tab close [tab] [--discard]");
    }
}
