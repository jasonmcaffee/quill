...
//! 3. **Unluminate's own flags are recognised anywhere on the line** — the command's own first, then the
...
//!    line** it takes every word left, and a `--flag` that is not one of Unluminate's own is part of the
//!    text — which is what lets `terminal send git log --oneline -n 5` send the whole shell command.
//!    A `--flag` that *is* one of Unluminate's is still Unluminate's, so `settings set appearance.font.size 20
...
//! After it, only the long spelling is Unluminate's. That is not tidiness: `git log -n 5` and `grep -q`
...
pub struct Global {
    /// Which Unluminate to talk to: a process id, a port, or part of a project's path.
...
pub const GLOBAL_FLAGS: &[(&str, Option<&str>, &str)] = &[
    ("instance", Some("pid|port|path"), "Which Unluminate to talk to when several are running: its process id, its port, or part of its project's path."),
    ...
    ("dry-run", None, "Print the command and the arguments that would be sent, and send nothing. Needs no running Unluminate."),
...
pub fn parse(words: &[String]) -> Result<Typed, Problem> {
    ...
    // `unluminate-cli --help` works, which is what everybody tries first.
    let mut at = 0;
    while at < words.len() {
        let word = &words[at];
        if !word.starts_with('-') {
        ...
        match take_global(words, &mut at, &mut global)? {
            ...
            Taken::No => {
                return Err(Problem::new(format!(
                    "{word} is not one of Unluminate's flags. Try `unluminate-cli --help`."
                )))
            }
        }
    }
    ...
    while next < words.len() {
        ...
        if !only_values && word.starts_with("--") {
            ...
            if known.is_none() {
                ...
                // Not a flag of Unluminate's at all. Inside a text argument that is ordinary text — a
            ...
            let Some(flag) = known else {
                ...
                while next < words.len() {
                    let word = words[next].clone();
                    // Unluminate's flags go on being Unluminate's, even here.
                    if word.starts_with("--") {
        ...
        next += 1;
        if taking_rest {
            // Everything left belongs to it, except a flag of Unluminate's own.
            while next < words.len() {
                let word = words[next].clone();
                if !only_values && word.starts_with("--") {
                    let (name, _) = split_flag(&word);
...
fn unknown(first: &str, second: Option<&String>) -> String {
    ...
    message.push_str(" `unluminate-cli commands` lists them all.");
    message
}
...
mod tests {
    ...
    fn a_text_argument_takes_the_rest_of_the_line_including_dashes() {
        // This is what makes the terminal usable: the shell's own flags must not be read as Unluminate's.
        ...
        assert!(!typed.global.json, "a flag the CLI does not have is not one of Unluminate's");
    }

    #[test]
    fn unluminates_own_flags_are_still_unluminates_after_a_text_argument() {
        ...
        // to set the size to the string "20 --json". A flag Unluminate has is Unluminate's wherever it sits.
    ...
    #[test]
    fn a_double_dash_makes_even_unluminates_own_flags_into_text() {
    ...
    #[test]
    fn unluminates_own_flags_go_before_the_text() {
    ...
    fn a_missing_required_argument_says_which_one_and_shows_the_usage() {
        ...
        assert!(problem.message.contains("unluminate-cli tab open <path>"), "{}", problem.message);
