...
`unluminate-cli` is the comfortable way to drive Unluminate, not the only one. Underneath it is a small open
...
| Platform | Folder |
|---|---|
| Windows | `%APPDATA%\Unluminate\instances` |
| macOS | `~/Library/Application Support/Unluminate/instances` |
| Everywhere else | `$XDG_CONFIG_HOME/unluminate/instances`, or `~/.config/unluminate/instances` |

`UNLUMINATE_INSTANCES` names another folder, which is what the tests use.
...
<- {"ok":true,"command":"tab.open","message":"Opened C:\\jason\\dev\\unluminate\\README.md in tab 1","result":{"tab":1,"path":"…","picture":false}}
```
...
many requests are queued behind this one. `unluminate-cli` and the MCP server both send it.

`unluminate-cli commands --json` prints every command's wire name, arguments and flags, which is the list
...
listing already laid out one row a line. `unluminate-cli` prints those and nothing else.
...
| `not-running` | Unluminate is closing, or could not be reached. |
| `several-instances` | Several Unluminates are running and none was named. |
...
def instances():
    folder = os.path.join(os.environ["APPDATA"], "Unluminate", "instances")  # see the table above
    for path in glob.glob(os.path.join(folder, "*.conf")):
        values = configparser.ConfigParser()
        values.read_string("[unluminate]\n" + io.open(path, encoding="utf-8").read())
        yield dict(values["unluminate"])
...
unluminate = next(iter(instances()))
print(ask(unluminate, "status")["result"]["project"])
ask(unluminate, "tab.open", path="README.md")
ask(unluminate, "window.screenshot", file="_agent_output/unluminate.png")
```

## What it will and will not do

**It is bound to `127.0.0.1` and never to anything else.** Nothing off the machine can reach it, and
there is a test in `crates/unluminate-app/src/services/control.rs` that fails if that ever changes.
