// Measure how well a local model drives Unluminate when it is given only the CLI documentation.
//
//   node unluminate-cli/agent-assessment/run.js --url http://localhost:8087/v1/chat/completions \
//        --model qwen3.8-27b --out _agent_output/task-1661-unluminate-cli/assessment
//
...
//   4. runs them against the live Unluminate;
...
const DOCS = path.join(REPO, 'unluminate-cli', 'docs', 'commands.md');
...
function systemPrompt(documentation) {
  return `You drive the Unluminate text editor from a command line tool called unluminate-cli.

A person tells you what they want in plain English. You answer with the unluminate-cli command line
...
- Every command begins with the word unluminate-cli.
...
Here is the complete documentation for unluminate-cli.
...
function commandsIn(answer) {
  return answer
    ...
    .map((line) => line.replace(/^[-*]\s+/, ''))
    .filter((line) => line.startsWith('unluminate-cli'))
    .map((line) => line.slice('unluminate-cli'.length).trim());
}
...
/** Run unluminate-cli and return the parsed reply, whatever the exit code. */
...
function grade(task, lines) {
  ...
  if (planned.some((plan) => plan === null)) {
    ...
    return { pass: false, why: `line ${at + 1} is not a valid command: unluminate-cli ${lines[at]}` };
  }
  ...
  for (const line of lines) {
    ...
    if (code !== 0) {
      ...
      return { pass: false, why: `unluminate-cli ${line} failed: ${why}` };
    }
  }
...
function main() {
  ...
  const out = flag('out', path.join(REPO, '_agent_output', 'task-1661-unluminate-cli', 'assessment'));
  const only = flag('only', null);
  const folder = flag(
    'project',
    path.join(REPO, '_agent_output', 'task-1661-unluminate-cli', 'assessment-project'),
  );
  ...
  if (!rooted || !rooted.ok) {
    console.error('could not point the running Unluminate at the sample project; is one running?');
    process.exit(2);
  }
