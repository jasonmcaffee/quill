# Qwen 3.8 27B driving Unluminate from the command line
...
1. a short system prompt saying it drives Unluminate through a tool called `unluminate-cli`, and that its
   answer must be command lines and nothing else;
2. the whole of `unluminate-cli/docs/commands.md` — 32,123 characters, about 8,000 tokens.
...
- **executed** — against a **live Unluminate window**, for real;
...
`_agent_output/task-1661-unluminate-cli/assessment/*.json`.
...
`unluminate-cli set-root src`, `unluminate-cli status set "step three done"`, `unluminate-cli open README.md`.

So the 97-point gap between 3.13% and 100% is what the documentation is worth. The model is not
recognising Unluminate from its training data, and it is not guessing a conventional CLI. It is reading.

## How it did, task by task

**It was strongest where the vocabulary was Unluminate's own.** All nine settings tasks passed every
...
- **`unluminate-cli version --json` printed a sentence.** The documentation tells an agent to pass
...
- **"What version of the Unluminate editor am I running?"** was answered with `unluminate-cli version`, which
...
3. **Errors that say what to do instead.** `action run open-file` does not fail with "not
   ...
   `unluminate-cli tab open <path>` instead."* Every refusal names its alternative, which is what turns a
...
node unluminate-cli/agent-assessment/run.js --label mine --temperature 0
node unluminate-cli/agent-assessment/run.js --label mine-control --temperature 0 --no-docs
```
